...
// File generated at Wed 16 Oct 2019 20:12:15
...
CE6SSM_low_scale_constraint<Semi_analytic>::CE6SSM_low_scale_constraint(
...
double CE6SSM_low_scale_constraint<Semi_analytic>::get_scale() const
{
   return scale;
}

void CE6SSM_low_scale_constraint<Semi_analytic>::apply()
{
   check_model_ptr();
   ...
   // initial settings / parameter restrictions
   {
      ...
      MODEL->set_vu(Re(WHICH(vu < 0,FSFlagProblem((MODEL->get_problems().
         flag_non_perturbative_parameter(CE6SSM_info::vu, vu, SCALE))) + (TanBeta*
         LowEnergyConstant(vev))/Sqrt(1 + Sqr(TanBeta)),vu > 3000,FSFlagProblem((
         MODEL->get_problems().flag_non_perturbative_parameter(CE6SSM_info::vu, vu
         , SCALE))) + (TanBeta*LowEnergyConstant(vev))/Sqrt(1 + Sqr(TanBeta)),True
         ,vu)));
      MODEL->set_vd(Re(WHICH(vd < 0,FSFlagProblem((MODEL->get_problems().
         flag_non_perturbative_parameter(CE6SSM_info::vd, vd, SCALE))) +
         LowEnergyConstant(vev)/Sqrt(1 + Sqr(TanBeta)),vd > 3000,FSFlagProblem((
         MODEL->get_problems().flag_non_perturbative_parameter(CE6SSM_info::vd, vd
         , SCALE))) + LowEnergyConstant(vev)/Sqrt(1 + Sqr(TanBeta)),True,vd)));
...
void CE6SSM_low_scale_constraint<Semi_analytic>::set_model(Model* model_)
{
   model = cast_model<CE6SSM<Semi_analytic>*>(model_);
}

void CE6SSM_low_scale_constraint<Semi_analytic>::clear()
{
   scale = 0.;
...
void CE6SSM_low_scale_constraint<Semi_analytic>::initialize()
{
   check_model_ptr();
...
void CE6SSM_low_scale_constraint<Semi_analytic>::update_scale()
{
   check_model_ptr();
...
void CE6SSM_low_scale_constraint<Semi_analytic>::calculate_threshold_corrections()
{
   check_model_ptr();
...
double CE6SSM_low_scale_constraint<Semi_analytic>::calculate_theta_w()
{
   check_model_ptr();

   double theta_w = std::asin(Electroweak_constants::sinThetaW);

   CE6SSM_weinberg_angle::Sm_parameters sm_pars;
   ...
   CE6SSM_weinberg_angle weinberg(MODEL, sm_pars);
...
void CE6SSM_low_scale_constraint<Semi_analytic>::calculate_DRbar_gauge_couplings()
{
   check_model_ptr();
...
double CE6SSM_low_scale_constraint<Semi_analytic>::calculate_delta_alpha_em(double alphaEm) const
{
   check_model_ptr();
...
double CE6SSM_low_scale_constraint<Semi_analytic>::calculate_delta_alpha_s(double alphaS) const
{
   check_model_ptr();
...
double CE6SSM_low_scale_constraint<Semi_analytic>::calculate_alpha_s_SM5_at(
...
void CE6SSM_low_scale_constraint<Semi_analytic>::calculate_running_SM_masses()
{
   check_model_ptr();
...
void CE6SSM_low_scale_constraint<Semi_analytic>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void CE6SSM_low_scale_constraint<Semi_analytic>::calculate_Yu_DRbar()
{
   check_model_ptr();
...
void CE6SSM_low_scale_constraint<Semi_analytic>::calculate_Yd_DRbar()
{
   check_model_ptr();
...
void CE6SSM_low_scale_constraint<Semi_analytic>::calculate_Ye_DRbar()
{
   check_model_ptr();
...
void CE6SSM_low_scale_constraint<Semi_analytic>::check_model_ptr() const
{
   if (!model)
      throw SetupError("CE6SSM_low_scale_constraint<Semi_analytic>: "
