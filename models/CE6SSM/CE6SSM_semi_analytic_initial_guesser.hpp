...
#include "CE6SSM_initial_guesser.hpp"
...
/**
 * @class CE6SSM_initial_guesser<Semi_analytic>
 * @brief initial guesser for the CE6SSM
 */

template<>
class CE6SSM_initial_guesser<Semi_analytic> : public Initial_guesser {
public:
   CE6SSM_initial_guesser(CE6SSM<Semi_analytic>*,
                               const softsusy::QedQcd&,
                               CE6SSM_low_scale_constraint<Semi_analytic>&,
                               CE6SSM_susy_scale_constraint<Semi_analytic>&,
                               CE6SSM_high_scale_constraint<Semi_analytic>&);
   virtual ~CE6SSM_initial_guesser() = default;
   virtual void guess() override; ///< initial guess

   void set_running_precision(double p) { running_precision = p; }

private:
   CE6SSM<Semi_analytic>* model{nullptr}; ///< pointer to model class
