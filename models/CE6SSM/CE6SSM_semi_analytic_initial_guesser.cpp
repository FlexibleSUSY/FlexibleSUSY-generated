...
// File generated at Wed 16 Oct 2019 20:12:16
...
CE6SSM_initial_guesser<Semi_analytic>::CE6SSM_initial_guesser(
   CE6SSM<Semi_analytic>* model_,
   const softsusy::QedQcd& qedqcd_,
   CE6SSM_low_scale_constraint<Semi_analytic>& low_constraint_,
   CE6SSM_susy_scale_constraint<Semi_analytic>& susy_constraint_,
   CE6SSM_high_scale_constraint<Semi_analytic>& high_constraint_
)
   : model(model_)
   , qedqcd(qedqcd_)
...
{
   if (!model)
      throw SetupError("CE6SSM_initial_guesser: Error: pointer to model"
                       " CE6SSM<Semi_analytic> must not be zero");
}
 ...
 */
void CE6SSM_initial_guesser<Semi_analytic>::guess()
{
 ...
 */
void CE6SSM_initial_guesser<Semi_analytic>::guess_susy_parameters()
{
...
void CE6SSM_initial_guesser<Semi_analytic>::initial_guess_low_scale_parameters()
{
   ...
   const auto m0SqGuess = INPUTPARAMETER(m0SqGuess);
   const auto m12Guess = INPUTPARAMETER(m12Guess);
   const auto AzeroGuess = INPUTPARAMETER(AzeroGuess);
   ...
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
   MODEL->set_m0Sq(Re(m0SqGuess));
   MODEL->set_m12(Re(m12Guess));
   MODEL->set_Azero(Re(AzeroGuess));
...
void CE6SSM_initial_guesser<Semi_analytic>::initial_guess_high_scale_parameters()
{
 ...
 * threshold corrections.
 */
void CE6SSM_initial_guesser<Semi_analytic>::solve_susy_parameters()
{
...
void CE6SSM_initial_guesser<Semi_analytic>::guess_soft_parameters()
{
