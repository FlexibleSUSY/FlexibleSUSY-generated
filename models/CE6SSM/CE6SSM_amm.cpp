...
#include "cxx_qft/CE6SSM_particle_aliases.hpp"
...
double get_MLCP(const CE6SSM_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(CE6SSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("CE6SSM_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("CE6SSM_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("CE6SSM_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("CE6SSM_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const CE6SSM_mass_eigenstates& model, double ml_pole, int idx)
{
   using namespace CE6SSM_cxx_diagrams::fields;

   const auto form_factors = CE6SSM_FFV_form_factors::calculate_form_factors<Lepton,Lepton,VP>(idx, idx, model, true);
...
template <typename Lepton>
double calculate_amm_impl(const CE6SSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("CE6SSM_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double CE6SSM_amm::calculate_amm(const CE6SSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   CE6SSM_mass_eigenstates model(model_);

   VERBOSE_MSG("CE6SSM_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("CE6SSM_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template double CE6SSM_amm::calculate_amm<CE6SSM_cxx_diagrams::fields::Fe>(const CE6SSM_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);

template <typename Lepton>
double CE6SSM_amm::calculate_amm_uncertainty(const CE6SSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   CE6SSM_mass_eigenstates model(model_);

   VERBOSE_MSG("CE6SSM_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("CE6SSM_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
// spliting templated function into header and cpp
// https://isocpp.org/wiki/faq/templates#separate-template-fn-defn-from-decl
template double CE6SSM_amm::calculate_amm_uncertainty<CE6SSM_cxx_diagrams::fields::Fe>(const CE6SSM_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);
