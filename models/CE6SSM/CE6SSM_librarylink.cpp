...
template <typename Solver_type>
void CE6SSM_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = CE6SSM_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSCE6SSMCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CE6SSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCE6SSMCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSCE6SSMCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
