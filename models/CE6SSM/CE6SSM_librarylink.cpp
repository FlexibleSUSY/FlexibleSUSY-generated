...
public:
   ...
   virtual void fill_slha_io(CE6SSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const CE6SSM_observables& get_observables() const = 0;
   virtual const CE6SSM_decays& get_decays() const = 0;
...
public:
   ...
   virtual void fill_slha_io(CE6SSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   ...
   virtual const CE6SSM_decays& get_decays() const override { return decays; }
...
private:
   std::tuple<CE6SSM<Solver_type>> models{};        ///< running parameters and pole masses
...
void put_spectrum(const CE6SSM_slha& model, MLINK link)
{
...
template <typename Solver_type>
void CE6SSM_spectrum_impl<Solver_type>::calculate_model_decays(
...
template <typename Solver_type>
void CE6SSM_spectrum_impl<Solver_type>::fill_slha_io(CE6SSM_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   CE6SSM_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = CE6SSM_info::get_multiplet_and_index_from_pdg(pid);
...
DLLEXPORT int FSCE6SSMCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CE6SSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCE6SSMCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSCE6SSMCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSCE6SSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSCE6SSMCalculateDecays", "error", e.what());
      put_error_output(link);
   }
