...
// File generated at Tue 22 Jan 2019 15:18:09

#include "CE6SSM_semi_analytic_spectrum_generator.hpp"
#include "CE6SSM_input_parameters.hpp"
#include "CE6SSM_semi_analytic_convergence_tester.hpp"
...
double CE6SSM_spectrum_generator<Semi_analytic>::get_pole_mass_scale() const
{
 ...
 */
void CE6SSM_spectrum_generator<Semi_analytic>::run_except(const softsusy::QedQcd& qedqcd,
                                const CE6SSM_input_parameters& input)
{
   ...
   CE6SSM_high_scale_constraint<Semi_analytic> high_scale_constraint(&model);
   CE6SSM_susy_scale_constraint<Semi_analytic> susy_scale_constraint(&model, qedqcd);
   CE6SSM_low_scale_constraint<Semi_analytic>  low_scale_constraint(&model, qedqcd);
   CE6SSM_soft_parameters_constraint<Semi_analytic> soft_constraint(&model, qedqcd);
   ...
   CE6SSM_susy_convergence_tester<Semi_analytic> inner_ct(
      &model, settings.get(Spectrum_generator_settings::precision));
   CE6SSM_convergence_tester<Semi_analytic> outer_ct(
      &model, settings.get(Spectrum_generator_settings::precision));
   ...
   CE6SSM_initial_guesser<Semi_analytic> initial_guesser(&model, qedqcd,
 ...
 */
void CE6SSM_spectrum_generator<Semi_analytic>::write_running_couplings(
   const std::string& filename) const
{
   CE6SSM_spectrum_generator_interface<Semi_analytic>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void CE6SSM_spectrum_generator<Semi_analytic>::calculate_spectrum()
{
