...
public:
   ...
   std::tuple<CE6SSM<T>> get_models_slha() const
   { return std::make_tuple(CE6SSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const CE6SSM<T>& get_model() const
   { return model; }
   CE6SSM<T>& get_model()
   { return model; }
   CE6SSM<T> get_model_slha() const
   { return CE6SSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return CE6SSM_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(CE6SSM_info::parameter_names),
      std::cend(CE6SSM_info::parameter_names));
