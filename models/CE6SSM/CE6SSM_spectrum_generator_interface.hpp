...
template <class T>
class CE6SSM_spectrum_generator_interface {
public:
   virtual ~CE6SSM_spectrum_generator_interface() = default;

   std::tuple<CE6SSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<CE6SSM_slha<CE6SSM<T>>> get_models_slha() const
   { return std::make_tuple(CE6SSM_slha<CE6SSM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const CE6SSM<T>& get_model() const
   { return model; }
   CE6SSM<T>& get_model()
   { return model; }
   CE6SSM_slha<CE6SSM<T>> get_model_slha() const
   { return CE6SSM_slha<CE6SSM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const CE6SSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "CE6SSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(CE6SSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const CE6SSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void CE6SSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void CE6SSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const CE6SSM_input_parameters& input)
{
 ...
 */
template <class T>
void CE6SSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   CE6SSM_mass_eigenstates tmp_model(model);
   try {
   ...
   CE6SSM_parameter_getter parameter_getter;
   Coupling_monitor<CE6SSM_mass_eigenstates, CE6SSM_parameter_getter>
 ...
 */
template <class T>
void CE6SSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   CE6SSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void CE6SSM_spectrum_generator_interface<T>::translate_exception_to_problem(CE6SSM<T>& model)
{
   try {
      throw;
