...
// File generated at Fri 20 Oct 2017 08:46:51
...
E6SSMEFTHiggs_low_scale_constraint<Two_scale>::E6SSMEFTHiggs_low_scale_constraint(
...
void E6SSMEFTHiggs_low_scale_constraint<Two_scale>::apply()
{
   check_model_ptr();
...
const Eigen::Matrix<std::complex<double>,3,3>& E6SSMEFTHiggs_low_scale_constraint<Two_scale>::get_ckm()
{
   return ckm;
}

const Eigen::Matrix<std::complex<double>,3,3>& E6SSMEFTHiggs_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}

double E6SSMEFTHiggs_low_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double E6SSMEFTHiggs_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void E6SSMEFTHiggs_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<E6SSMEFTHiggs<Two_scale>*>(model_);
}

void E6SSMEFTHiggs_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& E6SSMEFTHiggs_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void E6SSMEFTHiggs_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
...
void E6SSMEFTHiggs_low_scale_constraint<Two_scale>::initialize()
{
   check_model_ptr();
...
void E6SSMEFTHiggs_low_scale_constraint<Two_scale>::update_scale()
{
   check_model_ptr();
...
void E6SSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
   check_model_ptr();
...
double E6SSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();

   double theta_w = std::asin(Electroweak_constants::sinThetaW);

   E6SSMEFTHiggs_weinberg_angle::Sm_parameters sm_pars;
...
void E6SSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   check_model_ptr();
...
double E6SSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_delta_alpha_s(double alphaS) const
{
   check_model_ptr();
...
double E6SSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
{
   if (!model)
      throw SetupError("E6SSMEFTHiggs_low_scale_constraint<Two_scale>: "
