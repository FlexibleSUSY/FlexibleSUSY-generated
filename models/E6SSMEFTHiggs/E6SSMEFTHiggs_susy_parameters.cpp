...
// File generated at Tue 10 Oct 2017 21:23:48
...
#define CLASSNAME E6SSMEFTHiggs_susy_parameters
#define INPUT(parameter) input.parameter
#define TRACE_STRUCT susy_traces
#define TRACE_STRUCT_TYPE Susy_traces
#define CALCULATE_TRACES(l) calc_susy_traces(l);

const int E6SSMEFTHiggs_susy_parameters::numberOfParameters;

E6SSMEFTHiggs_susy_parameters::E6SSMEFTHiggs_susy_parameters(const E6SSMEFTHiggs_input_parameters& input_)
   : input(input_)
{
   set_number_of_parameters(numberOfParameters);
}

E6SSMEFTHiggs_susy_parameters::E6SSMEFTHiggs_susy_parameters(
   double scale_, int loops_, int thresholds_,
   const E6SSMEFTHiggs_input_parameters& input_
...
Eigen::ArrayXd E6SSMEFTHiggs_susy_parameters::beta() const
{
   return calc_beta().get().unaryExpr(Chop<double>(get_zero_threshold()));
}

E6SSMEFTHiggs_susy_parameters E6SSMEFTHiggs_susy_parameters::calc_beta(int loops) const
{
   ...
   return E6SSMEFTHiggs_susy_parameters(get_scale(), loops, get_thresholds(), input,
...
E6SSMEFTHiggs_susy_parameters E6SSMEFTHiggs_susy_parameters::calc_beta() const
{
   return calc_beta(get_loops());
}

void E6SSMEFTHiggs_susy_parameters::clear()
{
   reset();
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SlSl() const
{
   ...
   if (get_loops() > 1) {
      anomDim += (0.01*twoLoop*(10*(-20*(Ye.adjoint()*Ye*Ye.adjoint()*
         ...
         Quad(g1) + 275*Quad(g2) + 64*Quad(gN) + 20*Sqr(g2)*Sqr(gN) + 6*Sqr(g1)
         *(5*Sqr(g2) + 2*Sqr(gN)))*UNITMATRIX(3))).real();
   }
...
double CLASSNAME::get_SHdSHd() const
{
   ...
   if (get_loops() > 1) {
      anomDim += Re(twoLoop*(2.97*Quad(g1) + 8.25*Quad(g2) + 4.4325*
         Quad(gN) + 0.9*Sqr(g1)*Sqr(g2) - 0.09*Sqr(g1)*Sqr(gN) + 1.35*Sqr(g2)*
         Sqr(gN) - 3*Sqr(Conj(Lambdax))*Sqr(Lambdax) - 0.2*(2*Sqr(g1) - 80*Sqr(
         g3) + 3*Sqr(gN))*(Yd*Yd.adjoint()).trace() + 1.2*Sqr(g1)*(Ye*
...
double CLASSNAME::get_SHuSHu() const
{
   ...
   if (get_loops() > 1) {
      anomDim += Re(twoLoop*(-3*Sqr(Conj(Lambdax))*Sqr(Lambdax) + 0.5*
         ...
         Sqr(g3) - 3*Sqr(gN))*(Yu*Yu.adjoint()).trace() + 3*(99*Quad(g1) + 275*
         Quad(g2) + 64*Quad(gN) + 30*Sqr(g1)*Sqr(g2) + 12*Sqr(g1)*Sqr(gN) + 20*
         Sqr(g2)*Sqr(gN) - 100*(Yd*Yu.adjoint()*Yu*Yd.adjoint()).trace() - 300*
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SuRSuR() const
{
   ...
   if (get_loops() > 1) {
      anomDim += (twoLoop*(-0.4*(5*(Yu.conjugate()*Yd.transpose()*
         ...
         ).trace())) + 0.0002777777777777778*(19456*Quad(g1) + 25600*Quad(g3) +
         1701*Quad(gN) + 960*Sqr(g3)*Sqr(gN) + 256*Sqr(g1)*(40*Sqr(g3) + 3*Sqr
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SeRSeR() const
{
   ...
   if (get_loops() > 1) {
      anomDim += (twoLoop*(-0.4*(5*(Ye.conjugate()*Ye.transpose()*
         ...
         Quad(g1) + 63*Quad(gN) - 16*Sqr(g1)*Sqr(gN))*UNITMATRIX(3))).real();
   }
...
double CLASSNAME::get_SsRSsR() const
{
   ...
   if (get_loops() > 1) {
      anomDim += Re(twoLoop*(13.3125*Quad(gN) - 4*Sqr(Conj(Lambdax))*
...
Eigen::Matrix<double,2,2> CLASSNAME::get_SH1ISH1I() const
{
   ...
   if (get_loops() > 1) {
      anomDim += (twoLoop*(-((Lambda12).adjoint()*Lambda12*(Lambda12)
         ...
         *(Lambda12).adjoint()).trace()) + 0.0075*(396*Quad(g1) + 1100*Quad(g2)
         + 591*Quad(gN) + 12*Sqr(g1)*(10*Sqr(g2) - Sqr(gN)) + 180*Sqr(g2)*Sqr(
   ...
   if (get_loops() > 1) {
      anomDim += (twoLoop*(-((Kappa).adjoint()*Kappa*(Kappa).adjoint()
         *Kappa) + (Kappa).adjoint()*Kappa*(-2*AbsSqr(Lambdax) + Sqr(gN) - 3*(
         Kappa*(Kappa).adjoint()).trace() - 2*(Lambda12*(Lambda12).adjoint())
         .trace()) + 0.0002777777777777778*(4672*Quad(g1) + 25600*Quad(g3) +
         15957*Quad(gN) + 8640*Sqr(g3)*Sqr(gN) + 16*Sqr(g1)*(160*Sqr(g3) + 81*
...
double CLASSNAME::get_SHpSHp() const
{
   ...
   if (get_loops() > 1) {
      anomDim += Re(0.03*twoLoop*(99*Quad(g1) + 275*Quad(g2) + 64*Quad
...
Eigen::ArrayXd E6SSMEFTHiggs_susy_parameters::get() const
{
...
void E6SSMEFTHiggs_susy_parameters::print(std::ostream& ostr) const
{
...
void E6SSMEFTHiggs_susy_parameters::set(const Eigen::ArrayXd& pars)
{
   Yd(0,0) = pars(0);
   Yd(0,1) = pars(1);
   Yd(0,2) = pars(2);
...
const E6SSMEFTHiggs_input_parameters& E6SSMEFTHiggs_susy_parameters::get_input() const
{
   return input;
}

E6SSMEFTHiggs_input_parameters& E6SSMEFTHiggs_susy_parameters::get_input()
{
   return input;
}

void E6SSMEFTHiggs_susy_parameters::set_input_parameters(const E6SSMEFTHiggs_input_parameters& input_)
{
   input = input_;
}

E6SSMEFTHiggs_susy_parameters::Susy_traces E6SSMEFTHiggs_susy_parameters::calc_susy_traces(int loops) const
{
...
std::ostream& operator<<(std::ostream& ostr, const E6SSMEFTHiggs_susy_parameters& susy_pars)
{
