...
// File generated at Wed 16 Oct 2019 20:18:47
 ...
 * lambda is calculated from the tree-level E6SSMEFTHiggs parameters.
 *
 * @param sm SM parameters
 * @param model_0l E6SSMEFTHiggs tree-level parameters
...
Standard_model calculate_SM_tree_level(
   ...
   const E6SSMEFTHiggs_mass_eigenstates& model_0l,
   int idx)
{
   auto sm_0l = sm;
   sm_0l.get_problems().clear();
   sm_0l.set_Lambdax(Sqr(model_0l.get_Mhh(idx)/sm.get_v()));
...
/**
 * Calculates E6SSMEFTHiggs tree-level parameters (g1, g2, g3, Yu, Yd,
 * Ye, v) by performing a tree-level matching from the SM to the
 * E6SSMEFTHiggs.
 *
 * @param model E6SSMEFTHiggs parameters
 * @param sm SM parameters
 *
 * @return E6SSMEFTHiggs with tree-level parameters
 */
E6SSMEFTHiggs_mass_eigenstates calculate_E6SSMEFTHiggs_tree_level(
   const E6SSMEFTHiggs_mass_eigenstates& model,
   const Standard_model& sm)
{
   auto model_0l = model;
 ...
 */
Eigen::Matrix<double,3,1> calculate_tadpole_over_vevs(
   E6SSMEFTHiggs_mass_eigenstates model, int loop_order)
{
   if (loop_order == 0) {
...
/**
 * Calculates tree-level Higgs mass matrix in the E6SSMEFTHiggs without
 ...
 *
 * @param model E6SSMEFTHiggs parameters
 * @return tree-level Higgs mass matrix in the E6SSMEFTHiggs w/o tadpoles
 */
...
/**
 * Calculates squared Higgs pole mass in the E6SSMEFTHiggs,
 * \f$(M_h^{\text{E6SSMEFTHiggs}})^2\f$.
 *
 * @param model_0l tree-level E6SSMEFTHiggs parameters
 * @param model_1l 1-loop E6SSMEFTHiggs parameters
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 *
 * @return squared Higgs pole mass in the E6SSMEFTHiggs
 */
double calculate_Mh2_pole(
   const E6SSMEFTHiggs_mass_eigenstates& model_0l,
   const E6SSMEFTHiggs_mass_eigenstates& model_1l,
   int idx)
{
   ...
   const double p = model_0l.get_Mhh(idx);
   ...
   const auto M_loop = (mh2_tree - self_energy - Eigen::Matrix<double,3,3>(tadpole.asDiagonal())).eval();
 ...
 * lightest CP-even Higgs boson mass of the E6SSMEFTHiggs by requiring
 * that the Higgs pole masses are equal in both models.
 *
 * @param sm Standard Model
 * @param model_1l E6SSMEFTHiggs parameters
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 */
void match_high_to_low_scale_model_1loop(
   ...
   int idx)
{
   // tree-level E6SSMEFTHiggs parameters
   const auto model_0l = calculate_E6SSMEFTHiggs_tree_level(model_1l, sm);
...
double calculate_delta_alpha_em(double alpha_em, const E6SSMEFTHiggs_mass_eigenstates& model)
{
   ...
   delta_alpha_em += alpha_em/(2.*Pi)*(0.3333333333333333 - 1.3333333333333333*
      FiniteLog(Abs(MChaP/currentScale)) - 1.3333333333333333*FiniteLog(Abs(MCha(0
...
double calculate_delta_alpha_s(double alpha_s, const E6SSMEFTHiggs_mass_eigenstates& model)
{
   const double currentScale = model.get_scale();
...
Eigen::Matrix<double,3,3> calculate_MFu_DRbar_tree_level(const E6SSMEFTHiggs_mass_eigenstates& model)
{
...
Eigen::Matrix<double,3,3> calculate_MFd_DRbar_tree_level(const E6SSMEFTHiggs_mass_eigenstates& model)
{
   Eigen::Matrix<double,3,3> mf = ZEROMATRIX(3,3);
...
Eigen::Matrix<double,3,3> calculate_MFe_DRbar_tree_level(const E6SSMEFTHiggs_mass_eigenstates& model)
{
   Eigen::Matrix<double,3,3> mf = ZEROMATRIX(3,3);
...
double calculate_MW_pole_1loop(const E6SSMEFTHiggs_mass_eigenstates& model_0l)
{
   const double mw = model_0l.get_MVWm();
   const double p = model_0l.get_MVWm();
   const double self_energy = Re(model_0l.self_energy_VWm_1loop(p));
   const double M_loop = Sqr(mw) - self_energy;
...
double calculate_MZ_pole_1loop(const E6SSMEFTHiggs_mass_eigenstates& model_0l)
{
   ...
   const E6SSMEFTHiggs_mass_eigenstates& model_0l)
{
   ...
   const double mw2 = Sqr(MW_sm) - Sqr(MW_bsm) + Sqr(model_0l.get_MVWm());
...
/**
 * Calculates E6SSMEFTHiggs parameters at 1-loop level by performing a
 * 1-loop matching.
 *
 * @param sm_0l SM parameters (lambda is at tree-level)
 * @param sm_1l SM parameters (level is at 1-loop level)
 * @param model_0l E6SSMEFTHiggs parameters at tree-level
 * @param model_1l E6SSMEFTHiggs parameters at 1-loop level
 *
 * @return E6SSMEFTHiggs 1-loop parameters
 */
E6SSMEFTHiggs_mass_eigenstates calculate_E6SSMEFTHiggs_1loop(
   const Standard_model& sm_0l,
   ...
   const E6SSMEFTHiggs_mass_eigenstates& model_1l)
{
   ...
   // running E6SSMEFTHiggs W, Z masses (via 1L matching)
   ...
   // running E6SSMEFTHiggs quark and lepton masses (via 1L matching)
   ...
   // running E6SSMEFTHiggs gauge couplings (via 1L matching)
   const double g1_1L = AbsSqrt(4. * Pi * alpha_em * (1. + delta_alpha_em) * mZ2_1L / mW2_1L) / E6SSMEFTHiggs_info::normalization_g1;
   const double g2_1L = AbsSqrt(4. * Pi * alpha_em * (1. + delta_alpha_em) / (1. - mW2_1L/mZ2_1L)) / E6SSMEFTHiggs_info::normalization_g2;
   const double g3_1L = AbsSqrt(4. * Pi * alpha_s * (1. + delta_alpha_s)) / E6SSMEFTHiggs_info::normalization_g3;
   ...
   {
      auto MODEL = &model;
      const double VEV = 2. * AbsSqrt(mZ2_1L/(Sqr(g1_1L*E6SSMEFTHiggs_info::normalization_g1) + Sqr(g2_1L*E6SSMEFTHiggs_info::normalization_g2)));
   ...
   const auto vu = MODELPARAMETER(vu);
   const auto vd = MODELPARAMETER(vd);
   model.set_Yu(Diag((1.4142135623730951*upQuarksDRbar)/vu));
   model.set_Yd(Diag((1.4142135623730951*downQuarksDRbar)/vd));
   model.set_Ye(Diag((1.4142135623730951*downLeptonsDRbar)/vd));
 ...
 * the E6SSMEFTHiggs at the necessary loop level from the known Standard
 * Model couplings and the SM vev.
 *
 * @param model E6SSMEFTHiggs parameters (to be set)
 * @param sm SM parameters
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 */
...
{
   // tree-level E6SSMEFTHiggs parameters
   const auto model_0l = calculate_E6SSMEFTHiggs_tree_level(model, sm);
   const auto sm_0l = calculate_SM_tree_level(sm, model_0l, idx);

   // 1-loop parameters
   const auto model_1l = calculate_E6SSMEFTHiggs_1loop(sm_0l, sm, model_0l, model);

   model.set_g1(model_1l.get_g1());
   model.set_g2(model_1l.get_g2());
   model.set_g3(model_0l.get_g3());
   model.set_vd(model_1l.get_vd());
   model.set_vu(model_1l.get_vu());
 ...
 * CP-even Higgs boson mass of the E6SSMEFTHiggs.
 *
...
void match_high_to_low_scale_model_tree_level(
   Standard_model& sm, const E6SSMEFTHiggs_mass_eigenstates& model, int idx)
{
   auto model_tmp = model;
   model_tmp.calculate_DRbar_masses();
   sm.set_Lambdax(Sqr(model_tmp.get_Mhh(idx)/sm.get_v()));
   sm.calculate_DRbar_masses();
}

/**
 * Calculates the gauge and Yukawa couplings and the SM-like VEV in
 * the E6SSMEFTHiggs at the tree level from the known Standard Model
 * couplings and the SM vev.
 */
void match_low_to_high_scale_model_tree_level(
   E6SSMEFTHiggs_mass_eigenstates& model, const Standard_model& sm_input)
{
   auto sm = sm_input;
   sm.calculate_DRbar_masses();

   model.set_g1(sm.get_g1()*standard_model_info::normalization_g1/E6SSMEFTHiggs_info::normalization_g1);
   model.set_g2(sm.get_g2()*standard_model_info::normalization_g2/E6SSMEFTHiggs_info::normalization_g2);
   model.set_g3(sm.get_g3()*standard_model_info::normalization_g3/E6SSMEFTHiggs_info::normalization_g3);
...
/**
 * Calculates the gauge and Yukawa couplings and the SM-like VEV in
 * the E6SSMEFTHiggs at the 1-loop level from the known Standard Model
 * couplings and the SM vev.
 *
 ...
 * determine the E6SSMEFTHiggs at the right loop order to avoid the
...
void match_low_to_high_scale_model(
   E6SSMEFTHiggs_mass_eigenstates& model, const Standard_model& sm, int loop_order, int idx)
{
   if (loop_order == 0) {
 ...
 * CP-even Higgs boson mass of the E6SSMEFTHiggs by requiring that the
...
void match_high_to_low_scale_model(
   Standard_model& sm, const E6SSMEFTHiggs_mass_eigenstates& model, int loop_order, int idx)
{
   if (loop_order == 0) {
