...
public:
   ...
   std::tuple<E6SSMEFTHiggs<T>, standard_model::StandardModel<T>> get_models_slha() const
   { return std::make_tuple(E6SSMEFTHiggs<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.), eft); }

   const E6SSMEFTHiggs<T>& get_model() const
   { return model; }
   E6SSMEFTHiggs<T>& get_model()
   { return model; }
   E6SSMEFTHiggs<T> get_model_slha() const
   { return E6SSMEFTHiggs<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return E6SSMEFTHiggs_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(E6SSMEFTHiggs_info::parameter_names),
      std::cend(E6SSMEFTHiggs_info::parameter_names));
