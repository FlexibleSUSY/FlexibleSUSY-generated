...
#include "E6SSMEFTHiggs_utilities.hpp"
...
// E6SSMEFTHiggs_parameter_getter ////////////////////////////////////////

std::vector<std::string> E6SSMEFTHiggs_parameter_getter::get_mass_names(const std::string& head)
{
   using namespace E6SSMEFTHiggs_info;
...
std::array<std::string, E6SSMEFTHiggs_info::NUMBER_OF_MIXINGS> E6SSMEFTHiggs_parameter_getter::get_mixing_names()
{
   return E6SSMEFTHiggs_info::particle_mixing_names;
}

Eigen::ArrayXd E6SSMEFTHiggs_parameter_getter::get_parameters(const E6SSMEFTHiggs_mass_eigenstates& model)
{
   return model.get();
}

std::array<std::string, E6SSMEFTHiggs_info::NUMBER_OF_PARAMETERS> E6SSMEFTHiggs_parameter_getter::get_parameter_names()
{
   return E6SSMEFTHiggs_info::parameter_names;
}

std::array<std::string, E6SSMEFTHiggs_info::NUMBER_OF_PARTICLES> E6SSMEFTHiggs_parameter_getter::get_particle_names()
{
   return E6SSMEFTHiggs_info::particle_names;
}

std::vector<std::string> E6SSMEFTHiggs_parameter_getter::get_DRbar_mass_names()
{
   return get_mass_names();
}

std::vector<std::string> E6SSMEFTHiggs_parameter_getter::get_pole_mass_names()
{
   return get_mass_names("Pole");
}

std::array<std::string, E6SSMEFTHiggs_info::NUMBER_OF_MIXINGS> E6SSMEFTHiggs_parameter_getter::get_DRbar_mixing_names()
{
   return get_mixing_names();
}

std::array<std::string, E6SSMEFTHiggs_info::NUMBER_OF_MIXINGS> E6SSMEFTHiggs_parameter_getter::get_pole_mixing_names()
{
...
std::array<std::string, E6SSMEFTHiggs_info::NUMBER_OF_INPUT_PARAMETERS> E6SSMEFTHiggs_parameter_getter::get_input_parameter_names()
{
   return E6SSMEFTHiggs_info::input_parameter_names;
}

std::array<std::string, E6SSMEFTHiggs_info::NUMBER_OF_EXTRA_PARAMETERS> E6SSMEFTHiggs_parameter_getter::get_extra_parameter_names()
{
   return E6SSMEFTHiggs_info::extra_parameter_names;
}

decltype(E6SSMEFTHiggs_info::NUMBER_OF_MASSES) E6SSMEFTHiggs_parameter_getter::get_number_of_masses()
{
   return E6SSMEFTHiggs_info::NUMBER_OF_MASSES;
}

// E6SSMEFTHiggs_spectrum_plotter ////////////////////////////////////////

E6SSMEFTHiggs_spectrum_plotter::E6SSMEFTHiggs_spectrum_plotter(const E6SSMEFTHiggs_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void E6SSMEFTHiggs_spectrum_plotter::extract_spectrum(const E6SSMEFTHiggs_mass_eigenstates& model)
{
   spectrum.clear();
   scale = model.get_scale();

   spectrum.emplace_back(TParticle("Glu", "\\tilde{g}", to_valarray(PHYSICAL(MGlu))));
   spectrum.emplace_back(TParticle("ChaP", "\\tilde{\\chi}^{'-}", to_valarray(PHYSICAL(MChaP))));
   ...
   spectrum.emplace_back(TParticle("SDX", "\\tilde{x}", to_valarray(PHYSICAL(MSDX))));
   ...
   spectrum.emplace_back(TParticle("FDX", "x", to_valarray(PHYSICAL(MFDX))));
   spectrum.emplace_back(TParticle("SHI0", "h^{0,Inert}", to_valarray(PHYSICAL(MSHI0))));
   spectrum.emplace_back(TParticle("SHIp", "h^{-,Inert}", to_valarray(PHYSICAL(MSHIp))));
   spectrum.emplace_back(TParticle("ChaI", "\\tilde{\\chi}^{-,Inert}", to_valarray(PHYSICAL(MChaI))));
   spectrum.emplace_back(TParticle("ChiI", "\\tilde{\\chi}^{0,Inert}", to_valarray(PHYSICAL(MChiI))));
   spectrum.emplace_back(TParticle("SSI0", "s^{Inert}", to_valarray(PHYSICAL(MSSI0))));
   spectrum.emplace_back(TParticle("FSI", "\\tilde{S}^{Inert}", to_valarray(PHYSICAL(MFSI))));
   spectrum.emplace_back(TParticle("SHp0", "H^{'0}", to_valarray(PHYSICAL(MSHp0))));
   spectrum.emplace_back(TParticle("SHpp", "H^{'-}", to_valarray(PHYSICAL(MSHpp))));
   spectrum.emplace_back(TParticle("ChiP", "\\tilde{\\chi}^{'0}", to_valarray(PHYSICAL(MChiP))));
   spectrum.emplace_back(TParticle("VZp", "{Z'}", to_valarray(PHYSICAL(MVZp))));
...
void E6SSMEFTHiggs_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("E6SSMEFTHiggs_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("E6SSMEFTHiggs_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("E6SSMEFTHiggs_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void E6SSMEFTHiggs_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("E6SSMEFTHiggs_spectrum_plotter::write_spectrum: "
...
namespace E6SSMEFTHiggs_database {
 ...
 */
void to_database(
   const std::string& file_name, const E6SSMEFTHiggs_mass_eigenstates& model,
   ...
   const E6SSMEFTHiggs_observables* observables)
{
   ...
   append(names, E6SSMEFTHiggs_parameter_getter::get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, E6SSMEFTHiggs_parameter_getter::get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, E6SSMEFTHiggs_parameter_getter::get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, E6SSMEFTHiggs_parameter_getter::get_DRbar_mass_names());
   append(names, E6SSMEFTHiggs_parameter_getter::get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, E6SSMEFTHiggs_parameter_getter::get_pole_mass_names());
   append(names, E6SSMEFTHiggs_parameter_getter::get_pole_mixing_names());
   ...
   if (E6SSMEFTHiggs_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, E6SSMEFTHiggs_observables::get_names());
 ...
 */
E6SSMEFTHiggs_mass_eigenstates from_database(
   const std::string& file_name, long long entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, E6SSMEFTHiggs_observables* observables)
{
   E6SSMEFTHiggs_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = E6SSMEFTHiggs_parameter_getter::get_number_of_masses();
   const auto number_of_mixings = E6SSMEFTHiggs_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = E6SSMEFTHiggs_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = E6SSMEFTHiggs_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? E6SSMEFTHiggs_observables::NUMBER_OF_OBSERVABLES : 0);
