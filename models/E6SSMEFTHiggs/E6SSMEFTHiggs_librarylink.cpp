...
namespace E6SSMEFTHiggs_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSE6SSMEFTHiggsMessage", 1);
...
class E6SSMEFTHiggs_spectrum {
public:
   virtual ~E6SSMEFTHiggs_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(E6SSMEFTHiggs_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const E6SSMEFTHiggs_observables& get_observables() const = 0;
   virtual const E6SSMEFTHiggs_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const E6SSMEFTHiggs_input_parameters&) = 0;
...
class E6SSMEFTHiggs_spectrum_impl : public E6SSMEFTHiggs_spectrum
{
public:
   virtual ~E6SSMEFTHiggs_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(E6SSMEFTHiggs_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const E6SSMEFTHiggs_observables& get_observables() const override { return observables; }
   virtual const E6SSMEFTHiggs_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const E6SSMEFTHiggs_input_parameters&) override;
...
private:
   std::tuple<E6SSMEFTHiggs<Solver_type>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   E6SSMEFTHiggs_scales scales{};              ///< scale information
   E6SSMEFTHiggs_observables observables{};    ///< observables
   E6SSMEFTHiggs_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const E6SSMEFTHiggs_input_parameters& input_) { input = input_; }
...
private:
   E6SSMEFTHiggs_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<E6SSMEFTHiggs_spectrum> spectrum{nullptr};  ///< spectrum information

   E6SSMEFTHiggs_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSE6SSMEFTHiggsCalculateSpectrum[]?");
   }
}
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(gNInput), "gNInput");
   MLPutRuleTo(link, INPUTPARAMETER(M4Input), "M4Input");
   MLPutRuleTo(link, INPUTPARAMETER(mHp2Input), "mHp2Input");
   MLPutRuleTo(link, INPUTPARAMETER(mHpbar2Input), "mHpbar2Input");
   MLPutRuleTo(link, INPUTPARAMETER(MuPrInput), "MuPrInput");
   MLPutRuleTo(link, INPUTPARAMETER(BMuPrInput), "BMuPrInput");
   ...
   MLPutRuleTo(link, INPUTPARAMETER(ALambda12Input), "ALambda12Input");
   MLPutRuleTo(link, INPUTPARAMETER(KappaInput), "KappaInput");
   MLPutRuleTo(link, INPUTPARAMETER(AKappaInput), "AKappaInput");
   MLPutRuleTo(link, INPUTPARAMETER(mDx2Input), "mDx2Input");
   MLPutRuleTo(link, INPUTPARAMETER(mDxbar2Input), "mDxbar2Input");
   MLPutRuleTo(link, INPUTPARAMETER(mH1I2Input), "mH1I2Input");
   MLPutRuleTo(link, INPUTPARAMETER(mH2I2Input), "mH2I2Input");
   MLPutRuleTo(link, INPUTPARAMETER(msI2Input), "msI2Input");
...
E6SSMEFTHiggs_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const E6SSMEFTHiggs_slha& model, MLINK link)
{
   MLPutRule(link, E6SSMEFTHiggs_info::model_name);
   MLPutFunction(link, "List", 163);
...
template <typename Solver_type>
void E6SSMEFTHiggs_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void E6SSMEFTHiggs_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const E6SSMEFTHiggs_input_parameters& input
)
{
...
template <typename Solver_type>
void E6SSMEFTHiggs_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void E6SSMEFTHiggs_spectrum_impl<Solver_type>::calculate_model_decays(
...
template <typename Solver_type>
void E6SSMEFTHiggs_spectrum_impl<Solver_type>::fill_slha_io(E6SSMEFTHiggs_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   E6SSMEFTHiggs_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   E6SSMEFTHiggs_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = E6SSMEFTHiggs_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSE6SSMEFTHiggsCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSE6SSMEFTHiggsCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new E6SSMEFTHiggs_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   E6SSMEFTHiggs_input_parameters input;
   ...
   INPUTPARAMETER(gNInput) = pars[c++];
   INPUTPARAMETER(M4Input) = pars[c++];
   INPUTPARAMETER(mHp2Input) = pars[c++];
   INPUTPARAMETER(mHpbar2Input) = pars[c++];
   INPUTPARAMETER(MuPrInput) = pars[c++];
   INPUTPARAMETER(BMuPrInput) = pars[c++];
   ...
   INPUTPARAMETER(Lambda12Input(0,0)) = pars[c++];
   INPUTPARAMETER(Lambda12Input(0,1)) = pars[c++];
   INPUTPARAMETER(Lambda12Input(1,0)) = pars[c++];
   INPUTPARAMETER(Lambda12Input(1,1)) = pars[c++];
   INPUTPARAMETER(ALambda12Input(0,0)) = pars[c++];
   INPUTPARAMETER(ALambda12Input(0,1)) = pars[c++];
   INPUTPARAMETER(ALambda12Input(1,0)) = pars[c++];
   INPUTPARAMETER(ALambda12Input(1,1)) = pars[c++];
   INPUTPARAMETER(KappaInput(0,0)) = pars[c++];
   INPUTPARAMETER(KappaInput(0,1)) = pars[c++];
   INPUTPARAMETER(KappaInput(0,2)) = pars[c++];
   INPUTPARAMETER(KappaInput(1,0)) = pars[c++];
   INPUTPARAMETER(KappaInput(1,1)) = pars[c++];
   INPUTPARAMETER(KappaInput(1,2)) = pars[c++];
   INPUTPARAMETER(KappaInput(2,0)) = pars[c++];
   INPUTPARAMETER(KappaInput(2,1)) = pars[c++];
   INPUTPARAMETER(KappaInput(2,2)) = pars[c++];
   INPUTPARAMETER(AKappaInput(0,0)) = pars[c++];
   INPUTPARAMETER(AKappaInput(0,1)) = pars[c++];
   INPUTPARAMETER(AKappaInput(0,2)) = pars[c++];
   INPUTPARAMETER(AKappaInput(1,0)) = pars[c++];
   INPUTPARAMETER(AKappaInput(1,1)) = pars[c++];
   INPUTPARAMETER(AKappaInput(1,2)) = pars[c++];
   INPUTPARAMETER(AKappaInput(2,0)) = pars[c++];
   INPUTPARAMETER(AKappaInput(2,1)) = pars[c++];
   INPUTPARAMETER(AKappaInput(2,2)) = pars[c++];
   INPUTPARAMETER(mDx2Input(0,0)) = pars[c++];
   INPUTPARAMETER(mDx2Input(0,1)) = pars[c++];
   INPUTPARAMETER(mDx2Input(0,2)) = pars[c++];
   INPUTPARAMETER(mDx2Input(1,0)) = pars[c++];
   INPUTPARAMETER(mDx2Input(1,1)) = pars[c++];
   INPUTPARAMETER(mDx2Input(1,2)) = pars[c++];
   INPUTPARAMETER(mDx2Input(2,0)) = pars[c++];
   INPUTPARAMETER(mDx2Input(2,1)) = pars[c++];
   INPUTPARAMETER(mDx2Input(2,2)) = pars[c++];
   INPUTPARAMETER(mDxbar2Input(0,0)) = pars[c++];
   INPUTPARAMETER(mDxbar2Input(0,1)) = pars[c++];
   INPUTPARAMETER(mDxbar2Input(0,2)) = pars[c++];
   INPUTPARAMETER(mDxbar2Input(1,0)) = pars[c++];
   INPUTPARAMETER(mDxbar2Input(1,1)) = pars[c++];
   INPUTPARAMETER(mDxbar2Input(1,2)) = pars[c++];
   INPUTPARAMETER(mDxbar2Input(2,0)) = pars[c++];
   INPUTPARAMETER(mDxbar2Input(2,1)) = pars[c++];
   INPUTPARAMETER(mDxbar2Input(2,2)) = pars[c++];
   INPUTPARAMETER(mH1I2Input(0,0)) = pars[c++];
   INPUTPARAMETER(mH1I2Input(0,1)) = pars[c++];
   INPUTPARAMETER(mH1I2Input(1,0)) = pars[c++];
   INPUTPARAMETER(mH1I2Input(1,1)) = pars[c++];
   INPUTPARAMETER(mH2I2Input(0,0)) = pars[c++];
   INPUTPARAMETER(mH2I2Input(0,1)) = pars[c++];
   INPUTPARAMETER(mH2I2Input(1,0)) = pars[c++];
   INPUTPARAMETER(mH2I2Input(1,1)) = pars[c++];
   INPUTPARAMETER(msI2Input(0,0)) = pars[c++];
   INPUTPARAMETER(msI2Input(0,1)) = pars[c++];
   INPUTPARAMETER(msI2Input(1,0)) = pars[c++];
   INPUTPARAMETER(msI2Input(1,1)) = pars[c++];
...
DLLEXPORT int FSE6SSMEFTHiggsGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMEFTHiggsGetSettings"))
...
DLLEXPORT int FSE6SSMEFTHiggsGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMEFTHiggsGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSE6SSMEFTHiggsGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMEFTHiggsGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSE6SSMEFTHiggsOpenHandle(
...
DLLEXPORT int FSE6SSMEFTHiggsCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::E6SSMEFTHiggs_librarylink;
...
DLLEXPORT int FSE6SSMEFTHiggsSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::E6SSMEFTHiggs_librarylink;
...
DLLEXPORT int FSE6SSMEFTHiggsGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMEFTHiggsGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSE6SSMEFTHiggsToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMEFTHiggsToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSE6SSMEFTHiggsGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMEFTHiggsGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSE6SSMEFTHiggsCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMEFTHiggsCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSE6SSMEFTHiggsCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMEFTHiggsCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSE6SSMEFTHiggsCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSE6SSMEFTHiggsCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSE6SSMEFTHiggsCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSE6SSMEFTHiggsCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMEFTHiggsCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSE6SSMEFTHiggsCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSE6SSMEFTHiggsCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSE6SSMEFTHiggsCalculateDecays", "error", e.what());
      put_error_output(link);
   }
