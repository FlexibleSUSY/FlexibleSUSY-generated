...
#include "lowe.h"
#include "E6SSMEFTHiggs_unitarity.hpp"
...
template <typename Solver_type>
void E6SSMEFTHiggs_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = E6SSMEFTHiggs_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSE6SSMEFTHiggsCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMEFTHiggsCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSE6SSMEFTHiggsCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
