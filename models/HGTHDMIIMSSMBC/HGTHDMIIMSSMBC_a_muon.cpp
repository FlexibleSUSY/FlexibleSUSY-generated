...
#include "cxx_qft/HGTHDMIIMSSMBC_qft.hpp"
#include "HGTHDMIIMSSMBC_FFV_form_factors.hpp"
...
using namespace HGTHDMIIMSSMBC_cxx_diagrams;
...
void run_to_MSUSY(HGTHDMIIMSSMBC_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("HGTHDMIIMSSMBC_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("HGTHDMIIMSSMBC_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("HGTHDMIIMSSMBC_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("HGTHDMIIMSSMBC_a_muon: run_to_MSUSY(): "
...
double calculate_a_muon_impl(const HGTHDMIIMSSMBC_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   VERBOSE_MSG("HGTHDMIIMSSMBC_a_muon: calculating a_mu at Q = " << model.get_scale());

   context_base context{ model };

   using namespace HGTHDMIIMSSMBC_cxx_diagrams::fields;

   const auto form_factors = HGTHDMIIMSSMBC_FFV_form_factors::calculate_Fe_Fe_VP_form_factors(1, 1, model, true);
   ...
   valBarZee += std::complex<double> {1.000000000000000, 0} * AMuonBarZeeScalarLoop<typename conj<Hm>::type, Ah>::value({1}, context, qedqcd);
   ...
   valBarZee += std::complex<double> {1.000000000000000, 0} * AMuonBarZeeScalarLoop<typename conj<Hm>::type, hh>::value({1}, context, qedqcd);
...
std::pair<double,double> vary_scale(const HGTHDMIIMSSMBC_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model,&qedqcd] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("HGTHDMIIMSSMBC_a_muon: scale variation: " << e.what_detailed());
                     }
                     return amu;
                  });
...
double HGTHDMIIMSSMBC_a_muon::calculate_a_muon(const HGTHDMIIMSSMBC_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   HGTHDMIIMSSMBC_mass_eigenstates model(model_);

   VERBOSE_MSG("HGTHDMIIMSSMBC_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("HGTHDMIIMSSMBC_a_muon:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
double HGTHDMIIMSSMBC_a_muon::calculate_a_muon_uncertainty(const HGTHDMIIMSSMBC_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   HGTHDMIIMSSMBC_mass_eigenstates model(model_);

   VERBOSE_MSG("HGTHDMIIMSSMBC_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("HGTHDMIIMSSMBC_a_muon uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
