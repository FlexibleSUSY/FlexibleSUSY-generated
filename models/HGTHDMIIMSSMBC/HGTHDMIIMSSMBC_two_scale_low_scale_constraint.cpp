...
// File generated at Sun 26 Aug 2018 14:07:45
...
HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::HGTHDMIIMSSMBC_low_scale_constraint(
...
void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::apply()
{
   check_model_ptr();
...
const Eigen::Matrix<std::complex<double>,3,3>& HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::get_ckm()
{
   return ckm;
}

const Eigen::Matrix<std::complex<double>,3,3>& HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}

double HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<HGTHDMIIMSSMBC<Two_scale>*>(model_);
}

void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
...
void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::initialize()
{
   check_model_ptr();
...
void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::update_scale()
{
   check_model_ptr();
...
void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
   check_model_ptr();
...
double HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();

   double theta_w = std::asin(Electroweak_constants::sinThetaW);

   HGTHDMIIMSSMBC_weinberg_angle::Sm_parameters sm_pars;
   ...
   HGTHDMIIMSSMBC_weinberg_angle weinberg(MODEL, sm_pars);
...
void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   check_model_ptr();
   ...
   if (IsFinite(new_g1)) {
      model->get_problems().unflag_non_perturbative_parameter(HGTHDMIIMSSMBC_info
         ::g1);
   } else {
      model->get_problems().flag_non_perturbative_parameter(
         HGTHDMIIMSSMBC_info::g1, new_g1, get_scale());
   ...
   if (IsFinite(new_g2)) {
      ...
      model->get_problems().flag_non_perturbative_parameter(
         HGTHDMIIMSSMBC_info::g2, new_g2, get_scale());
      new_g2 = Electroweak_constants::g2;
   }
}

double HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   check_model_ptr();
   ...
   const double delta_alpha_em = 0.15915494309189535*alphaEm*(-1.3333333333333333*
      FiniteLog(Abs(MCha(0)/currentScale)) - 1.3333333333333333*FiniteLog(Abs(MCha
      (1)/currentScale)) - 0.3333333333333333*FiniteLog(Abs(MHm(1)/currentScale)))
      ;
...
double HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_delta_alpha_s(double alphaS) const
{
   check_model_ptr();
   ...
   const double delta_alpha_s = -0.3183098861837907*alphaS*FiniteLog(Abs(MGlu/
      currentScale));
...
double HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_running_SM_masses()
{
   check_model_ptr();
...
void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_Yu_DRbar()
{
   check_model_ptr();
...
void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_Yd_DRbar()
{
   check_model_ptr();
...
void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_Ye_DRbar()
{
   check_model_ptr();
...
void HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("HGTHDMIIMSSMBC_low_scale_constraint<Two_scale>: "
