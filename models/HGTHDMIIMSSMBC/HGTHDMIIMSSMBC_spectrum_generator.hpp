...
// File generated at Wed 29 Jun 2016 11:30:53

#ifndef HGTHDMIIMSSMBC_SPECTRUM_GENERATOR_H
#define HGTHDMIIMSSMBC_SPECTRUM_GENERATOR_H
...
template <class T>
class HGTHDMIIMSSMBC_spectrum_generator
   : public HGTHDMIIMSSMBC_spectrum_generator_interface<T> {
public:
   HGTHDMIIMSSMBC_spectrum_generator()
      : HGTHDMIIMSSMBC_spectrum_generator_interface<T>()
      ...
      , low_scale(0.)
   {}
   virtual ~HGTHDMIIMSSMBC_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const HGTHDMIIMSSMBC_input_parameters&);
   void write_running_couplings(const std::string& filename = "HGTHDMIIMSSMBC_rgflow.dat") const;

private:
   HGTHDMIIMSSMBC_high_scale_constraint<T> high_scale_constraint;
   HGTHDMIIMSSMBC_susy_scale_constraint<T> susy_scale_constraint;
   HGTHDMIIMSSMBC_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void HGTHDMIIMSSMBC_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const HGTHDMIIMSSMBC_input_parameters& input)
{
   HGTHDMIIMSSMBC<T>& model = this->model;
   model.clear();
   ...
   HGTHDMIIMSSMBC_convergence_tester<T> convergence_tester(
   ...
   HGTHDMIIMSSMBC_initial_guesser<T> initial_guesser(&model, qedqcd,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void HGTHDMIIMSSMBC_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   HGTHDMIIMSSMBC_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
