...
// File generated at Tue 22 Jan 2019 16:27:20
...
#include "HGTHDMIIMSSMBC_two_scale_spectrum_generator.hpp"
#endif
...
class HGTHDMIIMSSMBC_spectrum {
public:
   virtual ~HGTHDMIIMSSMBC_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(HGTHDMIIMSSMBC_slha_io&) const = 0;
...
class HGTHDMIIMSSMBC_spectrum_impl : public HGTHDMIIMSSMBC_spectrum
{
public:
   ...
   virtual void fill_slha_io(HGTHDMIIMSSMBC_slha_io&) const override;
...
private:
   std::tuple<HGTHDMIIMSSMBC_slha<HGTHDMIIMSSMBC<Solver_type>>> models{};        ///< running parameters and pole masses
...
public:
   ...
   void set_input_parameters(const HGTHDMIIMSSMBC_input_parameters& input_) { input = input_; }
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSHGTHDMIIMSSMBCCalculateSpectrum[]?");
   }
}
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, HGTHDMIIMSSMBC_info::particle_names[i].c_str());
      }
   }
}
...
HGTHDMIIMSSMBC_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const HGTHDMIIMSSMBC_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void HGTHDMIIMSSMBC_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void HGTHDMIIMSSMBC_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const HGTHDMIIMSSMBC_input_parameters& input
)
{
   HGTHDMIIMSSMBC_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void HGTHDMIIMSSMBC_spectrum_impl<Solver_type>::fill_slha_io(HGTHDMIIMSSMBC_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   HGTHDMIIMSSMBC_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSHGTHDMIIMSSMBCCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSHGTHDMIIMSSMBCCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new HGTHDMIIMSSMBC_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSHGTHDMIIMSSMBCGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HGTHDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSHGTHDMIIMSSMBCGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHGTHDMIIMSSMBCToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HGTHDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSHGTHDMIIMSSMBCToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHGTHDMIIMSSMBCGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HGTHDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSHGTHDMIIMSSMBCGetWarnings"))
      return LIBRARY_TYPE_ERROR;
