...
std::unique_ptr<HGTHDMIIMSSMBC_mass_eigenstates_interface> CLASSNAME::clone() const
{
   return std::make_unique<HGTHDMIIMSSMBC_mass_eigenstates>(*this);
}
...
const HGTHDMIIMSSMBC_input_parameters& CLASSNAME::get_input_parameters() const
{
   return get_input();
}

HGTHDMIIMSSMBC_input_parameters& CLASSNAME::get_input_parameters()
{
   return get_input();
}
...
std::complex<double> CLASSNAME::CpChiChaconjUHmPL(int gI1, int gI2, int gO1) const
{
   ...
   const std::complex<double> result = -0.7071067811865475*Conj(UM(gI2,1))*(g1dp*
      Conj(ZN(gI1,0)) + g1d*Conj(ZN(gI1,1)))*KroneckerDelta(0,gO1) + g1d*Conj(UM(
...
std::complex<double> CLASSNAME::CpbarChaUChiHmPR(int gI1, int gO1, int gI2) const
{
   ...
   const std::complex<double> result = g1d*KroneckerDelta(2,gO1)*UM(gI1,0)*ZP(gI2,
      0) - 0.7071067811865475*(g1dp*KroneckerDelta(0,gO1) + g1d*KroneckerDelta(1,
...
std::complex<double> CLASSNAME::CpUChiChaconjHmPL(int gO2, int gI2, int gI1) const
{
   ...
   const std::complex<double> result = -0.7071067811865475*Conj(UM(gI2,1))*(g1dp*
      KroneckerDelta(0,gO2) + g1d*KroneckerDelta(1,gO2))*ZP(gI1,0) + g1d*Conj(UM(
...
std::complex<double> CLASSNAME::CpbarUChaChiHmPR(int gO1, int gI2, int gI1) const
{
   ...
   const std::complex<double> result = -0.7071067811865475*KroneckerDelta(1,gO1)*(
      g1dp*ZN(gI2,0) + g1d*ZN(gI2,1))*ZP(gI1,0) + g1d*KroneckerDelta(0,gO1)*ZN(gI2
      ,2)*ZP(gI1,0);
...
std::complex<double> CLASSNAME::self_energy_VWm_1loop(double p ) const
{
   ...
   result += -0.3333333333333333*AbsSqr(CpconjVWmVPVWm())*(3*A0(Sqr(MVWm)) + 30*
      B00(Sqr(p),Sqr(MVWm),0) - 6*Sqr(MVWm) + 2*Sqr(p) + 3*B0(Sqr(p),Sqr(MVWm),0)*
      (Sqr(MVWm) + 4*Sqr(p)));
...
double CLASSNAME::calculate_MVZ_DRbar(double m_pole) const
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(HGTHDMIIMSSMBC_info::VZ);return m_pole;
   }
...
double CLASSNAME::calculate_MVWm_DRbar(double m_pole) const
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(HGTHDMIIMSSMBC_info::VWm);return m_pole;
   }
