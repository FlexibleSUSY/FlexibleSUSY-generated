...
// File generated at Sat 27 Aug 2016 11:55:06
...
public:
   HGTHDMIIMSSMBC_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~HGTHDMIIMSSMBC_spectrum_generator_interface() {}

   const HGTHDMIIMSSMBC<T>& get_model() const { return model; }
   HGTHDMIIMSSMBC<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const HGTHDMIIMSSMBC_input_parameters&) = 0;
   ...
   double start, double stop) const
{
   ...
   HGTHDMIIMSSMBC_parameter_getter parameter_getter;
   Coupling_monitor<HGTHDMIIMSSMBC_mass_eigenstates, HGTHDMIIMSSMBC_parameter_getter>
 ...
 */
template <class T>
void HGTHDMIIMSSMBC_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   HGTHDMIIMSSMBC_spectrum_plotter plotter;
...
void HGTHDMIIMSSMBC_spectrum_generator_interface<T>::translate_exception_to_problem(HGTHDMIIMSSMBC<T>& model)
{
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : HGTHDMIIMSSMBC_info::parameter_names[parameter_index];
