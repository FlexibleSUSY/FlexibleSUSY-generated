...
template <class T>
class HGTHDMIIMSSMBC_spectrum_generator_interface {
public:
   virtual ~HGTHDMIIMSSMBC_spectrum_generator_interface() = default;

   std::tuple<HGTHDMIIMSSMBC<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<HGTHDMIIMSSMBC<T>> get_models_slha() const
   { return std::make_tuple(HGTHDMIIMSSMBC<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const HGTHDMIIMSSMBC<T>& get_model() const
   { return model; }
   HGTHDMIIMSSMBC<T>& get_model()
   { return model; }
   HGTHDMIIMSSMBC<T> get_model_slha() const
   { return HGTHDMIIMSSMBC<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const HGTHDMIIMSSMBC_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "HGTHDMIIMSSMBC_spectrum.dat") const;

protected:
   HGTHDMIIMSSMBC<T> model;
   ...
   void translate_exception_to_problem(HGTHDMIIMSSMBC<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const HGTHDMIIMSSMBC_input_parameters&) = 0;
};
 ...
 */
template <class T>
void HGTHDMIIMSSMBC_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void HGTHDMIIMSSMBC_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const HGTHDMIIMSSMBC_input_parameters& input)
{
 ...
 */
template <class T>
void HGTHDMIIMSSMBC_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   HGTHDMIIMSSMBC_mass_eigenstates tmp_model(model);
   try {
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return HGTHDMIIMSSMBC_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(HGTHDMIIMSSMBC_info::parameter_names),
      std::cend(HGTHDMIIMSSMBC_info::parameter_names));
 ...
 */
template <class T>
void HGTHDMIIMSSMBC_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   HGTHDMIIMSSMBC_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void HGTHDMIIMSSMBC_spectrum_generator_interface<T>::translate_exception_to_problem(HGTHDMIIMSSMBC<T>& model)
{
   try {
      throw;
