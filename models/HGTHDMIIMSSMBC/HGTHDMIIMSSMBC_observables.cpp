...
#include "HGTHDMIIMSSMBC_edm.hpp"
#include "HGTHDMIIMSSMBC_l_to_lgamma.hpp"
//#include "HGTHDMIIMSSMBC_f_to_f_conversion.hpp"
...
const int HGTHDMIIMSSMBC_observables::NUMBER_OF_OBSERVABLES;

HGTHDMIIMSSMBC_observables::HGTHDMIIMSSMBC_observables()
   : a_muon(0)
...
Eigen::ArrayXd HGTHDMIIMSSMBC_observables::get() const
{
   Eigen::ArrayXd vec(HGTHDMIIMSSMBC_observables::NUMBER_OF_OBSERVABLES);
...
std::vector<std::string> HGTHDMIIMSSMBC_observables::get_names()
{
   std::vector<std::string> names(HGTHDMIIMSSMBC_observables::NUMBER_OF_OBSERVABLES);
...
void HGTHDMIIMSSMBC_observables::clear()
{
   a_muon = 0.;
   eff_cp_higgs_photon_photon = Eigen::Array<std::complex<double>,2,1>::Zero();
   eff_cp_higgs_gluon_gluon = Eigen::Array<std::complex<double>,2,1>::Zero();
...
void HGTHDMIIMSSMBC_observables::set(const Eigen::ArrayXd& vec)
{
   assert(vec.rows() == HGTHDMIIMSSMBC_observables::NUMBER_OF_OBSERVABLES);
...
HGTHDMIIMSSMBC_observables calculate_observables(HGTHDMIIMSSMBC_mass_eigenstates& model,
                                              ...
                                              double scale)
{
   ...
   if (scale > 0.) {
      try {
         ...
         return HGTHDMIIMSSMBC_observables();
      }
   }
                                              ...
                                              const Physical_input& physical_input)
{
   ...
   try {
      HGTHDMIIMSSMBC_effective_couplings effective_couplings(model, qedqcd, physical_input);
      effective_couplings.calculate_effective_couplings();

      observables.AMU = HGTHDMIIMSSMBC_a_muon::calculate_a_muon(MODEL, qedqcd);
