 ...
 * @param settings class of settings
 */
void MSSMNoFV_slha_io::set_LToLConversion_settings(const LToLConversion_settings& settings)
{
 ...
 */
void MSSMNoFV_slha_io::set_unitarity_infinite_s(
...
void MSSMNoFV_slha_io::set_effectivecouplings_block(const std::vector<std::tuple<int, int, int, double, std::string>>& effCouplings)
{
   ...
   if (spectrum_generator_settings.get(Spectrum_generator_settings::calculate_observables)) {
      {
         ...
         block << "Block FlexibleSUSYLowEnergy" << '\n'
               << FORMAT_ELEMENT(3, (OBSERVABLES.amm_Fe), "Delta(g-2)/2 of Fe (calculated with FlexibleSUSY)")
 ...
 * @param settings struct of spectrum generator settings to be filled
 */
void MSSMNoFV_slha_io::fill(LToLConversion_settings& settings) const
{
   slha_io.fill(settings);
}
