...
public:
   Eigen::ArrayXd get_parameters(const MSSMNoFV<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const MSSMNoFV<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const MSSMNoFV<T>&);
...
template <class T>
void MSSMNoFV_spectrum_plotter::extract_spectrum(const MSSMNoFV<T>& model)
{
   spectrum.clear();
