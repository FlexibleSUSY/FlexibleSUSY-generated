...
class MSSMNoFV_spectrum_generator
   : public MSSMNoFV_spectrum_generator_interface<T> {
public:
   MSSMNoFV_spectrum_generator()
      ...
      , low_scale(0.)
   {}
   virtual ~MSSMNoFV_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const MSSMNoFV_input_parameters&);
                                ...
                                const MSSMNoFV_input_parameters& input)
{
   ...
   MSSMNoFV_high_scale_constraint<T> high_scale_constraint(&model);
   MSSMNoFV_susy_scale_constraint<T> susy_scale_constraint(&model, qedqcd);
   MSSMNoFV_low_scale_constraint<T>  low_scale_constraint(&model, qedqcd);
 ...
 */
template <class T>
void MSSMNoFV_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   MSSMNoFV_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
