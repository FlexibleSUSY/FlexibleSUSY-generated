...
public:
   ...
   std::tuple<MSSMNoFV<T>> get_models_slha() const
   { return std::make_tuple(MSSMNoFV<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MSSMNoFV<T>& get_model() const
   { return model; }
   MSSMNoFV<T>& get_model()
   { return model; }
   MSSMNoFV<T> get_model_slha() const
   { return MSSMNoFV<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return MSSMNoFV_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(MSSMNoFV_info::parameter_names),
      std::cend(MSSMNoFV_info::parameter_names));
