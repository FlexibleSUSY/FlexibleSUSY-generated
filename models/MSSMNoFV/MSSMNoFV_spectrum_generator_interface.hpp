...
// File generated at Sun 26 Aug 2018 15:20:44
...
template <class T>
class MSSMNoFV_spectrum_generator_interface {
public:
   virtual ~MSSMNoFV_spectrum_generator_interface() = default;

   std::tuple<MSSMNoFV<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<MSSMNoFV_slha<MSSMNoFV<T>>> get_models_slha() const
   { return std::make_tuple(MSSMNoFV_slha<MSSMNoFV<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MSSMNoFV<T>& get_model() const
   { return model; }
   MSSMNoFV<T>& get_model()
   { return model; }
   MSSMNoFV_slha<MSSMNoFV<T>> get_model_slha() const
   { return MSSMNoFV_slha<MSSMNoFV<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MSSMNoFV_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MSSMNoFV_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(MSSMNoFV<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MSSMNoFV_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MSSMNoFV_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MSSMNoFV_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MSSMNoFV_input_parameters& input)
{
 ...
 */
template <class T>
void MSSMNoFV_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MSSMNoFV_mass_eigenstates tmp_model(model);
   try {
   ...
   MSSMNoFV_parameter_getter parameter_getter;
   Coupling_monitor<MSSMNoFV_mass_eigenstates, MSSMNoFV_parameter_getter>
 ...
 */
template <class T>
void MSSMNoFV_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   MSSMNoFV_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void MSSMNoFV_spectrum_generator_interface<T>::translate_exception_to_problem(MSSMNoFV<T>& model)
{
   try {
      throw;
