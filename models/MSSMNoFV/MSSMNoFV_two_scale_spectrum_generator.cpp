...
double MSSMNoFV_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void MSSMNoFV_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MSSMNoFV_input_parameters& input)
{
   ...
   MSSMNoFV_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MSSMNoFV_ewsb_solver<Two_scale> >(ewsb_solver));

   MSSMNoFV_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   MSSMNoFV_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   MSSMNoFV_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   MSSMNoFV_convergence_tester<Two_scale> convergence_tester(
   ...
   MSSMNoFV_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void MSSMNoFV_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MSSMNoFV_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void MSSMNoFV_spectrum_generator<Two_scale>::calculate_spectrum()
{
