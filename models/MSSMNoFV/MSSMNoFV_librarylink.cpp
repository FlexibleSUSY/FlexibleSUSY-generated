...
namespace MSSMNoFV_librarylink {
...
class MSSMNoFV_spectrum {
public:
   virtual ~MSSMNoFV_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSMNoFV_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSMNoFV_observables& get_observables() const = 0;

   virtual const UnitarityInfiniteS& get_unitarity() const = 0;
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMNoFV_input_parameters&) = 0;
...
class MSSMNoFV_spectrum_impl : public MSSMNoFV_spectrum
{
public:
   virtual ~MSSMNoFV_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSMNoFV_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSMNoFV_observables& get_observables() const override { return observables; }

   virtual const UnitarityInfiniteS& get_unitarity() const { return unitarityData; };
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMNoFV_input_parameters&) override;
   virtual void calculate_model_observables(
      const softsusy::QedQcd&,
...
private:
   std::tuple<MSSMNoFV<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSMNoFV_scales scales{};              ///< scale information
   MSSMNoFV_observables observables{};    ///< observables
...
public:
   ...
   void set_input_parameters(const MSSMNoFV_input_parameters& input_) { input = input_; }
...
private:
   ...
   std::unique_ptr<MSSMNoFV_spectrum> spectrum{nullptr}; ///< spectrum information

   MSSMNoFV_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSMSSMNoFVCalculateSpectrum[]?");
   }
}
...
MSSMNoFV_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const MSSMNoFV_slha& model, MLINK link)
{
   MLPutRule(link, MSSMNoFV_info::model_name);
...
template <typename Solver_type>
void MSSMNoFV_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSMNoFV_spectrum_impl<Solver_type>::calculate_spectrum(
...
template <typename Solver_type>
void MSSMNoFV_spectrum_impl<Solver_type>::calculate_model_observables(
   const softsusy::QedQcd& qedqcd,
...
template <typename Solver_type>
void MSSMNoFV_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = MSSMNoFV_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}

/******************************************************************/

template <typename Solver_type>
void MSSMNoFV_spectrum_impl<Solver_type>::fill_slha_io(MSSMNoFV_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   MSSMNoFV_observables observables = spectrum->get_observables();

   MLPutFunction(link, "List", 1);
   MLPutRule(link, MSSMNoFV_info::model_name);
   MLPutFunction(link, "List", 4);

   MLPutRuleTo(link, OBSERVABLE(amm_Fe), "Fe", {"FlexibleSUSYObservable`AMM"});
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMNoFVCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMNoFVCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSMNoFV_spectrum_impl<Two_scale>());
...
{
   using namespace flexiblesusy::MSSMNoFV_librarylink;
...
DLLEXPORT int FSMSSMNoFVGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFV_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFV_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFV_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVGetWarnings"))
      return LIBRARY_TYPE_ERROR;
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSMSSMNoFVCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSMSSMNoFVCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFV_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSMSSMNoFVCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
