...
/**
 * @file MSSMNoFV_FFV_form_factors.cpp
 *
...
std::valarray<std::complex<double>> calculate_Fe_Fe_VP_form_factors (
   ...
   std::array<int, 0> indices1 = {};
   std::array<int, 0> indices2 = {};

   std::valarray<std::complex<double>> val {0.0, 0.0, 0.0, 0.0};

   val += std::complex<double> {1., 0.} * FFV_FFS<Fe,Fe,VP,Cha,Cha,SveL>::value(indices1, indices2, context, discard_SM_contributions);
   ...
   val += std::complex<double> {1., 0.} * FFV_SSF<Fe,Fe,VP,Hpm,Hpm,Fve>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_VSF<Fe,Fe,VP,VWm,Hpm,Fve>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_SSF<Fe,Fe,VP,Se,Se,Chi>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_SVF<Fe,Fe,VP,Hpm,VWm,Fve>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_VVF<Fe,Fe,VP,VWm,VWm,Fve>::value(indices1, indices2, context, discard_SM_contributions);
...
std::valarray<std::complex<double>> calculate_Fm_Fm_VP_form_factors (
   ...
   val += std::complex<double> {1., 0.} * FFV_FFS<Fm,Fm,VP,Cha,Cha,SvmL>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_FFS<Fm,Fm,VP,Fm,Fm,Ah>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_FFS<Fm,Fm,VP,Fm,Fm,hh>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_FFV<Fm,Fm,VP,Fm,Fm,VP>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_FFV<Fm,Fm,VP,Fm,Fm,VZ>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_SSF<Fm,Fm,VP,Hpm,Hpm,Fvm>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_VSF<Fm,Fm,VP,VWm,Hpm,Fvm>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_SSF<Fm,Fm,VP,Sm,Sm,Chi>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_SVF<Fm,Fm,VP,Hpm,VWm,Fvm>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_VVF<Fm,Fm,VP,VWm,VWm,Fvm>::value(indices1, indices2, context, discard_SM_contributions);
...
>
calculate_form_factors(     const MSSMNoFV_mass_eigenstates& model, bool discard_SM_contributions) {
return calculate_Fe_Fe_VP_form_factors(     model, discard_SM_contributions);
}
template std::valarray<std::complex<double>> calculate_form_factors<Fe,Fe,VP>(     const MSSMNoFV_mass_eigenstates&, bool);

template <typename Fj, typename Fi, typename V>
std::enable_if_t<
   std::is_same_v<Fj, Fm> && std::is_same_v<Fi, Fm> && std::is_same_v<V, VP>,
   std::valarray<std::complex<double>>
>
calculate_form_factors(     const MSSMNoFV_mass_eigenstates& model, bool discard_SM_contributions) {
return calculate_Fm_Fm_VP_form_factors(     model, discard_SM_contributions);
}
template std::valarray<std::complex<double>> calculate_form_factors<Fm,Fm,VP>(     const MSSMNoFV_mass_eigenstates&, bool);
