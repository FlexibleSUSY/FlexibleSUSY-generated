...
class NMSSM_spectrum_generator
   : public NMSSM_spectrum_generator_interface<T> {
public:
   NMSSM_spectrum_generator()
      : NMSSM_spectrum_generator_interface<T>()
      ...
      , low_scale(0.)
   {}
   virtual ~NMSSM_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const NMSSM_input_parameters&);
 ...
 */
template <class T>
void NMSSM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const NMSSM_input_parameters& input)
{
   NMSSM<T>& model = this->model;
   model.clear();
   ...
   NMSSM_convergence_tester<T> convergence_tester(&model, this->precision_goal);
   if (this->max_iterations > 0)
      convergence_tester.set_max_iterations(this->max_iterations);

   NMSSM_initial_guesser<T> initial_guesser(&model, qedqcd,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void NMSSM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   NMSSM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
