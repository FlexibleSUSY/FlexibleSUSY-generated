...
template <class T>
class NMSSM_spectrum_generator {
public:
   NMSSM_spectrum_generator()
      : solver(), model()
   ...
   void run(const QedQcd& oneset, const NMSSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "NMSSM_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "NMSSM_spectrum.dat") const;

private:
   RGFlow<T> solver;
   NMSSM<T> model;
   NMSSM_high_scale_constraint<T> high_scale_constraint;
   NMSSM_susy_scale_constraint<T> susy_scale_constraint;
   NMSSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void NMSSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const NMSSM_input_parameters& input)
{
   ...
   NMSSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   NMSSM_initial_guesser<T> initial_guesser(&model, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void NMSSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   NMSSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   NMSSM_parameter_getter parameter_getter;
   Coupling_monitor<NMSSM<T>, NMSSM_parameter_getter>
 ...
 */
template <class T>
void NMSSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
