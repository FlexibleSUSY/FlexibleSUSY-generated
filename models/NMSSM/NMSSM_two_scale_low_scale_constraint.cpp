...
// File generated at Fri 26 Jun 2015 19:15:11
...
NMSSM_low_scale_constraint<Two_scale>::NMSSM_low_scale_constraint(
   NMSSM<Two_scale>* model_, const QedQcd& oneset_)
   : Constraint<Two_scale>()
   , model(model_)
   , oneset(oneset_)
   , new_g1(0.)
...
{
   assert(model && "Error: NMSSM_low_scale_constraint::apply():"
...
void NMSSM_low_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
   model = cast_model<NMSSM<Two_scale>*>(model_);
}

void NMSSM_low_scale_constraint<Two_scale>::set_sm_parameters(const QedQcd& oneset_)
{
   oneset = oneset_;
}

const QedQcd& NMSSM_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return oneset;
}
...
{
   assert(model && "NMSSM_low_scale_constraint<Two_scale>::"
...
double NMSSM_low_scale_constraint<Two_scale>::calculate_theta_w(double alpha_em_drbar)
{
   assert(model && "NMSSM_low_scale_constraint<Two_scale>::"
 ...
 */
void NMSSM_low_scale_constraint<Two_scale>::recalculate_mw_pole()
{
   assert(model && "NMSSM_low_scale_constraint<Two_scale>::"
