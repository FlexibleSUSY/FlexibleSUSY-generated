...
// File generated at Sun 26 Aug 2018 14:47:56
...
double NMSSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void NMSSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const NMSSM_input_parameters& input)
{
   ...
   NMSSM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   NMSSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   NMSSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   NMSSM_convergence_tester<Two_scale> convergence_tester(
   ...
   NMSSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void NMSSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   NMSSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void NMSSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
