...
// File generated at Tue 10 Oct 2017 22:39:45
 ...
 *
 * This file was generated at Tue 10 Oct 2017 22:39:45 with FlexibleSUSY
...
int CLASSNAME::solve_tree_level(NMSSM_mass_eigenstates& model)
{
   ...
   Kappa = Re((0.1*(40*mHd2*vd - 14.142135623730951*vS*vu*Conj(TLambdax) + 3*
      Cube(vd)*Sqr(g1) + 5*Cube(vd)*Sqr(g2) + 20*vd*AbsSqr(Lambdax)*Sqr(vS) + 20*
   ...
   ms2 = Re((0.25*(2*Kappa*vd*vS*vu*Conj(Lambdax) + 1.4142135623730951*vd*vu*
      Conj(TLambdax) + 2*Kappa*vd*vS*vu*Lambdax - 4*Cube(vS)*Sqr(Kappa) - 2*vS*
...
CLASSNAME::EWSB_vector_t CLASSNAME::ewsb_step(const NMSSM_mass_eigenstates& model) const
{
   ...
   vS = Re(0.22360679774997896*LOCALINPUT(SignvS)*Sqrt((40*vd*tadpole[0] - 40*
      vu*tadpole[1] - 3*Quad(vd)*Sqr(g1) + 3*Quad(vu)*Sqr(g1) - 5*Quad(vd)*Sqr(g2)
      + 5*Quad(vu)*Sqr(g2) - 40*mHd2*Sqr(vd) + 40*mHu2*Sqr(vu))/(AbsSqr(Lambdax)*
      Sqr(vd) - AbsSqr(Lambdax)*Sqr(vu))));
   Kappa = Re((0.1*(40*mHd2*vd - 14.142135623730951*vS*vu*Conj(TLambdax) - 40*
      ...
      *Sqr(vS) + 20*vd*AbsSqr(Lambdax)*Sqr(vu) - 3*vd*Sqr(g1)*Sqr(vu) - 5*vd*Sqr(
      ...
      Lambdax)*Sqr(vS)));
   ms2 = Re((0.25*(2*Kappa*vd*vS*vu*Conj(Lambdax) + 1.4142135623730951*vd*vu*
      Conj(TLambdax) + 2*Kappa*vd*vS*vu*Lambdax + 4*tadpole[2] - 4*Cube(vS)*Sqr(
      Kappa) - 2*vS*AbsSqr(Lambdax)*Sqr(vd) - 1.4142135623730951*Conj(TKappa)*Sqr(
      vS) - 2*vS*AbsSqr(Lambdax)*Sqr(vu) - 1.4142135623730951*Sqr(vS)*TKappa +
      1.4142135623730951*vd*vu*TLambdax))/vS);
