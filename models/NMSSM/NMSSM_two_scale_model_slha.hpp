...
/**
 * @file NMSSM_two_scale_model_slha.hpp
 * @brief contains wrapper class for model class in SLHA convention
 */

// File generated at Wed 29 Jun 2016 12:37:05

#ifndef NMSSM_TWO_SCALE_SLHA_H
#define NMSSM_TWO_SCALE_SLHA_H
...
/**
 * @class NMSSM_slha<Two_scale>
 * @brief model class wrapper in SLHA convention
 */

template<>
class NMSSM_slha<Two_scale> : public NMSSM<Two_scale> {
public:
   explicit NMSSM_slha(const NMSSM_input_parameters& input_ = NMSSM_input_parameters());
   explicit NMSSM_slha(const NMSSM<Two_scale>&, bool do_convert_masses_to_slha = true);
   virtual ~NMSSM_slha();
...
std::ostream& operator<<(std::ostream& ostr, const NMSSM_slha<Two_scale>& model);
