...
private:
   ...
   std::array<std::string, NMSSM_info::NUMBER_OF_MIXINGS> get_mixing_names() const {
      return NMSSM_info::particle_mixing_names;
   }

public:
   /// returns DR-bar parameters
   Eigen::ArrayXd get_parameters(const NMSSM_mass_eigenstates& model) {
      return model.get();
   }

   /// returns names of DR-bar parameters
   std::array<std::string, NMSSM_info::NUMBER_OF_PARAMETERS> get_parameter_names() const {
      return NMSSM_info::parameter_names;
   }

   /// returns names of particles
   std::array<std::string, NMSSM_info::NUMBER_OF_PARTICLES> get_particle_names() const {
   ...
   std::array<std::string, NMSSM_info::NUMBER_OF_MIXINGS> get_DRbar_mixing_names() const {
      return get_mixing_names();
   }

   /// returns names of pole mixing matrices
   std::array<std::string, NMSSM_info::NUMBER_OF_MIXINGS> get_pole_mixing_names() const {
   ...
   std::array<std::string, NMSSM_info::NUMBER_OF_INPUT_PARAMETERS> get_input_parameter_names() const {
      return NMSSM_info::input_parameter_names;
   }

   /// returns names of input parameters
   std::array<std::string, NMSSM_info::NUMBER_OF_EXTRA_PARAMETERS> get_extra_parameter_names() const {
      return NMSSM_info::extra_parameter_names;
   }

   /// returns number of particle masses
   decltype(NMSSM_info::NUMBER_OF_MASSES) get_number_of_masses() const {
...
public:
   NMSSM_spectrum_plotter() = default;
   explicit NMSSM_spectrum_plotter(const NMSSM_mass_eigenstates&);
...
void to_database(
   ...
   const NMSSM_observables* observables = nullptr);
...
NMSSM_mass_eigenstates from_database(
   ...
   NMSSM_observables* observables = nullptr);
