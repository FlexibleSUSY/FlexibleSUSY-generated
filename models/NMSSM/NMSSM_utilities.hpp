...
template <class Scalar, int M, int N>
std::valarray<double> NMSSM_spectrum_plotter::to_valarray(const Eigen::Array<Scalar, M, N>& v)
{
...
void to_database(const std::string&, const NMSSM_mass_eigenstates&, const softsusy::QedQcd* qedqcd = 0, const Observables* observables = 0);

/// fill model from an entry of the database
NMSSM_mass_eigenstates from_database(const std::string&, std::size_t, softsusy::QedQcd* qedqcd = 0, Observables* observables = 0);
