...
public:
   Eigen::ArrayXd get_parameters(const NMSSM<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const NMSSM<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const NMSSM<T>&);
...
template <class T>
void NMSSM_spectrum_plotter::extract_spectrum(const NMSSM<T>& model)
{
   spectrum.clear();
