...
// File generated at Mon 23 Feb 2015 13:32:13
...
public:
   ...
   template <class T> void set_extra(const NMSSM_slha<T>&, const NMSSM_scales&);
   ...
   static void convert_to_slha_convention(NMSSM_physical&);

   template <class T>
   static void fill_slhaea(SLHAea::Coll&, const NMSSM_slha<T>&, const QedQcd&, const NMSSM_scales&);

   template <class T>
   static SLHAea::Coll fill_slhaea(const NMSSM_slha<T>&, const QedQcd&);

   template <class T>
   static SLHAea::Coll fill_slhaea(const NMSSM_slha<T>&, const QedQcd&, const NMSSM_scales&);
...
private:
   ...
   template <class T> void fill_drbar_parameters(NMSSM_slha<T>&) const;
   template <class T> void fill_physical(NMSSM_slha<T>&) const;
};
 ...
 */
template <class T>
void NMSSM_slha_io::fill_drbar_parameters(NMSSM_slha<T>& model) const
{
 ...
 */
template <class T>
void NMSSM_slha_io::fill_physical(NMSSM_slha<T>& model) const
{
   {
      DEFINE_PARAMETER(ZD);
...
void NMSSM_slha_io::fill_slhaea(
   ...
   const QedQcd& qedqcd, const NMSSM_scales& scales)
{
...
SLHAea::Coll NMSSM_slha_io::fill_slhaea(
   const NMSSM_slha<T>& model, const QedQcd& qedqcd)
{
   NMSSM_scales scales;
...
SLHAea::Coll NMSSM_slha_io::fill_slhaea(
   const NMSSM_slha<T>& model, const QedQcd& qedqcd,
   const NMSSM_scales& scales)
{
   SLHAea::Coll slhaea;
   NMSSM_slha_io::fill_slhaea(slhaea, model, qedqcd, scales);
