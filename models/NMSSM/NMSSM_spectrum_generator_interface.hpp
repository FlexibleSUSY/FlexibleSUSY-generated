...
// File generated at Wed 16 Oct 2019 22:23:04

#ifndef NMSSM_SPECTRUM_GENERATOR_INTERFACE_H
#define NMSSM_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class NMSSM_spectrum_generator_interface {
public:
   virtual ~NMSSM_spectrum_generator_interface() = default;

   std::tuple<NMSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<NMSSM_slha<NMSSM<T>>> get_models_slha() const
   { return std::make_tuple(NMSSM_slha<NMSSM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const NMSSM<T>& get_model() const
   { return model; }
   NMSSM<T>& get_model()
   { return model; }
   NMSSM_slha<NMSSM<T>> get_model_slha() const
   { return NMSSM_slha<NMSSM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const NMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "NMSSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(NMSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const NMSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void NMSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void NMSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const NMSSM_input_parameters& input)
{
 ...
 */
template <class T>
void NMSSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   NMSSM_mass_eigenstates tmp_model(model);
   try {
   ...
   NMSSM_parameter_getter parameter_getter;
   Coupling_monitor<NMSSM_mass_eigenstates, NMSSM_parameter_getter>
 ...
 */
template <class T>
void NMSSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   NMSSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void NMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(NMSSM<T>& model)
{
   try {
      throw;
