...
// File generated at Sat 15 Oct 2016 15:46:45
...
public:
   NMSSM_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~NMSSM_spectrum_generator_interface() {}

   const NMSSM<T>& get_model() const { return model; }
   NMSSM<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const NMSSM_input_parameters&) = 0;
 ...
 */
template <class T>
void NMSSM_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   NMSSM_spectrum_plotter plotter;
...
void NMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(NMSSM<T>& model)
{
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : NMSSM_info::parameter_names[parameter_index];
