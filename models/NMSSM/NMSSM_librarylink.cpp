...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSNMSSMMessage", 1);
...
class NMSSM_spectrum {
public:
   virtual ~NMSSM_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(NMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const NMSSM_observables& get_observables() const = 0;
   virtual const NMSSM_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const NMSSM_input_parameters&) = 0;
...
class NMSSM_spectrum_impl : public NMSSM_spectrum
{
public:
   virtual ~NMSSM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(NMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const NMSSM_observables& get_observables() const override { return observables; }
   virtual const NMSSM_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const NMSSM_input_parameters&) override;
...
private:
   std::tuple<NMSSM<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   NMSSM_scales scales{};              ///< scale information
   NMSSM_observables observables{};    ///< observables
   NMSSM_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const NMSSM_input_parameters& input_) { input = input_; }
...
private:
   NMSSM_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<NMSSM_spectrum> spectrum{nullptr};  ///< spectrum information

   NMSSM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSNMSSMCalculateSpectrum[]?");
   }
}
...
NMSSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const NMSSM_slha& model, MLINK link)
{
   MLPutRule(link, NMSSM_info::model_name);
...
template <typename Solver_type>
void NMSSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void NMSSM_spectrum_impl<Solver_type>::calculate_spectrum(
...
{
   NMSSM_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void NMSSM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void NMSSM_spectrum_impl<Solver_type>::calculate_model_decays(
   const softsusy::QedQcd& qedqcd, const Physical_input& physical_input, const FlexibleDecay_settings& flexibledecay_settings)
{
   decays = NMSSM_decays(std::get<0>(models), qedqcd, physical_input, flexibledecay_settings);
...
template <typename Solver_type>
void NMSSM_spectrum_impl<Solver_type>::fill_slha_io(NMSSM_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   NMSSM_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   NMSSM_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = NMSSM_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSNMSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSNMSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new NMSSM_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSNMSSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMGetSettings"))
...
DLLEXPORT int FSNMSSMGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMOpenHandle(
...
DLLEXPORT int FSNMSSMCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::NMSSM_librarylink;
...
DLLEXPORT int FSNMSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSNMSSMCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSNMSSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSNMSSMCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSNMSSMCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSNMSSMCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSNMSSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSNMSSMCalculateDecays", "error", e.what());
      put_error_output(link);
   }
