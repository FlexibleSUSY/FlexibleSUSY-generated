...
template <typename Solver_type>
void NMSSM_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = NMSSM_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSNMSSMCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSNMSSMCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
