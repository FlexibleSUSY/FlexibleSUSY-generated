...
#define NMSSM_MASS_EIGENSTATES_H
 ...
 */
class NMSSM_mass_eigenstates : public NMSSM_soft_parameters {
public:
   explicit NMSSM_mass_eigenstates(const NMSSM_input_parameters& input_ = NMSSM_input_parameters());
   virtual ~NMSSM_mass_eigenstates();
   ...
   void set_physical(const NMSSM_physical&);
   double get_ewsb_iteration_precision() const;
   double get_ewsb_loop_order() const;
   const NMSSM_physical& get_physical() const;
   NMSSM_physical& get_physical();
   const Problems<NMSSM_info::NUMBER_OF_PARTICLES>& get_problems() const;
   Problems<NMSSM_info::NUMBER_OF_PARTICLES>& get_problems();
   ...
   double get_lsp(NMSSM_info::Particles&) const;
...
std::ostream& operator<<(std::ostream&, const NMSSM_mass_eigenstates&);
