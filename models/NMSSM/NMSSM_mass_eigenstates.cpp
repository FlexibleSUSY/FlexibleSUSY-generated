...
// File generated at Wed 12 Apr 2017 12:58:09
 ...
 * parameters.
 *
 * This file was generated at Wed 12 Apr 2017 12:58:09 with FlexibleSUSY
...
CLASSNAME::NMSSM_mass_eigenstates(const NMSSM_input_parameters& input_)
   ...
   , physical()
   , problems(NMSSM_info::particle_names)
...
CLASSNAME::~NMSSM_mass_eigenstates()
{
}
...
const Problems<NMSSM_info::NUMBER_OF_PARTICLES>& CLASSNAME::get_problems() const
{
   return problems;
}

Problems<NMSSM_info::NUMBER_OF_PARTICLES>& CLASSNAME::get_problems()
{
   return problems;
}
...
int CLASSNAME::tadpole_equations(const gsl_vector* x, void* params, gsl_vector* f)
{
   ...
   NMSSM_mass_eigenstates* model = ewsb_args->model;
...
)
{
   ...
   vS = LOCALINPUT(SignvS)*Abs(solver->get_solution(1));
...
int CLASSNAME::solve_ewsb_tree_level()
{
   ...
   vS = Re(0.22360679774997896*LOCALINPUT(SignvS)*Sqrt((-3*Power(vd,4)*Sqr(g1)
      + 3*Power(vu,4)*Sqr(g1) - 5*Power(vd,4)*Sqr(g2) + 5*Power(vu,4)*Sqr(g2) - 40
      *mHd2*Sqr(vd) + 40*mHu2*Sqr(vu))/(AbsSqr(Lambdax)*Sqr(vd) - AbsSqr(Lambdax)*
      Sqr(vu))));
   Kappa = Re((0.1*(40*mHd2*vd - 14.142135623730951*vS*vu*Conj(TLambdax) + 3*
   ...
   ms2 = Re((0.25*(1.4142135623730951*vd*vu*Conj(TLambdax) + 2*vd*vS*vu*Conj(
...
Eigen::Matrix<double, CLASSNAME::number_of_ewsb_equations, 1> CLASSNAME::ewsb_step() const
{
   ...
   vS = Re(0.22360679774997896*LOCALINPUT(SignvS)*Sqrt((40*vd*tadpole[0] - 40*
      vu*tadpole[1] - 3*Power(vd,4)*Sqr(g1) + 3*Power(vu,4)*Sqr(g1) - 5*Power(vd,4
      )*Sqr(g2) + 5*Power(vu,4)*Sqr(g2) - 40*mHd2*Sqr(vd) + 40*mHu2*Sqr(vu))/(
      AbsSqr(Lambdax)*Sqr(vd) - AbsSqr(Lambdax)*Sqr(vu))));
...
{
   if (PHYSICAL(MSd).tail<6>().minCoeff() < 0.) problems.flag_tachyon(NMSSM_info::Sd);
   if (PHYSICAL(MSv).tail<3>().minCoeff() < 0.) problems.flag_tachyon(NMSSM_info::Sv);
   if (PHYSICAL(MSu).tail<6>().minCoeff() < 0.) problems.flag_tachyon(NMSSM_info::Su);
   if (PHYSICAL(MSe).tail<6>().minCoeff() < 0.) problems.flag_tachyon(NMSSM_info::Se);
   if (PHYSICAL(Mhh).tail<3>().minCoeff() < 0.) problems.flag_tachyon(NMSSM_info::hh);
   if (PHYSICAL(MAh).tail<2>().minCoeff() < 0.) problems.flag_tachyon(NMSSM_info::Ah);
   if (PHYSICAL(MHpm).tail<1>().minCoeff() < 0.) problems.flag_tachyon(NMSSM_info::Hpm);
...
void CLASSNAME::calculate_MSd()
{
   ...
   if (MSd.minCoeff() < 0.) {
      problems.flag_tachyon(NMSSM_info::Sd);
   }
...
void CLASSNAME::calculate_MSv()
{
   ...
   if (MSv.minCoeff() < 0.) {
      problems.flag_tachyon(NMSSM_info::Sv);
   }
...
void CLASSNAME::calculate_MSu()
{
   ...
   if (MSu.minCoeff() < 0.) {
      problems.flag_tachyon(NMSSM_info::Su);
   }
...
void CLASSNAME::calculate_MSe()
{
   ...
   if (MSe.minCoeff() < 0.) {
      problems.flag_tachyon(NMSSM_info::Se);
   }
...
void CLASSNAME::calculate_Mhh()
{
   ...
   if (Mhh.minCoeff() < 0.) {
      problems.flag_tachyon(NMSSM_info::hh);
   }
...
void CLASSNAME::calculate_MAh()
{
   ...
   if (MAh.minCoeff() < 0.) {
      problems.flag_tachyon(NMSSM_info::Ah);
   }
...
void CLASSNAME::calculate_MHpm()
{
   ...
   if (MHpm.minCoeff() < 0.) {
      problems.flag_tachyon(NMSSM_info::Hpm);
   }
...
void CLASSNAME::calculate_MVWm()
{
   ...
   if (MVWm < 0.) {
      problems.flag_tachyon(NMSSM_info::VWm);
   }
...
{
   if (!force_output && problems.is_tachyon(NMSSM_info::VZ))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_tachyon(NMSSM_info::VZ);
...
{
   if (!force_output && problems.is_tachyon(NMSSM_info::Sd))
      return;
...
{
   if (!force_output && problems.is_tachyon(NMSSM_info::Sv))
      return;
...
{
   if (!force_output && problems.is_tachyon(NMSSM_info::Su))
      return;
...
{
   if (!force_output && problems.is_tachyon(NMSSM_info::Se))
      return;
...
{
   if (!force_output && problems.is_tachyon(NMSSM_info::hh))
      return;
...
{
   if (!force_output && problems.is_tachyon(NMSSM_info::Ah))
      return;
...
{
   if (!force_output && problems.is_tachyon(NMSSM_info::Hpm))
      return;
...
{
   if (!force_output && problems.is_tachyon(NMSSM_info::VWm))
      return;
