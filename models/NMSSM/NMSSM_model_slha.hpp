...
class NMSSM_slha : public Model {
public:
   explicit NMSSM_slha(const NMSSM_input_parameters& input_ = NMSSM_input_parameters());
   explicit NMSSM_slha(const Model&, bool do_convert_masses_to_slha = true);
   NMSSM_slha(const NMSSM_slha&) = default;
   NMSSM_slha(NMSSM_slha&&) = default;
   virtual ~NMSSM_slha() = default;
   NMSSM_slha& operator=(const NMSSM_slha&) = default;
   NMSSM_slha& operator=(NMSSM_slha&&) = default;
   ...
   const NMSSM_physical& get_physical_slha() const; ///< returns pole masses to SLHA convention
   NMSSM_physical& get_physical_slha(); ///< returns pole masses to SLHA convention
...
private:
   NMSSM_physical physical_slha{}; ///< contains the pole masses and mixings in slha convention
...
template <class Model>
NMSSM_slha<Model>::NMSSM_slha(const NMSSM_input_parameters& input_)
   : Model(input_)
{
}
 ...
 */
template <class Model>
NMSSM_slha<Model>::NMSSM_slha(const Model& model_,
...
template <class Model>
void NMSSM_slha<Model>::calculate_spectrum()
{
...
template <class Model>
void NMSSM_slha<Model>::convert_to_slha()
{
...
template <class Model>
void NMSSM_slha<Model>::calculate_ckm_matrix()
{
...
template <class Model>
void NMSSM_slha<Model>::calculate_pmns_matrix()
{
 ...
 */
template <class Model>
void NMSSM_slha<Model>::convert_yukawa_couplings_to_slha()
{
 ...
 */
template <class Model>
void NMSSM_slha<Model>::convert_trilinear_couplings_to_slha()
{
 ...
 */
template <class Model>
void NMSSM_slha<Model>::convert_soft_squared_masses_to_slha()
{
...
template <class Model>
const NMSSM_physical& NMSSM_slha<Model>::get_physical_slha() const
{
   return physical_slha;
}

template <class Model>
NMSSM_physical& NMSSM_slha<Model>::get_physical_slha()
{
   return physical_slha;
}

template <class Model>
void NMSSM_slha<Model>::print(std::ostream& ostr) const
{
   Model::print(ostr);
...
template <class Model>
void NMSSM_slha<Model>::set_convert_masses_to_slha(bool flag)
{
   convert_masses_to_slha = flag;
}

template <class Model>
std::ostream& operator<<(std::ostream& ostr, const NMSSM_slha<Model>& model)
{
   model.print(ostr);
   return ostr;
}
