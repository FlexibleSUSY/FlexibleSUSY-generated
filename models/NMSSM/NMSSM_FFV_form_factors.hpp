...
namespace NMSSM_FFV_form_factors {

std::valarray<std::complex<double>> calculate_Fe_Fe_VP_form_factors (
   int generationIndex1, int generationIndex2,
   const NMSSM_mass_eigenstates& model, bool discard_SM_contributions);

template <typename Fj, typename Fi, typename V>
std::enable_if_t<
   std::is_same_v<Fj, NMSSM_cxx_diagrams::fields::Fe> && std::is_same_v<Fi, NMSSM_cxx_diagrams::fields::Fe> && std::is_same_v<V, NMSSM_cxx_diagrams::fields::VP>,
   std::valarray<std::complex<double>>
>
calculate_form_factors(   int, int,
const NMSSM_mass_eigenstates&, bool);
