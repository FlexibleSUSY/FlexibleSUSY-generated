...
#define CLASSNAME NMSSM_susy_parameters
#define INPUT(parameter) input.parameter
#define TRACE_STRUCT susy_traces
#define TRACE_STRUCT_TYPE Susy_traces
#define CALCULATE_TRACES(l) calc_susy_traces(l);

const int NMSSM_susy_parameters::numberOfParameters;

NMSSM_susy_parameters::NMSSM_susy_parameters(const NMSSM_input_parameters& input_)
   : input(input_)
{
   set_number_of_parameters(numberOfParameters);
}

NMSSM_susy_parameters::NMSSM_susy_parameters(
   double scale_, int loops_, int thresholds_,
   const NMSSM_input_parameters& input_
...
Eigen::ArrayXd NMSSM_susy_parameters::beta() const
{
   return calc_beta().get().unaryExpr(Chop<double>(get_zero_threshold()));
}

NMSSM_susy_parameters NMSSM_susy_parameters::calc_beta(int loops) const
{
   ...
   return NMSSM_susy_parameters(get_scale(), loops, get_thresholds(), input,
                    beta_Yd, beta_Ye, beta_Lambdax, beta_Kappa, beta_Yu, beta_g1, beta_g2, beta_g3, beta_vd, beta_vu, beta_vS);
}

NMSSM_susy_parameters NMSSM_susy_parameters::calc_beta() const
{
   return calc_beta(get_loops());
}

void NMSSM_susy_parameters::clear()
{
   reset();
...
Eigen::ArrayXd NMSSM_susy_parameters::get() const
{
...
void NMSSM_susy_parameters::print(std::ostream& ostr) const
{
...
void NMSSM_susy_parameters::set(const Eigen::ArrayXd& pars)
{
   Yd(0,0) = pars(0);
   Yd(0,1) = pars(1);
   Yd(0,2) = pars(2);
...
const NMSSM_input_parameters& NMSSM_susy_parameters::get_input() const
{
   return input;
}

NMSSM_input_parameters& NMSSM_susy_parameters::get_input()
{
   return input;
}

void NMSSM_susy_parameters::set_input_parameters(const NMSSM_input_parameters& input_)
{
   input = input_;
}

NMSSM_susy_parameters::Susy_traces NMSSM_susy_parameters::calc_susy_traces(int loops) const
{
...
std::ostream& operator<<(std::ostream& ostr, const NMSSM_susy_parameters& susy_pars)
{
