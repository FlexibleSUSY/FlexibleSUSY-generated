...
// File generated at Sun 18 Oct 2015 12:56:09
...
{
   assert(model && "Error: NMSSM_high_scale_constraint::apply():"
          " model pointer must not be zero");

   if (std::fabs(model->get_g1()) > 3.54491) {
#ifdef ENABLE_VERBOSE
      ERROR("NMSSM_high_scale_constraint: Non-perturbative gauge "
...
void NMSSM_high_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
...
{
   assert(model && "NMSSM_high_scale_constraint<Two_scale>::"
...
void NMSSM_high_scale_constraint<Two_scale>::update_scale()
{
   ...
   const NMSSM_soft_parameters beta_functions(model->calc_beta());
   ...
   if (errno == ERANGE) {
#ifdef ENABLE_VERBOSE
      ERROR("NMSSM_high_scale_constraint<Two_scale>: Overflow error"
