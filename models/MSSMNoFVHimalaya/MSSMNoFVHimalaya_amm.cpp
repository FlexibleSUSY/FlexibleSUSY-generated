...
#include "cxx_qft/MSSMNoFVHimalaya_particle_aliases.hpp"
...
double get_MLCP(const MSSMNoFVHimalaya_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(MSSMNoFVHimalaya_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSMNoFVHimalaya_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSMNoFVHimalaya_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSMNoFVHimalaya_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSMNoFVHimalaya_amm: run_to_MSUSY(): "
...
template <typename Lepton>
std::enable_if_t<std::is_same_v<Lepton, MSSMNoFVHimalaya_cxx_diagrams::fields::Fm>, double>
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const MSSMNoFVHimalaya_mass_eigenstates& model, double ml_pole)
{
   using namespace MSSMNoFVHimalaya_cxx_diagrams::fields;

   const auto form_factors = MSSMNoFVHimalaya_FFV_form_factors::calculate_form_factors<Lepton,Lepton,VP>(model, true);
...
template <typename Lepton>
double calculate_amm_impl(const MSSMNoFVHimalaya_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings)
{
   VERBOSE_MSG("MSSMNoFVHimalaya_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double MSSMNoFVHimalaya_amm::calculate_amm(const MSSMNoFVHimalaya_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings)
{
   MSSMNoFVHimalaya_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMNoFVHimalaya_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMNoFVHimalaya_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template double MSSMNoFVHimalaya_amm::calculate_amm<MSSMNoFVHimalaya_cxx_diagrams::fields::Fm>(const MSSMNoFVHimalaya_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&);

template <typename Lepton>
double MSSMNoFVHimalaya_amm::calculate_amm_uncertainty(const MSSMNoFVHimalaya_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings)
{
   MSSMNoFVHimalaya_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMNoFVHimalaya_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMNoFVHimalaya_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
// spliting templated function into header and cpp
// https://isocpp.org/wiki/faq/templates#separate-template-fn-defn-from-decl
template double MSSMNoFVHimalaya_amm::calculate_amm_uncertainty<MSSMNoFVHimalaya_cxx_diagrams::fields::Fm>(const MSSMNoFVHimalaya_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&);
