...
#include "MSSMNoFVHimalaya_two_scale_spectrum_generator.hpp"
#endif
...
#include "lowe.h"
#include "MSSMNoFVHimalaya_unitarity.hpp"
...
namespace MSSMNoFVHimalaya_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSMNoFVHimalayaMessage", 1);
...
class MSSMNoFVHimalaya_spectrum {
public:
   virtual ~MSSMNoFVHimalaya_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSMNoFVHimalaya_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSMNoFVHimalaya_observables& get_observables() const = 0;

   virtual const UnitarityInfiniteS& get_unitarity() const = 0;
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMNoFVHimalaya_input_parameters&) = 0;
...
class MSSMNoFVHimalaya_spectrum_impl : public MSSMNoFVHimalaya_spectrum
{
public:
   virtual ~MSSMNoFVHimalaya_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSMNoFVHimalaya_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSMNoFVHimalaya_observables& get_observables() const override { return observables; }

   virtual const UnitarityInfiniteS& get_unitarity() const { return unitarityData; };
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMNoFVHimalaya_input_parameters&) override;
   virtual void calculate_model_observables(
      const softsusy::QedQcd&,
...
private:
   std::tuple<MSSMNoFVHimalaya<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSMNoFVHimalaya_scales scales{};              ///< scale information
   MSSMNoFVHimalaya_observables observables{};    ///< observables
...
public:
   ...
   void set_input_parameters(const MSSMNoFVHimalaya_input_parameters& input_) { input = input_; }
...
private:
   MSSMNoFVHimalaya_input_parameters input{};                    ///< model input parameters
   ...
   std::unique_ptr<MSSMNoFVHimalaya_spectrum> spectrum{nullptr}; ///< spectrum information

   MSSMNoFVHimalaya_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSMSSMNoFVHimalayaCalculateSpectrum[]?");
   }
}
...
MSSMNoFVHimalaya_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   MSSMNoFVHimalaya_slha_io slha_io;
...
void put_spectrum(const MSSMNoFVHimalaya_slha& model, MLINK link)
{
   MLPutRule(link, MSSMNoFVHimalaya_info::model_name);
...
template <typename Solver_type>
void MSSMNoFVHimalaya_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSMNoFVHimalaya_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MSSMNoFVHimalaya_input_parameters& input
)
{
   MSSMNoFVHimalaya_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void MSSMNoFVHimalaya_spectrum_impl<Solver_type>::calculate_model_observables(
   const softsusy::QedQcd& qedqcd,
...
template <typename Solver_type>
void MSSMNoFVHimalaya_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = MSSMNoFVHimalaya_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}

/******************************************************************/

template <typename Solver_type>
void MSSMNoFVHimalaya_spectrum_impl<Solver_type>::fill_slha_io(MSSMNoFVHimalaya_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   MSSMNoFVHimalaya_observables observables = spectrum->get_observables();
...
void Model_data::put_unitarity(MLINK link) const
{
   ...
   MLPutFunction(link, "List", 1);
   MLPutRule(link, "MSSMNoFVHimalaya");
   MLPutFunction(link, "List", 3);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMNoFVHimalayaCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMNoFVHimalayaCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSMNoFVHimalaya_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   MSSMNoFVHimalaya_input_parameters input;
...
DLLEXPORT int FSMSSMNoFVHimalayaGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVHimalayaGetSettings"))
...
DLLEXPORT int FSMSSMNoFVHimalayaGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVHimalayaGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVHimalayaGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVHimalayaGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVHimalayaOpenHandle(
...
DLLEXPORT int FSMSSMNoFVHimalayaCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MSSMNoFVHimalaya_librarylink;
...
DLLEXPORT int FSMSSMNoFVHimalayaSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::MSSMNoFVHimalaya_librarylink;
...
DLLEXPORT int FSMSSMNoFVHimalayaGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVHimalayaGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVHimalayaToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVHimalayaToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVHimalayaGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVHimalayaGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVHimalayaCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVHimalayaCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVHimalayaCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVHimalayaCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMNoFVHimalayaCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMNoFVHimalayaCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMNoFVHimalayaCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSMSSMNoFVHimalayaCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVHimalayaCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSMSSMNoFVHimalayaCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
