...
/**
 * @file MSSMNoFVHimalaya_mass_eigenstates_decoupling_scheme.cpp
 * @brief implementation of the MSSMNoFVHimalaya model class in the decoupling scheme
 *
...
#include "MSSMNoFVHimalaya_mass_eigenstates_decoupling_scheme.hpp"
...
#define CLASSNAME MSSMNoFVHimalaya_mass_eigenstates_decoupling_scheme
...
CLASSNAME::CLASSNAME(const MSSMNoFVHimalaya_mass_eigenstates& model)
{
   fill_from(model);
}
...
{
   return std::make_unique<MSSMNoFVHimalaya_mass_eigenstates_decoupling_scheme>(*this);
}
   ...
   // calculate new gauge couplings
   {
      ...
      if (IsFinite(new_g1)) {
         ...
         model->get_problems().flag_non_perturbative_parameter(
            MSSMNoFVHimalaya_info::g1, new_g1, get_scale());
...
void CLASSNAME::fill_from(const MSSMNoFVHimalaya_mass_eigenstates& model)
{
   set(model.get());
...
std::ostream& operator<<(std::ostream& ostr, const MSSMNoFVHimalaya_mass_eigenstates_decoupling_scheme& model)
{
   model.print(ostr);
   return ostr;
}
