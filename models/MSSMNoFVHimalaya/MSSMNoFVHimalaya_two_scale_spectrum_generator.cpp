...
double MSSMNoFVHimalaya_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void MSSMNoFVHimalaya_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MSSMNoFVHimalaya_input_parameters& input)
{
   ...
   MSSMNoFVHimalaya_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MSSMNoFVHimalaya_ewsb_solver<Two_scale> >(ewsb_solver));

   MSSMNoFVHimalaya_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   MSSMNoFVHimalaya_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   MSSMNoFVHimalaya_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   MSSMNoFVHimalaya_convergence_tester<Two_scale> convergence_tester(
   ...
   MSSMNoFVHimalaya_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void MSSMNoFVHimalaya_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MSSMNoFVHimalaya_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void MSSMNoFVHimalaya_spectrum_generator<Two_scale>::calculate_spectrum()
{
