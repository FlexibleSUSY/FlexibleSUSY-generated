...
// File generated at Sun 26 Aug 2018 13:50:21

/**
 * @file MSSMNoFVHimalaya_a_muon.cpp
 *
 * This file was generated at Sun 26 Aug 2018 13:50:21 with FlexibleSUSY
 * 2.2.0 and SARAH 4.13.0 .
 */

#include "MSSMNoFVHimalaya_a_muon.hpp"
#include "MSSMNoFVHimalaya_mass_eigenstates.hpp"

#include "MSSMNoFVHimalaya_cxx_diagrams.hpp"
...
double get_MSUSY(const MSSMNoFVHimalaya_mass_eigenstates& model)
{
...
void run_to_MSUSY(MSSMNoFVHimalaya_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSMNoFVHimalaya_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSMNoFVHimalaya_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSMNoFVHimalaya_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSMNoFVHimalaya_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(MSSMNoFVHimalaya_mass_eigenstates& model)
{
   VERBOSE_MSG("MSSMNoFVHimalaya_a_muon: calculating a_mu at Q = " << model.get_scale());
...
std::pair<double,double> vary_scale(const MSSMNoFVHimalaya_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("MSSMNoFVHimalaya_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double MSSMNoFVHimalaya_a_muon::calculate_a_muon(const MSSMNoFVHimalaya_mass_eigenstates& model_)
{
   MSSMNoFVHimalaya_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMNoFVHimalaya_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMNoFVHimalaya_a_muon:" << e.what());
...
double MSSMNoFVHimalaya_a_muon::calculate_a_muon_uncertainty(const MSSMNoFVHimalaya_mass_eigenstates& model_)
{
   MSSMNoFVHimalaya_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMNoFVHimalaya_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMNoFVHimalaya_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
