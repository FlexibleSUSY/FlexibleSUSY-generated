...
public:
   ...
   std::tuple<MSSMNoFVHimalaya<T>> get_models_slha() const
   { return std::make_tuple(MSSMNoFVHimalaya<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MSSMNoFVHimalaya<T>& get_model() const
   { return model; }
   MSSMNoFVHimalaya<T>& get_model()
   { return model; }
   MSSMNoFVHimalaya<T> get_model_slha() const
   { return MSSMNoFVHimalaya<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return MSSMNoFVHimalaya_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(MSSMNoFVHimalaya_info::parameter_names),
      std::cend(MSSMNoFVHimalaya_info::parameter_names));
