...
// MRSSM_parameter_getter ////////////////////////////////////////

std::vector<std::string> MRSSM_parameter_getter::get_mass_names(const std::string& head)
{
...
std::array<std::string, MRSSM_info::NUMBER_OF_MIXINGS> MRSSM_parameter_getter::get_mixing_names()
{
   return MRSSM_info::particle_mixing_names;
}

Eigen::ArrayXd MRSSM_parameter_getter::get_parameters(const MRSSM_mass_eigenstates& model)
{
   return model.get();
}

std::array<std::string, MRSSM_info::NUMBER_OF_PARAMETERS> MRSSM_parameter_getter::get_parameter_names()
{
   return MRSSM_info::parameter_names;
}

std::array<std::string, MRSSM_info::NUMBER_OF_PARTICLES> MRSSM_parameter_getter::get_particle_names()
{
   return MRSSM_info::particle_names;
}

std::vector<std::string> MRSSM_parameter_getter::get_DRbar_mass_names()
{
   return get_mass_names();
}

std::vector<std::string> MRSSM_parameter_getter::get_pole_mass_names()
{
   return get_mass_names("Pole");
}

std::array<std::string, MRSSM_info::NUMBER_OF_MIXINGS> MRSSM_parameter_getter::get_DRbar_mixing_names()
{
   return get_mixing_names();
}

std::array<std::string, MRSSM_info::NUMBER_OF_MIXINGS> MRSSM_parameter_getter::get_pole_mixing_names()
{
...
std::array<std::string, MRSSM_info::NUMBER_OF_INPUT_PARAMETERS> MRSSM_parameter_getter::get_input_parameter_names()
{
   return MRSSM_info::input_parameter_names;
}

std::array<std::string, MRSSM_info::NUMBER_OF_EXTRA_PARAMETERS> MRSSM_parameter_getter::get_extra_parameter_names()
{
   return MRSSM_info::extra_parameter_names;
}

decltype(MRSSM_info::NUMBER_OF_MASSES) MRSSM_parameter_getter::get_number_of_masses()
{
   return MRSSM_info::NUMBER_OF_MASSES;
}

// MRSSM_spectrum_plotter ////////////////////////////////////////
...
void MRSSM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("MRSSM_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("MRSSM_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("MRSSM_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void MRSSM_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("MRSSM_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const MRSSM_mass_eigenstates& model,
   ...
   const MRSSM_observables* observables)
{
   ...
   append(names, MRSSM_parameter_getter::get_input_parameter_names());
   ...
   if (MRSSM_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, MRSSM_observables::get_names());
