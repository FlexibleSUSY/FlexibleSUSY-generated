...
// File generated at Mon 23 Feb 2015 12:38:59
...
#define CLASSNAME MRSSM_susy_parameters
#define INPUT(parameter) input.parameter
#define TRACE_STRUCT susy_traces

MRSSM_susy_parameters::MRSSM_susy_parameters(const MRSSM_input_parameters& input_)
   : Beta_function()
...
Eigen::ArrayXd MRSSM_susy_parameters::beta() const
{
   return calc_beta().get();
}

MRSSM_susy_parameters MRSSM_susy_parameters::calc_beta() const
{
   ...
   double beta_LamTD(calc_beta_LamTD_one_loop(TRACE_STRUCT));
   double beta_LamTU(calc_beta_LamTU_one_loop(TRACE_STRUCT));
   double beta_LamSD(calc_beta_LamSD_one_loop(TRACE_STRUCT));
   double beta_LamSU(calc_beta_LamSU_one_loop(TRACE_STRUCT));
   Eigen::Matrix<double,3,3> beta_Yu(calc_beta_Yu_one_loop(TRACE_STRUCT));
   double beta_Mu(calc_beta_Mu_one_loop(TRACE_STRUCT));
   double beta_MuD(calc_beta_MuD_one_loop(TRACE_STRUCT));
   double beta_MuU(calc_beta_MuU_one_loop(TRACE_STRUCT));
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SlSl() const
{
   ...
   if (get_loops() > 1) {
      anomDim += twoLoop*(-2*(Ye.adjoint()*Ye*Ye.adjoint()*Ye) +
         ...
         Power(g1,4) + 55*Power(g2,4) + 6*Sqr(g1)*Sqr(g2))*UNITMATRIX(3));
   }
...
double CLASSNAME::get_SHdSHd() const
{
   ...
   anomDim = oneOver16PiSqr*(AbsSqr(LamSD) + 1.5*AbsSqr(LamTD) - 0.3*Sqr(
      g1) - 1.5*Sqr(g2) + 3*(Yd*Yd.adjoint()).trace() + (Ye*Ye.adjoint()).trace
      ());

   if (get_loops() > 1) {
      anomDim += twoLoop*(2.25*Power(g1,4) + 8.25*Power(g2,4) - AbsSqr
         (LamSD)*(2*AbsSqr(LamSU) + 3*AbsSqr(LamTD)) + 0.9*Sqr(g1)*Sqr(g2) +
         Conj(LamTD)*(-1.5*LamTD*AbsSqr(LamTU) + 6*LamTD*Sqr(g2)) - 3*Sqr(LamSD
         )*Sqr(Conj(LamSD)) - 3.75*Sqr(LamTD)*Sqr(Conj(LamTD)) - 0.4*Sqr(g1)*(
         Yd*Yd.adjoint()).trace() + 16*Sqr(g3)*(Yd*Yd.adjoint()).trace() + 1.2*
         Sqr(g1)*(Ye*Ye.adjoint()).trace() - 9*(Yd*Yd.adjoint()*Yd*Yd.adjoint()
         ).trace() - 3*(Yd*Yu.adjoint()*Yu*Yd.adjoint()).trace() - 3*(Ye*
         Ye.adjoint()*Ye*Ye.adjoint()).trace());
   }
...
double CLASSNAME::get_SHuSHu() const
{
   ...
   anomDim = oneOver16PiSqr*(AbsSqr(LamSU) - 0.3*(-5*AbsSqr(LamTU) + Sqr(
      g1) + 5*Sqr(g2) - 10*(Yu*Yu.adjoint()).trace()));

   if (get_loops() > 1) {
      anomDim += twoLoop*(2.25*Power(g1,4) + 8.25*Power(g2,4) - 2*
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SuRSuR() const
{
   ...
   if (get_loops() > 1) {
      anomDim += twoLoop*(-2*(Yu.conjugate()*Yd.transpose()*
         ...
         Power(g3,4) + 20*Sqr(g1)*Sqr(g3))*UNITMATRIX(3));
   }
...
double CLASSNAME::get_SsSs() const
{
   ...
   anomDim = 2*oneOver16PiSqr*(AbsSqr(LamSD) + AbsSqr(LamSU));

   if (get_loops() > 1) {
      anomDim += twoLoop*(-4*Sqr(LamSD)*Sqr(Conj(LamSD)) + Conj(LamSD)
         *(-6*LamSD*AbsSqr(LamTD) + 1.2*LamSD*Sqr(g1) + 6*LamSD*Sqr(g2) - 6*
         LamSD*(Yd*Yd.adjoint()).trace() - 2*LamSD*(Ye*Ye.adjoint()).trace()) -
...
double CLASSNAME::get_STST() const
{
   ...
   anomDim = oneOver16PiSqr*(AbsSqr(LamTD) + AbsSqr(LamTU) - 4*Sqr(g2));

   if (get_loops() > 1) {
      anomDim += twoLoop*(32*Power(g2,4) - 3*Sqr(LamTD)*Sqr(Conj(LamTD
         )) - 3*Sqr(LamTU)*Sqr(Conj(LamTU)) + 0.2*AbsSqr(LamTD)*(-10*AbsSqr(
         LamSD) + 3*Sqr(g1) - 5*Sqr(g2) - 15*(Yd*Yd.adjoint()).trace() - 5*(Ye*
         Ye.adjoint()).trace()) + Conj(LamTU)*(-2*LamTU*AbsSqr(LamSU) + 0.6*
         LamTU*Sqr(g1) - LamTU*Sqr(g2) - 3*LamTU*(Yu*Yu.adjoint()).trace()));
   }
...
double CLASSNAME::get_SOcSOc() const
{
   ...
   anomDim = -6*oneOver16PiSqr*Sqr(g3);

   if (get_loops() > 1) {
      anomDim += 36*Power(g3,4)*twoLoop;
   }
...
double CLASSNAME::get_SRdSRd() const
{
   ...
   anomDim = oneOver16PiSqr*(AbsSqr(LamSD) - 0.3*(-5*AbsSqr(LamTD) + Sqr(
      g1) + 5*Sqr(g2)));

   if (get_loops() > 1) {
      anomDim += twoLoop*(-3*Sqr(LamSD)*Sqr(Conj(LamSD)) + 0.15*(15*
...
double CLASSNAME::get_SRuSRu() const
{
   ...
   if (get_loops() > 1) {
      anomDim += twoLoop*(-2*AbsSqr(LamSD)*AbsSqr(LamSU) + 0.15*(15*
...
void MRSSM_susy_parameters::set(const Eigen::ArrayXd& pars)
{
   Yd(0,0) = pars(0);
   Yd(0,1) = pars(1);
   Yd(0,2) = pars(2);
...
const MRSSM_input_parameters& MRSSM_susy_parameters::get_input() const
{
   return input;
}

void MRSSM_susy_parameters::set_input_parameters(const MRSSM_input_parameters& input_)
{
   input = input_;
}

void MRSSM_susy_parameters::calc_susy_traces(Susy_traces& susy_traces) const
{
