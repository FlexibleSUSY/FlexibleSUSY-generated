...
// File generated at Tue 10 Oct 2017 21:40:27
...
double MRSSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void MRSSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MRSSM_input_parameters& input)
{
   ...
   MRSSM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MRSSM_ewsb_solver<Two_scale> >(ewsb_solver));

   MRSSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   MRSSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   MRSSM_convergence_tester<Two_scale> convergence_tester(
   ...
   MRSSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void MRSSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MRSSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_susy_scale());
}

void MRSSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
