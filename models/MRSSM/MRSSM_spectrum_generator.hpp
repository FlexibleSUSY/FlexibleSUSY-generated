...
class MRSSM_spectrum_generator
   : public MRSSM_spectrum_generator_interface<T> {
public:
   MRSSM_spectrum_generator()
      ...
      , low_scale(0.)
   {}
   virtual ~MRSSM_spectrum_generator() {}

   double get_high_scale() const { return 0.; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const MRSSM_input_parameters&);
                                ...
                                const MRSSM_input_parameters& input)
{
   ...
   MRSSM_convergence_tester<T> convergence_tester(&model, this->precision_goal);
 ...
 */
template <class T>
void MRSSM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   MRSSM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, susy_scale);
}
