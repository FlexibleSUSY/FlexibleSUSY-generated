...
// File generated at Mon 23 Feb 2015 12:52:18
...
template <class T>
class MRSSM_spectrum_generator {
public:
   ...
   void run(const QedQcd& oneset, const MRSSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "MRSSM_rge_running.dat") const;
 ...
 */
template <class T>
void MRSSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const MRSSM_input_parameters& input)
{
   ...
   MRSSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   MRSSM_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint);
 ...
 */
template <class T>
void MRSSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   MRSSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   MRSSM_parameter_getter parameter_getter;
   Coupling_monitor<MRSSM<T>, MRSSM_parameter_getter>
 ...
 */
template <class T>
void MRSSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
