...
public:
   Eigen::ArrayXd get_parameters(const MRSSM<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const MRSSM<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const MRSSM<T>&);
...
template <class T>
void MRSSM_spectrum_plotter::extract_spectrum(const MRSSM<T>& model)
{
   spectrum.clear();
   ...
   spectrum.push_back(TParticle("Rpm", "R^-", to_valarray(PHYSICAL(MRpm))));
