...
std::unique_ptr<MRSSM_mass_eigenstates_interface> CLASSNAME::clone() const
{
   return std::make_unique<MRSSM_mass_eigenstates>(*this);
}
...
const MRSSM_input_parameters& CLASSNAME::get_input_parameters() const
{
   return get_input();
}

MRSSM_input_parameters& CLASSNAME::get_input_parameters()
{
   return get_input();
}
...
double CLASSNAME::calculate_MVZ_DRbar(double m_pole) const
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(MRSSM_info::VZ);return m_pole;
   }
...
double CLASSNAME::calculate_MVWm_DRbar(double m_pole) const
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(MRSSM_info::VWm);return m_pole;
   }
