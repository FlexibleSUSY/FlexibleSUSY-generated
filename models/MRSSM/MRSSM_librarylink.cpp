...
// File generated at Wed 16 Oct 2019 21:46:46
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMRSSMMessage", 1);
...
public:
   ...
   virtual void fill_slha_io(MRSSM_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MRSSM_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MRSSM_input_parameters&) override;
...
public:
   ...
   void set_input_parameters(const MRSSM_input_parameters& input_) { input = input_; }
...
private:
   MRSSM_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<MRSSM_spectrum> spectrum{nullptr};  ///< spectrum information

   MRSSM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSMRSSMCalculateSpectrum[]?");
   }
}
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(BMuDInput), "BMuDInput");
   MLPutRuleTo(link, INPUTPARAMETER(BMuUInput), "BMuUInput");
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, MRSSM_info::particle_names[i].c_str());
      }
   }
}
...
MRSSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
template <typename Solver_type>
void MRSSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MRSSM_spectrum_impl<Solver_type>::calculate_spectrum(
...
template <typename Solver_type>
void MRSSM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MRSSM_spectrum_impl<Solver_type>::fill_slha_io(MRSSM_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   MRSSM_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMRSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMRSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MRSSM_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   INPUTPARAMETER(BMuDInput) = pars[c++];
   INPUTPARAMETER(BMuUInput) = pars[c++];
...
{
   using namespace flexiblesusy::MRSSM_librarylink;
...
DLLEXPORT int FSMRSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMRSSMCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMRSSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMRSSMCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
