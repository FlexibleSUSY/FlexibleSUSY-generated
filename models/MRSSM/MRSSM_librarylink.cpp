...
struct Model_data {
   ...
   MRSSM_slha<algorithm_type> model; ///< running parameters and pole masses
};
...
void put_spectrum(const MRSSM_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const MRSSM_observables& observables, MLINK link)
{
...
void check_spectrum(const Model_data& data, MLINK link)
{
   ...
   if (problems.have_problem()) {
      ...
      put_message(link, "FSMRSSMCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSMRSSMCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(Model_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSMRSSMCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = MRSSM_slha<algorithm_type>(
