...
// File generated at Tue 22 Jan 2019 16:56:05
...
template <class T>
class MRSSM_spectrum_generator_interface {
public:
   virtual ~MRSSM_spectrum_generator_interface() = default;

   std::tuple<MRSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<MRSSM_slha<MRSSM<T>>> get_models_slha() const
   { return std::make_tuple(MRSSM_slha<MRSSM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MRSSM<T>& get_model() const
   { return model; }
   MRSSM<T>& get_model()
   { return model; }
   MRSSM_slha<MRSSM<T>> get_model_slha() const
   { return MRSSM_slha<MRSSM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MRSSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MRSSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(MRSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MRSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MRSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MRSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MRSSM_input_parameters& input)
{
 ...
 */
template <class T>
void MRSSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MRSSM_mass_eigenstates tmp_model(model);
   try {
   ...
   MRSSM_parameter_getter parameter_getter;
   Coupling_monitor<MRSSM_mass_eigenstates, MRSSM_parameter_getter>
 ...
 */
template <class T>
void MRSSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   MRSSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void MRSSM_spectrum_generator_interface<T>::translate_exception_to_problem(MRSSM<T>& model)
{
   try {
      throw;
