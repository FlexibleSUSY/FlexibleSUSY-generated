...
// File generated at Sat 15 Oct 2016 15:32:36
...
public:
   MRSSM_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~MRSSM_spectrum_generator_interface() {}

   const MRSSM<T>& get_model() const { return model; }
   MRSSM<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const MRSSM_input_parameters&) = 0;
 ...
 */
template <class T>
void MRSSM_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   MRSSM_spectrum_plotter plotter;
