...
// File generated at Tue 10 Oct 2017 20:39:52
...
namespace NMSSMEFTHiggs_standard_model_matching {
 ...
 * lambda is calculated from the tree-level NMSSMEFTHiggs parameters.
 *
 * @param sm SM parameters
 * @param model_0l NMSSMEFTHiggs tree-level parameters
...
Standard_model calculate_SM_tree_level(
   ...
   const NMSSMEFTHiggs_mass_eigenstates& model_0l,
   int idx)
{
   auto sm_0l = sm;
...
/**
 * Calculates NMSSMEFTHiggs tree-level parameters (g1, g2, g3, Yu, Yd,
 * Ye, v) by performing a tree-level matching from the SM to the
 * NMSSMEFTHiggs.
 *
 * @param model NMSSMEFTHiggs parameters
 * @param sm SM parameters
 *
 * @return NMSSMEFTHiggs with tree-level parameters
 */
NMSSMEFTHiggs_mass_eigenstates calculate_NMSSMEFTHiggs_tree_level(
   const NMSSMEFTHiggs_mass_eigenstates& model,
   const Standard_model& sm)
{
   auto model_0l = model;
...
Eigen::Matrix<double,3,1> calculate_tadpole_over_vevs(
   NMSSMEFTHiggs_mass_eigenstates model, int loop_order)
{
   if (loop_order == 0) {
...
/**
 * Calculates tree-level Higgs mass matrix in the NMSSMEFTHiggs without
 ...
 *
 * @param model NMSSMEFTHiggs parameters
 * @return tree-level Higgs mass matrix in the NMSSMEFTHiggs w/o tadpoles
 */
auto calculate_mh2_tree_level(NMSSMEFTHiggs_mass_eigenstates model) -> decltype(model.get_mass_matrix_hh())
{
...
/**
 * Calculates squared Higgs pole mass in the NMSSMEFTHiggs,
 * \f$(M_h^{\text{NMSSMEFTHiggs}})^2\f$.
 *
 * @param model_0l tree-level NMSSMEFTHiggs parameters
 * @param model_1l 1-loop NMSSMEFTHiggs parameters
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 *
 * @return squared Higgs pole mass in the NMSSMEFTHiggs
 */
double calculate_Mh2_pole(
   const NMSSMEFTHiggs_mass_eigenstates& model_0l,
   const NMSSMEFTHiggs_mass_eigenstates& model_1l,
   int idx)
{
 ...
 * lightest CP-even Higgs boson mass of the NMSSMEFTHiggs by requiring
 * that the Higgs pole masses are equal in both models.
 *
 * @param sm Standard Model
 * @param model_1l NMSSMEFTHiggs parameters
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 */
void match_high_to_low_scale_model_1loop(
   ...
   int idx)
{
   // tree-level NMSSMEFTHiggs parameters
   const auto model_0l = calculate_NMSSMEFTHiggs_tree_level(model_1l, sm);
...
double calculate_delta_alpha_em(double alpha_em, const NMSSMEFTHiggs_mass_eigenstates& model)
{
...
double calculate_delta_alpha_s(double alpha_s, const NMSSMEFTHiggs_mass_eigenstates& model)
{
   const double currentScale = model.get_scale();
...
Eigen::Matrix<double,3,3> calculate_MFu_DRbar_tree_level(const NMSSMEFTHiggs_mass_eigenstates& model)
{
...
Eigen::Matrix<double,3,3> calculate_MFd_DRbar_tree_level(const NMSSMEFTHiggs_mass_eigenstates& model)
{
   Eigen::Matrix<double,3,3> mf = ZEROMATRIX(3,3);
...
Eigen::Matrix<double,3,3> calculate_MFe_DRbar_tree_level(const NMSSMEFTHiggs_mass_eigenstates& model)
{
   Eigen::Matrix<double,3,3> mf = ZEROMATRIX(3,3);
...
double calculate_MW_pole_1loop(const NMSSMEFTHiggs_mass_eigenstates& model_0l)
{
...
double calculate_MZ_pole_1loop(const NMSSMEFTHiggs_mass_eigenstates& model_0l)
{
...
/**
 * Calculates NMSSMEFTHiggs parameters at 1-loop level by performing a
 * 1-loop matching.
 *
 * @param sm_0l SM parameters (lambda is at tree-level)
 * @param sm_1l SM parameters (level is at 1-loop level)
 * @param model_0l NMSSMEFTHiggs parameters at tree-level
 * @param model_1l NMSSMEFTHiggs parameters at 1-loop level
 *
 * @return NMSSMEFTHiggs 1-loop parameters
 */
NMSSMEFTHiggs_mass_eigenstates calculate_NMSSMEFTHiggs_1loop(
   const Standard_model& sm_0l,
   ...
   const NMSSMEFTHiggs_mass_eigenstates& model_1l)
{
   ...
   // running NMSSMEFTHiggs W, Z masses (via 1L matching)
   ...
   // running NMSSMEFTHiggs quark and lepton masses (via 1L matching)
   ...
   // running NMSSMEFTHiggs gauge couplings (via 1L matching)
   const double g1_1L = AbsSqrt(4. * Pi * alpha_em * (1. + delta_alpha_em) * mZ2_1L / mW2_1L) / NMSSMEFTHiggs_info::normalization_g1;
   const double g2_1L = AbsSqrt(4. * Pi * alpha_em * (1. + delta_alpha_em) / (1. - mW2_1L/mZ2_1L)) / NMSSMEFTHiggs_info::normalization_g2;
   const double g3_1L = AbsSqrt(4. * Pi * alpha_s * (1. + delta_alpha_s)) / NMSSMEFTHiggs_info::normalization_g3;
   ...
   {
      auto MODEL = &model;
      const double VEV = 2. * AbsSqrt(mZ2_1L/(Sqr(g1_1L*NMSSMEFTHiggs_info::normalization_g1) + Sqr(g2_1L*NMSSMEFTHiggs_info::normalization_g2)));
 ...
 * the NMSSMEFTHiggs at the necessary loop level from the known Standard
 * Model couplings and the SM vev.
 *
 * @param model NMSSMEFTHiggs parameters (to be set)
 * @param sm SM parameters
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 */
...
{
   // tree-level NMSSMEFTHiggs parameters
   const auto model_0l = calculate_NMSSMEFTHiggs_tree_level(model, sm);
   const auto sm_0l = calculate_SM_tree_level(sm, model_0l, idx);

   // 1-loop parameters
   const auto model_1l = calculate_NMSSMEFTHiggs_1loop(sm_0l, sm, model_0l, model);
 ...
 * CP-even Higgs boson mass of the NMSSMEFTHiggs.
 *
...
void match_high_to_low_scale_model_tree_level(
   Standard_model& sm, const NMSSMEFTHiggs_mass_eigenstates& model, int idx)
{
...
/**
 * Calculates the gauge and Yukawa couplings and the SM-like VEV in
 * the NMSSMEFTHiggs at the tree level from the known Standard Model
 * couplings and the SM vev.
 */
void match_low_to_high_scale_model_tree_level(
   NMSSMEFTHiggs_mass_eigenstates& model, const Standard_model& sm_input)
{
   auto sm = sm_input;
   sm.calculate_DRbar_masses();

   model.set_g1(sm.get_g1()*standard_model_info::normalization_g1/NMSSMEFTHiggs_info::normalization_g1);
   model.set_g2(sm.get_g2()*standard_model_info::normalization_g2/NMSSMEFTHiggs_info::normalization_g2);
   model.set_g3(sm.get_g3()*standard_model_info::normalization_g3/NMSSMEFTHiggs_info::normalization_g3);
...
/**
 * Calculates the gauge and Yukawa couplings and the SM-like VEV in
 * the NMSSMEFTHiggs at the 1-loop level from the known Standard Model
 * couplings and the SM vev.
 *
 ...
 * determine the NMSSMEFTHiggs at the right loop order to avoid the
...
void match_low_to_high_scale_model(
   NMSSMEFTHiggs_mass_eigenstates& model, const Standard_model& sm, int loop_order, int idx)
{
   if (loop_order == 0) {
 ...
 * CP-even Higgs boson mass of the NMSSMEFTHiggs by requiring that the
...
void match_high_to_low_scale_model(
   Standard_model& sm, const NMSSMEFTHiggs_mass_eigenstates& model, int loop_order, int idx)
{
   if (loop_order == 0) {
