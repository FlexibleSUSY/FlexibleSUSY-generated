...
#ifndef NMSSMEFTHiggs_STANDARD_MODEL_TWO_SCALE_INITIAL_GUESSER_H
#define NMSSMEFTHiggs_STANDARD_MODEL_TWO_SCALE_INITIAL_GUESSER_H

#include "NMSSMEFTHiggs_initial_guesser.hpp"
...
template <class T>
class NMSSMEFTHiggs_standard_model_initial_guesser;

/**
 * @class NMSSMEFTHiggs_standard_model_initial_guesser<Two_scale>
 * @brief initial guesser for the NMSSMEFTHiggs tower
 */

template<>
class NMSSMEFTHiggs_standard_model_initial_guesser<Two_scale> : public Initial_guesser {
public:
   NMSSMEFTHiggs_standard_model_initial_guesser(NMSSMEFTHiggs<Two_scale>*,
                               standard_model::StandardModel<Two_scale>*,
                               const softsusy::QedQcd&,
                               const standard_model::Standard_model_low_scale_constraint<Two_scale>&,
                               const NMSSMEFTHiggs_susy_scale_constraint<Two_scale>&);
   virtual ~NMSSMEFTHiggs_standard_model_initial_guesser();
   virtual void guess(); ///< initial guess

   void set_running_precision(double p) { running_precision = p; }

private:
   NMSSMEFTHiggs<Two_scale>* model{nullptr}; ///< pointer to model class
   ...
   NMSSMEFTHiggs_susy_scale_constraint<Two_scale> susy_constraint{};
