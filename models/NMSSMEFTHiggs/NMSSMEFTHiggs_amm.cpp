...
/**
 * @file NMSSMEFTHiggs_amm.cpp
 *
 * This file was generated with FlexibleSUSY 2.8.0 and SARAH 4.15.1 .
 */

#include "NMSSMEFTHiggs_amm.hpp"
#include "NMSSMEFTHiggs_mass_eigenstates.hpp"

#include "cxx_qft/NMSSMEFTHiggs_qft.hpp"
#include "cxx_qft/NMSSMEFTHiggs_particle_aliases.hpp"
...
double get_MLCP(const NMSSMEFTHiggs_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(NMSSMEFTHiggs_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("NMSSMEFTHiggs_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("NMSSMEFTHiggs_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("NMSSMEFTHiggs_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("NMSSMEFTHiggs_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const NMSSMEFTHiggs_mass_eigenstates& model, double ml_pole, int idx)
{
   using namespace NMSSMEFTHiggs_cxx_diagrams::fields;

   const auto form_factors = NMSSMEFTHiggs_FFV_form_factors::calculate_form_factors<Lepton,Lepton,VP>(idx, idx, model, true);
...
template <typename Lepton>
double calculate_amm_impl(const NMSSMEFTHiggs_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("NMSSMEFTHiggs_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double NMSSMEFTHiggs_amm::calculate_amm(const NMSSMEFTHiggs_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   NMSSMEFTHiggs_mass_eigenstates model(model_);

   VERBOSE_MSG("NMSSMEFTHiggs_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NMSSMEFTHiggs_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template double NMSSMEFTHiggs_amm::calculate_amm<NMSSMEFTHiggs_cxx_diagrams::fields::Fe>(const NMSSMEFTHiggs_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);

template <typename Lepton>
double NMSSMEFTHiggs_amm::calculate_amm_uncertainty(const NMSSMEFTHiggs_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   NMSSMEFTHiggs_mass_eigenstates model(model_);

   VERBOSE_MSG("NMSSMEFTHiggs_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NMSSMEFTHiggs_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
// spliting templated function into header and cpp
// https://isocpp.org/wiki/faq/templates#separate-template-fn-defn-from-decl
template double NMSSMEFTHiggs_amm::calculate_amm_uncertainty<NMSSMEFTHiggs_cxx_diagrams::fields::Fe>(const NMSSMEFTHiggs_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);
