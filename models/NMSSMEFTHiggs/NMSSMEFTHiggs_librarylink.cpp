...
#include "lowe.h"
#include "NMSSMEFTHiggs_unitarity.hpp"
...
template <typename Solver_type>
void NMSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = NMSSMEFTHiggs_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSNMSSMEFTHiggsCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMEFTHiggsCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSNMSSMEFTHiggsCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
