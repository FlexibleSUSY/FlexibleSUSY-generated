...
// File generated at Sun 4 Aug 2019 17:42:12
...
public:
   ...
   virtual void fill_slha_io(NMSSMEFTHiggs_slha_io&) const = 0;
...
public:
   ...
   virtual void fill_slha_io(NMSSMEFTHiggs_slha_io&) const override;
...
private:
   std::tuple<NMSSMEFTHiggs_slha<NMSSMEFTHiggs<Solver_type>>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, NMSSMEFTHiggs_info::particle_names[i].c_str());
      }
   }
}
...
void put_spectrum(const NMSSMEFTHiggs_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void NMSSMEFTHiggs_spectrum_impl<Solver_type>::fill_slha_io(NMSSMEFTHiggs_slha_io& slha_io) const
{
