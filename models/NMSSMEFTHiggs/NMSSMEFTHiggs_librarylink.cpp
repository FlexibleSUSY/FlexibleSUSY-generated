...
#include "NMSSMEFTHiggs_two_scale_spectrum_generator.hpp"
#endif
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSNMSSMEFTHiggsMessage", 1);
...
class NMSSMEFTHiggs_spectrum {
public:
   virtual ~NMSSMEFTHiggs_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(NMSSMEFTHiggs_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const NMSSMEFTHiggs_observables& get_observables() const = 0;
   virtual const NMSSMEFTHiggs_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const NMSSMEFTHiggs_input_parameters&) = 0;
...
class NMSSMEFTHiggs_spectrum_impl : public NMSSMEFTHiggs_spectrum
{
public:
   virtual ~NMSSMEFTHiggs_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(NMSSMEFTHiggs_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const NMSSMEFTHiggs_observables& get_observables() const override { return observables; }
   virtual const NMSSMEFTHiggs_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const NMSSMEFTHiggs_input_parameters&) override;
...
private:
   std::tuple<NMSSMEFTHiggs<Solver_type>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   NMSSMEFTHiggs_scales scales{};              ///< scale information
   NMSSMEFTHiggs_observables observables{};    ///< observables
   NMSSMEFTHiggs_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const NMSSMEFTHiggs_input_parameters& input_) { input = input_; }
...
private:
   NMSSMEFTHiggs_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<NMSSMEFTHiggs_spectrum> spectrum{nullptr};  ///< spectrum information

   NMSSMEFTHiggs_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSNMSSMEFTHiggsCalculateSpectrum[]?");
   }
}
...
NMSSMEFTHiggs_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const NMSSMEFTHiggs_slha& model, MLINK link)
{
   MLPutRule(link, NMSSMEFTHiggs_info::model_name);
...
template <typename Solver_type>
void NMSSMEFTHiggs_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void NMSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_spectrum(
...
template <typename Solver_type>
void NMSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void NMSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_model_decays(
...
template <typename Solver_type>
void NMSSMEFTHiggs_spectrum_impl<Solver_type>::fill_slha_io(NMSSMEFTHiggs_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   NMSSMEFTHiggs_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   NMSSMEFTHiggs_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = NMSSMEFTHiggs_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSNMSSMEFTHiggsCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSNMSSMEFTHiggsCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new NMSSMEFTHiggs_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSNMSSMEFTHiggsGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMEFTHiggsGetSettings"))
...
DLLEXPORT int FSNMSSMEFTHiggsGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMEFTHiggsGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMEFTHiggsGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMEFTHiggsGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMEFTHiggsOpenHandle(
...
DLLEXPORT int FSNMSSMEFTHiggsCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::NMSSMEFTHiggs_librarylink;
...
DLLEXPORT int FSNMSSMEFTHiggsSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::NMSSMEFTHiggs_librarylink;
...
DLLEXPORT int FSNMSSMEFTHiggsGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMEFTHiggsGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMEFTHiggsToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMEFTHiggsToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMEFTHiggsGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMEFTHiggsGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMEFTHiggsCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMEFTHiggsCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMEFTHiggsCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMEFTHiggsCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSNMSSMEFTHiggsCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSNMSSMEFTHiggsCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSNMSSMEFTHiggsCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSNMSSMEFTHiggsCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSNMSSMEFTHiggsCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSNMSSMEFTHiggsCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSNMSSMEFTHiggsCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSNMSSMEFTHiggsCalculateDecays", "error", e.what());
      put_error_output(link);
   }
