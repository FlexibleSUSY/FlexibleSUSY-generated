...
void CMSSM_decay_table::print(std::ostream& ostr) const
{
   for (const auto& particle: decay_table) {
      ...
      for (const auto& decay: particle) {
         ...
         ostr << "Partial width["
              << CMSSM_info::get_particle_name_from_pdg(pdg)
              << " -> ";
         for (std::size_t i = 0; i < final_state_size; ++i) {
            ostr << CMSSM_info::get_particle_name_from_pdg(final_state[i]);
...
std::ostream& operator<<(std::ostream& ostr,
                         const CMSSM_decay_table& table)
{
   table.print(ostr);
   return ostr;
}

Decays_list& CMSSM_decay_table::get_hh_decays(int gI1)
{
   switch (gI1) {
...
const Decays_list& CMSSM_decay_table::get_hh_decays(int gI1) const
{
   switch (gI1) {
   case 0: return decay_table[0]; break;
   case 1: return decay_table[1]; break;
...
Decays_list& CMSSM_decay_table::get_Hpm_decays(int gI1)
{
   switch (gI1) {
...
const Decays_list& CMSSM_decay_table::get_Hpm_decays(int gI1) const
{
   switch (gI1) {
   case 1: return decay_table[2]; break;
   }
...
Decays_list& CMSSM_decay_table::get_Ah_decays(int gI1)
{
   switch (gI1) {
...
const Decays_list& CMSSM_decay_table::get_Ah_decays(int gI1) const
{
   switch (gI1) {
   case 1: return decay_table[3]; break;
   }
