...
libCMSSM = FileNameJoin[{Directory[], "models", "CMSSM", "CMSSM_librarylink.so"}];

FSCMSSMGetSettings = LibraryFunctionLoad[libCMSSM, "FSCMSSMGetSettings", LinkObject, LinkObject];
FSCMSSMGetSMInputParameters = LibraryFunctionLoad[libCMSSM, "FSCMSSMGetSMInputParameters", LinkObject, LinkObject];
FSCMSSMGetInputParameters = LibraryFunctionLoad[libCMSSM, "FSCMSSMGetInputParameters", LinkObject, LinkObject];
FSCMSSMGetProblems = LibraryFunctionLoad[libCMSSM, "FSCMSSMGetProblems", LinkObject, LinkObject];
FSCMSSMGetWarnings = LibraryFunctionLoad[libCMSSM, "FSCMSSMGetWarnings", LinkObject, LinkObject];
FSCMSSMToSLHA = LibraryFunctionLoad[libCMSSM, "FSCMSSMToSLHA", LinkObject, LinkObject];

FSCMSSMOpenHandleLib = LibraryFunctionLoad[libCMSSM, "FSCMSSMOpenHandle", {{Real,1}}, Integer];
FSCMSSMCloseHandle = LibraryFunctionLoad[libCMSSM, "FSCMSSMCloseHandle", {Integer}, Void];

FSCMSSMSetLib = LibraryFunctionLoad[libCMSSM, "FSCMSSMSet", {Integer, {Real,1}}, Void];
...
FSCMSSMMessage[s_] := Message[FSCMSSM::info, s];

FSCMSSMCheckIsNumeric[a_?NumericQ] := a;
FSCMSSMCheckIsNumeric[a_] := (Message[FSCMSSM::nonum, a]; Abort[]);
...
FSCMSSMOpenHandle[OptionsPattern[]] :=
    FSCMSSMOpenHandleLib[
        FSCMSSMCheckIsNumeric /@ {
...
Options[FSCMSSMSet] = Options[FSCMSSMOpenHandle];

FSCMSSMSet[handle_Integer, a___, (fsSettings | fsSMParameters | fsModelParameters) -> s_List, r___] :=
    FSCMSSMSet[handle, a, Sequence @@ s, r];

FSCMSSMSet[handle_Integer, p:OptionsPattern[]] :=
    FSCMSSMSetLib[
        handle,
        ReleaseHold[Hold[FSCMSSMCheckIsNumeric /@ {
            (* spectrum generator settings *)
            OptionValue[precisionGoal],
            OptionValue[maxIterations],
            OptionValue[solver],
