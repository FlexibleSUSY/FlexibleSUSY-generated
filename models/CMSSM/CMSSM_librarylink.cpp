...
template <typename Solver_type>
void CMSSM_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = CMSSM_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSCMSSMCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCMSSMCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSCMSSMCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
