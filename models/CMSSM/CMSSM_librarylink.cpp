...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSCMSSMMessage", 1);
...
class CMSSM_spectrum {
public:
   virtual ~CMSSM_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(CMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const CMSSM_observables& get_observables() const = 0;
   virtual const CMSSM_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const CMSSM_input_parameters&) = 0;
...
class CMSSM_spectrum_impl : public CMSSM_spectrum
{
public:
   virtual ~CMSSM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(CMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const CMSSM_observables& get_observables() const override { return observables; }
   virtual const CMSSM_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const CMSSM_input_parameters&) override;
...
private:
   std::tuple<CMSSM<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   CMSSM_scales scales{};              ///< scale information
   CMSSM_observables observables{};    ///< observables
   CMSSM_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const CMSSM_input_parameters& input_) { input = input_; }
...
private:
   CMSSM_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<CMSSM_spectrum> spectrum{nullptr};  ///< spectrum information

   CMSSM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSCMSSMCalculateSpectrum[]?");
   }
}
...
CMSSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const CMSSM_slha& model, MLINK link)
{
   MLPutRule(link, CMSSM_info::model_name);
...
template <typename Solver_type>
void CMSSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void CMSSM_spectrum_impl<Solver_type>::calculate_spectrum(
...
template <typename Solver_type>
void CMSSM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void CMSSM_spectrum_impl<Solver_type>::calculate_model_decays(
   const softsusy::QedQcd& qedqcd, const Physical_input& physical_input, const FlexibleDecay_settings& flexibledecay_settings)
{
   decays = CMSSM_decays(std::get<0>(models), qedqcd, physical_input, flexibledecay_settings);
...
template <typename Solver_type>
void CMSSM_spectrum_impl<Solver_type>::fill_slha_io(CMSSM_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   CMSSM_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   CMSSM_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = CMSSM_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSCMSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSCMSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new CMSSM_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSCMSSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCMSSMGetSettings"))
...
DLLEXPORT int FSCMSSMGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCMSSMGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCMSSMGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCMSSMGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCMSSMCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::CMSSM_librarylink;
...
DLLEXPORT int FSCMSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCMSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCMSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCMSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCMSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCMSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCMSSMCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCMSSMCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCMSSMCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCMSSMCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSCMSSMCalculateObservables", "warning",
      ...
      put_message(link, "FSCMSSMCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSCMSSMCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCMSSMCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSCMSSMCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSCMSSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSCMSSMCalculateDecays", "error", e.what());
      put_error_output(link);
   }
