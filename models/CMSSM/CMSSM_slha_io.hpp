...
public:
   ...
   template <class Model> void fill(CMSSM_slha<Model>&) const;
   ...
   template <class Model> void set_extra(const CMSSM_slha<Model>&, const CMSSM_scales&, const CMSSM_observables&);
   void set_input(const CMSSM_input_parameters&);
   ...
   template <class Model> void set_spectrum(const CMSSM_slha<Model>&);
   ...
   static void fill_imminpar_tuple(CMSSM_input_parameters&, int, double);
   static void fill_imextpar_tuple(CMSSM_input_parameters&, int, double);

   template <class Model>
   static void fill_slhaea(SLHAea::Coll&, const CMSSM_slha<Model>&, const softsusy::QedQcd&, const CMSSM_scales&, const CMSSM_observables&);

   template <class Model>
   static SLHAea::Coll fill_slhaea(const CMSSM_slha<Model>&, const softsusy::QedQcd&, const CMSSM_scales&, const CMSSM_observables&);
...
private:
   ...
   void set_imminpar(const CMSSM_input_parameters&);
   void set_imextpar(const CMSSM_input_parameters&);
   ...
   template <class Model> void set_model_parameters(const CMSSM_slha<Model>&);
 ...
 */
template <class Model>
void CMSSM_slha_io::fill(CMSSM_slha<Model>& model) const
{
...
void CMSSM_slha_io::fill_slhaea(
   SLHAea::Coll& slhaea, const CMSSM_slha<Model>& model,
...
SLHAea::Coll CMSSM_slha_io::fill_slhaea(
   const CMSSM_slha<Model>& model, const softsusy::QedQcd& qedqcd,
 ...
 */
template <class Model>
void CMSSM_slha_io::set_model_parameters(const CMSSM_slha<Model>& model)
{
   {
...
void CMSSM_slha_io::set_extra(
   const CMSSM_slha<Model>& model, const CMSSM_scales& scales,
   const CMSSM_observables& observables)
{
 ...
 */
template <class... Ts>
void CMSSM_slha_io::set_spectrum(const std::tuple<Ts...>& models)
{
   Set_spectrum<CMSSM_slha_io> ss(this);
...
{
   set_spectrum(CMSSM_slha<CMSSM<T> >(model));
}
 ...
 */
template <class Model>
void CMSSM_slha_io::set_spectrum(const CMSSM_slha<Model>& model)
{
   const CMSSM_physical physical(model.get_physical_slha());
