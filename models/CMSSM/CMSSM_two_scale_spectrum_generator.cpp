...
double CMSSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void CMSSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const CMSSM_input_parameters& input)
{
   ...
   CMSSM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   CMSSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   CMSSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   CMSSM_convergence_tester<Two_scale> convergence_tester(
   ...
   CMSSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void CMSSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   CMSSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void CMSSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
