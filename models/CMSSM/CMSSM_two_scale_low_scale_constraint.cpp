...
CMSSM_low_scale_constraint<Two_scale>::CMSSM_low_scale_constraint(
   CMSSM<Two_scale>* model_, const QedQcd& oneset_)
   : Constraint<Two_scale>()
   , model(model_)
   , oneset(oneset_)
   , new_g1(0.)
...
void CMSSM_low_scale_constraint<Two_scale>::set_sm_parameters(const QedQcd& oneset_)
{
   oneset = oneset_;
}

const QedQcd& CMSSM_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return oneset;
}
 ...
 */
void CMSSM_low_scale_constraint<Two_scale>::recalculate_mw_pole()
{
   assert(model && "CMSSM_low_scale_constraint<Two_scale>::"
