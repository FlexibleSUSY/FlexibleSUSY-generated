...
std::vector<std::string> CMSSM_parameter_getter::get_mass_names(const std::string& head)
{
...
std::array<std::string, CMSSM_info::NUMBER_OF_MIXINGS> CMSSM_parameter_getter::get_mixing_names()
{
   return CMSSM_info::particle_mixing_names;
}

Eigen::ArrayXd CMSSM_parameter_getter::get_parameters(const CMSSM_mass_eigenstates& model)
{
   return model.get();
}

std::array<std::string, CMSSM_info::NUMBER_OF_PARAMETERS> CMSSM_parameter_getter::get_parameter_names()
{
   return CMSSM_info::parameter_names;
}

std::array<std::string, CMSSM_info::NUMBER_OF_PARTICLES> CMSSM_parameter_getter::get_particle_names()
{
   return CMSSM_info::particle_names;
}

std::vector<std::string> CMSSM_parameter_getter::get_DRbar_mass_names()
{
   return get_mass_names();
}

std::vector<std::string> CMSSM_parameter_getter::get_pole_mass_names()
{
   return get_mass_names("Pole");
}

std::array<std::string, CMSSM_info::NUMBER_OF_MIXINGS> CMSSM_parameter_getter::get_DRbar_mixing_names()
{
   return get_mixing_names();
}

std::array<std::string, CMSSM_info::NUMBER_OF_MIXINGS> CMSSM_parameter_getter::get_pole_mixing_names()
{
...
std::array<std::string, CMSSM_info::NUMBER_OF_INPUT_PARAMETERS> CMSSM_parameter_getter::get_input_parameter_names()
{
   return CMSSM_info::input_parameter_names;
}

std::array<std::string, CMSSM_info::NUMBER_OF_EXTRA_PARAMETERS> CMSSM_parameter_getter::get_extra_parameter_names()
{
   return CMSSM_info::extra_parameter_names;
}

decltype(CMSSM_info::NUMBER_OF_MASSES) CMSSM_parameter_getter::get_number_of_masses()
{
   return CMSSM_info::NUMBER_OF_MASSES;
}

// CMSSM_spectrum_plotter ////////////////////////////////////////

CMSSM_spectrum_plotter::CMSSM_spectrum_plotter(const CMSSM_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void CMSSM_spectrum_plotter::extract_spectrum(const CMSSM_mass_eigenstates& model)
{
   spectrum.clear();
...
void CMSSM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("CMSSM_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("CMSSM_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("CMSSM_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void CMSSM_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("CMSSM_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const CMSSM_mass_eigenstates& model,
   ...
   const CMSSM_observables* observables)
{
   ...
   append(names, CMSSM_parameter_getter::get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, CMSSM_parameter_getter::get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, CMSSM_parameter_getter::get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, CMSSM_parameter_getter::get_DRbar_mass_names());
   append(names, CMSSM_parameter_getter::get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, CMSSM_parameter_getter::get_pole_mass_names());
   append(names, CMSSM_parameter_getter::get_pole_mixing_names());
   ...
   if (CMSSM_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, CMSSM_observables::get_names());
 ...
 */
CMSSM_mass_eigenstates from_database(
   const std::string& file_name, long long entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, CMSSM_observables* observables)
{
   CMSSM_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = CMSSM_parameter_getter::get_number_of_masses();
   const auto number_of_mixings = CMSSM_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = CMSSM_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = CMSSM_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? CMSSM_observables::NUMBER_OF_OBSERVABLES : 0);
