...
CMSSM_initial_guesser<Two_scale>::CMSSM_initial_guesser(
   CMSSM<Two_scale>* model_,
   const softsusy::QedQcd& qedqcd_,
   const CMSSM_low_scale_constraint<Two_scale>& low_constraint_,
...
{
   if (!model)
      throw SetupError("CMSSM_initial_guesser: Error: pointer to model"
                       " CMSSM<Two_scale> must not be zero");
}
 ...
 */
void CMSSM_initial_guesser<Two_scale>::guess()
{
 ...
 * \endcode
 */
void CMSSM_initial_guesser<Two_scale>::guess_susy_parameters()
{
...
void CMSSM_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void CMSSM_initial_guesser<Two_scale>::calculate_running_SM_masses()
{
 ...
 * corrections).
 */
void CMSSM_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
 ...
 */
void CMSSM_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
 ...
 * corrections).
 */
void CMSSM_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
   const auto vd = MODELPARAMETER(vd);
 ...
 */
void CMSSM_initial_guesser<Two_scale>::guess_soft_parameters()
{
