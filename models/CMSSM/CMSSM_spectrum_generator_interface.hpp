...
#ifndef CMSSM_SPECTRUM_GENERATOR_INTERFACE_H
#define CMSSM_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class CMSSM_spectrum_generator_interface {
public:
   virtual ~CMSSM_spectrum_generator_interface() = default;

   std::tuple<CMSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<CMSSM_slha<CMSSM<T>>> get_models_slha() const
   { return std::make_tuple(CMSSM_slha<CMSSM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const CMSSM<T>& get_model() const
   { return model; }
   CMSSM<T>& get_model()
   { return model; }
   CMSSM_slha<CMSSM<T>> get_model_slha() const
   { return CMSSM_slha<CMSSM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const CMSSM_input_parameters&);
...
protected:
   ...
   void translate_exception_to_problem(CMSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const CMSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void CMSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void CMSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const CMSSM_input_parameters& input)
{
 ...
 */
template <class T>
void CMSSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   CMSSM_mass_eigenstates tmp_model(model);
   try {
   ...
   Coupling_monitor<CMSSM_mass_eigenstates, CMSSM_parameter_getter>
 ...
 */
template <class T>
void CMSSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   CMSSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void CMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(CMSSM<T>& model)
{
   try {
      throw;
