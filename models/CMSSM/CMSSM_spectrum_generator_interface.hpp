...
public:
   ...
   {}
   virtual ~CMSSM_spectrum_generator_interface() {}

   const CMSSM<T>& get_model() const { return model; }
   CMSSM<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const CMSSM_input_parameters&) = 0;
 ...
 */
template <class T>
void CMSSM_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
...
void CMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(CMSSM<T>& model)
{
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : CMSSM_info::parameter_names[parameter_index];
