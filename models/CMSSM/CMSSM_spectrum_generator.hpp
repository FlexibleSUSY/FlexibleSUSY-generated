 ...
 */
template <class T>
void CMSSM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const CMSSM_input_parameters& input)
{
   ...
   CMSSM_high_scale_constraint<T> high_scale_constraint(&model);
   CMSSM_susy_scale_constraint<T> susy_scale_constraint(&model, qedqcd);
   CMSSM_low_scale_constraint<T>  low_scale_constraint(&model, qedqcd);
   ...
   CMSSM_convergence_tester<T> convergence_tester(
   ...
   CMSSM_initial_guesser<T> initial_guesser(&model, qedqcd,
