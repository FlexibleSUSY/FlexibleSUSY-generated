...
// File generated at Fri 20 Oct 2017 09:20:20
...
const CMSSM_input_parameters& CMSSM_susy_scale_constraint<Two_scale>::get_input_parameters() const
{
   check_model_ptr();
...
void CMSSM_susy_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<CMSSM<Two_scale>*>(model_);
}

void CMSSM_susy_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& CMSSM_susy_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}
...
void CMSSM_susy_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("CMSSM_susy_scale_constraint<Two_scale>: "
