...
// File generated at Thu 15 Dec 2016 12:39:02
...
#include "MSSMtower_two_scale_model_slha.hpp"
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSMtowerMessage", 1);
...
struct MSSMtower_data {
   ...
   MSSMtower_input_parameters input;     ///< model input parameters
   ...
   MSSMtower_slha<algorithm_type> model; ///< running parameters and pole masses
};

/// current handles
typedef std::map<Handle_id, MSSMtower_data> Handle_map;
Handle_map handles_MSSMtower;

/******************************************************************/

Handle_id get_new_MSSMtower_handle()
{
   static const std::size_t max_handles =
      static_cast<std::size_t>(std::exp2(8*sizeof(Handle_id)) - 1);

   if (handles_MSSMtower.size() >= max_handles)
      throw ENotEnoughFreeHandles(handles_MSSMtower.size());

   Handle_id hid = 0;

   while (handles_MSSMtower.find(hid) != handles_MSSMtower.end())
      hid++;
...
MSSMtower_data find_MSSMtower_data(Handle_id hid)
{
   const Handle_map::iterator handle = handles_MSSMtower.find(hid);

   if (handle == handles_MSSMtower.end())
...
void put_settings(const MSSMtower_data& data, MLINK link)
{
...
void put_sm_input_parameters(const MSSMtower_data& data, MLINK link)
{
...
void put_input_parameters(const MSSMtower_data& data, MLINK link)
{
...
void put_spectrum(const MSSMtower_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const MSSMtower_observables& observables, MLINK link)
{
...
void check_spectrum(const MSSMtower_data& data, MLINK link)
{
   const Problems<MSSMtower_info::NUMBER_OF_PARTICLES>& problems
      = data.model.get_problems();

   if (problems.have_problem()) {
      std::ostringstream msg;
      problems.print_problems(msg);
      put_message(link, "FSMSSMtowerCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSMSSMtowerCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(MSSMtower_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSMSSMtowerCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }

   MSSMtower_spectrum_generator<algorithm_type> spectrum_generator;
   ...
   data.model = MSSMtower_slha<algorithm_type>(
...
MSSMtower_data make_MSSMtower_data(double* pars, mint npars)
{
   MSSMtower_data data;

   const mint n_settings = 23, n_sm_parameters = 29, n_input_pars = 80;
...
DLLEXPORT int FSMSSMtowerGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSMSSMtowerGetSettings"))
      return LIBRARY_TYPE_ERROR;

   const Handle_id hid = get_handle_from(link);

   try {
      const MSSMtower_data data = find_MSSMtower_data(hid);
...
DLLEXPORT int FSMSSMtowerGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSMSSMtowerGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMtowerGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSMSSMtowerGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMtowerOpenHandle(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument Res)
{
   ...
   try {
      MSSMtower_data data = make_MSSMtower_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);

      const Handle_id hid = get_new_MSSMtower_handle();

      handles_MSSMtower.insert(std::make_pair(hid, std::move(data)));
...
DLLEXPORT int FSMSSMtowerCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   if (Argc != 1)
      return LIBRARY_TYPE_ERROR;
   ...
   if (handle != handles_MSSMtower.end())
      handles_MSSMtower.erase(handle);
   ...
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   ...
   if (handle == handles_MSSMtower.end()) {
      std::cerr << "Error: FSMSSMtowerSet: Unknown handle: "
                << hid << std::endl;
      return LIBRARY_FUNCTION_ERROR;
   }

   try {
      handle->second = make_MSSMtower_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);
   } catch (const flexiblesusy::Error& e) {
      std::cerr << e.what() << std::endl;
...
DLLEXPORT int FSMSSMtowerCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSMSSMtowerCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;

   const Handle_id hid = get_handle_from(link);

   try {
      MSSMtower_data data = find_MSSMtower_data(hid);
      ...
      handles_MSSMtower[hid] = std::move(data);
...
DLLEXPORT int FSMSSMtowerCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSMSSMtowerCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.model.get_scale() == 0.) {
         put_message(link,
            "FSMSSMtowerCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMtowerCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMtowerCalculateObservables", "error", e.what());
      put_error_output(link);
   }
