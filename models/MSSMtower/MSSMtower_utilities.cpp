...
#include "MSSMtower_utilities.hpp"
...
MSSMtower_spectrum_plotter::MSSMtower_spectrum_plotter()
   : spectrum()
   , scale(0.0)
   , width(16)
{
...
void MSSMtower_spectrum_plotter::extract_spectrum(const MSSMtower_mass_eigenstates& model)
{
   spectrum.clear();
...
void MSSMtower_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name.c_str(), std::ios::out);
   VERBOSE_MSG("MSSMtower_spectrum_plotter::write_to_file: opening file: "
               << file_name.c_str());
   if (filestr.fail()) {
      ERROR("MSSMtower_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("MSSMtower_spectrum_plotter::write_to_file: file written: "
               << file_name.c_str());
}

void MSSMtower_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   for (std::size_t s = 0; s < spectrum.size(); ++s) {
      if (!filestr.good()) {
         ERROR("MSSMtower_spectrum_plotter::write_spectrum: "
...
std::valarray<double> MSSMtower_spectrum_plotter::to_valarray(double v)
{
   return std::valarray<double>(&v, 1);
}

namespace MSSMtower_database {
 ...
 */
void to_database(
   const std::string& file_name, const MSSMtower_mass_eigenstates& model,
   ...
   const MSSMtower_observables* observables)
{
   ...
   append(names, MSSMtower_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, MSSMtower_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill pole masses and mixings
   append(names, MSSMtower_parameter_getter().get_mass_names());
   append(names, MSSMtower_parameter_getter().get_mixing_names());
   ...
   if (MSSMtower_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, MSSMtower_observables::get_names());
 ...
 */
MSSMtower_mass_eigenstates from_database(
   const std::string& file_name, std::size_t entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, MSSMtower_observables* observables)
{
   using utilities::append;

   MSSMtower_mass_eigenstates model;
   const std::size_t number_of_parameters = model.get_number_of_parameters();
   const std::size_t number_of_masses = MSSMtower_parameter_getter().get_number_of_masses();
   const std::size_t number_of_mixings = MSSMtower_info::NUMBER_OF_MIXINGS;
   const std::size_t number_of_input_parameters = MSSMtower_info::NUMBER_OF_INPUT_PARAMETERS;
   ...
   const std::size_t number_of_observables =
      (observables ? MSSMtower_observables::NUMBER_OF_OBSERVABLES : 0u);
