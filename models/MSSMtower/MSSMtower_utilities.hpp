...
#ifndef MSSMtower_UTILITIES_H
#define MSSMtower_UTILITIES_H
...
struct MSSMtower_observables;
class Physical_input;

class MSSMtower_parameter_getter {
public:
   Eigen::ArrayXd get_parameters(const MSSMtower_mass_eigenstates& model) {
      return model.get();
   }
...
class MSSMtower_spectrum_plotter {
public:
   MSSMtower_spectrum_plotter();

   void extract_spectrum(const MSSMtower_mass_eigenstates&);
...
template <class Scalar, int M, int N>
std::valarray<double> MSSMtower_spectrum_plotter::to_valarray(const Eigen::Array<Scalar, M, N>& v)
{
...
void to_database(
   const std::string&,
   const MSSMtower_mass_eigenstates&,
   const softsusy::QedQcd* qedqcd = 0,
   const Physical_input* physical_input = 0,
   const MSSMtower_observables* observables = 0);
...
MSSMtower_mass_eigenstates from_database(
   ...
   MSSMtower_observables* observables = 0);
