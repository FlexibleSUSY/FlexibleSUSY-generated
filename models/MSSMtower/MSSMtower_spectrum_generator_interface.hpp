...
#ifndef MSSMtower_SPECTRUM_GENERATOR_INTERFACE_H
#define MSSMtower_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class MSSMtower_spectrum_generator_interface {
public:
   MSSMtower_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~MSSMtower_spectrum_generator_interface() {}

   const MSSMtower<T>& get_model() const { return model; }
   MSSMtower<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const MSSMtower_input_parameters&) = 0;
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MSSMtower_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(MSSMtower<T>& model);
};
 ...
 */
template <class T>
void MSSMtower_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MSSMtower_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MSSMtower_mass_eigenstates tmp_model(model);
   try {
   ...
   MSSMtower_parameter_getter parameter_getter;
   Coupling_monitor<MSSMtower_mass_eigenstates, MSSMtower_parameter_getter>
 ...
 */
template <class T>
void MSSMtower_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   MSSMtower_spectrum_plotter plotter;
 ...
 */
template <class T>
void MSSMtower_spectrum_generator_interface<T>::translate_exception_to_problem(MSSMtower<T>& model)
{
   try {
      throw;
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : MSSMtower_info::parameter_names[parameter_index];
