...
#include "NUTNMSSM_spectrum_generator_interface.hpp"
...
template <class T>
class NUTNMSSM_spectrum_generator
   : public NUTNMSSM_spectrum_generator_interface<T> {
public:
   NUTNMSSM_spectrum_generator()
      ...
      , low_scale(0.)
   {}
   virtual ~NUTNMSSM_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const NUTNMSSM_input_parameters&);
 ...
 */
template <class T>
void NUTNMSSM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const NUTNMSSM_input_parameters& input)
{
   NUTNMSSM<T>& model = this->model;
   model.clear();
   ...
   NUTNMSSM_high_scale_constraint<T> high_scale_constraint(&model);
   NUTNMSSM_susy_scale_constraint<T> susy_scale_constraint(&model, qedqcd);
   NUTNMSSM_low_scale_constraint<T>  low_scale_constraint(&model, qedqcd);
   ...
   NUTNMSSM_convergence_tester<T> convergence_tester(
   ...
   NUTNMSSM_initial_guesser<T> initial_guesser(&model, qedqcd,
 ...
 */
template <class T>
void NUTNMSSM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   NUTNMSSM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
