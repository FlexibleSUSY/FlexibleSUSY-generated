...
public:
   ...
   void run(const QedQcd& oneset, const NUTNMSSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "NUTNMSSM_rge_running.dat") const;
 ...
 */
template <class T>
void NUTNMSSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const NUTNMSSM_input_parameters& input)
{
   ...
   NUTNMSSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   NUTNMSSM_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void NUTNMSSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   NUTNMSSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   NUTNMSSM_parameter_getter parameter_getter;
   Coupling_monitor<NUTNMSSM<T>, NUTNMSSM_parameter_getter>
 ...
 */
template <class T>
void NUTNMSSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
