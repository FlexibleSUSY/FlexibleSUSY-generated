...
double NUTNMSSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void NUTNMSSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const NUTNMSSM_input_parameters& input)
{
   ...
   NUTNMSSM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   NUTNMSSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   NUTNMSSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   NUTNMSSM_convergence_tester<Two_scale> convergence_tester(
   ...
   NUTNMSSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void NUTNMSSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   NUTNMSSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void NUTNMSSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
