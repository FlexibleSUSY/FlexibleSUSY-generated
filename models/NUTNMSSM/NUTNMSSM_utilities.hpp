...
public:
   Eigen::ArrayXd get_parameters(const NUTNMSSM<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const NUTNMSSM<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const NUTNMSSM<T>&);
...
template <class T>
void NUTNMSSM_spectrum_plotter::extract_spectrum(const NUTNMSSM<T>& model)
{
   spectrum.clear();
