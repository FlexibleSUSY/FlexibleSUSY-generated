...
NUTNMSSM_spectrum_plotter::NUTNMSSM_spectrum_plotter()
   : spectrum()
   , scale(0.0)
   , width(16)
{
...
std::valarray<double> NUTNMSSM_spectrum_plotter::to_valarray(double v)
{
   ...
   const NUTNMSSM_observables* observables)
{
   ...
   append(names, NUTNMSSM_parameter_getter().get_mass_names());
   append(names, NUTNMSSM_parameter_getter().get_mixing_names());
   ...
   Physical_input* physical_input, NUTNMSSM_observables* observables)
{
   ...
   const std::size_t number_of_masses = NUTNMSSM_parameter_getter().get_number_of_masses();
   const std::size_t number_of_mixings = NUTNMSSM_info::NUMBER_OF_MIXINGS;
   const std::size_t number_of_input_parameters = NUTNMSSM_info::NUMBER_OF_INPUT_PARAMETERS;
   ...
   const std::size_t number_of_observables =
      (observables ? NUTNMSSM_observables::NUMBER_OF_OBSERVABLES : 0u);
