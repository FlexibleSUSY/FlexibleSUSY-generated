...
#include "lowe.h"
#include "NUTNMSSM_unitarity.hpp"
...
void put_spectrum(const NUTNMSSM_slha& model, MLINK link)
{
...
template <typename Solver_type>
void NUTNMSSM_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = NUTNMSSM_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSNUTNMSSMCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTNMSSMCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSNUTNMSSMCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
