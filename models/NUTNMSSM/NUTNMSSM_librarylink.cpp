...
struct NUTNMSSM_data {
   ...
   NUTNMSSM_slha<algorithm_type> model; ///< running parameters and pole masses
};

/// current handles
typedef std::map<Handle_id, NUTNMSSM_data> Handle_map;
Handle_map handles_NUTNMSSM;

/******************************************************************/

Handle_id get_new_NUTNMSSM_handle()
{
   static const std::size_t max_handles =
      static_cast<std::size_t>(std::exp2(8*sizeof(Handle_id)) - 1);

   if (handles_NUTNMSSM.size() >= max_handles)
      throw ENotEnoughFreeHandles(handles_NUTNMSSM.size());

   Handle_id hid = 0;

   while (handles_NUTNMSSM.find(hid) != handles_NUTNMSSM.end())
      hid++;
...
NUTNMSSM_data find_NUTNMSSM_data(Handle_id hid)
{
   const Handle_map::iterator handle = handles_NUTNMSSM.find(hid);

   if (handle == handles_NUTNMSSM.end())
...
void put_settings(const NUTNMSSM_data& data, MLINK link)
{
...
void put_sm_input_parameters(const NUTNMSSM_data& data, MLINK link)
{
...
void put_input_parameters(const NUTNMSSM_data& data, MLINK link)
{
...
void put_spectrum(const NUTNMSSM_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const NUTNMSSM_observables& observables, MLINK link)
{
...
void check_spectrum(const NUTNMSSM_data& data, MLINK link)
{
   ...
   if (problems.have_problem()) {
      ...
      put_message(link, "FSNUTNMSSMCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSNUTNMSSMCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(NUTNMSSM_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSNUTNMSSMCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = NUTNMSSM_slha<algorithm_type>(
...
NUTNMSSM_data make_NUTNMSSM_data(double* pars, mint npars)
{
   NUTNMSSM_data data;
...
DLLEXPORT int FSNUTNMSSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      const NUTNMSSM_data data = find_NUTNMSSM_data(hid);
   ...
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument Res)
{
   ...
   try {
      NUTNMSSM_data data = make_NUTNMSSM_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);

      const Handle_id hid = get_new_NUTNMSSM_handle();

      handles_NUTNMSSM.insert(std::make_pair(hid, std::move(data)));
   ...
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   ...
   if (handle == handles_NUTNMSSM.end()) {
      std::cerr << "Error: FSNUTNMSSMSet: Unknown handle: "
                << hid << std::endl;
      return LIBRARY_FUNCTION_ERROR;
   }

   try {
      handle->second = make_NUTNMSSM_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);
   } catch (const flexiblesusy::Error& e) {
      std::cerr << e.what() << std::endl;
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      NUTNMSSM_data data = find_NUTNMSSM_data(hid);
      ...
      handles_NUTNMSSM[hid] = std::move(data);
