...
// File generated at Sun 26 Aug 2018 14:45:13
...
namespace NUTNMSSM_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSNUTNMSSMMessage", 1);
...
class NUTNMSSM_spectrum_impl : public NUTNMSSM_spectrum
{
public:
   virtual ~NUTNMSSM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(NUTNMSSM_slha_io&) const override;
...
private:
   std::tuple<NUTNMSSM_slha<NUTNMSSM<Solver_type>>> models{};        ///< running parameters and pole masses
...
public:
   ...
   void set_input_parameters(const NUTNMSSM_input_parameters& input_) { input = input_; }
...
private:
   NUTNMSSM_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<NUTNMSSM_spectrum> spectrum{nullptr};  ///< spectrum information

   NUTNMSSM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSNUTNMSSMCalculateSpectrum[]?");
   }
}
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(MuEff), "MuEff");
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, NUTNMSSM_info::particle_names[i].c_str());
      }
   }
}
...
NUTNMSSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const NUTNMSSM_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void NUTNMSSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void NUTNMSSM_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const NUTNMSSM_input_parameters& input
)
{
   NUTNMSSM_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void NUTNMSSM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void NUTNMSSM_spectrum_impl<Solver_type>::fill_slha_io(NUTNMSSM_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   NUTNMSSM_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSNUTNMSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSNUTNMSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new NUTNMSSM_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSNUTNMSSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTNMSSMGetSettings"))
...
DLLEXPORT int FSNUTNMSSMGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTNMSSMGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUTNMSSMGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTNMSSMGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUTNMSSMOpenHandle(
...
DLLEXPORT int FSNUTNMSSMCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::NUTNMSSM_librarylink;
...
DLLEXPORT int FSNUTNMSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTNMSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUTNMSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTNMSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUTNMSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTNMSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUTNMSSMCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTNMSSMCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUTNMSSMCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTNMSSMCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSNUTNMSSMCalculateObservables", "warning",
      ...
      put_message(link, "FSNUTNMSSMCalculateObservables", "error", e.what());
      put_error_output(link);
   }
