...
/**
 * @class NUTNMSSM_slha<Two_scale>
 * @brief model class wrapper in SLHA convention
 */

template<>
class NUTNMSSM_slha<Two_scale> : public NUTNMSSM<Two_scale> {
public:
   explicit NUTNMSSM_slha(const NUTNMSSM_input_parameters& input_ = NUTNMSSM_input_parameters());
   explicit NUTNMSSM_slha(const NUTNMSSM<Two_scale>&, bool do_convert_masses_to_slha = true);
   virtual ~NUTNMSSM_slha();
...
std::ostream& operator<<(std::ostream& ostr, const NUTNMSSM_slha<Two_scale>& model);
