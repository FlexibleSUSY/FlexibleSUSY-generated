...
/**
 * @file NUTNMSSM_amm.cpp
 *
 * This file was generated with FlexibleSUSY 2.8.0 and SARAH 4.15.1 .
 */

#include "NUTNMSSM_amm.hpp"
#include "NUTNMSSM_mass_eigenstates.hpp"

#include "cxx_qft/NUTNMSSM_qft.hpp"
#include "cxx_qft/NUTNMSSM_particle_aliases.hpp"
...
double get_MLCP(const NUTNMSSM_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(NUTNMSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("NUTNMSSM_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("NUTNMSSM_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("NUTNMSSM_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("NUTNMSSM_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const NUTNMSSM_mass_eigenstates& model, double ml_pole, int idx)
{
   using namespace NUTNMSSM_cxx_diagrams::fields;

   const auto form_factors = NUTNMSSM_FFV_form_factors::calculate_form_factors<Lepton,Lepton,VP>(idx, idx, model, true);
...
template <typename Lepton>
double calculate_amm_impl(const NUTNMSSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("NUTNMSSM_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double NUTNMSSM_amm::calculate_amm(const NUTNMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   NUTNMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("NUTNMSSM_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUTNMSSM_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template double NUTNMSSM_amm::calculate_amm<NUTNMSSM_cxx_diagrams::fields::Fe>(const NUTNMSSM_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);

template <typename Lepton>
double NUTNMSSM_amm::calculate_amm_uncertainty(const NUTNMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   NUTNMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("NUTNMSSM_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUTNMSSM_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
// spliting templated function into header and cpp
// https://isocpp.org/wiki/faq/templates#separate-template-fn-defn-from-decl
template double NUTNMSSM_amm::calculate_amm_uncertainty<NUTNMSSM_cxx_diagrams::fields::Fe>(const NUTNMSSM_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);
