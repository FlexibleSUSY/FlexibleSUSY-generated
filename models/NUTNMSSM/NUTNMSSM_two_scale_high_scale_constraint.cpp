...
void NUTNMSSM_high_scale_constraint<Two_scale>::apply()
{
   ...
   if (std::fabs(model->get_g1()) > 3.0) {
      ...
      ERROR("NUTNMSSM_high_scale_constraint: Non-perturbative gauge "
   ...
   MODEL->set_TKappa(AKappaInput*Kappa);
...
void NUTNMSSM_high_scale_constraint<Two_scale>::set_input_parameters(const NUTNMSSM_input_parameters& inputPars_)
{
   inputPars = inputPars_;
}
