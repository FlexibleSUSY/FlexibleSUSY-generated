...
// File generated at Tue 10 Oct 2017 22:37:13

/**
 * @file NUTNMSSM_mass_eigenstates.cpp
 ...
 * parameters.
 *
 * This file was generated at Tue 10 Oct 2017 22:37:13 with FlexibleSUSY
 * 2.0.0 (git commit: e7cd01524dc37f9ba34ce6090bb584b8c724259f) and SARAH 4.12.0 .
 */

#include "NUTNMSSM_mass_eigenstates.hpp"
#include "NUTNMSSM_ewsb_solver_interface.hpp"
...
#define CLASSNAME NUTNMSSM_mass_eigenstates
...
CLASSNAME::NUTNMSSM_mass_eigenstates(const NUTNMSSM_input_parameters& input_)
   : NUTNMSSM_soft_parameters(input_)
#if defined(ENABLE_TWO_SCALE_SOLVER)
   , ewsb_solver(new NUTNMSSM_ewsb_solver<Two_scale>())
#endif
{
}
...
const NUTNMSSM_physical& CLASSNAME::get_physical() const
{
   return physical;
}

NUTNMSSM_physical& CLASSNAME::get_physical()
{
   return physical;
}

void CLASSNAME::set_physical(const NUTNMSSM_physical& physical_)
{
   physical = physical_;
}
...
void CLASSNAME::set_ewsb_solver(const std::shared_ptr<NUTNMSSM_ewsb_solver_interface>& solver)
{
   ewsb_solver = solver;
}
...
{
   if (!ewsb_solver) {
      throw SetupError("NUTNMSSM_mass_eigenstates::solve_ewsb_tree_level: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("NUTNMSSM_mass_eigenstates::solve_ewsb_one_loop: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("NUTNMSSM_mass_eigenstates::solve_ewsb: "
                       "no EWSB solver set");
   }

   VERBOSE_MSG("\t\tSolving NUTNMSSM EWSB at " << ewsb_loop_order << "-loop order");
...
void CLASSNAME::print(std::ostream& ostr) const
{
   ostr << "========================================\n"
           "NUTNMSSM\n"
           "========================================\n";
   NUTNMSSM_soft_parameters::print(ostr);
   ostr << "----------------------------------------\n"
...
{
   if (PHYSICAL(MSd).tail<6>().minCoeff() < 0.) problems.flag_pole_tachyon(NUTNMSSM_info::Sd);
   if (PHYSICAL(MSv).tail<3>().minCoeff() < 0.) problems.flag_pole_tachyon(NUTNMSSM_info::Sv);
   if (PHYSICAL(MSu).tail<6>().minCoeff() < 0.) problems.flag_pole_tachyon(NUTNMSSM_info::Su);
   if (PHYSICAL(MSe).tail<6>().minCoeff() < 0.) problems.flag_pole_tachyon(NUTNMSSM_info::Se);
   if (PHYSICAL(Mhh).tail<3>().minCoeff() < 0.) problems.flag_pole_tachyon(NUTNMSSM_info::hh);
   if (PHYSICAL(MAh).tail<2>().minCoeff() < 0.) problems.flag_pole_tachyon(NUTNMSSM_info::Ah);
   if (PHYSICAL(MHpm).tail<1>().minCoeff() < 0.) problems.flag_pole_tachyon(NUTNMSSM_info::Hpm);
...
{
   NUTNMSSM_soft_parameters::clear();
   clear_DRbar_parameters();
   physical.clear();
   problems.clear();
}
...
void CLASSNAME::run_to(double scale, double eps)
{
   if (eps < 0.0)
      eps = precision;
   NUTNMSSM_soft_parameters::run_to(scale, eps);
}
 ...
 * @return mass of LSP
 */
double CLASSNAME::get_lsp(NUTNMSSM_info::Particles& particle_type) const
{
   double lsp_mass = std::numeric_limits<double>::max();
   double tmp_mass;
   particle_type = NUTNMSSM_info::NUMBER_OF_PARTICLES;

   tmp_mass = Abs(PHYSICAL(MChi(0)));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = NUTNMSSM_info::Chi;
   }

   tmp_mass = Abs(PHYSICAL(MSv(0)));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = NUTNMSSM_info::Sv;
   }

   tmp_mass = Abs(PHYSICAL(MSu(0)));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = NUTNMSSM_info::Su;
   }

   tmp_mass = Abs(PHYSICAL(MSd(0)));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = NUTNMSSM_info::Sd;
   }

   tmp_mass = Abs(PHYSICAL(MSe(0)));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = NUTNMSSM_info::Se;
   }

   tmp_mass = Abs(PHYSICAL(MCha(0)));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = NUTNMSSM_info::Cha;
   }

   tmp_mass = Abs(PHYSICAL(MGlu));
   if (tmp_mass < lsp_mass) {
      lsp_mass = tmp_mass;
      particle_type = NUTNMSSM_info::Glu;
   }
...
void CLASSNAME::calculate_MSd()
{
   ...
   problems.flag_bad_mass(NUTNMSSM_info::Sd, eigenvalue_error > precision
      * Abs(MSd(0)));
#else
   ...
   if (MSd.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUTNMSSM_info::Sd);
   }
...
void CLASSNAME::calculate_MSv()
{
   ...
   problems.flag_bad_mass(NUTNMSSM_info::Sv, eigenvalue_error > precision
      * Abs(MSv(0)));
#else
   ...
   if (MSv.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUTNMSSM_info::Sv);
   }
...
void CLASSNAME::calculate_MSu()
{
   ...
   problems.flag_bad_mass(NUTNMSSM_info::Su, eigenvalue_error > precision
      * Abs(MSu(0)));
#else
   ...
   if (MSu.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUTNMSSM_info::Su);
   }
...
void CLASSNAME::calculate_MSe()
{
   ...
   problems.flag_bad_mass(NUTNMSSM_info::Se, eigenvalue_error > precision
      * Abs(MSe(0)));
#else
   ...
   if (MSe.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUTNMSSM_info::Se);
   }
...
void CLASSNAME::calculate_Mhh()
{
   ...
   problems.flag_bad_mass(NUTNMSSM_info::hh, eigenvalue_error > precision
      * Abs(Mhh(0)));
#else
   ...
   if (Mhh.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUTNMSSM_info::hh);
   }
...
void CLASSNAME::calculate_MAh()
{
   ...
   problems.flag_bad_mass(NUTNMSSM_info::Ah, eigenvalue_error > precision
      * Abs(MAh(0)));
#else
   ...
   if (MAh.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUTNMSSM_info::Ah);
   }
...
void CLASSNAME::calculate_MHpm()
{
   ...
   problems.flag_bad_mass(NUTNMSSM_info::Hpm, eigenvalue_error >
   ...
   if (MHpm.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUTNMSSM_info::Hpm);
   }
...
void CLASSNAME::calculate_MChi()
{
   ...
   problems.flag_bad_mass(NUTNMSSM_info::Chi, eigenvalue_error >
...
void CLASSNAME::calculate_MCha()
{
   ...
   problems.flag_bad_mass(NUTNMSSM_info::Cha, eigenvalue_error >
...
void CLASSNAME::calculate_MFe()
{
   ...
   problems.flag_bad_mass(NUTNMSSM_info::Fe, eigenvalue_error > precision
      * Abs(MFe(0)));
#else
...
void CLASSNAME::calculate_MFd()
{
   ...
   problems.flag_bad_mass(NUTNMSSM_info::Fd, eigenvalue_error > precision
      * Abs(MFd(0)));
#else
...
void CLASSNAME::calculate_MFu()
{
   ...
   problems.flag_bad_mass(NUTNMSSM_info::Fu, eigenvalue_error > precision
      * Abs(MFu(0)));
#else
...
void CLASSNAME::calculate_MVWm()
{
   ...
   if (MVWm < 0.) {
      problems.flag_running_tachyon(NUTNMSSM_info::VWm);
   }
...
{
   if (!force_output && problems.is_running_tachyon(NUTNMSSM_info::VZ))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(NUTNMSSM_info::VZ);
...
{
   if (!force_output && problems.is_running_tachyon(NUTNMSSM_info::Sd))
      return;
   ...
   for (int es = 0; es < 6; ++es) {
      ...
      Eigen::Matrix<double,6,6> mix_ZD;
         ...
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZD,
            eigenvalue_error);
         problems.flag_bad_mass(NUTNMSSM_info::Sd, eigenvalue_error
...
{
   if (!force_output && problems.is_running_tachyon(NUTNMSSM_info::Sv))
      return;
   ...
   for (int es = 0; es < 3; ++es) {
      ...
      Eigen::Matrix<double,3,3> mix_ZV;
         ...
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZV,
            eigenvalue_error);
         problems.flag_bad_mass(NUTNMSSM_info::Sv, eigenvalue_error
            > precision * Abs(eigen_values(0)));
      #else
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZV);
      #endif
...
{
   if (!force_output && problems.is_running_tachyon(NUTNMSSM_info::Su))
      return;
   ...
   for (int es = 0; es < 6; ++es) {
      ...
      Eigen::Matrix<double,6,6> mix_ZU;
         ...
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZU,
            eigenvalue_error);
         problems.flag_bad_mass(NUTNMSSM_info::Su, eigenvalue_error
            > precision * Abs(eigen_values(0)));
      #else
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZU);
      #endif
...
{
   if (!force_output && problems.is_running_tachyon(NUTNMSSM_info::Se))
      return;
   ...
   for (int es = 0; es < 6; ++es) {
      ...
      Eigen::Matrix<double,6,6> mix_ZE;
         ...
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZE,
            eigenvalue_error);
         problems.flag_bad_mass(NUTNMSSM_info::Se, eigenvalue_error
            > precision * Abs(eigen_values(0)));
      #else
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZE);
      #endif
...
{
   if (!force_output && problems.is_running_tachyon(NUTNMSSM_info::hh))
      return;
      ...
      for (int i = 0; i < 3; i++) {
         for (int k = 0; k < 3; k++) {
            if (!std::isfinite(self_energy_2l(i,k))) {
               ...
               problems.flag_bad_mass(NUTNMSSM_info::hh);
            }
         }
      }
   }
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(NUTNMSSM_info::hh);
   else
      problems.unflag_no_pole_mass_convergence(NUTNMSSM_info::hh);
}

void CLASSNAME::calculate_MAh_pole()
{
   if (!force_output && problems.is_running_tachyon(NUTNMSSM_info::Ah))
      return;
   ...
   if (pole_mass_loop_order > 1) {
      ...
      for (int i = 0; i < 3; i++) {
         for (int k = 0; k < 3; k++) {
            if (!std::isfinite(self_energy_2l(i,k))) {
               self_energy_2l(i,k) = 0.;
               problems.flag_bad_mass(NUTNMSSM_info::Ah);
            }
         }
      }
   }
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(NUTNMSSM_info::Ah);
   else
      problems.unflag_no_pole_mass_convergence(NUTNMSSM_info::Ah);
}

void CLASSNAME::calculate_MHpm_pole()
{
   if (!force_output && problems.is_running_tachyon(NUTNMSSM_info::Hpm))
      return;
   ...
   do {
      ...
      for (int es = 0; es < 2; ++es) {
         ...
         Eigen::Matrix<double,2,2> mix_ZP;
            ...
            problems.flag_bad_mass(NUTNMSSM_info::Hpm,
               eigenvalue_error > precision * Abs(eigen_values(0)));
         #else
            fs_diagonalize_hermitian(M_loop, eigen_values,
               mix_ZP);
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(NUTNMSSM_info::Hpm);
   else
      problems.unflag_no_pole_mass_convergence(NUTNMSSM_info::Hpm);
}
   ...
   for (int es = 0; es < 5; ++es) {
      ...
      decltype(ZN) mix_ZN;
         ...
         fs_diagonalize_symmetric(M_loop, eigen_values, mix_ZN,
            eigenvalue_error);
         problems.flag_bad_mass(NUTNMSSM_info::Chi,
            eigenvalue_error > precision * Abs(eigen_values(0)));
      #else
         fs_diagonalize_symmetric(M_loop, eigen_values, mix_ZN);
      #endif
...
void CLASSNAME::calculate_MFe_pole()
{
   ...
   for (int es = 0; es < 3; ++es) {
      ...
      fs_svd(M_loop, eigen_values, mix_ZEL, mix_ZER, eigenvalue_error)
         ;
      problems.flag_bad_mass(NUTNMSSM_info::Fe, eigenvalue_error >
         precision * Abs(eigen_values(0)));
   #else
...
void CLASSNAME::calculate_MFd_pole()
{
   ...
   for (int es = 0; es < 3; ++es) {
      ...
      fs_svd(M_loop, eigen_values, mix_ZDL, mix_ZDR, eigenvalue_error)
         ;
      problems.flag_bad_mass(NUTNMSSM_info::Fd, eigenvalue_error >
         precision * Abs(eigen_values(0)));
   #else
...
void CLASSNAME::calculate_MFu_pole()
{
   ...
   for (int es = 0; es < 3; ++es) {
      ...
      fs_svd(M_loop, eigen_values, mix_ZUL, mix_ZUR, eigenvalue_error)
         ;
      problems.flag_bad_mass(NUTNMSSM_info::Fu, eigenvalue_error >
         precision * Abs(eigen_values(0)));
   #else
...
{
   if (!force_output && problems.is_running_tachyon(NUTNMSSM_info::VWm))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(NUTNMSSM_info::VWm);
...
std::ostream& operator<<(std::ostream& ostr, const NUTNMSSM_mass_eigenstates& model)
{
   model.print(ostr);
   return ostr;
}
