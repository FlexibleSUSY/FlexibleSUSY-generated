...
/**
 * @file NUTNMSSM_model_slha.hpp
 * @brief contains wrapper class for model class in SLHA convention
 */

// File generated at Fri 20 Oct 2017 09:00:40

#ifndef NUTNMSSM_SLHA_H
#define NUTNMSSM_SLHA_H
...
class NUTNMSSM_slha : public Model {
public:
   explicit NUTNMSSM_slha(const NUTNMSSM_input_parameters& input_ = NUTNMSSM_input_parameters());
   explicit NUTNMSSM_slha(const Model&, bool do_convert_masses_to_slha = true);
   NUTNMSSM_slha(const NUTNMSSM_slha&) = default;
   NUTNMSSM_slha(NUTNMSSM_slha&&) = default;
   virtual ~NUTNMSSM_slha() = default;
   NUTNMSSM_slha& operator=(const NUTNMSSM_slha&) = default;
   NUTNMSSM_slha& operator=(NUTNMSSM_slha&&) = default;
   ...
   const NUTNMSSM_physical& get_physical_slha() const; ///< returns pole masses to SLHA convention
   NUTNMSSM_physical& get_physical_slha(); ///< returns pole masses to SLHA convention
...
private:
   NUTNMSSM_physical physical_slha{}; ///< contains the pole masses and mixings in slha convention
...
template <class Model>
NUTNMSSM_slha<Model>::NUTNMSSM_slha(const NUTNMSSM_input_parameters& input_)
   : Model(input_)
{
}
 ...
 */
template <class Model>
NUTNMSSM_slha<Model>::NUTNMSSM_slha(const Model& model_,
...
template <class Model>
void NUTNMSSM_slha<Model>::clear()
{
   Model::clear();
   physical_slha.clear();
}
...
template <class Model>
void NUTNMSSM_slha<Model>::calculate_spectrum()
{
...
template <class Model>
void NUTNMSSM_slha<Model>::convert_to_slha()
{
...
template <class Model>
void NUTNMSSM_slha<Model>::calculate_ckm_matrix()
{
...
template <class Model>
void NUTNMSSM_slha<Model>::calculate_pmns_matrix()
{
 ...
 */
template <class Model>
void NUTNMSSM_slha<Model>::convert_yukawa_couplings_to_slha()
{
 ...
 */
template <class Model>
void NUTNMSSM_slha<Model>::convert_trilinear_couplings_to_slha()
{
 ...
 */
template <class Model>
void NUTNMSSM_slha<Model>::convert_soft_squared_masses_to_slha()
{
...
template <class Model>
const NUTNMSSM_physical& NUTNMSSM_slha<Model>::get_physical_slha() const
{
   return physical_slha;
}

template <class Model>
NUTNMSSM_physical& NUTNMSSM_slha<Model>::get_physical_slha()
{
   return physical_slha;
}

template <class Model>
void NUTNMSSM_slha<Model>::print(std::ostream& ostr) const
{
   Model::print(ostr);
...
template <class Model>
void NUTNMSSM_slha<Model>::set_convert_masses_to_slha(bool flag)
{
   convert_masses_to_slha = flag;
}

template <class Model>
std::ostream& operator<<(std::ostream& ostr, const NUTNMSSM_slha<Model>& model)
{
   model.print(ostr);
   return ostr;
}
