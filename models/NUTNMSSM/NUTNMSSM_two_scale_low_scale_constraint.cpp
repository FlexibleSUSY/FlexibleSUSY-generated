...
// File generated at Tue 8 Sep 2015 13:08:15
...
NUTNMSSM_low_scale_constraint<Two_scale>::NUTNMSSM_low_scale_constraint(
   NUTNMSSM<Two_scale>* model_, const softsusy::QedQcd& oneset_)
   : Constraint<Two_scale>()
   , model(model_)
   , oneset(oneset_)
   , new_g1(0.)
...
{
   assert(model && "Error: NUTNMSSM_low_scale_constraint::apply():"
...
void NUTNMSSM_low_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
...
{
   assert(model && "NUTNMSSM_low_scale_constraint<Two_scale>::"
...
double NUTNMSSM_low_scale_constraint<Two_scale>::calculate_theta_w(double alpha_em_drbar)
{
   assert(model && "NUTNMSSM_low_scale_constraint<Two_scale>::"
 ...
 */
void NUTNMSSM_low_scale_constraint<Two_scale>::recalculate_mw_pole()
{
   assert(model && "NUTNMSSM_low_scale_constraint<Two_scale>::"
