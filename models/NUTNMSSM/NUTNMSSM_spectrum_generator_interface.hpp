...
// File generated at Tue 22 Jan 2019 17:34:13
...
template <class T>
class NUTNMSSM_spectrum_generator_interface {
public:
   virtual ~NUTNMSSM_spectrum_generator_interface() = default;

   std::tuple<NUTNMSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<NUTNMSSM_slha<NUTNMSSM<T>>> get_models_slha() const
   { return std::make_tuple(NUTNMSSM_slha<NUTNMSSM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const NUTNMSSM<T>& get_model() const
   { return model; }
   NUTNMSSM<T>& get_model()
   { return model; }
   NUTNMSSM_slha<NUTNMSSM<T>> get_model_slha() const
   { return NUTNMSSM_slha<NUTNMSSM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const NUTNMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "NUTNMSSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(NUTNMSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const NUTNMSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void NUTNMSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void NUTNMSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const NUTNMSSM_input_parameters& input)
{
 ...
 */
template <class T>
void NUTNMSSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   NUTNMSSM_mass_eigenstates tmp_model(model);
   try {
   ...
   NUTNMSSM_parameter_getter parameter_getter;
   Coupling_monitor<NUTNMSSM_mass_eigenstates, NUTNMSSM_parameter_getter>
 ...
 */
template <class T>
void NUTNMSSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   NUTNMSSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void NUTNMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(NUTNMSSM<T>& model)
{
   try {
      throw;
