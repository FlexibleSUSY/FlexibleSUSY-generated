...
public:
   NUTNMSSM_spectrum_generator_interface()
      : model()
      , settings()
   ...
   virtual void run(const softsusy::QedQcd&, const NUTNMSSM_input_parameters&) = 0;
 ...
 */
template <class T>
void NUTNMSSM_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   NUTNMSSM_spectrum_plotter plotter;
...
void NUTNMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(NUTNMSSM<T>& model)
{
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : NUTNMSSM_info::parameter_names[parameter_index];
