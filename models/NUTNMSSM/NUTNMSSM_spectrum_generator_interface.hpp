...
public:
   ...
   std::tuple<NUTNMSSM<T>> get_models_slha() const
   { return std::make_tuple(NUTNMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const NUTNMSSM<T>& get_model() const
   { return model; }
   NUTNMSSM<T>& get_model()
   { return model; }
   NUTNMSSM<T> get_model_slha() const
   { return NUTNMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return NUTNMSSM_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(NUTNMSSM_info::parameter_names),
      std::cend(NUTNMSSM_info::parameter_names));
