...
void run_to_MSUSY(NUTNMSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("NUTNMSSM_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("NUTNMSSM_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("NUTNMSSM_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("NUTNMSSM_a_muon: run_to_MSUSY(): "
...
double calculate_a_muon_impl(const NUTNMSSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   VERBOSE_MSG("NUTNMSSM_a_muon: calculating a_mu at Q = " << model.get_scale());

   context_base context{ model };

   using namespace NUTNMSSM_cxx_diagrams::fields;

   const auto form_factors = NUTNMSSM_FFV_form_factors::calculate_Fe_Fe_VP_form_factors(1, 1, model, true);
...
std::pair<double,double> vary_scale(const NUTNMSSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model,&qedqcd] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("NUTNMSSM_a_muon: scale variation: " << e.what_detailed());
                     }
                     return amu;
                  });
...
double NUTNMSSM_a_muon::calculate_a_muon(const NUTNMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   NUTNMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("NUTNMSSM_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUTNMSSM_a_muon:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
double NUTNMSSM_a_muon::calculate_a_muon_uncertainty(const NUTNMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   NUTNMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("NUTNMSSM_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUTNMSSM_a_muon uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
