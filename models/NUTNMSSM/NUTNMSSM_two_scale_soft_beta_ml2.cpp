 ...
 */
Eigen::Matrix<double,3,3> NUTNMSSM_soft_parameters::calc_beta_ml2_one_loop(const Soft_traces& soft_traces) const
{
 ...
 */
Eigen::Matrix<double,3,3> NUTNMSSM_soft_parameters::calc_beta_ml2_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_ml2 = twoLoop*(-6*traceconjTYdTpTYd*(Ye.adjoint()*Ye) - 2*
      ...
      *MassB*(Ye.adjoint()*Ye) - 20*(Ye.adjoint()*TYe) + 3*(69*MassB*Sqr(g1) +
      5*(2*MassB + MassWB)*Sqr(g2))*UNITMATRIX(3)));
