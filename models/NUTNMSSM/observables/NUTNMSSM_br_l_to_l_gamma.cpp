...
/**
 * @file NUTNMSSM_br_l_to_l_gamma.cpp
 *
...
#include "NUTNMSSM_br_l_to_l_gamma.hpp"
...
void run_to_MSUSY(NUTNMSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("NUTNMSSM_l_to_lgamma: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("NUTNMSSM_l_to_lgamma:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("NUTNMSSM_l_to_lgamma: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("NUTNMSSM_l_to_lgamma: run_to_MSUSY(): "
...
namespace NUTNMSSM_br_l_to_l_gamma {
