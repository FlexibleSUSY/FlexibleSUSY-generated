...
void lowMSSM_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
...
double lowMSSM_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();

   double theta_w = std::asin(Electroweak_constants::sinThetaW);

   lowMSSM_weinberg_angle::Sm_parameters sm_pars;
   ...
   lowMSSM_weinberg_angle weinberg(MODEL, sm_pars);
...
double lowMSSM_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
void lowMSSM_low_scale_constraint<Two_scale>::calculate_running_SM_masses()
{
   check_model_ptr();
...
void lowMSSM_low_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("lowMSSM_low_scale_constraint<Two_scale>: "
