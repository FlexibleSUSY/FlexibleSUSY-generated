...
#ifndef lowMSSM_SPECTRUM_GENERATOR_INTERFACE_H
#define lowMSSM_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class lowMSSM_spectrum_generator_interface {
public:
   virtual ~lowMSSM_spectrum_generator_interface() = default;

   std::tuple<lowMSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<lowMSSM_slha<lowMSSM<T>>> get_models_slha() const
   { return std::make_tuple(lowMSSM_slha<lowMSSM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const lowMSSM<T>& get_model() const
   { return model; }
   lowMSSM<T>& get_model()
   { return model; }
   lowMSSM_slha<lowMSSM<T>> get_model_slha() const
   { return lowMSSM_slha<lowMSSM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const lowMSSM_input_parameters&);
...
protected:
   ...
   void translate_exception_to_problem(lowMSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const lowMSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void lowMSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void lowMSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const lowMSSM_input_parameters& input)
{
 ...
 */
template <class T>
void lowMSSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   lowMSSM_mass_eigenstates tmp_model(model);
   try {
   ...
   Coupling_monitor<lowMSSM_mass_eigenstates, lowMSSM_parameter_getter>
 ...
 */
template <class T>
void lowMSSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   lowMSSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void lowMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(lowMSSM<T>& model)
{
   try {
      throw;
