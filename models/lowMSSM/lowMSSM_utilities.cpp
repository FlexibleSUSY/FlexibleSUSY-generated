...
// File generated at Tue 10 Oct 2017 22:46:36
...
lowMSSM_spectrum_plotter::lowMSSM_spectrum_plotter(const lowMSSM_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void lowMSSM_spectrum_plotter::extract_spectrum(const lowMSSM_mass_eigenstates& model)
{
   spectrum.clear();
...
void lowMSSM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("lowMSSM_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("lowMSSM_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("lowMSSM_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void lowMSSM_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("lowMSSM_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const lowMSSM_mass_eigenstates& model,
   ...
   const lowMSSM_observables* observables)
{
   ...
   append(names, lowMSSM_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, lowMSSM_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, lowMSSM_parameter_getter().get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, lowMSSM_parameter_getter().get_DRbar_mass_names());
   append(names, lowMSSM_parameter_getter().get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, lowMSSM_parameter_getter().get_pole_mass_names());
   append(names, lowMSSM_parameter_getter().get_pole_mixing_names());
   ...
   if (lowMSSM_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, lowMSSM_observables::get_names());
...
lowMSSM_mass_eigenstates from_database(
   ...
   Physical_input* physical_input, lowMSSM_observables* observables)
{
   using utilities::append;

   lowMSSM_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = lowMSSM_parameter_getter().get_number_of_masses();
   const auto number_of_mixings = lowMSSM_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = lowMSSM_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = lowMSSM_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? lowMSSM_observables::NUMBER_OF_OBSERVABLES : 0);
