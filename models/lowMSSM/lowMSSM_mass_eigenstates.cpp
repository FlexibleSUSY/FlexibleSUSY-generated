...
// File generated at Sun 26 Aug 2018 14:54:50
 ...
 * parameters.
 *
 * This file was generated at Sun 26 Aug 2018 14:54:50 with FlexibleSUSY
...
#include "lowMSSM_two_scale_ewsb_solver.hpp"
#endif
...
CLASSNAME::lowMSSM_mass_eigenstates(const lowMSSM_input_parameters& input_)
   ...
   , ewsb_solver(new lowMSSM_ewsb_solver<Two_scale>())
#endif
{
}
...
void CLASSNAME::set_ewsb_solver(const std::shared_ptr<lowMSSM_ewsb_solver_interface>& solver)
{
   ewsb_solver = solver;
}
...
{
   if (!ewsb_solver) {
      throw SetupError("lowMSSM_mass_eigenstates::solve_ewsb_tree_level: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("lowMSSM_mass_eigenstates::solve_ewsb_one_loop: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("lowMSSM_mass_eigenstates::solve_ewsb: "
                       "no EWSB solver set");
   }

   VERBOSE_MSG("\t\tSolving lowMSSM EWSB at " << ewsb_loop_order << "-loop order");
...
{
   if (!force_output && problems.is_running_tachyon(lowMSSM_info::VZ))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(lowMSSM_info::VZ);
...
{
   if (!force_output && problems.is_running_tachyon(lowMSSM_info::Sd))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(lowMSSM_info::Sv))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(lowMSSM_info::Su))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(lowMSSM_info::Se))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(lowMSSM_info::hh))
      return;
      ...
      for (int es = 0; es < 2; ++es) {
         ...
         Eigen::Matrix<double,2,2> mix_ZH;
            ...
            fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZH,
               eigenvalue_error);
            problems.flag_bad_mass(lowMSSM_info::hh, eigenvalue_error >
...
{
   if (!force_output && problems.is_running_tachyon(lowMSSM_info::Ah))
      return;
      ...
      for (int es = 0; es < 2; ++es) {
         ...
         Eigen::Matrix<double,2,2> mix_ZA;
            ...
            fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZA,
               eigenvalue_error);
            problems.flag_bad_mass(lowMSSM_info::Ah, eigenvalue_error >
               precision * Abs(eigen_values(0)));
         #else
...
{
   if (!force_output && problems.is_running_tachyon(lowMSSM_info::Hpm))
      return;
      ...
      for (int es = 0; es < 2; ++es) {
         ...
         Eigen::Matrix<double,2,2> mix_ZP;
            ...
            fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZP,
               eigenvalue_error);
            problems.flag_bad_mass(lowMSSM_info::Hpm, eigenvalue_error >
               precision * Abs(eigen_values(0)));
         #else
...
{
   if (!force_output && problems.is_running_tachyon(lowMSSM_info::VWm))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(lowMSSM_info::VWm);
