...
// File generated at Thu 15 Dec 2016 13:04:36
...
struct lowMSSM_data {
   ...
   lowMSSM_slha<algorithm_type> model; ///< running parameters and pole masses
};

/// current handles
typedef std::map<Handle_id, lowMSSM_data> Handle_map;
Handle_map handles_lowMSSM;

/******************************************************************/

Handle_id get_new_lowMSSM_handle()
{
   static const std::size_t max_handles =
      static_cast<std::size_t>(std::exp2(8*sizeof(Handle_id)) - 1);

   if (handles_lowMSSM.size() >= max_handles)
      throw ENotEnoughFreeHandles(handles_lowMSSM.size());

   Handle_id hid = 0;

   while (handles_lowMSSM.find(hid) != handles_lowMSSM.end())
      hid++;
...
lowMSSM_data find_lowMSSM_data(Handle_id hid)
{
   const Handle_map::iterator handle = handles_lowMSSM.find(hid);

   if (handle == handles_lowMSSM.end())
...
void put_settings(const lowMSSM_data& data, MLINK link)
{
...
void put_sm_input_parameters(const lowMSSM_data& data, MLINK link)
{
...
void put_input_parameters(const lowMSSM_data& data, MLINK link)
{
...
void put_spectrum(const lowMSSM_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const lowMSSM_observables& observables, MLINK link)
{
...
void check_spectrum(const lowMSSM_data& data, MLINK link)
{
   const Problems<lowMSSM_info::NUMBER_OF_PARTICLES>& problems
      = data.model.get_problems();

   if (problems.have_problem()) {
      std::ostringstream msg;
      problems.print_problems(msg);
      put_message(link, "FSlowMSSMCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSlowMSSMCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(lowMSSM_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSlowMSSMCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = lowMSSM_slha<algorithm_type>(
...
lowMSSM_data make_lowMSSM_data(double* pars, mint npars)
{
   lowMSSM_data data;

   const mint n_settings = 23, n_sm_parameters = 29, n_input_pars = 78;
...
DLLEXPORT int FSlowMSSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      const lowMSSM_data data = find_lowMSSM_data(hid);
   ...
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument Res)
{
   ...
   try {
      lowMSSM_data data = make_lowMSSM_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);

      const Handle_id hid = get_new_lowMSSM_handle();

      handles_lowMSSM.insert(std::make_pair(hid, std::move(data)));
   ...
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   ...
   if (handle == handles_lowMSSM.end()) {
      std::cerr << "Error: FSlowMSSMSet: Unknown handle: "
                << hid << std::endl;
      return LIBRARY_FUNCTION_ERROR;
   }

   try {
      handle->second = make_lowMSSM_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);
   } catch (const flexiblesusy::Error& e) {
      std::cerr << e.what() << std::endl;
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      lowMSSM_data data = find_lowMSSM_data(hid);
      ...
      handles_lowMSSM[hid] = std::move(data);
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSlowMSSMCalculateObservables", "error", e.what());
      put_error_output(link);
   }
