...
public:
   Eigen::ArrayXd get_parameters(const lowMSSM<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const lowMSSM<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const lowMSSM<T>&);
...
template <class T>
void lowMSSM_spectrum_plotter::extract_spectrum(const lowMSSM<T>& model)
{
   spectrum.clear();
