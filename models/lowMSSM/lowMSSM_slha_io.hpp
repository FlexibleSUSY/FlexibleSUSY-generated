...
// File generated at Mon 23 Feb 2015 13:40:06
...
struct lowMSSM_scales {
   lowMSSM_scales() : HighScale(0.), SUSYScale(0.), LowScale(0.) {}
...
public:
   ...
   template <class T> void fill(lowMSSM_slha<T>&) const;
   void fill(Spectrum_generator_settings&) const;
   double get_parameter_output_scale() const;
   const SLHA_io& get_slha_io() const { return slha_io; }
   void read_from_file(const std::string&);
   void set_extpar(const lowMSSM_input_parameters&);
   template <class T> void set_extra(const lowMSSM_slha<T>&, const lowMSSM_scales&);
   void set_minpar(const lowMSSM_input_parameters&);
   void set_sminputs(const softsusy::QedQcd&);
   template <class T> void set_spectrum(const lowMSSM_slha<T>&);
   ...
   template <class T>
   static void fill_slhaea(SLHAea::Coll&, const lowMSSM_slha<T>&, const QedQcd&, const lowMSSM_scales&);

   template <class T>
   static SLHAea::Coll fill_slhaea(const lowMSSM_slha<T>&, const QedQcd&);

   template <class T>
   static SLHAea::Coll fill_slhaea(const lowMSSM_slha<T>&, const QedQcd&, const lowMSSM_scales&);
...
private:
   ...
   template <class T> void set_model_parameters(const lowMSSM_slha<T>&);
   double read_scale() const;
   template <class T> void fill_drbar_parameters(lowMSSM_slha<T>&) const;
   template <class T> void fill_physical(lowMSSM_slha<T>&) const;
};
 ...
 */
template <class T>
void lowMSSM_slha_io::fill(lowMSSM_slha<T>& model) const
{
 ...
 */
template <class T>
void lowMSSM_slha_io::fill_drbar_parameters(lowMSSM_slha<T>& model) const
{
 ...
 */
template <class T>
void lowMSSM_slha_io::fill_physical(lowMSSM_slha<T>& model) const
{
   {
      DEFINE_PARAMETER(ZD);
...
void lowMSSM_slha_io::fill_slhaea(
   SLHAea::Coll& slhaea, const lowMSSM_slha<T>& model,
   const QedQcd& qedqcd, const lowMSSM_scales& scales)
{
...
SLHAea::Coll lowMSSM_slha_io::fill_slhaea(
   const lowMSSM_slha<T>& model, const QedQcd& qedqcd)
{
...
{
   SLHAea::Coll slhaea;
   lowMSSM_slha_io::fill_slhaea(slhaea, model, qedqcd, scales);
 ...
 */
template <class T>
void lowMSSM_slha_io::set_model_parameters(const lowMSSM_slha<T>& model)
{
   {
...
void lowMSSM_slha_io::set_extra(
   const lowMSSM_slha<T>& model, const lowMSSM_scales& scales)
{
...
{
   const lowMSSM_slha<T> model_slha(model);
 ...
 */
template <class T>
void lowMSSM_slha_io::set_spectrum(const lowMSSM_slha<T>& model)
{
   const lowMSSM_physical physical(model.get_physical_slha());
