...
/**
 * @file lowMSSM_br_l_to_l_gamma.cpp
 *
...
#include "lowMSSM_br_l_to_l_gamma.hpp"
...
void run_to_MSUSY(lowMSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("lowMSSM_l_to_lgamma: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("lowMSSM_l_to_lgamma:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("lowMSSM_l_to_lgamma: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("lowMSSM_l_to_lgamma: run_to_MSUSY(): "
...
namespace lowMSSM_br_l_to_l_gamma {
