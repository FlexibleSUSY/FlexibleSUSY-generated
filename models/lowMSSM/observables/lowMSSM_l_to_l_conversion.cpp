...
/**
 * @file lowMSSM_l_to_l_conversion.cpp
 *
...
#include "lowMSSM_l_to_l_conversion.hpp"
...
void run_to_MSUSY(lowMSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("lowMSSM_l_to_l_conversion: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("lowMSSM_l_to_l_conversion:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("lowMSSM_l_to_l_conversion: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("lowMSSM_l_to_l_conversion: run_to_MSUSY(): "
...
namespace lowMSSM_l_to_l_conversion {
...
typedef std::array<std::complex<double>, 10> (*npf_function)
   (const lowMSSM_mass_eigenstates&, const std::array<int,4>&, const std::array<Eigen::Vector4d,0>&);
...
std::complex<double> left(const lowMSSM_mass_eigenstates& model, int g) {
 ...
 */
template <class Q, class A>
std::complex<double> right(const lowMSSM_mass_eigenstates& model, int g) {
    context_base context {model};
    using vertex = Vertex<typename Q::lorentz_conjugate, Q, A>;
    std::array<int, 2> indices {g, g};
    const auto value =  vertex::evaluate(indices, context);
...
Eigen::Array<std::complex<double>,10,1> fix_tensors_sign(
   const lowMSSM_mass_eigenstates& model, int in, int out, int g) {
 ...
 *       for photon, as in lowMSSM, using eq. (t.3).
 ...
 *          <out:q4,l3| T exp(i L_lowMSSM dx) |in:q2,l1> =: G4
...
Eigen::Array<std::complex<double>, 13, 1> forge_conversion(int in, int out,
   const lowMSSM_l_to_l_conversion::Nucleus nucleus,
   ...
   try {
      ...
      ERROR("lowMSSM_l_to_l_conversion:" << e.what_detailed());
