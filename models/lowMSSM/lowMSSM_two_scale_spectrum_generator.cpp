...
double lowMSSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void lowMSSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const lowMSSM_input_parameters& input)
{
   ...
   lowMSSM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<lowMSSM_ewsb_solver<Two_scale> >(ewsb_solver));

   lowMSSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   lowMSSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   lowMSSM_convergence_tester<Two_scale> convergence_tester(
   ...
   lowMSSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void lowMSSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   lowMSSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_susy_scale());
}

void lowMSSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
