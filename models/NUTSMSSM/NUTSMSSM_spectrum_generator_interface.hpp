...
// File generated at Tue 22 Jan 2019 17:31:04

#ifndef NUTSMSSM_SPECTRUM_GENERATOR_INTERFACE_H
#define NUTSMSSM_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class NUTSMSSM_spectrum_generator_interface {
public:
   virtual ~NUTSMSSM_spectrum_generator_interface() = default;

   std::tuple<NUTSMSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<NUTSMSSM_slha<NUTSMSSM<T>>> get_models_slha() const
   { return std::make_tuple(NUTSMSSM_slha<NUTSMSSM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const NUTSMSSM<T>& get_model() const
   { return model; }
   NUTSMSSM<T>& get_model()
   { return model; }
   NUTSMSSM_slha<NUTSMSSM<T>> get_model_slha() const
   { return NUTSMSSM_slha<NUTSMSSM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const NUTSMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "NUTSMSSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(NUTSMSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const NUTSMSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void NUTSMSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void NUTSMSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const NUTSMSSM_input_parameters& input)
{
 ...
 */
template <class T>
void NUTSMSSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   NUTSMSSM_mass_eigenstates tmp_model(model);
   try {
   ...
   NUTSMSSM_parameter_getter parameter_getter;
   Coupling_monitor<NUTSMSSM_mass_eigenstates, NUTSMSSM_parameter_getter>
 ...
 */
template <class T>
void NUTSMSSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   NUTSMSSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void NUTSMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(NUTSMSSM<T>& model)
{
   try {
      throw;
