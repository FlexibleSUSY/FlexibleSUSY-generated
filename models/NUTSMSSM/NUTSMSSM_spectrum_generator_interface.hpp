...
// File generated at Sat 15 Oct 2016 15:43:46
...
public:
   NUTSMSSM_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~NUTSMSSM_spectrum_generator_interface() {}

   const NUTSMSSM<T>& get_model() const { return model; }
   NUTSMSSM<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const NUTSMSSM_input_parameters&) = 0;
 ...
 */
template <class T>
void NUTSMSSM_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   NUTSMSSM_spectrum_plotter plotter;
...
void NUTSMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(NUTSMSSM<T>& model)
{
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : NUTSMSSM_info::parameter_names[parameter_index];
