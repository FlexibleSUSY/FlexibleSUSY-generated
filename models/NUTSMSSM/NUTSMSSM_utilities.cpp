...
// NUTSMSSM_parameter_getter ////////////////////////////////////////

std::vector<std::string> NUTSMSSM_parameter_getter::get_mass_names(const std::string& head)
{
...
std::array<std::string, NUTSMSSM_info::NUMBER_OF_MIXINGS> NUTSMSSM_parameter_getter::get_mixing_names()
{
   return NUTSMSSM_info::particle_mixing_names;
}

Eigen::ArrayXd NUTSMSSM_parameter_getter::get_parameters(const NUTSMSSM_mass_eigenstates& model)
{
   return model.get();
}

std::array<std::string, NUTSMSSM_info::NUMBER_OF_PARAMETERS> NUTSMSSM_parameter_getter::get_parameter_names()
{
   return NUTSMSSM_info::parameter_names;
}

std::array<std::string, NUTSMSSM_info::NUMBER_OF_PARTICLES> NUTSMSSM_parameter_getter::get_particle_names()
{
   return NUTSMSSM_info::particle_names;
}

std::vector<std::string> NUTSMSSM_parameter_getter::get_DRbar_mass_names()
{
   return get_mass_names();
}

std::vector<std::string> NUTSMSSM_parameter_getter::get_pole_mass_names()
{
   return get_mass_names("Pole");
}

std::array<std::string, NUTSMSSM_info::NUMBER_OF_MIXINGS> NUTSMSSM_parameter_getter::get_DRbar_mixing_names()
{
   return get_mixing_names();
}

std::array<std::string, NUTSMSSM_info::NUMBER_OF_MIXINGS> NUTSMSSM_parameter_getter::get_pole_mixing_names()
{
...
std::array<std::string, NUTSMSSM_info::NUMBER_OF_INPUT_PARAMETERS> NUTSMSSM_parameter_getter::get_input_parameter_names()
{
   return NUTSMSSM_info::input_parameter_names;
}

std::array<std::string, NUTSMSSM_info::NUMBER_OF_EXTRA_PARAMETERS> NUTSMSSM_parameter_getter::get_extra_parameter_names()
{
   return NUTSMSSM_info::extra_parameter_names;
}

decltype(NUTSMSSM_info::NUMBER_OF_MASSES) NUTSMSSM_parameter_getter::get_number_of_masses()
{
   return NUTSMSSM_info::NUMBER_OF_MASSES;
}

// NUTSMSSM_spectrum_plotter ////////////////////////////////////////

NUTSMSSM_spectrum_plotter::NUTSMSSM_spectrum_plotter(const NUTSMSSM_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void NUTSMSSM_spectrum_plotter::extract_spectrum(const NUTSMSSM_mass_eigenstates& model)
{
   spectrum.clear();
...
void NUTSMSSM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("NUTSMSSM_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("NUTSMSSM_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("NUTSMSSM_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void NUTSMSSM_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("NUTSMSSM_spectrum_plotter::write_spectrum: "
   ...
   const NUTSMSSM_observables* observables)
{
   ...
   append(names, NUTSMSSM_parameter_getter::get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, NUTSMSSM_parameter_getter::get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, NUTSMSSM_parameter_getter::get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, NUTSMSSM_parameter_getter::get_DRbar_mass_names());
   append(names, NUTSMSSM_parameter_getter::get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, NUTSMSSM_parameter_getter::get_pole_mass_names());
   append(names, NUTSMSSM_parameter_getter::get_pole_mixing_names());
   ...
   Physical_input* physical_input, NUTSMSSM_observables* observables)
{
   ...
   const auto number_of_masses = NUTSMSSM_parameter_getter::get_number_of_masses();
   const auto number_of_mixings = NUTSMSSM_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = NUTSMSSM_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = NUTSMSSM_info::NUMBER_OF_EXTRA_PARAMETERS;
