...
// File generated at Mon 9 May 2016 13:07:07

#ifndef NUTSMSSM_SPECTRUM_GENERATOR_H
#define NUTSMSSM_SPECTRUM_GENERATOR_H
...
#include "NUTSMSSM_two_scale_convergence_tester.hpp"
#include "NUTSMSSM_two_scale_initial_guesser.hpp"
...
template <class T>
class NUTSMSSM_spectrum_generator
   : public NUTSMSSM_spectrum_generator_interface<T> {
public:
   NUTSMSSM_spectrum_generator()
      : NUTSMSSM_spectrum_generator_interface<T>()
      ...
      , low_scale(0.)
   {}
   virtual ~NUTSMSSM_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const NUTSMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename = "NUTSMSSM_rgflow.dat") const;

private:
   NUTSMSSM_high_scale_constraint<T> high_scale_constraint;
   NUTSMSSM_susy_scale_constraint<T> susy_scale_constraint;
   NUTSMSSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void NUTSMSSM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const NUTSMSSM_input_parameters& input)
{
   NUTSMSSM<T>& model = this->model;
   model.clear();
   ...
   NUTSMSSM_convergence_tester<T> convergence_tester(&model, this->precision_goal);
   if (this->max_iterations > 0)
      convergence_tester.set_max_iterations(this->max_iterations);

   NUTSMSSM_initial_guesser<T> initial_guesser(&model, qedqcd,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void NUTSMSSM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   NUTSMSSM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
