...
double NUTSMSSM_susy_parameters::calc_beta_MS_1_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_MS = Re(4*MS*(AbsSqr(Kappa) + AbsSqr(Lambdax)));


   return oneLoop * beta_MS;
}
...
double NUTSMSSM_susy_parameters::calc_beta_MS_2_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_MS = Re(-0.8*MS*(15*traceYdAdjYd*AbsSqr(Lambdax) + 5*traceYeAdjYe*
 ...
 */
double NUTSMSSM_susy_parameters::calc_beta_MS_5_loop(const Susy_traces& susy_traces) const
{
   DEFINE_PROJECTOR(3,3,3,3)
