...
#include "cxx_qft/NUTSMSSM_qft.hpp"
#include "NUTSMSSM_FFV_form_factors.hpp"
...
using namespace NUTSMSSM_cxx_diagrams;
...
double get_MSUSY(const NUTSMSSM_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(NUTSMSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("NUTSMSSM_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("NUTSMSSM_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("NUTSMSSM_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("NUTSMSSM_a_muon: run_to_MSUSY(): "
...
double calculate_a_muon_impl(const NUTSMSSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   VERBOSE_MSG("NUTSMSSM_a_muon: calculating a_mu at Q = " << model.get_scale());

   context_base context{ model };

   using namespace NUTSMSSM_cxx_diagrams::fields;
...
std::pair<double,double> vary_scale(const NUTSMSSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model,&qedqcd] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("NUTSMSSM_a_muon: scale variation: " << e.what_detailed());
                     }
                     return amu;
                  });
...
double NUTSMSSM_a_muon::calculate_a_muon(const NUTSMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   NUTSMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("NUTSMSSM_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUTSMSSM_a_muon:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
double NUTSMSSM_a_muon::calculate_a_muon_uncertainty(const NUTSMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   NUTSMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("NUTSMSSM_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUTSMSSM_a_muon uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
