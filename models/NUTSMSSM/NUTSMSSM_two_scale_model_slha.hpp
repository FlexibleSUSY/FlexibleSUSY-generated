...
/**
 * @file NUTSMSSM_two_scale_model_slha.hpp
 * @brief contains wrapper class for model class in SLHA convention
 */

// File generated at Tue 12 Jul 2016 11:44:20

#ifndef NUTSMSSM_TWO_SCALE_SLHA_H
#define NUTSMSSM_TWO_SCALE_SLHA_H
...
/**
 * @class NUTSMSSM_slha<Two_scale>
 * @brief model class wrapper in SLHA convention
 */

template<>
class NUTSMSSM_slha<Two_scale> : public NUTSMSSM<Two_scale> {
public:
   explicit NUTSMSSM_slha(const NUTSMSSM_input_parameters& input_ = NUTSMSSM_input_parameters());
   explicit NUTSMSSM_slha(const NUTSMSSM<Two_scale>&, bool do_convert_masses_to_slha = true);
   virtual ~NUTSMSSM_slha();
...
std::ostream& operator<<(std::ostream& ostr, const NUTSMSSM_slha<Two_scale>& model);
