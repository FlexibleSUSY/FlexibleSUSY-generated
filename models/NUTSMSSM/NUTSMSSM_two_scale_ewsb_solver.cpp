...
// File generated at Fri 20 Oct 2017 09:01:20

/**
 * @file NUTSMSSM_two_scale_ewsb_solver.cpp
 *
 * @brief implementation of EWSB solver for two-scale iteration
 *
 * This file was generated at Fri 20 Oct 2017 09:01:20 with FlexibleSUSY
...
#define CLASSNAME NUTSMSSM_ewsb_solver<Two_scale>

/**
 * This method solves the EWSB conditions iteratively, trying several
 * root finding methods until a solution is found.
 */
int CLASSNAME::solve_iteratively(NUTSMSSM_mass_eigenstates& model_to_solve)
{
...
int CLASSNAME::solve_iteratively_with(
   NUTSMSSM_mass_eigenstates& model_to_solve, EWSB_solver* solver, const EWSB_vector_t& x_init)
{
 ...
 * @param solver solver
 */
void CLASSNAME::set_ewsb_solution(NUTSMSSM_mass_eigenstates& model, const EWSB_solver* solver)
{
...
void CLASSNAME::set_best_ewsb_solution(NUTSMSSM_mass_eigenstates& model, It first, It last)
{
...
int CLASSNAME::solve_iteratively_at(NUTSMSSM_mass_eigenstates& model_to_solve, int l)
{
...
int CLASSNAME::solve(NUTSMSSM_mass_eigenstates& model_to_solve)
{
   if (loop_order == 0) {
...
int CLASSNAME::solve_tree_level(NUTSMSSM_mass_eigenstates& model)
{
   int error = EWSB_solver::SUCCESS;

   const auto LL1 = MODELPARAMETER(LL1);
   const auto BMu = MODELPARAMETER(BMu);
   const auto BMS = MODELPARAMETER(BMS);
...
CLASSNAME::EWSB_vector_t CLASSNAME::initial_guess(const NUTSMSSM_mass_eigenstates& model) const
{
...
CLASSNAME::EWSB_vector_t CLASSNAME::tadpole_equations(const NUTSMSSM_mass_eigenstates& model) const
{
 ...
 */
CLASSNAME::EWSB_vector_t CLASSNAME::ewsb_step(const NUTSMSSM_mass_eigenstates& model) const
{
   ...
   ms2 = Re((0.25*(-4*vS*AbsSqr(MS) - 2*vS*BMS - 2.8284271247461903*MS*Conj(L1)
      ...
      1.4142135623730951*vd*vu*Conj(TLambdax) - 4*AbsSqr(Kappa)*Cube(vS) - 4*vS*
      Conj(L1)*Kappa + 2*vd*vS*vu*Conj(Lambdax)*Kappa + 1.4142135623730951*vd*vu*
      Conj(MS)*Lambdax + 2*vd*vS*vu*Conj(Kappa)*Lambdax + 4*tadpole[2] -
