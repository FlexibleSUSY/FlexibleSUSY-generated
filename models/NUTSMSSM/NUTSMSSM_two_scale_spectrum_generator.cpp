...
// File generated at Wed 16 Oct 2019 22:40:11
...
double NUTSMSSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void NUTSMSSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const NUTSMSSM_input_parameters& input)
{
   ...
   NUTSMSSM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<NUTSMSSM_ewsb_solver<Two_scale> >(ewsb_solver));

   NUTSMSSM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   NUTSMSSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   NUTSMSSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   NUTSMSSM_convergence_tester<Two_scale> convergence_tester(
   ...
   NUTSMSSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void NUTSMSSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   NUTSMSSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void NUTSMSSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
