...
// File generated at Mon 27 Feb 2017 13:38:57
...
NUTSMSSM_susy_parameters::NUTSMSSM_susy_parameters(const NUTSMSSM_input_parameters& input_)
   ...
   )), Lambdax(0), Kappa(0), Yu(Eigen::Matrix<double,3,3>::Zero()), Mu(0), MS(0
   ), L1(0), g1(0), g2(0), g3(0), vd(0), vu(0), vS(0)
...
NUTSMSSM_susy_parameters NUTSMSSM_susy_parameters::calc_beta() const
{
   ...
   if (get_loops() > 0) {
      ...
      beta_L1 += calc_beta_L1_one_loop(TRACE_STRUCT);
      ...
      if (get_loops() > 1) {
         ...
         beta_L1 += calc_beta_L1_two_loop(TRACE_STRUCT);
   ...
   return NUTSMSSM_susy_parameters(get_scale(), get_loops(), get_thresholds(), input,
                    beta_Yd, beta_Ye, beta_Lambdax, beta_Kappa, beta_Yu, beta_Mu, beta_MS, beta_L1, beta_g1, beta_g2, beta_g3, beta_vd, beta_vu, beta_vS);
}

NUTSMSSM_susy_parameters NUTSMSSM_susy_parameters::calc_beta(unsigned loops) const
{
   NUTSMSSM_susy_parameters p(*this);
   p.set_loops(loops);
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SlSl() const
{
   ...
   if (get_loops() > 1) {
      anomDim += (twoLoop*(-2*(Ye.adjoint()*Ye*Ye.adjoint()*Ye) +
         ...
         Power(g2,4) + 30*Sqr(g1)*Sqr(g2))*UNITMATRIX(3))).real();
   }
...
double CLASSNAME::get_SHdSHd() const
{
   ...
   if (get_loops() > 1) {
      anomDim += Re(twoLoop*(2.07*Power(g1,4) + 3.75*Power(g2,4) - 2*
...
void NUTSMSSM_susy_parameters::calc_susy_traces(Susy_traces& susy_traces) const
{
   if (get_loops() > 0) {
