...
/**
 * @file NUTSMSSM_amm.cpp
 *
 * This file was generated with FlexibleSUSY 2.8.0 and SARAH 4.15.1 .
 */

#include "NUTSMSSM_amm.hpp"
#include "NUTSMSSM_mass_eigenstates.hpp"

#include "cxx_qft/NUTSMSSM_qft.hpp"
#include "cxx_qft/NUTSMSSM_particle_aliases.hpp"
...
double get_MLCP(const NUTSMSSM_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(NUTSMSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("NUTSMSSM_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("NUTSMSSM_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("NUTSMSSM_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("NUTSMSSM_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const NUTSMSSM_mass_eigenstates& model, double ml_pole, int idx)
{
...
template <typename Lepton>
double calculate_amm_impl(const NUTSMSSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("NUTSMSSM_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double NUTSMSSM_amm::calculate_amm(const NUTSMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   NUTSMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("NUTSMSSM_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUTSMSSM_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template <typename Lepton>
double NUTSMSSM_amm::calculate_amm_uncertainty(const NUTSMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   NUTSMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("NUTSMSSM_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUTSMSSM_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
