...
// File generated at Fri 20 Oct 2017 09:01:14
...
{
   const double gY = MODEL->get_g1() * NUTSMSSM_info::normalization_g1;
   const double g2 = MODEL->get_g2() * NUTSMSSM_info::normalization_g2;
...
double CLASSNAME::calculate_delta_rho_hat(double sinThetaW) const
{
   ...
   if (number_of_loops > 1) {
      ...
      deltaRhoHat2LoopSM = (6.015223977354103e-6*((64*ALPHAS*Pi*Sqr(g1)*Sqr(g2)*(
         ...
         ,j2))*ZUR(2,j1))*ZUL(2,j2))))) - Sqr(Abs(Conj(ZA(1,1))*(SUM(j2,0,2,Conj(ZUL(
         2,j2))*SUM(j1,0,2,Conj(ZUR(2,j1))*Yu(j1,j2))) + SUM(j2,0,2,SUM(j1,0,2,Conj(
         Yu(j1,j2))*ZUR(2,j1))*ZUL(2,j2)))))) + RHO2(MAh(2)/MT)*(Sqr(Abs(Conj(ZA(2,1)
         )*(SUM(j2,0,2,Conj(ZUL(2,j2))*SUM(j1,0,2,Conj(ZUR(2,j1))*Yu(j1,j2))) - SUM(
         j2,0,2,SUM(j1,0,2,Conj(Yu(j1,j2))*ZUR(2,j1))*ZUL(2,j2))))) - Sqr(Abs(Conj(ZA
         (2,1))*(SUM(j2,0,2,Conj(ZUL(2,j2))*SUM(j1,0,2,Conj(ZUR(2,j1))*Yu(j1,j2))) +
         SUM(j2,0,2,SUM(j1,0,2,Conj(Yu(j1,j2))*ZUR(2,j1))*ZUL(2,j2)))))) - RHO2(Mhh(0
         )/MT)*(Sqr(Abs(Conj(ZH(0,1))*(SUM(j2,0,2,Conj(ZUL(2,j2))*SUM(j1,0,2,Conj(ZUR
