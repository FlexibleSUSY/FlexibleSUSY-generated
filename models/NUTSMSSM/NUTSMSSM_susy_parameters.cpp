...
// File generated at Tue 10 Oct 2017 22:29:57
...
#define CLASSNAME NUTSMSSM_susy_parameters
#define INPUT(parameter) input.parameter
#define TRACE_STRUCT susy_traces
#define TRACE_STRUCT_TYPE Susy_traces
#define CALCULATE_TRACES(l) calc_susy_traces(l);

const int NUTSMSSM_susy_parameters::numberOfParameters;

NUTSMSSM_susy_parameters::NUTSMSSM_susy_parameters(const NUTSMSSM_input_parameters& input_)
   : input(input_)
{
   set_number_of_parameters(numberOfParameters);
}

NUTSMSSM_susy_parameters::NUTSMSSM_susy_parameters(
   double scale_, int loops_, int thresholds_,
   const NUTSMSSM_input_parameters& input_
   ...
   double Mu_, double MS_, double L1_, double g1_, double g2_, double g3_,
   double vd_, double vu_, double vS_

)
   : Beta_function()
   , Yd(Yd_), Ye(Ye_), Lambdax(Lambdax_), Kappa(Kappa_), Yu(Yu_), Mu(Mu_), MS(
   MS_), L1(L1_), g1(g1_), g2(g2_), g3(g3_), vd(vd_), vu(vu_), vS(vS_)
...
Eigen::ArrayXd NUTSMSSM_susy_parameters::beta() const
{
   return calc_beta().get().unaryExpr(Chop<double>(get_zero_threshold()));
}

NUTSMSSM_susy_parameters NUTSMSSM_susy_parameters::calc_beta(int loops) const
{
   ...
   if (loops > 0) {
      ...
      beta_L1 += calc_beta_L1_1_loop(TRACE_STRUCT);
      ...
      if (loops > 1) {
         ...
         beta_L1 += calc_beta_L1_2_loop(TRACE_STRUCT);
   ...
   return NUTSMSSM_susy_parameters(get_scale(), loops, get_thresholds(), input,
                    beta_Yd, beta_Ye, beta_Lambdax, beta_Kappa, beta_Yu, beta_Mu, beta_MS, beta_L1, beta_g1, beta_g2, beta_g3, beta_vd, beta_vu, beta_vS);
}

NUTSMSSM_susy_parameters NUTSMSSM_susy_parameters::calc_beta() const
{
   return calc_beta(get_loops());
}

void NUTSMSSM_susy_parameters::clear()
{
   reset();
...
void NUTSMSSM_susy_parameters::set(const Eigen::ArrayXd& pars)
{
   Yd(0,0) = pars(0);
   Yd(0,1) = pars(1);
   Yd(0,2) = pars(2);
...
const NUTSMSSM_input_parameters& NUTSMSSM_susy_parameters::get_input() const
{
   return input;
}

NUTSMSSM_input_parameters& NUTSMSSM_susy_parameters::get_input()
{
   return input;
}

void NUTSMSSM_susy_parameters::set_input_parameters(const NUTSMSSM_input_parameters& input_)
{
   input = input_;
}

NUTSMSSM_susy_parameters::Susy_traces NUTSMSSM_susy_parameters::calc_susy_traces(int loops) const
{
