...
#include "lowe.h"
#include "NUTSMSSM_unitarity.hpp"
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSNUTSMSSMMessage", 1);
...
class NUTSMSSM_spectrum {
public:
   virtual ~NUTSMSSM_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(NUTSMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const NUTSMSSM_observables& get_observables() const = 0;

   virtual const UnitarityInfiniteS& get_unitarity() const = 0;
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const NUTSMSSM_input_parameters&) = 0;
...
class NUTSMSSM_spectrum_impl : public NUTSMSSM_spectrum
{
public:
   virtual ~NUTSMSSM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(NUTSMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const NUTSMSSM_observables& get_observables() const override { return observables; }

   virtual const UnitarityInfiniteS& get_unitarity() const { return unitarityData; };
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const NUTSMSSM_input_parameters&) override;
   virtual void calculate_model_observables(
      const softsusy::QedQcd&,
...
private:
   std::tuple<NUTSMSSM<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   NUTSMSSM_scales scales{};              ///< scale information
   NUTSMSSM_observables observables{};    ///< observables
...
public:
   ...
   void set_input_parameters(const NUTSMSSM_input_parameters& input_) { input = input_; }
...
private:
   NUTSMSSM_input_parameters input{};                    ///< model input parameters
   ...
   std::unique_ptr<NUTSMSSM_spectrum> spectrum{nullptr}; ///< spectrum information

   NUTSMSSM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSNUTSMSSMCalculateSpectrum[]?");
   }
}
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(LambdaSInput), "LambdaSInput");
   MLPutRuleTo(link, INPUTPARAMETER(L1Input), "L1Input");
   ...
   MLPutRuleTo(link, INPUTPARAMETER(LInput), "LInput");
...
NUTSMSSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const NUTSMSSM_slha& model, MLINK link)
{
   MLPutRule(link, NUTSMSSM_info::model_name);
   MLPutFunction(link, "List", 104);
   ...
   MLPutRuleTo(link, MODELPARAMETER(MS), "MS");
   MLPutRuleTo(link, MODELPARAMETER(L1), "L1");
   MLPutRuleTo(link, MODELPARAMETER(g1), "g1");
   MLPutRuleTo(link, MODELPARAMETER(g2), "g2");
   MLPutRuleTo(link, MODELPARAMETER(g3), "g3");
   MLPutRuleTo(link, MODELPARAMETER(vd), "vd");
   ...
   MLPutRuleTo(link, MODELPARAMETER(BMS), "MS", {"B"});
   MLPutRuleTo(link, MODELPARAMETER(LL1), "L1", {"L"});
...
template <typename Solver_type>
void NUTSMSSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void NUTSMSSM_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const NUTSMSSM_input_parameters& input
)
{
...
template <typename Solver_type>
void NUTSMSSM_spectrum_impl<Solver_type>::calculate_model_observables(
   const softsusy::QedQcd& qedqcd,
...
template <typename Solver_type>
void NUTSMSSM_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = NUTSMSSM_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}

/******************************************************************/

template <typename Solver_type>
void NUTSMSSM_spectrum_impl<Solver_type>::fill_slha_io(NUTSMSSM_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   NUTSMSSM_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSNUTSMSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSNUTSMSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new NUTSMSSM_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   INPUTPARAMETER(LambdaSInput) = pars[c++];
   INPUTPARAMETER(L1Input) = pars[c++];
   INPUTPARAMETER(MSInput) = pars[c++];
   INPUTPARAMETER(BInput) = pars[c++];
   INPUTPARAMETER(MuInput) = pars[c++];
   INPUTPARAMETER(LInput) = pars[c++];
...
{
   using namespace flexiblesusy::NUTSMSSM_librarylink;
...
DLLEXPORT int FSNUTSMSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTSMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTSMSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUTSMSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTSMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTSMSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUTSMSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTSMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTSMSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSNUTSMSSMCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSNUTSMSSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSNUTSMSSMCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSNUTSMSSMCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUTSMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUTSMSSMCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSNUTSMSSMCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
