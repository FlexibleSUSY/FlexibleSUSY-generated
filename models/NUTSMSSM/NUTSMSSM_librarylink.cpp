...
// File generated at Wed 16 Oct 2019 22:40:22
...
public:
   ...
   virtual void fill_slha_io(NUTSMSSM_slha_io&) const = 0;
...
public:
   ...
   virtual void fill_slha_io(NUTSMSSM_slha_io&) const override;
...
private:
   std::tuple<NUTSMSSM_slha<NUTSMSSM<Solver_type>>> models{};        ///< running parameters and pole masses
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, NUTSMSSM_info::particle_names[i].c_str());
      }
   }
}
...
void put_spectrum(const NUTSMSSM_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void NUTSMSSM_spectrum_impl<Solver_type>::fill_slha_io(NUTSMSSM_slha_io& slha_io) const
{
