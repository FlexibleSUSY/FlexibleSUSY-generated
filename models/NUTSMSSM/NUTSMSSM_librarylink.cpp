...
// File generated at Wed 12 Apr 2017 13:01:23
...
struct Model_data {
   ...
   NUTSMSSM_slha<algorithm_type> model; ///< running parameters and pole masses
};
...
void put_spectrum(const NUTSMSSM_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const NUTSMSSM_observables& observables, MLINK link)
{
...
void check_spectrum(const Model_data& data, MLINK link)
{
   ...
   if (problems.have_problem()) {
      ...
      put_message(link, "FSNUTSMSSMCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSNUTSMSSMCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(Model_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSNUTSMSSMCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = NUTSMSSM_slha<algorithm_type>(
