...
E6SSM_initial_guesser<Two_scale>::E6SSM_initial_guesser(
   E6SSM<Two_scale>* model_,
   const softsusy::QedQcd& qedqcd_,
   const E6SSM_low_scale_constraint<Two_scale>& low_constraint_,
   const E6SSM_susy_scale_constraint<Two_scale>& susy_constraint_,
   const E6SSM_high_scale_constraint<Two_scale>& high_constraint_
)
   : model(model_)
   , qedqcd(qedqcd_)
...
{
   if (!model)
      throw SetupError("E6SSM_initial_guesser: Error: pointer to model"
                       " E6SSM<Two_scale> must not be zero");
}
 ...
 */
void E6SSM_initial_guesser<Two_scale>::guess()
{
 ...
 * \code{.cpp}
   ...
   MODEL->set_BMuPr(Re(Sqr(LowEnergyConstant(MZ))));
   MODEL->set_MuPr(Re(LowEnergyConstant(MZ)));
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
 ...
 * \endcode
 */
void E6SSM_initial_guesser<Two_scale>::guess_susy_parameters()
{
...
void E6SSM_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void E6SSM_initial_guesser<Two_scale>::calculate_running_SM_masses()
{
 ...
 * corrections).
 */
void E6SSM_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
 ...
 */
void E6SSM_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
 ...
 * corrections).
 */
void E6SSM_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
   const auto vd = MODELPARAMETER(vd);
 ...
 */
void E6SSM_initial_guesser<Two_scale>::guess_soft_parameters()
{
