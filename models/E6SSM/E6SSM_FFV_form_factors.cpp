...
/**
 * @file E6SSM_FFV_form_factors.cpp
 *
...
std::valarray<std::complex<double>> calculate_Fe_Fe_VP_form_factors (
   ...
   val += std::complex<double> {1., 0.} * FFV_FFV<Fe,Fe,VP,Fe,Fe,VZp>::value(indices1, indices2, context, discard_SM_contributions);
...
template std::valarray<std::complex<double>> calculate_form_factors<Fe,Fe,VP>(   int, int,
   const E6SSM_mass_eigenstates&, bool);
