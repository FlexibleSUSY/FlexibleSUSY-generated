...
template <class T>
class E6SSM_spectrum_generator_interface {
public:
   virtual ~E6SSM_spectrum_generator_interface() = default;

   std::tuple<E6SSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<E6SSM<T>> get_models_slha() const
   { return std::make_tuple(E6SSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const E6SSM<T>& get_model() const
   { return model; }
   E6SSM<T>& get_model()
   { return model; }
   E6SSM<T> get_model_slha() const
   { return E6SSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const E6SSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "E6SSM_spectrum.dat") const;
...
protected:
   ...
   virtual void run_except(const softsusy::QedQcd&, const E6SSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void E6SSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const E6SSM_input_parameters& input)
{
 ...
 */
template <class T>
void E6SSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   E6SSM_mass_eigenstates tmp_model(model);
   try {
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return E6SSM_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(E6SSM_info::parameter_names),
      std::cend(E6SSM_info::parameter_names));
 ...
 */
template <class T>
void E6SSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   E6SSM_spectrum_plotter plotter(model);
