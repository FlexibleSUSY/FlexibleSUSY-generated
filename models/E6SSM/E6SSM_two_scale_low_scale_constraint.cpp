...
// File generated at Tue 22 Jan 2019 17:09:11
...
void E6SSM_low_scale_constraint<Two_scale>::apply()
{
   check_model_ptr();
...
double E6SSM_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void E6SSM_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<E6SSM<Two_scale>*>(model_);
}

void E6SSM_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& E6SSM_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void E6SSM_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
...
void E6SSM_low_scale_constraint<Two_scale>::initialize()
{
   check_model_ptr();
...
void E6SSM_low_scale_constraint<Two_scale>::update_scale()
{
   check_model_ptr();
...
void E6SSM_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
   check_model_ptr();
...
double E6SSM_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();

   double theta_w = std::asin(Electroweak_constants::sinThetaW);

   E6SSM_weinberg_angle::Sm_parameters sm_pars;
   ...
   E6SSM_weinberg_angle weinberg(MODEL, sm_pars);
...
void E6SSM_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   ...
   if (IsFinite(new_g1)) {
      ...
      model->get_problems().flag_non_perturbative_parameter(
         E6SSM_info::g1, new_g1, get_scale());
   ...
   if (IsFinite(new_g2)) {
      ...
      model->get_problems().flag_non_perturbative_parameter(
         E6SSM_info::g2, new_g2, get_scale());
...
double E6SSM_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
void E6SSM_low_scale_constraint<Two_scale>::calculate_running_SM_masses()
{
   check_model_ptr();
...
void E6SSM_low_scale_constraint<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void E6SSM_low_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("E6SSM_low_scale_constraint<Two_scale>: "
