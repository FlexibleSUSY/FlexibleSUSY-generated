...
#include "E6SSM_amm.hpp"
#include "E6SSM_mass_eigenstates.hpp"

#include "cxx_qft/E6SSM_qft.hpp"
#include "cxx_qft/E6SSM_particle_aliases.hpp"
...
double get_MLCP(const E6SSM_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(E6SSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("E6SSM_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("E6SSM_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("E6SSM_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("E6SSM_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const E6SSM_mass_eigenstates& model, double ml_pole, int idx)
{
   using namespace E6SSM_cxx_diagrams::fields;

   const auto form_factors = E6SSM_FFV_form_factors::calculate_form_factors<Lepton,Lepton,VP>(idx, idx, model, true);
...
template <typename Lepton>
double calculate_amm_impl(const E6SSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("E6SSM_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double E6SSM_amm::calculate_amm(const E6SSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   E6SSM_mass_eigenstates model(model_);

   VERBOSE_MSG("E6SSM_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("E6SSM_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template double E6SSM_amm::calculate_amm<E6SSM_cxx_diagrams::fields::Fe>(const E6SSM_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);

template <typename Lepton>
double E6SSM_amm::calculate_amm_uncertainty(const E6SSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   E6SSM_mass_eigenstates model(model_);

   VERBOSE_MSG("E6SSM_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("E6SSM_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
// spliting templated function into header and cpp
// https://isocpp.org/wiki/faq/templates#separate-template-fn-defn-from-decl
template double E6SSM_amm::calculate_amm_uncertainty<E6SSM_cxx_diagrams::fields::Fe>(const E6SSM_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);
