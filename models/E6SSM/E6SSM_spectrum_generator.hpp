...
class E6SSM_spectrum_generator
   : public E6SSM_spectrum_generator_interface<T> {
public:
   E6SSM_spectrum_generator()
      : E6SSM_spectrum_generator_interface<T>()
      , high_scale(0.)
      , susy_scale(0.)
      , low_scale(0.)
   {}
   virtual ~E6SSM_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const E6SSM_input_parameters&);
 ...
 */
template <class T>
void E6SSM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const E6SSM_input_parameters& input)
{
   E6SSM<T>& model = this->model;
   model.clear();
   ...
   E6SSM_high_scale_constraint<T> high_scale_constraint(&model);
   E6SSM_susy_scale_constraint<T> susy_scale_constraint(&model, qedqcd);
   E6SSM_low_scale_constraint<T>  low_scale_constraint(&model, qedqcd);
   ...
   E6SSM_convergence_tester<T> convergence_tester(
   ...
   E6SSM_initial_guesser<T> initial_guesser(&model, qedqcd,
 ...
 */
template <class T>
void E6SSM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   E6SSM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
