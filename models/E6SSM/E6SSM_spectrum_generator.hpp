...
// File generated at Sun 31 May 2015 12:38:22
...
template <class T>
class E6SSM_spectrum_generator {
public:
   E6SSM_spectrum_generator()
      : solver(), model()
   ...
   void run(const QedQcd& oneset, const E6SSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "E6SSM_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "E6SSM_spectrum.dat") const;

private:
   RGFlow<T> solver;
   E6SSM<T> model;
   E6SSM_high_scale_constraint<T> high_scale_constraint;
   E6SSM_susy_scale_constraint<T> susy_scale_constraint;
   E6SSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void E6SSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const E6SSM_input_parameters& input)
{
   ...
   E6SSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   E6SSM_initial_guesser<T> initial_guesser(&model, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void E6SSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   E6SSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   E6SSM_parameter_getter parameter_getter;
   Coupling_monitor<E6SSM<T>, E6SSM_parameter_getter>
 ...
 */
template <class T>
void E6SSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
