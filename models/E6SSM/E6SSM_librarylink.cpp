...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSE6SSMMessage", 1);
...
class E6SSM_spectrum {
public:
   virtual ~E6SSM_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(E6SSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const E6SSM_observables& get_observables() const = 0;
   virtual const E6SSM_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const E6SSM_input_parameters&) = 0;
...
class E6SSM_spectrum_impl : public E6SSM_spectrum
{
public:
   virtual ~E6SSM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(E6SSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const E6SSM_observables& get_observables() const override { return observables; }
   virtual const E6SSM_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const E6SSM_input_parameters&) override;
...
private:
   std::tuple<E6SSM<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   E6SSM_scales scales{};              ///< scale information
   E6SSM_observables observables{};    ///< observables
   E6SSM_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const E6SSM_input_parameters& input_) { input = input_; }
...
private:
   E6SSM_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<E6SSM_spectrum> spectrum{nullptr};  ///< spectrum information

   E6SSM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSE6SSMCalculateSpectrum[]?");
   }
}
...
E6SSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const E6SSM_slha& model, MLINK link)
{
   MLPutRule(link, E6SSM_info::model_name);
...
template <typename Solver_type>
void E6SSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void E6SSM_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const E6SSM_input_parameters& input
)
{
...
template <typename Solver_type>
void E6SSM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void E6SSM_spectrum_impl<Solver_type>::calculate_model_decays(
   const softsusy::QedQcd& qedqcd, const Physical_input& physical_input, const FlexibleDecay_settings& flexibledecay_settings)
{
   decays = E6SSM_decays(std::get<0>(models), qedqcd, physical_input, flexibledecay_settings);
...
template <typename Solver_type>
void E6SSM_spectrum_impl<Solver_type>::fill_slha_io(E6SSM_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   E6SSM_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   E6SSM_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = E6SSM_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSE6SSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSE6SSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new E6SSM_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSE6SSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSM_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMGetSettings"))
...
DLLEXPORT int FSE6SSMGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSM_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSE6SSMGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSM_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSE6SSMOpenHandle(
...
DLLEXPORT int FSE6SSMCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::E6SSM_librarylink;
...
DLLEXPORT int FSE6SSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSM_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSE6SSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSM_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSE6SSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSM_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSE6SSMCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSM_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSE6SSMCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSM_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSE6SSMCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSE6SSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSE6SSMCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSE6SSMCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::E6SSM_librarylink;

   if (!check_number_of_args(link, 1, "FSE6SSMCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSE6SSMCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSE6SSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSE6SSMCalculateDecays", "error", e.what());
      put_error_output(link);
   }
