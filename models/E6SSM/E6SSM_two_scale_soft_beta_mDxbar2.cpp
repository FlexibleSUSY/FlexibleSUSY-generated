...
// File generated at Tue 27 Oct 2015 15:15:02
...
Eigen::Matrix<double,3,3> E6SSM_soft_parameters::calc_beta_mDxbar2_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_mDxbar2 = (twoLoop*(-6*traceconjTKappaTpTKappa*((Kappa).transpose
      ()*Kappa.conjugate()) - 4*traceconjTLambda12TpTLambda12*((Kappa)
      .transpose()*Kappa.conjugate()) - 12*ms2*traceKappaAdjKappa*((Kappa)
      .transpose()*Kappa.conjugate()) - 6*traceKappaAdjKappaconjmDx2*((Kappa)
      .transpose()*Kappa.conjugate()) - 6*traceKappaconjmDxbar2AdjKappa*((Kappa
      ).transpose()*Kappa.conjugate()) - 8*ms2*traceLambda12AdjLambda12*((Kappa
      ).transpose()*Kappa.conjugate()) - 4*traceLambda12AdjLambda12conjmH2I2*((
      Kappa).transpose()*Kappa.conjugate()) - 4*tracemH1I2AdjLambda12Lambda12*(
      (Kappa).transpose()*Kappa.conjugate()) - 4*mHd2*AbsSqr(Lambdax)*((Kappa)
      ...
      *(200*(2*MassBp*((Kappa).transpose()*Kappa.conjugate()) - (TKappa)
      ...
      (2*MassBp + MassG)*Sqr(g3) + 1773*MassBp*Sqr(gN))*UNITMATRIX(3)))).real()
      ;
