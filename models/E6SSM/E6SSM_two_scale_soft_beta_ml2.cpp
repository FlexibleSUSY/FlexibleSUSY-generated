...
// File generated at Mon 23 Feb 2015 13:02:20
...
Eigen::Matrix<double,3,3> E6SSM_soft_parameters::calc_beta_ml2_one_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_ml2 = oneOver16PiSqr*(2*mHd2*(Ye.adjoint()*Ye) + 2*((TYe).adjoint
      ...
      *ml2 - 0.7745966692414834*g1*Tr11*UNITMATRIX(3) + 0.6324555320336759*gN*
      Tr14*UNITMATRIX(3) - 1.2*AbsSqr(MassB)*Sqr(g1)*UNITMATRIX(3) - 6*AbsSqr(
      MassWB)*Sqr(g2)*UNITMATRIX(3) - 0.8*AbsSqr(MassBp)*Sqr(gN)*UNITMATRIX(3))
      ;
...
Eigen::Matrix<double,3,3> E6SSM_soft_parameters::calc_beta_ml2_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_ml2 = twoLoop*(-6*traceconjTYdTpTYd*(Ye.adjoint()*Ye) - 2*
      ...
      *Ye) + 2.4*mHd2*Sqr(g1)*(Ye.adjoint()*Ye) + 0.6*mHd2*Sqr(gN)*(Ye.adjoint(
      )*Ye) - 6*traceconjTYdTpYd*(Ye.adjoint()*TYe) - 2*traceconjTYeTpYe*(
      Ye.adjoint()*TYe) - 2*Conj(TLambdax)*Lambdax*(Ye.adjoint()*TYe) - 6*
      traceAdjYdTYd*((TYe).adjoint()*Ye) - 2*traceAdjYeTYe*((TYe).adjoint()*Ye)
      - 2.4*MassB*Sqr(g1)*((TYe).adjoint()*Ye) - 0.6*MassBp*Sqr(gN)*((TYe)
      ...
      *TYe) + 0.6*Sqr(gN)*((TYe).adjoint()*TYe) - 3*traceYdAdjYd*(ml2*
      Ye.adjoint()*Ye) - traceYeAdjYe*(ml2*Ye.adjoint()*Ye) - AbsSqr(Lambdax)*(
      ml2*Ye.adjoint()*Ye) + 1.2*Sqr(g1)*(ml2*Ye.adjoint()*Ye) + 0.3*Sqr(gN)*(
      ml2*Ye.adjoint()*Ye) - 6*traceYdAdjYd*(Ye.adjoint()*me2*Ye) - 2*
      traceYeAdjYe*(Ye.adjoint()*me2*Ye) - 2*AbsSqr(Lambdax)*(Ye.adjoint()*me2*
      Ye) + 2.4*Sqr(g1)*(Ye.adjoint()*me2*Ye) + 0.6*Sqr(gN)*(Ye.adjoint()*me2*
      Ye) - 3*traceYdAdjYd*(Ye.adjoint()*Ye*ml2) - traceYeAdjYe*(Ye.adjoint()*
      Ye*ml2) - AbsSqr(Lambdax)*(Ye.adjoint()*Ye*ml2) + 1.2*Sqr(g1)*(Ye.adjoint
      ()*Ye*ml2) + 0.3*Sqr(gN)*(Ye.adjoint()*Ye*ml2) - 8*mHd2*(Ye.adjoint()*Ye*
      ...
      *Sqr(g1)*Sqr(g2)*UNITMATRIX(3) + 0.8*Tr2U144*Sqr(gN)*UNITMATRIX(3) + 2.4*
      ...
      Sqr(g2)*Sqr(gN)*UNITMATRIX(3) + 0.12*Conj(MassBp)*Sqr(gN)*(10*MassBp*(
      Ye.adjoint()*Ye) - 5*(Ye.adjoint()*TYe) + 2*(3*(MassB + 2*MassBp)*Sqr(g1)
      + 5*(2*MassBp + MassWB)*Sqr(g2) + 96*MassBp*Sqr(gN))*UNITMATRIX(3)) +
