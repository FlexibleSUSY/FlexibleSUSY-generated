...
// File generated at Sun 26 Aug 2018 14:28:16
...
double E6SSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void E6SSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const E6SSM_input_parameters& input)
{
   ...
   E6SSM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   E6SSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   E6SSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   E6SSM_convergence_tester<Two_scale> convergence_tester(
   ...
   E6SSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void E6SSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   E6SSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void E6SSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
