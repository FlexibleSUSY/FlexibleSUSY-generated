...
// File generated at Mon 19 Sep 2016 09:36:20

#include "NMSSMtower_two_scale_high_scale_constraint.hpp"
...
NMSSMtower_high_scale_constraint<Two_scale>::NMSSMtower_high_scale_constraint()
...
void NMSSMtower_high_scale_constraint<Two_scale>::apply()
{
   assert(model && "Error: NMSSMtower_high_scale_constraint::apply():"
...
bool NMSSMtower_high_scale_constraint<Two_scale>::check_non_perturbative()
{
   bool problem = false;
...
double NMSSMtower_high_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double NMSSMtower_high_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

const NMSSMtower_input_parameters& NMSSMtower_high_scale_constraint<Two_scale>::get_input_parameters() const
{
   return model->get_input();
}

NMSSMtower<Two_scale>* NMSSMtower_high_scale_constraint<Two_scale>::get_model() const
{
   return model;
}

void NMSSMtower_high_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
   model = cast_model<NMSSMtower<Two_scale>*>(model_);
}

void NMSSMtower_high_scale_constraint<Two_scale>::set_scale(double s)
{
   scale = s;
}

void NMSSMtower_high_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
   initial_scale_guess = 0.;
   model = NULL;
...
void NMSSMtower_high_scale_constraint<Two_scale>::initialize()
{
   assert(model && "NMSSMtower_high_scale_constraint<Two_scale>::"
...
void NMSSMtower_high_scale_constraint<Two_scale>::update_scale()
{
   assert(model && "NMSSMtower_high_scale_constraint<Two_scale>::"
          "update_scale(): model pointer is zero.");

   const double currentScale = model->get_scale();
   const NMSSMtower_soft_parameters beta_functions(model->calc_beta());
   ...
   if (errno == ERANGE) {
      ...
      ERROR("NMSSMtower_high_scale_constraint<Two_scale>: Overflow error"
