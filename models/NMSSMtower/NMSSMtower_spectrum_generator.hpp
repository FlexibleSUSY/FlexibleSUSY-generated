...
private:
   RGFlow<T> solver;
   NMSSMtower_susy_scale_constraint<T> susy_scale_constraint;
   standard_model::Standard_model_low_scale_constraint<T>  low_scale_constraint;
   NMSSMtower_standard_model_Matching<T> matching;
 ...
 */
template <class T>
void NMSSMtower_spectrum_generator<T>::run(const softsusy::QedQcd& oneset,
                                const NMSSMtower_input_parameters& input)
{
   ...
   NMSSMtower_convergence_tester<T> convergence_tester(
   ...
   NMSSMtower_standard_model_initial_guesser<T> initial_guesser(&model, &eft, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint);
