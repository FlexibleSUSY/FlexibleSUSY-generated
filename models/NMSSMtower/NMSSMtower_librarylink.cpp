...
// File generated at Thu 15 Dec 2016 12:40:07
...
#include "NMSSMtower_two_scale_model_slha.hpp"
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSNMSSMtowerMessage", 1);
...
struct NMSSMtower_data {
   ...
   NMSSMtower_input_parameters input;     ///< model input parameters
   ...
   NMSSMtower_slha<algorithm_type> model; ///< running parameters and pole masses
};

/// current handles
typedef std::map<Handle_id, NMSSMtower_data> Handle_map;
Handle_map handles_NMSSMtower;

/******************************************************************/

Handle_id get_new_NMSSMtower_handle()
{
   static const std::size_t max_handles =
      static_cast<std::size_t>(std::exp2(8*sizeof(Handle_id)) - 1);

   if (handles_NMSSMtower.size() >= max_handles)
      throw ENotEnoughFreeHandles(handles_NMSSMtower.size());

   Handle_id hid = 0;

   while (handles_NMSSMtower.find(hid) != handles_NMSSMtower.end())
      hid++;
...
NMSSMtower_data find_NMSSMtower_data(Handle_id hid)
{
   const Handle_map::iterator handle = handles_NMSSMtower.find(hid);

   if (handle == handles_NMSSMtower.end())
...
void put_settings(const NMSSMtower_data& data, MLINK link)
{
...
void put_sm_input_parameters(const NMSSMtower_data& data, MLINK link)
{
...
void put_input_parameters(const NMSSMtower_data& data, MLINK link)
{
...
void put_spectrum(const NMSSMtower_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const NMSSMtower_observables& observables, MLINK link)
{
...
void check_spectrum(const NMSSMtower_data& data, MLINK link)
{
   const Problems<NMSSMtower_info::NUMBER_OF_PARTICLES>& problems
      = data.model.get_problems();

   if (problems.have_problem()) {
      std::ostringstream msg;
      problems.print_problems(msg);
      put_message(link, "FSNMSSMtowerCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSNMSSMtowerCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(NMSSMtower_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSNMSSMtowerCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = NMSSMtower_slha<algorithm_type>(
...
NMSSMtower_data make_NMSSMtower_data(double* pars, mint npars)
{
   NMSSMtower_data data;

   const mint n_settings = 23, n_sm_parameters = 29, n_input_pars = 82;
...
DLLEXPORT int FSNMSSMtowerGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSNMSSMtowerGetSettings"))
      return LIBRARY_TYPE_ERROR;

   const Handle_id hid = get_handle_from(link);

   try {
      const NMSSMtower_data data = find_NMSSMtower_data(hid);
...
DLLEXPORT int FSNMSSMtowerGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSNMSSMtowerGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMtowerGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSNMSSMtowerGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNMSSMtowerOpenHandle(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument Res)
{
   ...
   try {
      NMSSMtower_data data = make_NMSSMtower_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);

      const Handle_id hid = get_new_NMSSMtower_handle();

      handles_NMSSMtower.insert(std::make_pair(hid, std::move(data)));
...
DLLEXPORT int FSNMSSMtowerCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   if (Argc != 1)
      return LIBRARY_TYPE_ERROR;
   ...
   if (handle != handles_NMSSMtower.end())
      handles_NMSSMtower.erase(handle);
   ...
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   ...
   if (handle == handles_NMSSMtower.end()) {
      std::cerr << "Error: FSNMSSMtowerSet: Unknown handle: "
                << hid << std::endl;
      return LIBRARY_FUNCTION_ERROR;
   }

   try {
      handle->second = make_NMSSMtower_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);
   } catch (const flexiblesusy::Error& e) {
      std::cerr << e.what() << std::endl;
...
DLLEXPORT int FSNMSSMtowerCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSNMSSMtowerCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;

   const Handle_id hid = get_handle_from(link);

   try {
      NMSSMtower_data data = find_NMSSMtower_data(hid);
      ...
      handles_NMSSMtower[hid] = std::move(data);
...
DLLEXPORT int FSNMSSMtowerCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSNMSSMtowerCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.model.get_scale() == 0.) {
         put_message(link,
            "FSNMSSMtowerCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSNMSSMtowerCalculateSpectrum[]?");
      }

      NMSSMtower_observables observables;
      ...
      put_message(link, "FSNMSSMtowerCalculateObservables", "error", e.what());
      put_error_output(link);
   }
