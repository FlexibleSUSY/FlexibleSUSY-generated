...
#ifndef NMSSMtower_UTILITIES_H
#define NMSSMtower_UTILITIES_H
...
struct NMSSMtower_observables;
class Physical_input;

class NMSSMtower_parameter_getter {
public:
   Eigen::ArrayXd get_parameters(const NMSSMtower_mass_eigenstates& model) {
      return model.get();
   }
...
class NMSSMtower_spectrum_plotter {
public:
   NMSSMtower_spectrum_plotter();

   void extract_spectrum(const NMSSMtower_mass_eigenstates&);
...
template <class Scalar, int M, int N>
std::valarray<double> NMSSMtower_spectrum_plotter::to_valarray(const Eigen::Array<Scalar, M, N>& v)
{
...
void to_database(
   const std::string&,
   const NMSSMtower_mass_eigenstates&,
   const softsusy::QedQcd* qedqcd = 0,
   const Physical_input* physical_input = 0,
   const NMSSMtower_observables* observables = 0);
...
NMSSMtower_mass_eigenstates from_database(
   ...
   NMSSMtower_observables* observables = 0);
