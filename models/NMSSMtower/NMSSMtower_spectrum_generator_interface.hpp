...
#ifndef NMSSMtower_SPECTRUM_GENERATOR_INTERFACE_H
#define NMSSMtower_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class NMSSMtower_spectrum_generator_interface {
public:
   NMSSMtower_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~NMSSMtower_spectrum_generator_interface() {}

   const NMSSMtower<T>& get_model() const { return model; }
   NMSSMtower<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const NMSSMtower_input_parameters&) = 0;
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "NMSSMtower_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(NMSSMtower<T>& model);
};
 ...
 */
template <class T>
void NMSSMtower_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void NMSSMtower_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   NMSSMtower_mass_eigenstates tmp_model(model);
   try {
   ...
   NMSSMtower_parameter_getter parameter_getter;
   Coupling_monitor<NMSSMtower_mass_eigenstates, NMSSMtower_parameter_getter>
 ...
 */
template <class T>
void NMSSMtower_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   NMSSMtower_spectrum_plotter plotter;
 ...
 */
template <class T>
void NMSSMtower_spectrum_generator_interface<T>::translate_exception_to_problem(NMSSMtower<T>& model)
{
   try {
      throw;
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : NMSSMtower_info::parameter_names[parameter_index];
