...
#include "NMSSMtower_utilities.hpp"
...
NMSSMtower_spectrum_plotter::NMSSMtower_spectrum_plotter()
   : spectrum()
   , scale(0.0)
   , width(16)
{
...
void NMSSMtower_spectrum_plotter::extract_spectrum(const NMSSMtower_mass_eigenstates& model)
{
   spectrum.clear();
...
void NMSSMtower_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name.c_str(), std::ios::out);
   VERBOSE_MSG("NMSSMtower_spectrum_plotter::write_to_file: opening file: "
               << file_name.c_str());
   if (filestr.fail()) {
      ERROR("NMSSMtower_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("NMSSMtower_spectrum_plotter::write_to_file: file written: "
               << file_name.c_str());
}

void NMSSMtower_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   for (std::size_t s = 0; s < spectrum.size(); ++s) {
      if (!filestr.good()) {
         ERROR("NMSSMtower_spectrum_plotter::write_spectrum: "
...
std::valarray<double> NMSSMtower_spectrum_plotter::to_valarray(double v)
{
   return std::valarray<double>(&v, 1);
}

namespace NMSSMtower_database {
 ...
 */
void to_database(
   const std::string& file_name, const NMSSMtower_mass_eigenstates& model,
   ...
   const NMSSMtower_observables* observables)
{
   ...
   append(names, NMSSMtower_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, NMSSMtower_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill pole masses and mixings
   append(names, NMSSMtower_parameter_getter().get_mass_names());
   append(names, NMSSMtower_parameter_getter().get_mixing_names());
   ...
   if (NMSSMtower_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, NMSSMtower_observables::get_names());
 ...
 */
NMSSMtower_mass_eigenstates from_database(
   const std::string& file_name, std::size_t entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, NMSSMtower_observables* observables)
{
   using utilities::append;

   NMSSMtower_mass_eigenstates model;
   const std::size_t number_of_parameters = model.get_number_of_parameters();
   const std::size_t number_of_masses = NMSSMtower_parameter_getter().get_number_of_masses();
   const std::size_t number_of_mixings = NMSSMtower_info::NUMBER_OF_MIXINGS;
   const std::size_t number_of_input_parameters = NMSSMtower_info::NUMBER_OF_INPUT_PARAMETERS;
   ...
   const std::size_t number_of_observables =
      (observables ? NMSSMtower_observables::NUMBER_OF_OBSERVABLES : 0u);
