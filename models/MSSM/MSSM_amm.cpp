...
#include "cxx_qft/MSSM_particle_aliases.hpp"
...
double get_MLCP(const MSSM_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(MSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSM_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSM_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSM_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSM_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const MSSM_mass_eigenstates& model, double ml_pole, int idx)
{
   using namespace MSSM_cxx_diagrams::fields;

   const auto form_factors = MSSM_FFV_form_factors::calculate_form_factors<Lepton,Lepton,VP>(idx, idx, model, true);
...
template <typename Lepton>
double calculate_amm_impl(const MSSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("MSSM_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double MSSM_amm::calculate_amm(const MSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   MSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSM_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSM_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template double MSSM_amm::calculate_amm<MSSM_cxx_diagrams::fields::Fe>(const MSSM_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);

template <typename Lepton>
double MSSM_amm::calculate_amm_uncertainty(const MSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   MSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSM_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSM_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
// spliting templated function into header and cpp
// https://isocpp.org/wiki/faq/templates#separate-template-fn-defn-from-decl
template double MSSM_amm::calculate_amm_uncertainty<MSSM_cxx_diagrams::fields::Fe>(const MSSM_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);
