...
// File generated at Wed 12 Apr 2017 13:32:11
 ...
 * parameters.
 *
 * This file was generated at Wed 12 Apr 2017 13:32:11 with FlexibleSUSY
...
#define CLASSNAME MSSM_mass_eigenstates
...
CLASSNAME::MSSM_mass_eigenstates(const MSSM_input_parameters& input_)
...
CLASSNAME::~MSSM_mass_eigenstates()
{
}
...
MSSM_physical& CLASSNAME::get_physical()
{
   return physical;
}

void CLASSNAME::set_physical(const MSSM_physical& physical_)
{
   physical = physical_;
}
...
int CLASSNAME::tadpole_equations(const gsl_vector* x, void* params, gsl_vector* f)
{
   ...
   MSSM_mass_eigenstates* model = ewsb_args->model;
...
{
   if (PHYSICAL(MSd).tail<6>().minCoeff() < 0.) problems.flag_tachyon(MSSM_info::Sd);
   if (PHYSICAL(MSv).tail<3>().minCoeff() < 0.) problems.flag_tachyon(MSSM_info::Sv);
   if (PHYSICAL(MSu).tail<6>().minCoeff() < 0.) problems.flag_tachyon(MSSM_info::Su);
   if (PHYSICAL(MSe).tail<6>().minCoeff() < 0.) problems.flag_tachyon(MSSM_info::Se);
   if (PHYSICAL(Mhh).tail<2>().minCoeff() < 0.) problems.flag_tachyon(MSSM_info::hh);
   if (PHYSICAL(MAh).tail<1>().minCoeff() < 0.) problems.flag_tachyon(MSSM_info::Ah);
   if (PHYSICAL(MHpm).tail<1>().minCoeff() < 0.) problems.flag_tachyon(MSSM_info::Hpm);
...
void CLASSNAME::calculate_MSd()
{
   ...
   problems.flag_bad_mass(MSSM_info::Sd, eigenvalue_error > precision *
      Abs(MSd(0)));
#else
...
void CLASSNAME::calculate_MSv()
{
   ...
   problems.flag_bad_mass(MSSM_info::Sv, eigenvalue_error > precision *
      Abs(MSv(0)));
#else
...
void CLASSNAME::calculate_MSu()
{
   ...
   problems.flag_bad_mass(MSSM_info::Su, eigenvalue_error > precision *
      Abs(MSu(0)));
#else
...
void CLASSNAME::calculate_MSe()
{
   ...
   problems.flag_bad_mass(MSSM_info::Se, eigenvalue_error > precision *
      Abs(MSe(0)));
#else
...
void CLASSNAME::calculate_Mhh()
{
   ...
   problems.flag_bad_mass(MSSM_info::hh, eigenvalue_error > precision *
      Abs(Mhh(0)));
#else
...
void CLASSNAME::calculate_MAh()
{
   ...
   problems.flag_bad_mass(MSSM_info::Ah, eigenvalue_error > precision *
      Abs(MAh(0)));
#else
...
void CLASSNAME::calculate_MHpm()
{
   ...
   problems.flag_bad_mass(MSSM_info::Hpm, eigenvalue_error > precision *
      Abs(MHpm(0)));
#else
...
void CLASSNAME::calculate_MChi()
{
   ...
   problems.flag_bad_mass(MSSM_info::Chi, eigenvalue_error > precision *
      Abs(MChi(0)));
#else
...
void CLASSNAME::calculate_MCha()
{
   ...
   problems.flag_bad_mass(MSSM_info::Cha, eigenvalue_error > precision *
      Abs(MCha(0)));
#else
...
void CLASSNAME::calculate_MFe()
{
   ...
   problems.flag_bad_mass(MSSM_info::Fe, eigenvalue_error > precision *
      Abs(MFe(0)));
#else
...
void CLASSNAME::calculate_MFd()
{
   ...
   problems.flag_bad_mass(MSSM_info::Fd, eigenvalue_error > precision *
      Abs(MFd(0)));
#else
...
void CLASSNAME::calculate_MFu()
{
   ...
   problems.flag_bad_mass(MSSM_info::Fu, eigenvalue_error > precision *
      Abs(MFu(0)));
#else
...
{
   if (!force_output && problems.is_tachyon(MSSM_info::VZ))
      return;
...
{
   if (!force_output && problems.is_tachyon(MSSM_info::Sd))
      return;
   ...
   for (unsigned es = 0; es < 6; ++es) {
      ...
      Eigen::Matrix<double,6,6> mix_ZD;
         ...
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZD,
            eigenvalue_error);
         problems.flag_bad_mass(MSSM_info::Sd, eigenvalue_error >
...
{
   if (!force_output && problems.is_tachyon(MSSM_info::Sv))
      return;
   ...
   for (unsigned es = 0; es < 3; ++es) {
      ...
      Eigen::Matrix<double,3,3> mix_ZV;
         ...
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZV,
            eigenvalue_error);
         problems.flag_bad_mass(MSSM_info::Sv, eigenvalue_error >
            precision * Abs(eigen_values(0)));
      #else
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZV);
      #endif
...
{
   if (!force_output && problems.is_tachyon(MSSM_info::Su))
      return;
   ...
   for (unsigned es = 0; es < 6; ++es) {
      ...
      Eigen::Matrix<double,6,6> mix_ZU;
         ...
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZU,
            eigenvalue_error);
         problems.flag_bad_mass(MSSM_info::Su, eigenvalue_error >
            precision * Abs(eigen_values(0)));
      #else
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZU);
      #endif
...
{
   if (!force_output && problems.is_tachyon(MSSM_info::Se))
      return;
   ...
   for (unsigned es = 0; es < 6; ++es) {
      ...
      Eigen::Matrix<double,6,6> mix_ZE;
         ...
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZE,
            eigenvalue_error);
         problems.flag_bad_mass(MSSM_info::Se, eigenvalue_error >
            precision * Abs(eigen_values(0)));
      #else
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZE);
      #endif
...
{
   if (!force_output && problems.is_tachyon(MSSM_info::hh))
      return;
         ...
         for (unsigned i = 0; i < 2; i++) {
            for (unsigned k = 0; k < 2; k++) {
               if (!std::isfinite(self_energy_2l(i,k))) {
                  ...
                  problems.flag_bad_mass(MSSM_info::hh);
               }
            }
         }
      }
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(MSSM_info::hh);
   else
      problems.unflag_no_pole_mass_convergence(MSSM_info::hh);
}

void CLASSNAME::calculate_MAh_pole()
{
   if (!force_output && problems.is_tachyon(MSSM_info::Ah))
      return;
      ...
      if (pole_mass_loop_order > 1) {
         ...
         for (unsigned i = 0; i < 2; i++) {
            for (unsigned k = 0; k < 2; k++) {
               if (!std::isfinite(self_energy_2l(i,k))) {
                  self_energy_2l(i,k) = 0.;
                  problems.flag_bad_mass(MSSM_info::Ah);
               }
            }
         }
      }
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(MSSM_info::Ah);
   else
      problems.unflag_no_pole_mass_convergence(MSSM_info::Ah);
}

void CLASSNAME::calculate_MHpm_pole()
{
   if (!force_output && problems.is_tachyon(MSSM_info::Hpm))
      return;
   ...
   do {
      ...
      for (unsigned es = 0; es < 2; ++es) {
         ...
         Eigen::Matrix<double,2,2> mix_ZP;
            ...
            problems.flag_bad_mass(MSSM_info::Hpm,
               eigenvalue_error > precision * Abs(eigen_values(0)));
         #else
            fs_diagonalize_hermitian(M_loop, eigen_values,
               mix_ZP);
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(MSSM_info::Hpm);
   else
      problems.unflag_no_pole_mass_convergence(MSSM_info::Hpm);
}
   ...
   for (unsigned es = 0; es < 4; ++es) {
      ...
      decltype(ZN) mix_ZN;
         ...
         fs_diagonalize_symmetric(M_loop, eigen_values, mix_ZN,
            eigenvalue_error);
         problems.flag_bad_mass(MSSM_info::Chi, eigenvalue_error >
            precision * Abs(eigen_values(0)));
      #else
         fs_diagonalize_symmetric(M_loop, eigen_values, mix_ZN);
      #endif
...
void CLASSNAME::calculate_MCha_pole()
{
   ...
   for (unsigned es = 0; es < 2; ++es) {
      ...
      problems.flag_bad_mass(MSSM_info::Cha, eigenvalue_error >
         precision * Abs(eigen_values(0)));
   #else
...
void CLASSNAME::calculate_MFe_pole()
{
   ...
   for (unsigned es = 0; es < 3; ++es) {
      ...
      fs_svd(M_loop, eigen_values, mix_ZEL, mix_ZER, eigenvalue_error)
         ;
      problems.flag_bad_mass(MSSM_info::Fe, eigenvalue_error >
         precision * Abs(eigen_values(0)));
   #else
...
void CLASSNAME::calculate_MFd_pole()
{
   ...
   for (unsigned es = 0; es < 3; ++es) {
      ...
      fs_svd(M_loop, eigen_values, mix_ZDL, mix_ZDR, eigenvalue_error)
         ;
      problems.flag_bad_mass(MSSM_info::Fd, eigenvalue_error >
         precision * Abs(eigen_values(0)));
   #else
...
void CLASSNAME::calculate_MFu_pole()
{
   ...
   for (unsigned es = 0; es < 3; ++es) {
      ...
      fs_svd(M_loop, eigen_values, mix_ZUL, mix_ZUR, eigenvalue_error)
         ;
      problems.flag_bad_mass(MSSM_info::Fu, eigenvalue_error >
         precision * Abs(eigen_values(0)));
   #else
...
{
   if (!force_output && problems.is_tachyon(MSSM_info::VWm))
      return;
...
std::ostream& operator<<(std::ostream& ostr, const MSSM_mass_eigenstates& model)
{
   model.print(ostr);
   return ostr;
}
