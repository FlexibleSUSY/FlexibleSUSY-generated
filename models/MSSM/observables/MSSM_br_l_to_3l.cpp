...
class nPointFeFeFeFe_3917884759387016 : public correlation_function_context<4,0>
{
   ...
   public:
   nPointFeFeFeFe_3917884759387016( const MSSM_mass_eigenstates &model,
...
}; // End of nPointFeFeFeFe_3917884759387016

std::array<std::complex<double>, 10> scalars1loop(const MSSM_mass_eigenstates &model,
const std::array<int, 4> &indices,
const std::array<Eigen::Vector4d, 0> &momenta
) {
   nPointFeFeFeFe_3917884759387016 helper{model, indices, momenta};
   return helper.calculate();
}
class nPointFeFeFeFe_3917884759688858 : public correlation_function_context<4,0>
{
   using generic_sum_base = correlation_function_context<4,0>;
   ...
   public:
   nPointFeFeFeFe_3917884759688858( const MSSM_mass_eigenstates &model,
   const std::array<int, 4> &indices,
   const std::array<Eigen::Vector4d, 0> &momenta
    ) :
   correlation_function_context<4,0> { model, indices, momenta }
...
}; // End of nPointFeFeFeFe_3917884759688858

std::array<std::complex<double>, 10> vectors1loop(const MSSM_mass_eigenstates &model,
const std::array<int, 4> &indices,
const std::array<Eigen::Vector4d, 0> &momenta
) {
   nPointFeFeFeFe_3917884759688858 helper{model, indices, momenta};
   return helper.calculate();
}
class nPointFeFeFeFe_3917884760017687 : public correlation_function_context<4,0>
{
   using generic_sum_base = correlation_function_context<4,0>;
   ...
   std::array<std::complex<double>,10> genericSum3( void ) {
      ...
      using GenericInsertions = boost::mpl::vector<
         ...
         boost::mpl::vector<fields::Cha, fields::Cha, fields::Sv, fields::Sv>,
         boost::mpl::vector<fields::Chi, fields::Chi, fields::Se, fields::Se>
         >;
      using combinatorial_factors = boost::mpl::vector<
         boost::mpl::int_<1>,
   ...
   public:
   nPointFeFeFeFe_3917884760017687( const MSSM_mass_eigenstates &model,
   const std::array<int, 4> &indices,
   const std::array<Eigen::Vector4d, 0> &momenta
    ) :
   correlation_function_context<4,0> { model, indices, momenta }
...
}; // End of nPointFeFeFeFe_3917884760017687

std::array<std::complex<double>, 10> boxes1loop(const MSSM_mass_eigenstates &model,
const std::array<int, 4> &indices,
const std::array<Eigen::Vector4d, 0> &momenta
) {
   nPointFeFeFeFe_3917884760017687 helper{model, indices, momenta};
   return helper.calculate();
}
...
void run_to_MSUSY(MSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSM_br_l_to_3l: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSM_br_l_to_3l:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSM_br_l_to_3l: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSM_br_l_to_3l: run_to_MSUSY(): "
...
std::complex<double> left(const MSSM_mass_eigenstates& model, int nA) {
 ...
 */
template <class Lepton, class Photon>
std::complex<double> right(const MSSM_mass_eigenstates& model, int nA) {
    context_base context {model};
    using vertex = Vertex<typename Lepton::lorentz_conjugate, Lepton, Photon>;
    std::array<int, 2> indices {nA, nA};
    const auto value =  vertex::evaluate(indices, context);
...
Eigen::Array<std::complex<double>,10,1> fix_tensors_sign(
   const MSSM_mass_eigenstates& model, int nI, int nO, int nA) {
...
Eigen::Array<std::complex<double>,13,1> forge(int nI, int nO, int nA,
   const MSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd) {
   ...
   try {
      ...
      ERROR("MSSM_br_l_to_3l:" << e.what_detailed());
