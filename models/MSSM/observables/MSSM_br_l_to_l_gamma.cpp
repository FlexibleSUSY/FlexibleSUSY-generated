...
#include "MSSM_br_l_to_l_gamma.hpp"
...
void run_to_MSUSY(MSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSM_l_to_lgamma: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSM_l_to_lgamma:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSM_l_to_lgamma: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSM_l_to_lgamma: run_to_MSUSY(): "
...
double calculate_Fe_to_Fe_VP (
   ...
   const MSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Physical_input& physical_input) {
   ...
   try {
      ...
      ERROR("MSSM_l_to_lgamma:" << e.what_detailed());
