...
// File generated at Sun 26 Aug 2018 15:27:07
...
double MSSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void MSSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MSSM_input_parameters& input)
{
   ...
   MSSM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MSSM_ewsb_solver<Two_scale> >(ewsb_solver));

   MSSM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   MSSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   MSSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   MSSM_convergence_tester<Two_scale> convergence_tester(
   ...
   MSSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void MSSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MSSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void MSSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
