...
public:
   ...
   std::tuple<MSSM<T>> get_models_slha() const
   { return std::make_tuple(MSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MSSM<T>& get_model() const
   { return model; }
   MSSM<T>& get_model()
   { return model; }
   MSSM<T> get_model_slha() const
   { return MSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return MSSM_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(MSSM_info::parameter_names),
      std::cend(MSSM_info::parameter_names));
