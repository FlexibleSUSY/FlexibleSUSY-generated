...
// File generated at Sun 26 Aug 2018 15:26:52
...
template <class T>
class MSSM_spectrum_generator_interface {
public:
   virtual ~MSSM_spectrum_generator_interface() = default;

   std::tuple<MSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<MSSM_slha<MSSM<T>>> get_models_slha() const
   { return std::make_tuple(MSSM_slha<MSSM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MSSM<T>& get_model() const
   { return model; }
   MSSM<T>& get_model()
   { return model; }
   MSSM_slha<MSSM<T>> get_model_slha() const
   { return MSSM_slha<MSSM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MSSM_input_parameters&);
...
protected:
   ...
   void translate_exception_to_problem(MSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MSSM_input_parameters& input)
{
 ...
 */
template <class T>
void MSSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MSSM_mass_eigenstates tmp_model(model);
   try {
   ...
   Coupling_monitor<MSSM_mass_eigenstates, MSSM_parameter_getter>
 ...
 */
template <class T>
void MSSM_spectrum_generator_interface<T>::translate_exception_to_problem(MSSM<T>& model)
{
   try {
      throw;
