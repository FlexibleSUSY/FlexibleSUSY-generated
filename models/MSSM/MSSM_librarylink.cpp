...
class MSSM_spectrum {
public:
   virtual ~MSSM_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSM_observables& get_observables() const = 0;
   virtual const MSSM_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSM_input_parameters&) = 0;
...
class MSSM_spectrum_impl : public MSSM_spectrum
{
public:
   virtual ~MSSM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSM_observables& get_observables() const override { return observables; }
   virtual const MSSM_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSM_input_parameters&) override;
...
private:
   std::tuple<MSSM<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSM_scales scales{};              ///< scale information
   MSSM_observables observables{};    ///< observables
   MSSM_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const MSSM_input_parameters& input_) { input = input_; }
...
private:
   ...
   std::unique_ptr<MSSM_spectrum> spectrum{nullptr};  ///< spectrum information

   MSSM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSMSSMCalculateSpectrum[]?");
   }
}
...
MSSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const MSSM_slha& model, MLINK link)
{
   MLPutRule(link, MSSM_info::model_name);
...
template <typename Solver_type>
void MSSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSM_spectrum_impl<Solver_type>::calculate_spectrum(
...
template <typename Solver_type>
void MSSM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MSSM_spectrum_impl<Solver_type>::calculate_model_decays(
   const softsusy::QedQcd& qedqcd, const Physical_input& physical_input, const FlexibleDecay_settings& flexibledecay_settings)
{
   decays = MSSM_decays(std::get<0>(models), qedqcd, physical_input, flexibledecay_settings);
...
template <typename Solver_type>
void MSSM_spectrum_impl<Solver_type>::fill_slha_io(MSSM_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   MSSM_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   MSSM_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = MSSM_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSM_spectrum_impl<Two_scale>());
...
{
   using namespace flexiblesusy::MSSM_librarylink;
...
DLLEXPORT int FSMSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSMSSMCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSMSSMCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSMSSMCalculateDecays", "error", e.what());
      put_error_output(link);
   }
