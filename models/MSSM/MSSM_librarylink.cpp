...
template <typename Solver_type>
void MSSM_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = MSSM_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSMSSMCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSMSSMCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
