...
// File generated at Mon 27 Feb 2017 13:57:21

#ifndef MSSM_SPECTRUM_GENERATOR_H
#define MSSM_SPECTRUM_GENERATOR_H

#include "MSSM_spectrum_generator_interface.hpp"
...
template <class T>
class MSSM_spectrum_generator
   : public MSSM_spectrum_generator_interface<T> {
public:
   MSSM_spectrum_generator()
      : MSSM_spectrum_generator_interface<T>()
      , high_scale(0.)
      , susy_scale(0.)
      , low_scale(0.)
   {}
   virtual ~MSSM_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const MSSM_input_parameters&);
   void write_running_couplings(const std::string& filename = "MSSM_rgflow.dat") const;
 ...
 */
template <class T>
void MSSM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const MSSM_input_parameters& input)
{
   MSSM<T>& model = this->model;
   model.clear();
   ...
   MSSM_high_scale_constraint<T> high_scale_constraint(&model);
   MSSM_susy_scale_constraint<T> susy_scale_constraint(&model, qedqcd);
   MSSM_low_scale_constraint<T>  low_scale_constraint(&model, qedqcd);
   ...
   MSSM_convergence_tester<T> convergence_tester(
   ...
   MSSM_initial_guesser<T> initial_guesser(&model, qedqcd,
 ...
 */
template <class T>
void MSSM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   MSSM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
