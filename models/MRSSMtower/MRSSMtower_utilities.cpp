...
MRSSMtower_spectrum_plotter::MRSSMtower_spectrum_plotter()
   : spectrum()
   , scale(0.0)
   , width(16)
{
...
void MRSSMtower_spectrum_plotter::extract_spectrum(const MRSSMtower_mass_eigenstates& model)
{
   spectrum.clear();
...
void MRSSMtower_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;
...
void MRSSMtower_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   for (std::size_t s = 0; s < spectrum.size(); ++s) {
      if (!filestr.good()) {
         ERROR("MRSSMtower_spectrum_plotter::write_spectrum: "
...
std::valarray<double> MRSSMtower_spectrum_plotter::to_valarray(double v)
{
 ...
 */
void to_database(
   const std::string& file_name, const MRSSMtower_mass_eigenstates& model,
   ...
   const MRSSMtower_observables* observables)
{
   ...
   append(names, MRSSMtower_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, MRSSMtower_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill pole masses and mixings
   append(names, MRSSMtower_parameter_getter().get_mass_names());
   append(names, MRSSMtower_parameter_getter().get_mixing_names());
   ...
   if (MRSSMtower_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, MRSSMtower_observables::get_names());
...
MRSSMtower_mass_eigenstates from_database(
   ...
   Physical_input* physical_input, MRSSMtower_observables* observables)
{
   using utilities::append;

   MRSSMtower_mass_eigenstates model;
   const std::size_t number_of_parameters = model.get_number_of_parameters();
   const std::size_t number_of_masses = MRSSMtower_parameter_getter().get_number_of_masses();
   const std::size_t number_of_mixings = MRSSMtower_info::NUMBER_OF_MIXINGS;
   const std::size_t number_of_input_parameters = MRSSMtower_info::NUMBER_OF_INPUT_PARAMETERS;
   ...
   const std::size_t number_of_observables =
      (observables ? MRSSMtower_observables::NUMBER_OF_OBSERVABLES : 0u);
