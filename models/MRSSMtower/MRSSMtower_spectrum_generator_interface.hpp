...
#ifndef MRSSMtower_SPECTRUM_GENERATOR_INTERFACE_H
#define MRSSMtower_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class MRSSMtower_spectrum_generator_interface {
public:
   MRSSMtower_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~MRSSMtower_spectrum_generator_interface() {}

   const MRSSMtower<T>& get_model() const { return model; }
   MRSSMtower<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const MRSSMtower_input_parameters&) = 0;
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MRSSMtower_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(MRSSMtower<T>& model);
};
 ...
 */
template <class T>
void MRSSMtower_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MRSSMtower_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MRSSMtower_mass_eigenstates tmp_model(model);
   try {
   ...
   MRSSMtower_parameter_getter parameter_getter;
   Coupling_monitor<MRSSMtower_mass_eigenstates, MRSSMtower_parameter_getter>
 ...
 */
template <class T>
void MRSSMtower_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   MRSSMtower_spectrum_plotter plotter;
 ...
 */
template <class T>
void MRSSMtower_spectrum_generator_interface<T>::translate_exception_to_problem(MRSSMtower<T>& model)
{
   try {
      throw;
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : MRSSMtower_info::parameter_names[parameter_index];
