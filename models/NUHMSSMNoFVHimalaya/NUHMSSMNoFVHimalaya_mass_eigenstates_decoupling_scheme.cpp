...
/**
 * @file NUHMSSMNoFVHimalaya_mass_eigenstates_decoupling_scheme.cpp
 * @brief implementation of the NUHMSSMNoFVHimalaya model class in the decoupling scheme
 *
...
#include "NUHMSSMNoFVHimalaya_mass_eigenstates_decoupling_scheme.hpp"
...
#define CLASSNAME NUHMSSMNoFVHimalaya_mass_eigenstates_decoupling_scheme
...
CLASSNAME::CLASSNAME(const NUHMSSMNoFVHimalaya_mass_eigenstates& model)
{
   fill_from(model);
}
...
{
   return std::make_unique<NUHMSSMNoFVHimalaya_mass_eigenstates_decoupling_scheme>(*this);
}
   ...
   // calculate new gauge couplings
   {
      ...
      if (IsFinite(new_g1)) {
         ...
         model->get_problems().flag_non_perturbative_parameter(
            NUHMSSMNoFVHimalaya_info::g1, new_g1, get_scale());
      ...
      if (IsFinite(new_g2)) {
         ...
         model->get_problems().flag_non_perturbative_parameter(
            NUHMSSMNoFVHimalaya_info::g2, new_g2, get_scale());
...
void CLASSNAME::fill_from(const NUHMSSMNoFVHimalaya_mass_eigenstates& model)
{
   set(model.get());
...
std::ostream& operator<<(std::ostream& ostr, const NUHMSSMNoFVHimalaya_mass_eigenstates_decoupling_scheme& model)
{
   model.print(ostr);
   return ostr;
}
