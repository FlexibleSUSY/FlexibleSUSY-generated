...
/**
 * @file NUHMSSMNoFVHimalaya_amm.cpp
 *
 * This file was generated with FlexibleSUSY 2.8.0 and SARAH 4.15.1 .
 */

#include "NUHMSSMNoFVHimalaya_amm.hpp"
#include "NUHMSSMNoFVHimalaya_mass_eigenstates.hpp"

#include "cxx_qft/NUHMSSMNoFVHimalaya_qft.hpp"
#include "cxx_qft/NUHMSSMNoFVHimalaya_particle_aliases.hpp"
#include "NUHMSSMNoFVHimalaya_FFV_form_factors.hpp"
...
double get_MLCP(const NUHMSSMNoFVHimalaya_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(NUHMSSMNoFVHimalaya_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("NUHMSSMNoFVHimalaya_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("NUHMSSMNoFVHimalaya_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("NUHMSSMNoFVHimalaya_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("NUHMSSMNoFVHimalaya_amm: run_to_MSUSY(): "
...
template <typename Lepton>
std::enable_if_t<std::is_same_v<Lepton, NUHMSSMNoFVHimalaya_cxx_diagrams::fields::Fm>, double>
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const NUHMSSMNoFVHimalaya_mass_eigenstates& model, double ml_pole)
{
   using namespace NUHMSSMNoFVHimalaya_cxx_diagrams::fields;

   const auto form_factors = NUHMSSMNoFVHimalaya_FFV_form_factors::calculate_form_factors<Lepton,Lepton,VP>(model, true);
...
template <typename Lepton>
double calculate_amm_impl(const NUHMSSMNoFVHimalaya_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings)
{
   VERBOSE_MSG("NUHMSSMNoFVHimalaya_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double NUHMSSMNoFVHimalaya_amm::calculate_amm(const NUHMSSMNoFVHimalaya_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings)
{
   NUHMSSMNoFVHimalaya_mass_eigenstates model(model_);

   VERBOSE_MSG("NUHMSSMNoFVHimalaya_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUHMSSMNoFVHimalaya_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template double NUHMSSMNoFVHimalaya_amm::calculate_amm<NUHMSSMNoFVHimalaya_cxx_diagrams::fields::Fm>(const NUHMSSMNoFVHimalaya_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&);

template <typename Lepton>
double NUHMSSMNoFVHimalaya_amm::calculate_amm_uncertainty(const NUHMSSMNoFVHimalaya_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings)
{
   NUHMSSMNoFVHimalaya_mass_eigenstates model(model_);

   VERBOSE_MSG("NUHMSSMNoFVHimalaya_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUHMSSMNoFVHimalaya_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
// spliting templated function into header and cpp
// https://isocpp.org/wiki/faq/templates#separate-template-fn-defn-from-decl
template double NUHMSSMNoFVHimalaya_amm::calculate_amm_uncertainty<NUHMSSMNoFVHimalaya_cxx_diagrams::fields::Fm>(const NUHMSSMNoFVHimalaya_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&);
