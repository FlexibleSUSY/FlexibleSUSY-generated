...
#include "NUHMSSMNoFVHimalaya_two_scale_convergence_tester.hpp"
...
double NUHMSSMNoFVHimalaya_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void NUHMSSMNoFVHimalaya_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const NUHMSSMNoFVHimalaya_input_parameters& input)
{
   ...
   NUHMSSMNoFVHimalaya_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<NUHMSSMNoFVHimalaya_ewsb_solver<Two_scale> >(ewsb_solver));

   NUHMSSMNoFVHimalaya_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   NUHMSSMNoFVHimalaya_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   NUHMSSMNoFVHimalaya_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   NUHMSSMNoFVHimalaya_convergence_tester<Two_scale> convergence_tester(
   ...
   NUHMSSMNoFVHimalaya_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void NUHMSSMNoFVHimalaya_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   NUHMSSMNoFVHimalaya_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void NUHMSSMNoFVHimalaya_spectrum_generator<Two_scale>::calculate_spectrum()
{
