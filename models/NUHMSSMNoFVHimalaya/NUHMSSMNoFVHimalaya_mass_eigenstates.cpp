...
/**
 * @file NUHMSSMNoFVHimalaya_mass_eigenstates.cpp
...
#include "NUHMSSMNoFVHimalaya_ewsb_solver_interface.hpp"
#include "config.h"
...
#include "NUHMSSMNoFVHimalaya_two_scale_ewsb_solver.hpp"
#endif
...
#define CLASSNAME NUHMSSMNoFVHimalaya_mass_eigenstates
...
CLASSNAME::CLASSNAME(const NUHMSSMNoFVHimalaya_input_parameters& input_)
   : NUHMSSMNoFVHimalaya_soft_parameters(input_)
#if defined(ENABLE_TWO_SCALE_SOLVER)
   , ewsb_solver(new NUHMSSMNoFVHimalaya_ewsb_solver<Two_scale>())
#endif
{
}

std::unique_ptr<NUHMSSMNoFVHimalaya_mass_eigenstates_interface> CLASSNAME::clone() const
{
   return std::make_unique<NUHMSSMNoFVHimalaya_mass_eigenstates>(*this);
}
...
const NUHMSSMNoFVHimalaya_physical& CLASSNAME::get_physical() const
{
   return physical;
}

NUHMSSMNoFVHimalaya_physical& CLASSNAME::get_physical()
{
   return physical;
}

void CLASSNAME::set_physical(const NUHMSSMNoFVHimalaya_physical& physical_)
{
   physical = physical_;
}
...
void CLASSNAME::set_ewsb_solver(const std::shared_ptr<NUHMSSMNoFVHimalaya_ewsb_solver_interface>& solver)
{
   ewsb_solver = solver;
}
...
int CLASSNAME::solve_ewsb()
{
   ...
   VERBOSE_MSG("\t\tSolving NUHMSSMNoFVHimalaya EWSB at " << ewsb_loop_order << "-loop order");
...
void CLASSNAME::print(std::ostream& ostr) const
{
   ostr << "========================================\n"
           "NUHMSSMNoFVHimalaya\n"
           "========================================\n";
   NUHMSSMNoFVHimalaya_soft_parameters::print(ostr);
   ostr << "----------------------------------------\n"
...
{
   if (PHYSICAL(MSveL) < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::SveL); }
   if (PHYSICAL(MSvmL) < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::SvmL); }
   if (PHYSICAL(MSvtL) < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::SvtL); }
   if (PHYSICAL(MSd).tail<2>().minCoeff() < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::Sd); }
   if (PHYSICAL(MSu).tail<2>().minCoeff() < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::Su); }
   if (PHYSICAL(MSe).tail<2>().minCoeff() < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::Se); }
   if (PHYSICAL(MSm).tail<2>().minCoeff() < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::Sm); }
   if (PHYSICAL(MStau).tail<2>().minCoeff() < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::Stau); }
   if (PHYSICAL(MSs).tail<2>().minCoeff() < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::Ss); }
   if (PHYSICAL(MSc).tail<2>().minCoeff() < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::Sc); }
   if (PHYSICAL(MSb).tail<2>().minCoeff() < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::Sb); }
   if (PHYSICAL(MSt).tail<2>().minCoeff() < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::St); }
   if (PHYSICAL(Mhh).tail<2>().minCoeff() < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::hh); }
   if (PHYSICAL(MAh).tail<1>().minCoeff() < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::Ah); }
   if (PHYSICAL(MHpm).tail<1>().minCoeff() < 0.) { problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::Hpm); }
}
...
{
   NUHMSSMNoFVHimalaya_soft_parameters::clear();
   clear_DRbar_parameters();
   physical.clear();
   problems.clear();
}
...
void CLASSNAME::run_to(double scale, double eps)
{
   if (eps < 0.0)
      eps = precision;
   NUHMSSMNoFVHimalaya_soft_parameters::run_to(scale, eps);
}
...
const NUHMSSMNoFVHimalaya_input_parameters& CLASSNAME::get_input_parameters() const
{
   return get_input();
}

NUHMSSMNoFVHimalaya_input_parameters& CLASSNAME::get_input_parameters()
{
   return get_input();
}
...
void CLASSNAME::calculate_MSveL()
{
   ...
   if (MSveL < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::SveL);
   }
...
void CLASSNAME::calculate_MSvmL()
{
   ...
   if (MSvmL < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::SvmL);
   }
...
void CLASSNAME::calculate_MSvtL()
{
   ...
   if (MSvtL < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::SvtL);
   }
...
void CLASSNAME::calculate_MSd()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Sd, eigenvalue_error >
      precision * Abs(MSd(0)));
#else
   ...
   if (MSd.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::Sd);
   }
...
void CLASSNAME::calculate_MSu()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Su, eigenvalue_error >
      precision * Abs(MSu(0)));
#else
   ...
   if (MSu.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::Su);
   }
...
void CLASSNAME::calculate_MSe()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Se, eigenvalue_error >
      precision * Abs(MSe(0)));
#else
   ...
   if (MSe.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::Se);
   }
...
void CLASSNAME::calculate_MSm()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Sm, eigenvalue_error >
      precision * Abs(MSm(0)));
#else
   ...
   if (MSm.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::Sm);
   }
...
void CLASSNAME::calculate_MStau()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Stau, eigenvalue_error >
   ...
   if (MStau.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::Stau);
   }
...
void CLASSNAME::calculate_MSs()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Ss, eigenvalue_error >
      precision * Abs(MSs(0)));
#else
   ...
   if (MSs.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::Ss);
   }
...
void CLASSNAME::calculate_MSc()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Sc, eigenvalue_error >
      precision * Abs(MSc(0)));
#else
   ...
   if (MSc.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::Sc);
   }
...
void CLASSNAME::calculate_MSb()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Sb, eigenvalue_error >
      precision * Abs(MSb(0)));
#else
   ...
   if (MSb.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::Sb);
   }
...
void CLASSNAME::calculate_MSt()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::St, eigenvalue_error >
      precision * Abs(MSt(0)));
#else
   ...
   if (MSt.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::St);
   }
...
void CLASSNAME::calculate_Mhh()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::hh, eigenvalue_error >
      precision * Abs(Mhh(0)));
#else
   ...
   if (Mhh.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::hh);
   }
...
void CLASSNAME::calculate_MAh()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Ah, eigenvalue_error >
      precision * Abs(MAh(0)));
#else
   ...
   if (MAh.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::Ah);
   }
...
void CLASSNAME::calculate_MHpm()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Hpm, eigenvalue_error >
   ...
   if (MHpm.minCoeff() < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::Hpm);
   }
...
void CLASSNAME::calculate_MChi()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Chi, eigenvalue_error >
...
void CLASSNAME::calculate_MCha()
{
   ...
   problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Cha, eigenvalue_error >
...
void CLASSNAME::calculate_MVWm()
{
   ...
   if (MVWm < 0.) {
      problems.flag_running_tachyon(NUHMSSMNoFVHimalaya_info::VWm);
   }
...
{
   if (!force_output && problems.is_running_tachyon(NUHMSSMNoFVHimalaya_info::
      VZ))
      return;
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::VZ);
   }
   ...
   for (int es = 0; es < 2; ++es) {
      ...
      Eigen::Matrix<double,2,2> mix_ZTau;
         ...
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZTau,
            eigenvalue_error);
         problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Stau,
...
void CLASSNAME::calculate_Mhh_pole()
{
      ...
      for (int i = 0; i < 2; i++) {
         for (int k = 0; k < 2; k++) {
            if (!std::isfinite(self_energy_2l(i,k))) {
               ...
               problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::hh);
            }
         }
      }
   }
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(NUHMSSMNoFVHimalaya_info::hh);
   else
      problems.unflag_no_pole_mass_convergence(NUHMSSMNoFVHimalaya_info::hh);
}
...
void CLASSNAME::calculate_MAh_pole()
{
   ...
   if (pole_mass_loop_order > 1) {
      ...
      for (int i = 0; i < 2; i++) {
         for (int k = 0; k < 2; k++) {
            if (!std::isfinite(self_energy_2l(i,k))) {
               self_energy_2l(i,k) = 0.;
               problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Ah);
            }
         }
      }
   }
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(NUHMSSMNoFVHimalaya_info::Ah);
   else
      problems.unflag_no_pole_mass_convergence(NUHMSSMNoFVHimalaya_info::Ah);
}
...
void CLASSNAME::calculate_MHpm_pole()
{
      ...
      for (int es = 0; es < 2; ++es) {
         ...
         Eigen::Matrix<double,2,2> mix_ZP;
            ...
            fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZP,
               eigenvalue_error);
            problems.flag_bad_mass(NUHMSSMNoFVHimalaya_info::Hpm,
               eigenvalue_error > precision * Abs(eigen_values(0)));
         #else
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(NUHMSSMNoFVHimalaya_info::Hpm);
   else
      problems.unflag_no_pole_mass_convergence(NUHMSSMNoFVHimalaya_info::Hpm);
}
...
void CLASSNAME::calculate_MVWm_pole()
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::VWm);
   }
...
double CLASSNAME::calculate_MVZ_DRbar(double m_pole) const
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::VZ);return m_pole;
   }
...
double CLASSNAME::calculate_MVWm_DRbar(double m_pole) const
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(NUHMSSMNoFVHimalaya_info::VWm);return m_pole;
   }
