...
#ifndef NUHMSSMNoFVHimalaya_SPECTRUM_GENERATOR_INTERFACE_H
#define NUHMSSMNoFVHimalaya_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class NUHMSSMNoFVHimalaya_spectrum_generator_interface {
public:
   virtual ~NUHMSSMNoFVHimalaya_spectrum_generator_interface() = default;

   std::tuple<NUHMSSMNoFVHimalaya<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<NUHMSSMNoFVHimalaya_slha<NUHMSSMNoFVHimalaya<T>>> get_models_slha() const
   { return std::make_tuple(NUHMSSMNoFVHimalaya_slha<NUHMSSMNoFVHimalaya<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const NUHMSSMNoFVHimalaya<T>& get_model() const
   { return model; }
   NUHMSSMNoFVHimalaya<T>& get_model()
   { return model; }
   NUHMSSMNoFVHimalaya_slha<NUHMSSMNoFVHimalaya<T>> get_model_slha() const
   { return NUHMSSMNoFVHimalaya_slha<NUHMSSMNoFVHimalaya<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const NUHMSSMNoFVHimalaya_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "NUHMSSMNoFVHimalaya_spectrum.dat") const;

protected:
   NUHMSSMNoFVHimalaya<T> model;
   ...
   void translate_exception_to_problem(NUHMSSMNoFVHimalaya<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const NUHMSSMNoFVHimalaya_input_parameters&) = 0;
};
 ...
 */
template <class T>
void NUHMSSMNoFVHimalaya_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void NUHMSSMNoFVHimalaya_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const NUHMSSMNoFVHimalaya_input_parameters& input)
{
 ...
 */
template <class T>
void NUHMSSMNoFVHimalaya_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   NUHMSSMNoFVHimalaya_mass_eigenstates tmp_model(model);
   try {
   ...
   NUHMSSMNoFVHimalaya_parameter_getter parameter_getter;
   Coupling_monitor<NUHMSSMNoFVHimalaya_mass_eigenstates, NUHMSSMNoFVHimalaya_parameter_getter>
 ...
 */
template <class T>
void NUHMSSMNoFVHimalaya_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   NUHMSSMNoFVHimalaya_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void NUHMSSMNoFVHimalaya_spectrum_generator_interface<T>::translate_exception_to_problem(NUHMSSMNoFVHimalaya<T>& model)
{
   try {
      throw;
