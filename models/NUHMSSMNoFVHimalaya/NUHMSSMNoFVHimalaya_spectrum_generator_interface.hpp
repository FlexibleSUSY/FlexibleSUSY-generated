...
public:
   ...
   std::tuple<NUHMSSMNoFVHimalaya<T>> get_models_slha() const
   { return std::make_tuple(NUHMSSMNoFVHimalaya<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const NUHMSSMNoFVHimalaya<T>& get_model() const
   { return model; }
   NUHMSSMNoFVHimalaya<T>& get_model()
   { return model; }
   NUHMSSMNoFVHimalaya<T> get_model_slha() const
   { return NUHMSSMNoFVHimalaya<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return NUHMSSMNoFVHimalaya_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(NUHMSSMNoFVHimalaya_info::parameter_names),
      std::cend(NUHMSSMNoFVHimalaya_info::parameter_names));
