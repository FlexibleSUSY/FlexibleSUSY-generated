...
#include "lowe.h"
#include "NUHMSSMNoFVHimalaya_unitarity.hpp"
...
namespace NUHMSSMNoFVHimalaya_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSNUHMSSMNoFVHimalayaMessage", 1);
...
class NUHMSSMNoFVHimalaya_spectrum {
public:
   virtual ~NUHMSSMNoFVHimalaya_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(NUHMSSMNoFVHimalaya_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const NUHMSSMNoFVHimalaya_observables& get_observables() const = 0;

   virtual const UnitarityInfiniteS& get_unitarity() const = 0;
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const NUHMSSMNoFVHimalaya_input_parameters&) = 0;
...
class NUHMSSMNoFVHimalaya_spectrum_impl : public NUHMSSMNoFVHimalaya_spectrum
{
public:
   virtual ~NUHMSSMNoFVHimalaya_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(NUHMSSMNoFVHimalaya_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const NUHMSSMNoFVHimalaya_observables& get_observables() const override { return observables; }

   virtual const UnitarityInfiniteS& get_unitarity() const { return unitarityData; };
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const NUHMSSMNoFVHimalaya_input_parameters&) override;
   virtual void calculate_model_observables(
      const softsusy::QedQcd&,
...
private:
   std::tuple<NUHMSSMNoFVHimalaya<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   NUHMSSMNoFVHimalaya_scales scales{};              ///< scale information
   NUHMSSMNoFVHimalaya_observables observables{};    ///< observables
...
public:
   ...
   void set_input_parameters(const NUHMSSMNoFVHimalaya_input_parameters& input_) { input = input_; }
...
private:
   NUHMSSMNoFVHimalaya_input_parameters input{};                    ///< model input parameters
   ...
   std::unique_ptr<NUHMSSMNoFVHimalaya_spectrum> spectrum{nullptr}; ///< spectrum information

   NUHMSSMNoFVHimalaya_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSNUHMSSMNoFVHimalayaCalculateSpectrum[]?");
   }
}
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(MuIN), "MuIN");
   MLPutRuleTo(link, INPUTPARAMETER(mA2IN), "mA2IN");
...
NUHMSSMNoFVHimalaya_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const NUHMSSMNoFVHimalaya_slha& model, MLINK link)
{
   MLPutRule(link, NUHMSSMNoFVHimalaya_info::model_name);
...
template <typename Solver_type>
void NUHMSSMNoFVHimalaya_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void NUHMSSMNoFVHimalaya_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const NUHMSSMNoFVHimalaya_input_parameters& input
)
{
...
template <typename Solver_type>
void NUHMSSMNoFVHimalaya_spectrum_impl<Solver_type>::calculate_model_observables(
   const softsusy::QedQcd& qedqcd,
...
template <typename Solver_type>
void NUHMSSMNoFVHimalaya_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = NUHMSSMNoFVHimalaya_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}

/******************************************************************/

template <typename Solver_type>
void NUHMSSMNoFVHimalaya_spectrum_impl<Solver_type>::fill_slha_io(NUHMSSMNoFVHimalaya_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   NUHMSSMNoFVHimalaya_observables observables = spectrum->get_observables();

   MLPutFunction(link, "List", 1);
   MLPutRule(link, NUHMSSMNoFVHimalaya_info::model_name);
   MLPutFunction(link, "List", 4);

   MLPutRuleTo(link, OBSERVABLE(amm_Fm), "Fm", {"FlexibleSUSYObservable`AMM"});
   MLPutRuleTo(link, OBSERVABLE(amm_uncertainty_Fm), "Fm", {"FlexibleSUSYObservable`AMMUncertainty"});
...
void Model_data::put_unitarity(MLINK link) const
{
   ...
   MLPutFunction(link, "List", 1);
   MLPutRule(link, "NUHMSSMNoFVHimalaya");
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSNUHMSSMNoFVHimalayaCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSNUHMSSMNoFVHimalayaCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new NUHMSSMNoFVHimalaya_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   INPUTPARAMETER(MuIN) = pars[c++];
   INPUTPARAMETER(mA2IN) = pars[c++];
...
DLLEXPORT int FSNUHMSSMNoFVHimalayaGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMNoFVHimalayaGetSettings"))
...
DLLEXPORT int FSNUHMSSMNoFVHimalayaGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMNoFVHimalayaGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUHMSSMNoFVHimalayaGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMNoFVHimalayaGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUHMSSMNoFVHimalayaOpenHandle(
...
DLLEXPORT int FSNUHMSSMNoFVHimalayaCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::NUHMSSMNoFVHimalaya_librarylink;
...
DLLEXPORT int FSNUHMSSMNoFVHimalayaSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::NUHMSSMNoFVHimalaya_librarylink;
...
DLLEXPORT int FSNUHMSSMNoFVHimalayaGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMNoFVHimalayaGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUHMSSMNoFVHimalayaToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMNoFVHimalayaToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUHMSSMNoFVHimalayaGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMNoFVHimalayaGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUHMSSMNoFVHimalayaCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMNoFVHimalayaCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUHMSSMNoFVHimalayaCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMNoFVHimalayaCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSNUHMSSMNoFVHimalayaCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSNUHMSSMNoFVHimalayaCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSNUHMSSMNoFVHimalayaCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSNUHMSSMNoFVHimalayaCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSMNoFVHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMNoFVHimalayaCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSNUHMSSMNoFVHimalayaCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
