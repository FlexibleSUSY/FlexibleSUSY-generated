...
// File generated at Wed 16 Oct 2019 19:03:05
...
public:
   ...
   virtual void fill_slha_io(NUHMSSMNoFVHimalaya_slha_io&) const = 0;
...
public:
   ...
   virtual void fill_slha_io(NUHMSSMNoFVHimalaya_slha_io&) const override;
...
private:
   std::tuple<NUHMSSMNoFVHimalaya_slha<NUHMSSMNoFVHimalaya<Solver_type>>> models{};        ///< running parameters and pole masses
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, NUHMSSMNoFVHimalaya_info::particle_names[i].c_str());
      }
   }
}
...
void put_spectrum(const NUHMSSMNoFVHimalaya_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void NUHMSSMNoFVHimalaya_spectrum_impl<Solver_type>::fill_slha_io(NUHMSSMNoFVHimalaya_slha_io& slha_io) const
{
...
void Model_data::put_observables(MLINK link) const
{
   ...
   MLPutRuleTo(link, OBSERVABLE(a_muon_uncertainty), "FlexibleSUSYObservable`aMuonUncertainty");
