...
// File generated at Fri 10 Apr 2020 17:35:22

/**
 * @file NUHMSSMNoFVHimalaya_a_muon.cpp
 *
 * This file was generated at Fri 10 Apr 2020 17:35:22 with FlexibleSUSY
...
double get_MSUSY(const NUHMSSMNoFVHimalaya_mass_eigenstates& model)
{
...
void run_to_MSUSY(NUHMSSMNoFVHimalaya_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("NUHMSSMNoFVHimalaya_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("NUHMSSMNoFVHimalaya_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("NUHMSSMNoFVHimalaya_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("NUHMSSMNoFVHimalaya_a_muon: run_to_MSUSY(): "
...
double calculate_a_muon_impl(const NUHMSSMNoFVHimalaya_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   VERBOSE_MSG("NUHMSSMNoFVHimalaya_a_muon: calculating a_mu at Q = " << model.get_scale());

   context_base context{ model };
   
   using namespace NUHMSSMNoFVHimalaya_cxx_diagrams::fields;

   const auto form_factors = NUHMSSMNoFVHimalaya_FFV_form_factors::calculate_Fm_Fm_VP_form_factors(model, true);
...
std::pair<double,double> vary_scale(const NUHMSSMNoFVHimalaya_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model,&qedqcd] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("NUHMSSMNoFVHimalaya_a_muon: scale variation: " << e.what_detailed());
                     }
                     return amu;
                  });
...
double NUHMSSMNoFVHimalaya_a_muon::calculate_a_muon(const NUHMSSMNoFVHimalaya_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   NUHMSSMNoFVHimalaya_mass_eigenstates model(model_);

   VERBOSE_MSG("NUHMSSMNoFVHimalaya_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUHMSSMNoFVHimalaya_a_muon:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
double NUHMSSMNoFVHimalaya_a_muon::calculate_a_muon_uncertainty(const NUHMSSMNoFVHimalaya_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   NUHMSSMNoFVHimalaya_mass_eigenstates model(model_);

   VERBOSE_MSG("NUHMSSMNoFVHimalaya_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUHMSSMNoFVHimalaya_a_muon uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
