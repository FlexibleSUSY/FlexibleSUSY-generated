...
TMSSM_susy_scale_constraint<Two_scale>::TMSSM_susy_scale_constraint(
...
void TMSSM_susy_scale_constraint<Two_scale>::apply()
{
   check_model_ptr();
...
double TMSSM_susy_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}
...
void TMSSM_susy_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<TMSSM<Two_scale>*>(model_);
}

void TMSSM_susy_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& TMSSM_susy_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void TMSSM_susy_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
...
void TMSSM_susy_scale_constraint<Two_scale>::initialize()
{
   check_model_ptr();
...
void TMSSM_susy_scale_constraint<Two_scale>::update_scale()
{
   check_model_ptr();
...
void TMSSM_susy_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("TMSSM_susy_scale_constraint<Two_scale>: "
