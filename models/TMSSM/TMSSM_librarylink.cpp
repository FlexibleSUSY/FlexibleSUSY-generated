...
template <typename Solver_type>
void TMSSM_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = TMSSM_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSTMSSMCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::TMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSTMSSMCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSTMSSMCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
