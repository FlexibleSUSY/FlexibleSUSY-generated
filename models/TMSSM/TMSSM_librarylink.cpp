...
// File generated at Sun 26 Aug 2018 14:16:45
...
public:
   ...
   virtual void fill_slha_io(TMSSM_slha_io&) const = 0;
...
public:
   ...
   virtual void fill_slha_io(TMSSM_slha_io&) const override;
...
private:
   std::tuple<TMSSM_slha<TMSSM<Solver_type>>> models{};        ///< running parameters and pole masses
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, TMSSM_info::particle_names[i].c_str());
      }
   }
}
...
void put_spectrum(const TMSSM_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void TMSSM_spectrum_impl<Solver_type>::fill_slha_io(TMSSM_slha_io& slha_io) const
{
