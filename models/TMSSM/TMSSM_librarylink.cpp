...
// File generated at Thu 15 Dec 2016 12:47:41
...
struct TMSSM_data {
   ...
   TMSSM_slha<algorithm_type> model; ///< running parameters and pole masses
};

/// current handles
typedef std::map<Handle_id, TMSSM_data> Handle_map;
Handle_map handles_TMSSM;

/******************************************************************/

Handle_id get_new_TMSSM_handle()
{
   static const std::size_t max_handles =
      static_cast<std::size_t>(std::exp2(8*sizeof(Handle_id)) - 1);

   if (handles_TMSSM.size() >= max_handles)
      throw ENotEnoughFreeHandles(handles_TMSSM.size());

   Handle_id hid = 0;

   while (handles_TMSSM.find(hid) != handles_TMSSM.end())
      hid++;
...
TMSSM_data find_TMSSM_data(Handle_id hid)
{
   const Handle_map::iterator handle = handles_TMSSM.find(hid);

   if (handle == handles_TMSSM.end())
...
void put_settings(const TMSSM_data& data, MLINK link)
{
...
void put_sm_input_parameters(const TMSSM_data& data, MLINK link)
{
...
void put_input_parameters(const TMSSM_data& data, MLINK link)
{
...
void put_spectrum(const TMSSM_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const TMSSM_observables& observables, MLINK link)
{
...
void check_spectrum(const TMSSM_data& data, MLINK link)
{
   ...
   if (problems.have_problem()) {
      ...
      put_message(link, "FSTMSSMCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSTMSSMCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(TMSSM_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSTMSSMCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = TMSSM_slha<algorithm_type>(
...
TMSSM_data make_TMSSM_data(double* pars, mint npars)
{
   TMSSM_data data;
...
DLLEXPORT int FSTMSSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      const TMSSM_data data = find_TMSSM_data(hid);
   ...
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument Res)
{
   ...
   try {
      TMSSM_data data = make_TMSSM_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);

      const Handle_id hid = get_new_TMSSM_handle();

      handles_TMSSM.insert(std::make_pair(hid, std::move(data)));
   ...
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   ...
   if (handle == handles_TMSSM.end()) {
      std::cerr << "Error: FSTMSSMSet: Unknown handle: "
                << hid << std::endl;
      return LIBRARY_FUNCTION_ERROR;
   }

   try {
      handle->second = make_TMSSM_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);
   } catch (const flexiblesusy::Error& e) {
      std::cerr << e.what() << std::endl;
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      TMSSM_data data = find_TMSSM_data(hid);
      ...
      handles_TMSSM[hid] = std::move(data);
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSTMSSMCalculateObservables", "error", e.what());
      put_error_output(link);
   }
