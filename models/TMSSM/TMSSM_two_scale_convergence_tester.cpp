...
// File generated at Sun 18 Oct 2015 11:31:44
...
TMSSM_convergence_tester<Two_scale>::TMSSM_convergence_tester(TMSSM<Two_scale>* model, double accuracy_goal)
   : Convergence_tester_DRbar<TMSSM<Two_scale> >(model, accuracy_goal)
{
}

TMSSM_convergence_tester<Two_scale>::~TMSSM_convergence_tester()
{
}

double TMSSM_convergence_tester<Two_scale>::max_rel_diff() const
{
   const TMSSM<Two_scale>& ol = get_last_iteration_model();
   const TMSSM<Two_scale>& ne = get_model();
   ...
   for (unsigned i = 0; i < 5; i++) {
      diff[i + 32] = MaxRelDiff(OLD1(MChi,i),NEW1(MChi,i));
   }
   for (unsigned i = 0; i < 3; i++) {
      diff[i + 37] = MaxRelDiff(OLD1(MCha,i),NEW1(MCha,i));
   }

   return *std::max_element(diff, diff + 40);
