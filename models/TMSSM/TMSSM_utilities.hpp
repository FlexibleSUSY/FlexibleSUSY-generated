...
public:
   Eigen::ArrayXd get_parameters(const TMSSM<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const TMSSM<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const TMSSM<T>&);
...
template <class T>
void TMSSM_spectrum_plotter::extract_spectrum(const TMSSM<T>& model)
{
   spectrum.clear();
