...
// File generated at Fri 8 Jan 2016 15:10:23

#ifndef TMSSM_SPECTRUM_GENERATOR_H
#define TMSSM_SPECTRUM_GENERATOR_H

#include "TMSSM_spectrum_generator_interface.hpp"
...
#include "TMSSM_two_scale_convergence_tester.hpp"
...
template <class T>
class TMSSM_spectrum_generator
   : public TMSSM_spectrum_generator_interface<T> {
public:
   TMSSM_spectrum_generator()
      : TMSSM_spectrum_generator_interface<T>()
      , solver()
      ...
      , low_scale(0.)
   {}
   virtual ~TMSSM_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const TMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename = "TMSSM_rgflow.dat") const;

private:
   RGFlow<T> solver;
   TMSSM_high_scale_constraint<T> high_scale_constraint;
   TMSSM_susy_scale_constraint<T> susy_scale_constraint;
   TMSSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void TMSSM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const TMSSM_input_parameters& input)
{
   TMSSM<T>& model = this->model;
   model.clear();
   ...
   TMSSM_convergence_tester<T> convergence_tester(&model, this->precision_goal);
   if (this->max_iterations > 0)
      convergence_tester.set_max_iterations(this->max_iterations);

   TMSSM_initial_guesser<T> initial_guesser(&model, qedqcd,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
   ...
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : TMSSM_info::parameter_names[parameter_index];
 ...
 */
template <class T>
void TMSSM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   TMSSM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
