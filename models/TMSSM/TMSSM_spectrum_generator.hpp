 ...
 */
template <class T>
void TMSSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const TMSSM_input_parameters& input)
{
   ...
   TMSSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   TMSSM_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void TMSSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   TMSSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   TMSSM_parameter_getter parameter_getter;
   Coupling_monitor<TMSSM<T>, TMSSM_parameter_getter>
 ...
 */
template <class T>
void TMSSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   TMSSM_spectrum_plotter plotter;
