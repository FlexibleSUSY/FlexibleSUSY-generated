...
// File generated at Sun 26 Aug 2018 14:15:52
...
TMSSM_initial_guesser<Two_scale>::TMSSM_initial_guesser(
   TMSSM<Two_scale>* model_,
   const softsusy::QedQcd& qedqcd_,
   const TMSSM_low_scale_constraint<Two_scale>& low_constraint_,
   const TMSSM_susy_scale_constraint<Two_scale>& susy_constraint_,
   const TMSSM_high_scale_constraint<Two_scale>& high_constraint_
)
   : model(model_)
   , qedqcd(qedqcd_)
...
{
   if (!model)
      throw SetupError("TMSSM_initial_guesser: Error: pointer to model"
                       " TMSSM<Two_scale> must not be zero");
}
 ...
 */
void TMSSM_initial_guesser<Two_scale>::guess()
{
 ...
 * \endcode
 */
void TMSSM_initial_guesser<Two_scale>::guess_susy_parameters()
{
...
void TMSSM_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void TMSSM_initial_guesser<Two_scale>::calculate_running_SM_masses()
{
 ...
 * corrections).
 */
void TMSSM_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
 ...
 */
void TMSSM_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
 ...
 * corrections).
 */
void TMSSM_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
   const auto vd = MODELPARAMETER(vd);
 ...
 */
void TMSSM_initial_guesser<Two_scale>::guess_soft_parameters()
{
