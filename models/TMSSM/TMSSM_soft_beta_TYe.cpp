...
// File generated at Tue 10 Oct 2017 21:34:54
...
Eigen::Matrix<double,3,3> TMSSM_soft_parameters::calc_beta_TYe_2_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_TYe = (twoLoop*(0.05*(-2*(4*Ye*(135*MassB*Quad(g1) + Sqr(g1)*(2*(
      ...
      *traceAdjYeTYe + 4*MassWB*Sqr(g2) + 3*Conj(Lambdax)*TLambdax)*(Ye*
      Ye.adjoint()*Ye) + (-12*traceYdAdjYd - 4*traceYeAdjYe - 6*AbsSqr(Lambdax)
      + 1.2*Sqr(g1) + 6*Sqr(g2))*(Ye*Ye.adjoint()*TYe) + (-15*traceYdAdjYd - 5
      *traceYeAdjYe - 7.5*AbsSqr(Lambdax) - 1.2*Sqr(g1) + 12*Sqr(g2))*(TYe*
