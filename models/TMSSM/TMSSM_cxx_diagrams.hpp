...
// File generated at Tue 10 Oct 2017 21:54:07

/**
 * @file TMSSM_cxx_diagrams.hpp
 *
 * This file was generated at Tue 10 Oct 2017 21:54:07 with FlexibleSUSY
 * 2.0.0 and SARAH 4.12.0 .
 */

#ifndef TMSSM_CXXDIAGRAMS_H
#define TMSSM_CXXDIAGRAMS_H
...
*/
struct EvaluationContext {
   TMSSM_mass_eigenstates& model; ///< The model object.
...
Vertex<VP, bar<Cha>::type, Cha>::evaluate(const indices_type& indices, const EvaluationContext& context)
{
   ...
   const std::complex<double> left = 0.5*(2*g2*Conj(UM(gt3,0))*Sin(ThetaW)*UM(gt2,0) + Conj(UM(gt3,1))*(0.7745966692414834*g1*Cos(ThetaW) + g2*Sin(ThetaW))*UM(gt2,1) + 2*g2*Conj(UM(gt3,2))*Sin(ThetaW)*UM(gt2,2));

   const std::complex<double> right = 0.5*(2*g2*Conj(UP(gt2,0))*Sin(ThetaW)*UP(gt3,0) + Conj(UP(gt2,1))*(0.7745966692414834*g1*Cos(ThetaW) + g2*Sin(ThetaW))*UP(gt3,1) + 2*g2*Conj(UP(gt2,2))*Sin(ThetaW)*UP(gt3,2));
