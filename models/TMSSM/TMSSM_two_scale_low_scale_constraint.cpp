...
// File generated at Sun 26 Aug 2018 14:15:51

#include "TMSSM_two_scale_low_scale_constraint.hpp"
...
TMSSM_low_scale_constraint<Two_scale>::TMSSM_low_scale_constraint(
   TMSSM<Two_scale>* model_, const softsusy::QedQcd& qedqcd_)
   : model(model_)
   , qedqcd(qedqcd_)
{
   initialize();
...
void TMSSM_low_scale_constraint<Two_scale>::apply()
{
   check_model_ptr();
...
const Eigen::Matrix<std::complex<double>,3,3>& TMSSM_low_scale_constraint<Two_scale>::get_ckm()
{
   return ckm;
}

const Eigen::Matrix<std::complex<double>,3,3>& TMSSM_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}

double TMSSM_low_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double TMSSM_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void TMSSM_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<TMSSM<Two_scale>*>(model_);
}

void TMSSM_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& TMSSM_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void TMSSM_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
...
void TMSSM_low_scale_constraint<Two_scale>::initialize()
{
   check_model_ptr();
...
void TMSSM_low_scale_constraint<Two_scale>::update_scale()
{
   check_model_ptr();
...
void TMSSM_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
   check_model_ptr();
...
double TMSSM_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();

   double theta_w = std::asin(Electroweak_constants::sinThetaW);

   TMSSM_weinberg_angle::Sm_parameters sm_pars;
   ...
   TMSSM_weinberg_angle weinberg(MODEL, sm_pars);
...
void TMSSM_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   check_model_ptr();
...
double TMSSM_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   check_model_ptr();
...
double TMSSM_low_scale_constraint<Two_scale>::calculate_delta_alpha_s(double alphaS) const
{
   check_model_ptr();
...
double TMSSM_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
void TMSSM_low_scale_constraint<Two_scale>::calculate_running_SM_masses()
{
   check_model_ptr();
...
void TMSSM_low_scale_constraint<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void TMSSM_low_scale_constraint<Two_scale>::calculate_Yu_DRbar()
{
   check_model_ptr();
...
void TMSSM_low_scale_constraint<Two_scale>::calculate_Yd_DRbar()
{
   check_model_ptr();
...
void TMSSM_low_scale_constraint<Two_scale>::calculate_Ye_DRbar()
{
   check_model_ptr();
...
void TMSSM_low_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("TMSSM_low_scale_constraint<Two_scale>: "
