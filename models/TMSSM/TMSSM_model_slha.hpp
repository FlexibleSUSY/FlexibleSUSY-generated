...
class TMSSM_slha : public Model {
public:
   explicit TMSSM_slha(const TMSSM_input_parameters& input_ = TMSSM_input_parameters());
   explicit TMSSM_slha(const Model&, bool do_convert_masses_to_slha = true);
   TMSSM_slha(const TMSSM_slha&) = default;
   TMSSM_slha(TMSSM_slha&&) = default;
   virtual ~TMSSM_slha() = default;
   TMSSM_slha& operator=(const TMSSM_slha&) = default;
   TMSSM_slha& operator=(TMSSM_slha&&) = default;
   ...
   const TMSSM_physical& get_physical_slha() const; ///< returns pole masses to SLHA convention
   TMSSM_physical& get_physical_slha(); ///< returns pole masses to SLHA convention
   ...
   const Eigen::Array<double,3,1>& get_MCha_pole_slha() const { return PHYSICAL_SLHA(MCha); }
   ...
   const Eigen::Matrix<std::complex<double>,3,3>& get_UM_pole_slha() const { return PHYSICAL_SLHA(UM); }
   std::complex<double> get_UM_pole_slha(int i, int k) const { return PHYSICAL_SLHA(UM(i,k)); }
   const Eigen::Matrix<std::complex<double>,3,3>& get_UP_pole_slha() const { return PHYSICAL_SLHA(UP); }
   std::complex<double> get_UP_pole_slha(int i, int k) const { return PHYSICAL_SLHA(UP(i,k)); }
...
private:
   TMSSM_physical physical_slha{}; ///< contains the pole masses and mixings in slha convention
...
template <class Model>
TMSSM_slha<Model>::TMSSM_slha(const TMSSM_input_parameters& input_)
   : Model(input_)
{
}
 ...
 */
template <class Model>
TMSSM_slha<Model>::TMSSM_slha(const Model& model_,
...
template <class Model>
void TMSSM_slha<Model>::calculate_spectrum()
{
...
template <class Model>
void TMSSM_slha<Model>::convert_to_slha()
{
...
template <class Model>
void TMSSM_slha<Model>::calculate_ckm_matrix()
{
...
template <class Model>
void TMSSM_slha<Model>::calculate_pmns_matrix()
{
 ...
 */
template <class Model>
void TMSSM_slha<Model>::convert_yukawa_couplings_to_slha()
{
 ...
 */
template <class Model>
void TMSSM_slha<Model>::convert_trilinear_couplings_to_slha()
{
 ...
 */
template <class Model>
void TMSSM_slha<Model>::convert_soft_squared_masses_to_slha()
{
...
template <class Model>
const TMSSM_physical& TMSSM_slha<Model>::get_physical_slha() const
{
   return physical_slha;
}

template <class Model>
TMSSM_physical& TMSSM_slha<Model>::get_physical_slha()
{
   return physical_slha;
}

template <class Model>
void TMSSM_slha<Model>::print(std::ostream& ostr) const
{
   Model::print(ostr);
...
template <class Model>
void TMSSM_slha<Model>::set_convert_masses_to_slha(bool flag)
{
   convert_masses_to_slha = flag;
}

template <class Model>
std::ostream& operator<<(std::ostream& ostr, const TMSSM_slha<Model>& model)
{
   model.print(ostr);
   return ostr;
}
