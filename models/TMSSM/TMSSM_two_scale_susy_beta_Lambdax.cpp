...
double TMSSM_susy_parameters::calc_beta_Lambdax_one_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_Lambdax = oneOver16PiSqr*(3*traceYdAdjYd*Lambdax + traceYeAdjYe*
      Lambdax + 3*traceYuAdjYu*Lambdax - 0.6*Lambdax*Sqr(g1) - 7*Lambdax*Sqr(g2
      ) + 4*Conj(Lambdax)*Sqr(Lambdax));
...
double TMSSM_susy_parameters::calc_beta_Lambdax_two_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_Lambdax = -0.02*twoLoop*Lambdax*(-207*Power(g1,4) - 2075*Power(g2
