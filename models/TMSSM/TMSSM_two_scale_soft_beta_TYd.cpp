...
// File generated at Thu 15 Dec 2016 12:44:58
...
Eigen::Matrix<double,3,3> TMSSM_soft_parameters::calc_beta_TYd_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_TYd = (twoLoop*(-0.044444444444444446*Yd*(287*Power(g1,4)*MassB +
      5*(-32*Power(g3,4)*MassG + 243*Power(g2,4)*MassWB + 27*(6*
      ...
      Power(g1,4) + 13.5*Power(g2,4) - 1.7777777777777777*Power(g3,4) - 9*
      ...
      *Sqr(g3)) + 16*traceYdAdjYd*Sqr(g3) + 8*Sqr(g2)*Sqr(g3))*TYd - 3.75*
      Lambdax*Sqr(Conj(Lambdax))*(Lambdax*TYd + 4*Yd*TLambdax) - 1.5*Conj(
      Lambdax)*((3*traceYuAdjYu*Lambdax - 4*Lambdax*Sqr(g2))*TYd + 2*Yd*(3*
      traceAdjYuTYu*Lambdax + 4*MassWB*Lambdax*Sqr(g2) + 3*traceYuAdjYu*
      TLambdax - 4*Sqr(g2)*TLambdax)) + (-0.4*(4*MassB*Sqr(g1) + 15*(3*
      traceAdjYdTYd + traceAdjYeTYe + 2*MassWB*Sqr(g2))) - 9*Conj(Lambdax)*
      TLambdax)*(Yd*Yd.adjoint()*Yd) + (-12*traceYdAdjYd - 4*traceYeAdjYe - 6*
      AbsSqr(Lambdax) + 1.2*Sqr(g1) + 6*Sqr(g2))*(Yd*Yd.adjoint()*TYd) + (-6*
      traceAdjYuTYu - 1.6*MassB*Sqr(g1) - 3*Conj(Lambdax)*TLambdax)*(Yd*
      Yu.adjoint()*Yu) + (-6*traceYuAdjYu - 3*AbsSqr(Lambdax) + 1.6*Sqr(g1))*(
      Yd*Yu.adjoint()*TYu) + (-15*traceYdAdjYd - 5*traceYeAdjYe - 7.5*AbsSqr(
      Lambdax) + 1.2*Sqr(g1) + 12*Sqr(g2))*(TYd*Yd.adjoint()*Yd) + (-3*
      traceYuAdjYu - 1.5*AbsSqr(Lambdax) + 0.8*Sqr(g1))*(TYd*Yu.adjoint()*Yu) -
 ...
 */
Eigen::Matrix<double,3,3> TMSSM_soft_parameters::calc_beta_TYd_three_loop(const Soft_traces& soft_traces) const
{
