...
#ifndef TMSSM_TWO_SCALE_INITIAL_GUESSER_H
#define TMSSM_TWO_SCALE_INITIAL_GUESSER_H

#include "TMSSM_initial_guesser.hpp"
...
/**
 * @class TMSSM_initial_guesser<Two_scale>
 * @brief initial guesser for the TMSSM
 */

template<>
class TMSSM_initial_guesser<Two_scale> : public Initial_guesser<Two_scale> {
public:
   TMSSM_initial_guesser(TMSSM<Two_scale>*,
                               const TMSSM_input_parameters&,
                               const QedQcd&,
                               const TMSSM_low_scale_constraint<Two_scale>&,
                               const TMSSM_susy_scale_constraint<Two_scale>&,
                               const TMSSM_high_scale_constraint<Two_scale>&);
   virtual ~TMSSM_initial_guesser();
   virtual void guess(); ///< initial guess

   void set_running_precision(double p) { running_precision = p; }

private:
   TMSSM<Two_scale>* model; ///< pointer to model class
   TMSSM_input_parameters input_pars;
   ...
   TMSSM_low_scale_constraint<Two_scale> low_constraint;
   TMSSM_susy_scale_constraint<Two_scale> susy_constraint;
   TMSSM_high_scale_constraint<Two_scale> high_constraint;
