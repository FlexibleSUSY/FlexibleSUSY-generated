...
/**
 * @file TMSSM_two_scale_model_slha.hpp
 * @brief contains wrapper class for model class in SLHA convention
 */

// File generated at Mon 27 Feb 2017 13:28:15

#ifndef TMSSM_TWO_SCALE_SLHA_H
#define TMSSM_TWO_SCALE_SLHA_H
...
/**
 * @class TMSSM_slha<Two_scale>
 * @brief model class wrapper in SLHA convention
 */

template<>
class TMSSM_slha<Two_scale> : public TMSSM<Two_scale> {
public:
   explicit TMSSM_slha(const TMSSM_input_parameters& input_ = TMSSM_input_parameters());
   explicit TMSSM_slha(const TMSSM<Two_scale>&, bool do_convert_masses_to_slha = true);
   virtual ~TMSSM_slha();
...
std::ostream& operator<<(std::ostream& ostr, const TMSSM_slha<Two_scale>& model);
