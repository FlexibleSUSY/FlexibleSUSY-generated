...
Vertex<fields::VP, typename bar<fields::Cha>::type, fields::Cha>::evaluate(const indices_type& indices, const context_base& context)
{
   ...
   const std::complex<double> left = 0.5*(2*g2*Conj(UM(gt2,0))*Sin(ThetaW)*UM(gt1,0) + Conj(UM(gt2,1))*(0.7745966692414834*g1*Cos(ThetaW) + g2*Sin(ThetaW))*UM(gt1,1) + 2*g2*Conj(UM(gt2,2))*Sin(ThetaW)*UM(gt1,2));

   const std::complex<double> right = 0.5*(2*g2*Conj(UP(gt1,0))*Sin(ThetaW)*UP(gt2,0) + Conj(UP(gt1,1))*(0.7745966692414834*g1*Cos(ThetaW) + g2*Sin(ThetaW))*UP(gt2,1) + 2*g2*Conj(UP(gt1,2))*Sin(ThetaW)*UP(gt2,2));
