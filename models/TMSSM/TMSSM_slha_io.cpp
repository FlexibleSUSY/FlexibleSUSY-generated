...
char const * const TMSSM_slha_io::drbar_blocks[NUMBER_OF_DRBAR_BLOCKS] =
 ...
 */
void TMSSM_slha_io::set_spinfo(const Problems<TMSSM_info::NUMBER_OF_PARTICLES>& problems)
{
 ...
 */
void TMSSM_slha_io::write_to_file(const std::string& file_name)
{
...
void TMSSM_slha_io::fill(TMSSM_input_parameters& input) const
{
   SLHA_io::Tuple_processor minpar_processor
      = boost::bind(&TMSSM_slha_io::fill_minpar_tuple, boost::ref(input), _1, _2);
   SLHA_io::Tuple_processor extpar_processor
      = boost::bind(&TMSSM_slha_io::fill_extpar_tuple, boost::ref(input), _1, _2);
...
void TMSSM_slha_io::fill(Spectrum_generator_settings& settings) const
{
   SLHA_io::Tuple_processor flexiblesusy_processor
      = boost::bind(&TMSSM_slha_io::fill_flexiblesusy_tuple, boost::ref(settings), _1, _2);
...
void TMSSM_slha_io::fill_flexiblesusy_tuple(Spectrum_generator_settings& settings,
                                                  int key, double value)
{
 ...
 */
void TMSSM_slha_io::convert_to_slha_convention(TMSSM_physical& physical)
{
