...
// File generated at Tue 10 Oct 2017 21:34:50
...
const int TMSSM_soft_parameters::numberOfParameters;

TMSSM_soft_parameters::TMSSM_soft_parameters(const TMSSM_input_parameters& input_)
   : TMSSM_susy_parameters(input_)
{
   set_number_of_parameters(numberOfParameters);
}

TMSSM_soft_parameters::TMSSM_soft_parameters(
   const TMSSM_susy_parameters& susy_model
   , const Eigen::Matrix<double,3,3>& TYd_, const Eigen::Matrix<double,3,3>&
   TYe_, double TLambdax_, const Eigen::Matrix<double,3,3>& TYu_, double BMu_,
   double BMT_, const Eigen::Matrix<double,3,3>& mq2_, const Eigen::Matrix<
   ...
   3,3>& me2_, double mT2_, double MassB_, double MassWB_, double MassG_

)
   : TMSSM_susy_parameters(susy_model)
   , TYd(TYd_), TYe(TYe_), TLambdax(TLambdax_), TYu(TYu_), BMu(BMu_), BMT(BMT_)
   , mq2(mq2_), ml2(ml2_), mHd2(mHd2_), mHu2(mHu2_), md2(md2_), mu2(mu2_), me2(
   me2_), mT2(mT2_), MassB(MassB_), MassWB(MassWB_), MassG(MassG_)

{
   set_number_of_parameters(numberOfParameters);
}

Eigen::ArrayXd TMSSM_soft_parameters::beta() const
{
   return calc_beta().get().unaryExpr(Chop<double>(get_zero_threshold()));
}

TMSSM_soft_parameters TMSSM_soft_parameters::calc_beta(int loops) const
{
   ...
   if (loops > 0) {
      ...
      beta_BMT += calc_beta_BMT_1_loop(TRACE_STRUCT);
      ...
      if (loops > 1) {
         ...
         beta_BMT += calc_beta_BMT_2_loop(TRACE_STRUCT);
   ...
   const TMSSM_susy_parameters susy_betas(TMSSM_susy_parameters::calc_beta(loops));
...
TMSSM_soft_parameters TMSSM_soft_parameters::calc_beta() const
{
...
void TMSSM_soft_parameters::print(std::ostream& ostr) const
{
   TMSSM_susy_parameters::print(ostr);
...
void TMSSM_soft_parameters::set(const Eigen::ArrayXd& pars)
{
   TMSSM_susy_parameters::set(pars);
...
TMSSM_soft_parameters::Soft_traces TMSSM_soft_parameters::calc_soft_traces(int loops) const
{
   ...
   if (loops > 0) {
      ...
      TRACE_STRUCT.Tr31 = Re(0.012909944487358056*g1*(45*(mHd2 - mHu2)*AbsSqr(
      ...
      TRACE_STRUCT.Tr22 = Re(0.5*(mHd2 + mHu2 + 4*mT2 + (ml2).trace() + 3*(mq2)
         .trace()));
...
std::ostream& operator<<(std::ostream& ostr, const TMSSM_soft_parameters& soft_pars)
{
