...
// File generated at Sun 26 Aug 2018 14:15:54
...
double TMSSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void TMSSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const TMSSM_input_parameters& input)
{
   ...
   TMSSM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<TMSSM_ewsb_solver<Two_scale> >(ewsb_solver));

   TMSSM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   TMSSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   TMSSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   TMSSM_convergence_tester<Two_scale> convergence_tester(
   ...
   TMSSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void TMSSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   TMSSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void TMSSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
