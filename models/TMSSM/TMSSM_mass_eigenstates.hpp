...
#define TMSSM_MASS_EIGENSTATES_H
 ...
 */
class TMSSM_mass_eigenstates : public TMSSM_soft_parameters {
public:
   explicit TMSSM_mass_eigenstates(const TMSSM_input_parameters& input_ = TMSSM_input_parameters());
   virtual ~TMSSM_mass_eigenstates();
   ...
   void set_physical(const TMSSM_physical&);
   double get_ewsb_iteration_precision() const;
   double get_ewsb_loop_order() const;
   const TMSSM_physical& get_physical() const;
   TMSSM_physical& get_physical();
   const Problems<TMSSM_info::NUMBER_OF_PARTICLES>& get_problems() const;
   Problems<TMSSM_info::NUMBER_OF_PARTICLES>& get_problems();
   ...
   double get_lsp(TMSSM_info::Particles&) const;
   ...
   const Eigen::Array<double,3,1>& get_MCha() const { return MCha; }
   ...
   const Eigen::Matrix<std::complex<double>,3,3>& get_UM() const { return UM; }
   const std::complex<double>& get_UM(int i, int k) const { return UM(i,k); }
   const Eigen::Matrix<std::complex<double>,3,3>& get_UP() const { return UP; }
   ...
   void calculate_MChi();
   Eigen::Matrix<double,3,3> get_mass_matrix_Cha() const;
   void calculate_MCha();
   ...
   std::complex<double> CpVPconjHpmHpm(unsigned gI1, unsigned gI2) const;
...
private:
   ...
   Eigen::Matrix<std::complex<double>,3,3> UM;
   Eigen::Matrix<std::complex<double>,3,3> UP;
...
std::ostream& operator<<(std::ostream&, const TMSSM_mass_eigenstates&);
