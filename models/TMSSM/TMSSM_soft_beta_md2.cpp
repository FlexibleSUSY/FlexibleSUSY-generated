...
// File generated at Fri 20 Oct 2017 08:42:04
...
Eigen::Matrix<double,3,3> TMSSM_soft_parameters::calc_beta_md2_2_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_md2 = (twoLoop*((-12*traceconjTYdTpTYd - 4*traceconjTYeTpTYe - 12
      ...
      *Sqr(g1) + 6*Sqr(g2))*(md2*Yd*Yd.adjoint()) + (-12*traceYdAdjYd - 4*
      ...
      MassB + MassG)*Sqr(g3)) + 5*(3*(g1*(g1*Tr2U111 + 3.872983346207417*Tr31)
      + 20*Tr23*Quad(g3)) + 8*Conj(MassG)*Sqr(g3)*((MassB + 2*MassG)*Sqr(g1) -
      30*MassG*Sqr(g3))))*UNITMATRIX(3))).real();
