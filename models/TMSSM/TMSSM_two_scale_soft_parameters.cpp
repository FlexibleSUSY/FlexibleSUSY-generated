...
// File generated at Wed 12 Apr 2017 11:36:19
...
TMSSM_soft_parameters::TMSSM_soft_parameters(const TMSSM_input_parameters& input_)
   ...
   ::Zero()), TLambdax(0), TYu(Eigen::Matrix<double,3,3>::Zero()), BMu(0), BMT(
   ...
   mT2(0), MassB(0), MassWB(0), MassG(0)
...
TMSSM_soft_parameters TMSSM_soft_parameters::calc_beta() const
{
   ...
   if (get_loops() > 0) {
      ...
      beta_BMT += calc_beta_BMT_one_loop(TRACE_STRUCT);
      ...
      if (get_loops() > 1) {
         ...
         beta_BMT += calc_beta_BMT_two_loop(TRACE_STRUCT);
...
void TMSSM_soft_parameters::calc_soft_traces(Soft_traces& soft_traces) const
{
   if (get_loops() > 0) {
