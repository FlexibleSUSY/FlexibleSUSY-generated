...
// TMSSM_parameter_getter ////////////////////////////////////////

std::vector<std::string> TMSSM_parameter_getter::get_mass_names(const std::string& head)
{
...
std::array<std::string, TMSSM_info::NUMBER_OF_MIXINGS> TMSSM_parameter_getter::get_mixing_names()
{
   return TMSSM_info::particle_mixing_names;
}

Eigen::ArrayXd TMSSM_parameter_getter::get_parameters(const TMSSM_mass_eigenstates& model)
{
   return model.get();
}

std::array<std::string, TMSSM_info::NUMBER_OF_PARAMETERS> TMSSM_parameter_getter::get_parameter_names()
{
   return TMSSM_info::parameter_names;
}

std::array<std::string, TMSSM_info::NUMBER_OF_PARTICLES> TMSSM_parameter_getter::get_particle_names()
{
   return TMSSM_info::particle_names;
}

std::vector<std::string> TMSSM_parameter_getter::get_DRbar_mass_names()
{
   return get_mass_names();
}

std::vector<std::string> TMSSM_parameter_getter::get_pole_mass_names()
{
   return get_mass_names("Pole");
}

std::array<std::string, TMSSM_info::NUMBER_OF_MIXINGS> TMSSM_parameter_getter::get_DRbar_mixing_names()
{
   return get_mixing_names();
}

std::array<std::string, TMSSM_info::NUMBER_OF_MIXINGS> TMSSM_parameter_getter::get_pole_mixing_names()
{
...
std::array<std::string, TMSSM_info::NUMBER_OF_INPUT_PARAMETERS> TMSSM_parameter_getter::get_input_parameter_names()
{
   return TMSSM_info::input_parameter_names;
}

std::array<std::string, TMSSM_info::NUMBER_OF_EXTRA_PARAMETERS> TMSSM_parameter_getter::get_extra_parameter_names()
{
   return TMSSM_info::extra_parameter_names;
}

decltype(TMSSM_info::NUMBER_OF_MASSES) TMSSM_parameter_getter::get_number_of_masses()
{
   return TMSSM_info::NUMBER_OF_MASSES;
}

// TMSSM_spectrum_plotter ////////////////////////////////////////

TMSSM_spectrum_plotter::TMSSM_spectrum_plotter(const TMSSM_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void TMSSM_spectrum_plotter::extract_spectrum(const TMSSM_mass_eigenstates& model)
{
   spectrum.clear();
...
void TMSSM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;
...
{
   if (!filestr.good()) {
      ERROR("TMSSM_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const TMSSM_mass_eigenstates& model,
   ...
   const TMSSM_observables* observables)
{
   ...
   append(names, TMSSM_parameter_getter::get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, TMSSM_parameter_getter::get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, TMSSM_parameter_getter::get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, TMSSM_parameter_getter::get_DRbar_mass_names());
   append(names, TMSSM_parameter_getter::get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, TMSSM_parameter_getter::get_pole_mass_names());
   append(names, TMSSM_parameter_getter::get_pole_mixing_names());
   ...
   if (TMSSM_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, TMSSM_observables::get_names());
 ...
 */
TMSSM_mass_eigenstates from_database(
   const std::string& file_name, long long entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, TMSSM_observables* observables)
{
   TMSSM_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = TMSSM_parameter_getter::get_number_of_masses();
   const auto number_of_mixings = TMSSM_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = TMSSM_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = TMSSM_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? TMSSM_observables::NUMBER_OF_OBSERVABLES : 0);
