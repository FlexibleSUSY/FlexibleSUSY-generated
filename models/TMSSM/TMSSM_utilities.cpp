...
TMSSM_spectrum_plotter::TMSSM_spectrum_plotter()
   : spectrum()
   , scale(0.0)
   , width(16)
{
...
std::valarray<double> TMSSM_spectrum_plotter::to_valarray(double v)
{
   ...
   const TMSSM_observables* observables)
{
   ...
   append(names, TMSSM_parameter_getter().get_mass_names());
   append(names, TMSSM_parameter_getter().get_mixing_names());
   ...
   Physical_input* physical_input, TMSSM_observables* observables)
{
   ...
   const std::size_t number_of_masses = TMSSM_parameter_getter().get_number_of_masses();
   const std::size_t number_of_mixings = TMSSM_info::NUMBER_OF_MIXINGS;
   const std::size_t number_of_input_parameters = TMSSM_info::NUMBER_OF_INPUT_PARAMETERS;
   ...
   const std::size_t number_of_observables =
      (observables ? TMSSM_observables::NUMBER_OF_OBSERVABLES : 0u);
