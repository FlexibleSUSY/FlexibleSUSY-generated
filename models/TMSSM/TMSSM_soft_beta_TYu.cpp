...
// File generated at Fri 20 Oct 2017 08:41:48
...
Eigen::Matrix<double,3,3> TMSSM_soft_parameters::calc_beta_TYu_2_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_TYu = (twoLoop*(-0.008888888888888889*Yu*(2743*MassB*Quad(g1) +
      ...
      traceYuAdjYuTYuAdjYu) + 243*MassWB*Quad(g2) - 32*MassG*Quad(g3) - 144*(
      ...
      *Sqr(g3)) + 5*Sqr(g1)*(45*(MassB + MassWB)*Sqr(g2) + 4*(-9*traceAdjYuTYu
      ...
      + 13.5*Quad(g2) - 1.7777777777777777*Quad(g3) + 16*traceYuAdjYu*Sqr(g3) +
      8*Sqr(g2)*Sqr(g3) + Sqr(g1)*(Sqr(g2) + 0.08888888888888889*(9*
      traceYuAdjYu + 34*Sqr(g3))))*TYu - 3.75*Lambdax*Sqr(Conj(Lambdax))*(
      Lambdax*TYu + 4*Yu*TLambdax) - 1.5*Conj(Lambdax)*(Lambdax*(3*traceYdAdjYd
      + traceYeAdjYe - 4*Sqr(g2))*TYu + 2*Yu*(Lambdax*(3*traceAdjYdTYd +
      traceAdjYeTYe + 4*MassWB*Sqr(g2)) + (3*traceYdAdjYd + traceYeAdjYe - 4*
      Sqr(g2))*TLambdax)) + (-2*(3*traceAdjYdTYd + traceAdjYeTYe) - 0.8*MassB*
      Sqr(g1) - 3*Conj(Lambdax)*TLambdax)*(Yu*Yd.adjoint()*Yd) + (-2*(3*
      traceYdAdjYd + traceYeAdjYe) - 3*AbsSqr(Lambdax) + 0.8*Sqr(g1))*(Yu*
      Yd.adjoint()*TYd) + (-18*traceAdjYuTYu - 0.8*MassB*Sqr(g1) - 12*MassWB*
      Sqr(g2) - 9*Conj(Lambdax)*TLambdax)*(Yu*Yu.adjoint()*Yu) + 1.2*(-5*AbsSqr
      (Lambdax) + Sqr(g1) + 5*(-2*traceYuAdjYu + Sqr(g2)))*(Yu*Yu.adjoint()*TYu
      ) + (-3*traceYdAdjYd - traceYeAdjYe - 1.5*AbsSqr(Lambdax) + 0.4*Sqr(g1))*
      (TYu*Yd.adjoint()*Yd) + 1.5*(-10*traceYuAdjYu - 5*AbsSqr(Lambdax) + 8*Sqr
      (g2))*(TYu*Yu.adjoint()*Yu) - 4*(Yu*Yd.adjoint()*Yd*Yd.adjoint()*TYd) - 2
      *(Yu*Yd.adjoint()*Yd*Yu.adjoint()*TYu) - 4*(Yu*Yd.adjoint()*TYd*
