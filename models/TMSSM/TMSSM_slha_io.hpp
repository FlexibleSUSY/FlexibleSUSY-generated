...
public:
   ...
   static void convert_to_slha_convention(TMSSM_physical&);

   template <class T>
   static void fill_slhaea(SLHAea::Coll&, const TMSSM_slha<T>&, const QedQcd&, const TMSSM_scales&);

   template <class T>
   static SLHAea::Coll fill_slhaea(const TMSSM_slha<T>&, const QedQcd&);

   template <class T>
   static SLHAea::Coll fill_slhaea(const TMSSM_slha<T>&, const QedQcd&, const TMSSM_scales&);
...
private:
   ...
   template <class T> void fill_drbar_parameters(TMSSM_slha<T>&) const;
   template <class T> void fill_physical(TMSSM_slha<T>&) const;
};
 ...
 */
template <class T>
void TMSSM_slha_io::fill_drbar_parameters(TMSSM_slha<T>& model) const
{
 ...
 */
template <class T>
void TMSSM_slha_io::fill_physical(TMSSM_slha<T>& model) const
{
   {
      DEFINE_PARAMETER(ZD);
   ...
   PHYSICAL(MCha)(2) = slha_io.read_entry("MASS", 1000047);
...
void TMSSM_slha_io::fill_slhaea(
   ...
   const QedQcd& qedqcd, const TMSSM_scales& scales)
{
...
SLHAea::Coll TMSSM_slha_io::fill_slhaea(
   const TMSSM_slha<T>& model, const QedQcd& qedqcd)
{
   TMSSM_scales scales;
