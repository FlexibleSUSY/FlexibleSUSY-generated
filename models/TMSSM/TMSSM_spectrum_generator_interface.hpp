...
template <class T>
class TMSSM_spectrum_generator_interface {
public:
   virtual ~TMSSM_spectrum_generator_interface() = default;

   std::tuple<TMSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<TMSSM<T>> get_models_slha() const
   { return std::make_tuple(TMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const TMSSM<T>& get_model() const
   { return model; }
   TMSSM<T>& get_model()
   { return model; }
   TMSSM<T> get_model_slha() const
   { return TMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const TMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "TMSSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(TMSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const TMSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void TMSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void TMSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const TMSSM_input_parameters& input)
{
 ...
 */
template <class T>
void TMSSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   TMSSM_mass_eigenstates tmp_model(model);
   try {
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return TMSSM_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(TMSSM_info::parameter_names),
      std::cend(TMSSM_info::parameter_names));
 ...
 */
template <class T>
void TMSSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   TMSSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void TMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(TMSSM<T>& model)
{
   try {
      throw;
