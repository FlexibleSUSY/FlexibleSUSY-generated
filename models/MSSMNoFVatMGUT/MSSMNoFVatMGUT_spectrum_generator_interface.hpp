...
// File generated at Fri 10 Apr 2020 20:43:42
...
template <class T>
class MSSMNoFVatMGUT_spectrum_generator_interface {
public:
   virtual ~MSSMNoFVatMGUT_spectrum_generator_interface() = default;

   std::tuple<MSSMNoFVatMGUT<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<MSSMNoFVatMGUT_slha<MSSMNoFVatMGUT<T>>> get_models_slha() const
   { return std::make_tuple(MSSMNoFVatMGUT_slha<MSSMNoFVatMGUT<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MSSMNoFVatMGUT<T>& get_model() const
   { return model; }
   MSSMNoFVatMGUT<T>& get_model()
   { return model; }
   MSSMNoFVatMGUT_slha<MSSMNoFVatMGUT<T>> get_model_slha() const
   { return MSSMNoFVatMGUT_slha<MSSMNoFVatMGUT<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MSSMNoFVatMGUT_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MSSMNoFVatMGUT_spectrum.dat") const;

protected:
   MSSMNoFVatMGUT<T> model;
   ...
   void translate_exception_to_problem(MSSMNoFVatMGUT<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MSSMNoFVatMGUT_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MSSMNoFVatMGUT_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MSSMNoFVatMGUT_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MSSMNoFVatMGUT_input_parameters& input)
{
 ...
 */
template <class T>
void MSSMNoFVatMGUT_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MSSMNoFVatMGUT_mass_eigenstates tmp_model(model);
   try {
   ...
   MSSMNoFVatMGUT_parameter_getter parameter_getter;
   Coupling_monitor<MSSMNoFVatMGUT_mass_eigenstates, MSSMNoFVatMGUT_parameter_getter>
 ...
 */
template <class T>
void MSSMNoFVatMGUT_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   MSSMNoFVatMGUT_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void MSSMNoFVatMGUT_spectrum_generator_interface<T>::translate_exception_to_problem(MSSMNoFVatMGUT<T>& model)
{
   try {
      throw;
