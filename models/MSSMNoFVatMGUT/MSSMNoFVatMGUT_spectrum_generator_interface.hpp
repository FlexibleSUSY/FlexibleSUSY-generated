...
// File generated at Wed 12 Apr 2017 14:12:54
...
public:
   MSSMNoFVatMGUT_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~MSSMNoFVatMGUT_spectrum_generator_interface() {}

   const MSSMNoFVatMGUT<T>& get_model() const { return model; }
   MSSMNoFVatMGUT<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const MSSMNoFVatMGUT_input_parameters&) = 0;
 ...
 */
template <class T>
void MSSMNoFVatMGUT_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   MSSMNoFVatMGUT_spectrum_plotter plotter;
...
void MSSMNoFVatMGUT_spectrum_generator_interface<T>::translate_exception_to_problem(MSSMNoFVatMGUT<T>& model)
{
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : MSSMNoFVatMGUT_info::parameter_names[parameter_index];
