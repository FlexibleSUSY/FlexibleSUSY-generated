...
public:
   ...
   std::tuple<MSSMNoFVatMGUT<T>> get_models_slha() const
   { return std::make_tuple(MSSMNoFVatMGUT<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MSSMNoFVatMGUT<T>& get_model() const
   { return model; }
   MSSMNoFVatMGUT<T>& get_model()
   { return model; }
   MSSMNoFVatMGUT<T> get_model_slha() const
   { return MSSMNoFVatMGUT<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return MSSMNoFVatMGUT_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(MSSMNoFVatMGUT_info::parameter_names),
      std::cend(MSSMNoFVatMGUT_info::parameter_names));
