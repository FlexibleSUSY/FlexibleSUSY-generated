...
/**
 * @file MSSMNoFVatMGUT_l_to_l_conversion.cpp
 *
 * This file was generated at Sun 25 Feb 2024 21:11:46 with FlexibleSUSY
...
#include "MSSMNoFVatMGUT_l_to_l_conversion.hpp"
...
#include "cxx_qft/MSSMNoFVatMGUT_npointfunctions_wilsoncoeffs.hpp"
...
double get_MSUSY(const MSSMNoFVatMGUT_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(MSSMNoFVatMGUT_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSMNoFVatMGUT_l_to_l_conversion: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSMNoFVatMGUT_l_to_l_conversion:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSMNoFVatMGUT_l_to_l_conversion: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSMNoFVatMGUT_l_to_l_conversion: run_to_MSUSY(): "
...
using namespace MSSMNoFVatMGUT_FFV_form_factors;

namespace MSSMNoFVatMGUT_l_to_l_conversion {
...
typedef std::valarray<std::complex<double>>  (*ffv_function)
   (int, int, const MSSMNoFVatMGUT_mass_eigenstates&, bool);

typedef std::array<std::complex<double>, 10> (*npf_function)
   (const MSSMNoFVatMGUT_mass_eigenstates&, const std::array<int,4>&, const std::array<Eigen::Vector4d,0>&);
...
std::complex<double> left(const MSSMNoFVatMGUT_mass_eigenstates& model, int g) {
 ...
 */
template <class Q, class A>
std::complex<double> right(const MSSMNoFVatMGUT_mass_eigenstates& model, int g) {
    context_base context {model};
    using vertex = Vertex<typename Q::lorentz_conjugate, Q, A>;
    std::array<int, 2> indices {g, g};
    const auto value =  vertex::evaluate(indices, context);
...
Eigen::Array<std::complex<double>,10,1> embed_photon(
   const MSSMNoFVatMGUT_mass_eigenstates& model, const T& ff, int g) {
...
Eigen::Array<std::complex<double>,10,1> fix_tensors_sign(
   const MSSMNoFVatMGUT_mass_eigenstates& model, int in, int out, int g) {
 ...
 *    <pi, q| T exp(i L_MSSMNoFVatMGUT dx)|pj> =
 *    i * ubari
 ...
 *       for photon, as in MSSMNoFVatMGUT, using eq. (t.3).
 ...
 *          <out:q4,l3| T exp(i L_MSSMNoFVatMGUT dx) |in:q2,l1> =: G4
...
Eigen::Array<std::complex<double>, 13, 1> forge_conversion(int in, int out,
   const MSSMNoFVatMGUT_l_to_l_conversion::Nucleus nucleus,
   ...
   try {
      ...
      ERROR("MSSMNoFVatMGUT_l_to_l_conversion:" << e.what_detailed());
