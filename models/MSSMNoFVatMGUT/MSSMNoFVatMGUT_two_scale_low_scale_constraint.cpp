...
void MSSMNoFVatMGUT_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   ...
   if (IsFinite(new_g1)) {
      model->get_problems().unflag_non_perturbative_parameter(MSSMNoFVatMGUT_info
         ::g1);
   } else {
...
void MSSMNoFVatMGUT_low_scale_constraint<Two_scale>::calculate_running_SM_masses()
{
   check_model_ptr();
