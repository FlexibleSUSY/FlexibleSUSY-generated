...
/**
 * @file MSSMNoFVatMGUT_amm.cpp
 *
 * This file was generated with FlexibleSUSY 2.8.0 and SARAH 4.15.1 .
 */

#include "MSSMNoFVatMGUT_amm.hpp"
#include "MSSMNoFVatMGUT_mass_eigenstates.hpp"

#include "cxx_qft/MSSMNoFVatMGUT_qft.hpp"
#include "cxx_qft/MSSMNoFVatMGUT_particle_aliases.hpp"
#include "MSSMNoFVatMGUT_FFV_form_factors.hpp"
...
using namespace MSSMNoFVatMGUT_cxx_diagrams;
...
double get_MLCP(const MSSMNoFVatMGUT_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(MSSMNoFVatMGUT_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSMNoFVatMGUT_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSMNoFVatMGUT_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSMNoFVatMGUT_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSMNoFVatMGUT_amm: run_to_MSUSY(): "
...
template <typename Lepton>
std::enable_if_t<std::is_same_v<Lepton, MSSMNoFVatMGUT_cxx_diagrams::fields::Fe>, double>
...
template <typename Lepton>
std::enable_if_t<std::is_same_v<Lepton, MSSMNoFVatMGUT_cxx_diagrams::fields::Fm>, double>
lepton_pole_mass(const softsusy::QedQcd& qedqcd)
{
...
template <typename Lepton>
std::enable_if_t<std::is_same_v<Lepton, MSSMNoFVatMGUT_cxx_diagrams::fields::Ftau>, double>
lepton_pole_mass(const softsusy::QedQcd& qedqcd)
{
   return qedqcd.displayPoleMtau();
}

/// 1-loop scalar/fermionic contributions
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const MSSMNoFVatMGUT_mass_eigenstates& model, double ml_pole)
{
   using namespace MSSMNoFVatMGUT_cxx_diagrams::fields;
...
template <typename Lepton>
double calculate_amm_impl(const MSSMNoFVatMGUT_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings)
{
   VERBOSE_MSG("MSSMNoFVatMGUT_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double MSSMNoFVatMGUT_amm::calculate_amm(const MSSMNoFVatMGUT_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings)
{
   MSSMNoFVatMGUT_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMNoFVatMGUT_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMNoFVatMGUT_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template <typename Lepton>
double MSSMNoFVatMGUT_amm::calculate_amm_uncertainty(const MSSMNoFVatMGUT_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings)
{
   MSSMNoFVatMGUT_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMNoFVatMGUT_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMNoFVatMGUT_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
