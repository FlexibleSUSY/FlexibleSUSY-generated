...
class MSSMNoFVatMGUT_spectrum_generator
   : public MSSMNoFVatMGUT_spectrum_generator_interface<T> {
public:
   MSSMNoFVatMGUT_spectrum_generator()
      ...
      , low_scale(0.)
   {}
   virtual ~MSSMNoFVatMGUT_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const MSSMNoFVatMGUT_input_parameters&);
 ...
 */
template <class T>
void MSSMNoFVatMGUT_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const MSSMNoFVatMGUT_input_parameters& input)
{
   MSSMNoFVatMGUT<T>& model = this->model;
   model.clear();
   ...
   MSSMNoFVatMGUT_convergence_tester<T> convergence_tester(&model, this->precision_goal);
   if (this->max_iterations > 0)
      convergence_tester.set_max_iterations(this->max_iterations);

   MSSMNoFVatMGUT_initial_guesser<T> initial_guesser(&model, qedqcd,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void MSSMNoFVatMGUT_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   MSSMNoFVatMGUT_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
