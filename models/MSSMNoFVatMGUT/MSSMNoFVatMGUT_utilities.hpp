...
public:
   Eigen::ArrayXd get_parameters(const MSSMNoFVatMGUT<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const MSSMNoFVatMGUT<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const MSSMNoFVatMGUT<T>&);
...
template <class T>
void MSSMNoFVatMGUT_spectrum_plotter::extract_spectrum(const MSSMNoFVatMGUT<T>& model)
{
   spectrum.clear();
