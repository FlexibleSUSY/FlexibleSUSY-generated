...
double MSSMNoFVatMGUT_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void MSSMNoFVatMGUT_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MSSMNoFVatMGUT_input_parameters& input)
{
   ...
   MSSMNoFVatMGUT_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MSSMNoFVatMGUT_ewsb_solver<Two_scale> >(ewsb_solver));

   MSSMNoFVatMGUT_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   MSSMNoFVatMGUT_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   MSSMNoFVatMGUT_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   MSSMNoFVatMGUT_convergence_tester<Two_scale> convergence_tester(
   ...
   MSSMNoFVatMGUT_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void MSSMNoFVatMGUT_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MSSMNoFVatMGUT_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void MSSMNoFVatMGUT_spectrum_generator<Two_scale>::calculate_spectrum()
{
