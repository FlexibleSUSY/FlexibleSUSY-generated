...
#include "MSSMNoFVatMGUT_two_scale_spectrum_generator.hpp"
#endif
...
#include "lowe.h"
#include "MSSMNoFVatMGUT_unitarity.hpp"
...
namespace MSSMNoFVatMGUT_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSMNoFVatMGUTMessage", 1);
...
class MSSMNoFVatMGUT_spectrum {
public:
   virtual ~MSSMNoFVatMGUT_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSMNoFVatMGUT_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSMNoFVatMGUT_observables& get_observables() const = 0;

   virtual const UnitarityInfiniteS& get_unitarity() const = 0;
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMNoFVatMGUT_input_parameters&) = 0;
...
class MSSMNoFVatMGUT_spectrum_impl : public MSSMNoFVatMGUT_spectrum
{
public:
   virtual ~MSSMNoFVatMGUT_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSMNoFVatMGUT_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSMNoFVatMGUT_observables& get_observables() const override { return observables; }

   virtual const UnitarityInfiniteS& get_unitarity() const { return unitarityData; };
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMNoFVatMGUT_input_parameters&) override;
   virtual void calculate_model_observables(
      const softsusy::QedQcd&,
...
private:
   std::tuple<MSSMNoFVatMGUT<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSMNoFVatMGUT_scales scales{};              ///< scale information
   MSSMNoFVatMGUT_observables observables{};    ///< observables
...
public:
   ...
   void set_input_parameters(const MSSMNoFVatMGUT_input_parameters& input_) { input = input_; }
...
private:
   MSSMNoFVatMGUT_input_parameters input{};                    ///< model input parameters
   ...
   std::unique_ptr<MSSMNoFVatMGUT_spectrum> spectrum{nullptr}; ///< spectrum information

   MSSMNoFVatMGUT_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSMSSMNoFVatMGUTCalculateSpectrum[]?");
   }
}
...
MSSMNoFVatMGUT_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const MSSMNoFVatMGUT_slha& model, MLINK link)
{
   MLPutRule(link, MSSMNoFVatMGUT_info::model_name);
...
template <typename Solver_type>
void MSSMNoFVatMGUT_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSMNoFVatMGUT_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MSSMNoFVatMGUT_input_parameters& input
)
{
...
template <typename Solver_type>
void MSSMNoFVatMGUT_spectrum_impl<Solver_type>::calculate_model_observables(
   const softsusy::QedQcd& qedqcd,
...
template <typename Solver_type>
void MSSMNoFVatMGUT_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = MSSMNoFVatMGUT_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}

/******************************************************************/

template <typename Solver_type>
void MSSMNoFVatMGUT_spectrum_impl<Solver_type>::fill_slha_io(MSSMNoFVatMGUT_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   MSSMNoFVatMGUT_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMNoFVatMGUTCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMNoFVatMGUTCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSMNoFVatMGUT_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSMSSMNoFVatMGUTGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUT_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTGetSettings"))
...
DLLEXPORT int FSMSSMNoFVatMGUTGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUT_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVatMGUTGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUT_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVatMGUTOpenHandle(
...
DLLEXPORT int FSMSSMNoFVatMGUTCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MSSMNoFVatMGUT_librarylink;
...
DLLEXPORT int FSMSSMNoFVatMGUTSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::MSSMNoFVatMGUT_librarylink;
...
DLLEXPORT int FSMSSMNoFVatMGUTGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUT_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVatMGUTToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUT_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVatMGUTGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUT_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVatMGUTCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUT_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVatMGUTCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUT_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMNoFVatMGUTCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMNoFVatMGUTCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMNoFVatMGUTCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSMSSMNoFVatMGUTCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUT_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSMSSMNoFVatMGUTCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
