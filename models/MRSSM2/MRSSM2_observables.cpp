...
#include "observables/MRSSM2_br_l_to_3l.hpp"
#include "observables/MRSSM2_br_l_to_l_gamma.hpp"
#include "observables/MRSSM2_l_to_l_conversion.hpp"
...
const int MRSSM2_observables::NUMBER_OF_OBSERVABLES;

MRSSM2_observables::MRSSM2_observables()
   : amm_Fe_0(0)
   , amm_Fe_1(0)
   , amm_Fe_2(0)
   , edm_Fe_0(0)
...
Eigen::ArrayXd MRSSM2_observables::get() const
{
   Eigen::ArrayXd vec(MRSSM2_observables::NUMBER_OF_OBSERVABLES);
   ...
   vec(59) = amm_uncertainty_Fe_1;

   return vec;
}

std::vector<std::string> MRSSM2_observables::get_names()
{
   std::vector<std::string> names(MRSSM2_observables::NUMBER_OF_OBSERVABLES);
   ...
   names[59] = "amm_uncertainty_Fe_1";

   return names;
}

void MRSSM2_observables::clear()
{
   amm_Fe_0 = 0.;
   amm_Fe_1 = 0.;
   amm_Fe_2 = 0.;
...
void MRSSM2_observables::set(const Eigen::ArrayXd& vec)
{
   assert(vec.rows() == MRSSM2_observables::NUMBER_OF_OBSERVABLES);
   ...
   amm_uncertainty_Fe_1 = vec(59);

}

MRSSM2_observables calculate_observables(const MRSSM2_mass_eigenstates& model,
                                              ...
                                              double scale)
{
   ...
   if (scale > 0.) {
      try {
         ...
         MRSSM2_observables observables;
                                              ...
                                              const Spectrum_generator_settings& settings)
{
   ...
   try {
      ...
      observables.AMM1(Fe, 0) = MRSSM2_amm::calculate_amm<MRSSM2_cxx_diagrams::fields::Fe>(MODEL, qedqcd, settings,0);
      observables.AMM1(Fe, 1) = MRSSM2_amm::calculate_amm<MRSSM2_cxx_diagrams::fields::Fe>(MODEL, qedqcd, settings,1);
      observables.AMM1(Fe, 2) = MRSSM2_amm::calculate_amm<MRSSM2_cxx_diagrams::fields::Fe>(MODEL, qedqcd, settings,2);
      observables.EDM1(Fe, 0) = MRSSM2_edm::calculate_edm<MRSSM2_cxx_diagrams::fields::Fe>(MODEL, qedqcd, 0);
      observables.EDM1(Fe, 1) = MRSSM2_edm::calculate_edm<MRSSM2_cxx_diagrams::fields::Fe>(MODEL, qedqcd, 1);
      observables.EDM1(Fe, 2) = MRSSM2_edm::calculate_edm<MRSSM2_cxx_diagrams::fields::Fe>(MODEL, qedqcd, 2);
      observables.Fe1_to_Fe0_VP = MRSSM2_br_l_to_l_gamma::calculate_Fe_to_Fe_VP(1, 0, model, qedqcd, physical_input);
      observables.Fe2to11bar1_All_1loop = MRSSM2_br_l_to_3l::calculate_Fe_to_FeFebarFe_for_All_1loop(1, 0, 0, model, qedqcd);
      observables.Fe2Fe1inAl_All_1loop = MRSSM2_l_to_l_conversion::calculate_FeFe_forAll_1loop(1, 0, MRSSM2_l_to_l_conversion::Nucleus::Al, model, ltolconversion_settings, qedqcd);
      observables.AMMUNCERTAINTY1(Fe, 1) = MRSSM2_amm::calculate_amm_uncertainty<MRSSM2_cxx_diagrams::fields::Fe>(MODEL, qedqcd, settings, 1);
   } catch (const NonPerturbativeRunningError& e) {
