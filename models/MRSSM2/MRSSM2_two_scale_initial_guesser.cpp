...
MRSSM2_initial_guesser<Two_scale>::MRSSM2_initial_guesser(
   MRSSM2<Two_scale>* model_,
   const softsusy::QedQcd& qedqcd_,
   const MRSSM2_low_scale_constraint<Two_scale>& low_constraint_,
   const MRSSM2_susy_scale_constraint<Two_scale>& susy_constraint_
)
   : model(model_)
   , qedqcd(qedqcd_)
...
{
   if (!model)
      throw SetupError("MRSSM2_initial_guesser: Error: pointer to model"
                       " MRSSM2<Two_scale> must not be zero");
}
 ...
 */
void MRSSM2_initial_guesser<Two_scale>::guess()
{
 ...
 * \endcode
 */
void MRSSM2_initial_guesser<Two_scale>::guess_susy_parameters()
{
...
void MRSSM2_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void MRSSM2_initial_guesser<Two_scale>::calculate_running_SM_masses()
{
 ...
 * corrections).
 */
void MRSSM2_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
 ...
 */
void MRSSM2_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
 ...
 * corrections).
 */
void MRSSM2_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
   const auto vd = MODELPARAMETER(vd);
 ...
 */
void MRSSM2_initial_guesser<Two_scale>::guess_soft_parameters()
{
