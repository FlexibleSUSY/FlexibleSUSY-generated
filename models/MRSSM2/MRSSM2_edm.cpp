...
double calculate_edm_Fe( int generationIndex, const MRSSM2_mass_eigenstates& model )
{
   ...
   val += 1 * EDMVertexCorrectionSF<Fe, typename conj<Se>::type, typename bar<Chi>::type>::value(indices, context);
   val += 1 * EDMVertexCorrectionSF<Fe, typename conj<Hpm>::type, typename bar<Fv>::type>::value(indices, context);
   val += 1 * EDMVertexCorrectionFS<Fe, Cha1, typename conj<Sv>::type>::value(indices, context);
