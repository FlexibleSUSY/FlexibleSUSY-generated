...
// MRSSM2_parameter_getter ////////////////////////////////////////

std::vector<std::string> MRSSM2_parameter_getter::get_mass_names(const std::string& head)
{
   using namespace MRSSM2_info;
...
std::array<std::string, MRSSM2_info::NUMBER_OF_MIXINGS> MRSSM2_parameter_getter::get_mixing_names()
{
   return MRSSM2_info::particle_mixing_names;
}

Eigen::ArrayXd MRSSM2_parameter_getter::get_parameters(const MRSSM2_mass_eigenstates& model)
{
   return model.get();
}

std::array<std::string, MRSSM2_info::NUMBER_OF_PARAMETERS> MRSSM2_parameter_getter::get_parameter_names()
{
   return MRSSM2_info::parameter_names;
}

std::array<std::string, MRSSM2_info::NUMBER_OF_PARTICLES> MRSSM2_parameter_getter::get_particle_names()
{
   return MRSSM2_info::particle_names;
}

std::vector<std::string> MRSSM2_parameter_getter::get_DRbar_mass_names()
{
   return get_mass_names();
}

std::vector<std::string> MRSSM2_parameter_getter::get_pole_mass_names()
{
   return get_mass_names("Pole");
}

std::array<std::string, MRSSM2_info::NUMBER_OF_MIXINGS> MRSSM2_parameter_getter::get_DRbar_mixing_names()
{
   return get_mixing_names();
}

std::array<std::string, MRSSM2_info::NUMBER_OF_MIXINGS> MRSSM2_parameter_getter::get_pole_mixing_names()
{
...
std::array<std::string, MRSSM2_info::NUMBER_OF_INPUT_PARAMETERS> MRSSM2_parameter_getter::get_input_parameter_names()
{
   return MRSSM2_info::input_parameter_names;
}

std::array<std::string, MRSSM2_info::NUMBER_OF_EXTRA_PARAMETERS> MRSSM2_parameter_getter::get_extra_parameter_names()
{
   return MRSSM2_info::extra_parameter_names;
}

decltype(MRSSM2_info::NUMBER_OF_MASSES) MRSSM2_parameter_getter::get_number_of_masses()
{
   return MRSSM2_info::NUMBER_OF_MASSES;
}

// MRSSM2_spectrum_plotter ////////////////////////////////////////

MRSSM2_spectrum_plotter::MRSSM2_spectrum_plotter(const MRSSM2_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void MRSSM2_spectrum_plotter::extract_spectrum(const MRSSM2_mass_eigenstates& model)
{
   spectrum.clear();
...
void MRSSM2_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("MRSSM2_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("MRSSM2_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("MRSSM2_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void MRSSM2_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("MRSSM2_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const MRSSM2_mass_eigenstates& model,
   ...
   const MRSSM2_observables* observables)
{
   ...
   append(names, MRSSM2_parameter_getter::get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, MRSSM2_parameter_getter::get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, MRSSM2_parameter_getter::get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, MRSSM2_parameter_getter::get_DRbar_mass_names());
   append(names, MRSSM2_parameter_getter::get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, MRSSM2_parameter_getter::get_pole_mass_names());
   append(names, MRSSM2_parameter_getter::get_pole_mixing_names());
   ...
   if (MRSSM2_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, MRSSM2_observables::get_names());
...
MRSSM2_mass_eigenstates from_database(
   ...
   Physical_input* physical_input, MRSSM2_observables* observables)
{
   MRSSM2_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = MRSSM2_parameter_getter::get_number_of_masses();
   const auto number_of_mixings = MRSSM2_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = MRSSM2_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = MRSSM2_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? MRSSM2_observables::NUMBER_OF_OBSERVABLES : 0);
