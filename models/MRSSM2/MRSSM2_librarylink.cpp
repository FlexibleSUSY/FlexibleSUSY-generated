...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMRSSM2Message", 1);
...
class MRSSM2_spectrum {
public:
   virtual ~MRSSM2_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MRSSM2_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MRSSM2_observables& get_observables() const = 0;
   virtual const MRSSM2_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MRSSM2_input_parameters&) = 0;
...
class MRSSM2_spectrum_impl : public MRSSM2_spectrum
{
public:
   virtual ~MRSSM2_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MRSSM2_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MRSSM2_observables& get_observables() const override { return observables; }
   virtual const MRSSM2_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MRSSM2_input_parameters&) override;
...
private:
   std::tuple<MRSSM2<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MRSSM2_scales scales{};              ///< scale information
   MRSSM2_observables observables{};    ///< observables
   MRSSM2_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const MRSSM2_input_parameters& input_) { input = input_; }
...
private:
   MRSSM2_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<MRSSM2_spectrum> spectrum{nullptr};  ///< spectrum information

   MRSSM2_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSMRSSM2CalculateSpectrum[]?");
   }
}
...
MRSSM2_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const MRSSM2_slha& model, MLINK link)
{
   MLPutRule(link, MRSSM2_info::model_name);
...
template <typename Solver_type>
void MRSSM2_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MRSSM2_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MRSSM2_input_parameters& input
)
{
...
template <typename Solver_type>
void MRSSM2_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MRSSM2_spectrum_impl<Solver_type>::calculate_model_decays(
...
template <typename Solver_type>
void MRSSM2_spectrum_impl<Solver_type>::fill_slha_io(MRSSM2_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   MRSSM2_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   MRSSM2_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = MRSSM2_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMRSSM2CalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMRSSM2CalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MRSSM2_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSMRSSM2GetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM2_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSM2GetSettings"))
...
DLLEXPORT int FSMRSSM2GetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM2_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSM2GetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSM2GetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM2_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSM2GetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSM2OpenHandle(
...
DLLEXPORT int FSMRSSM2CloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MRSSM2_librarylink;
...
DLLEXPORT int FSMRSSM2GetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM2_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSM2GetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSM2ToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM2_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSM2ToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSM2GetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM2_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSM2GetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSM2CalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM2_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSM2CalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSM2CalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM2_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSM2CalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMRSSM2CalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMRSSM2CalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMRSSM2CalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSMRSSM2CalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM2_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSM2CalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMRSSM2CalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMRSSM2CalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMRSSM2CalculateDecays", "error", e.what());
      put_error_output(link);
   }
