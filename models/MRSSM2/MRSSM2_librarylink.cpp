...
template <typename Solver_type>
void MRSSM2_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = MRSSM2_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
void Model_data::put_observables(MLINK link) const
{
   ...
   MLPutInteger(link, 1);
   MLPutSymbol(link, "VP");
   MLPutReal(link, OBSERVABLE(Fe1_to_Fe0_VP));MLPutRuleTo(link, OBSERVABLE(Fe2to11bar1_All_1loop), "FlexibleSUSYObservable`BrLTo3L[Fe, 2 -> {1, 1, 1}, All, 1]");
...
DLLEXPORT int FSMRSSM2CalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSM2_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSM2CalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSMRSSM2CalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
