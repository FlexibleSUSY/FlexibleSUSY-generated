...
void run_to_MSUSY(MRSSM2_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MRSSM2_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MRSSM2_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MRSSM2_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MRSSM2_a_muon: run_to_MSUSY(): "
...
double calculate_a_muon_impl(const MRSSM2_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   VERBOSE_MSG("MRSSM2_a_muon: calculating a_mu at Q = " << model.get_scale());

   context_base context{ model };

   using namespace MRSSM2_cxx_diagrams::fields;

   const auto form_factors = MRSSM2_FFV_form_factors::calculate_Fe_Fe_VP_form_factors(1, 1, model, true);
...
std::pair<double,double> vary_scale(const MRSSM2_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model,&qedqcd] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("MRSSM2_a_muon: scale variation: " << e.what_detailed());
                     }
                     return amu;
                  });
...
double MRSSM2_a_muon::calculate_a_muon(const MRSSM2_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   MRSSM2_mass_eigenstates model(model_);

   VERBOSE_MSG("MRSSM2_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MRSSM2_a_muon:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
double MRSSM2_a_muon::calculate_a_muon_uncertainty(const MRSSM2_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   MRSSM2_mass_eigenstates model(model_);

   VERBOSE_MSG("MRSSM2_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MRSSM2_a_muon uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
