...
#include "MRSSM2_two_scale_convergence_tester.hpp"
#include "MRSSM2_two_scale_ewsb_solver.hpp"
#include "MRSSM2_two_scale_initial_guesser.hpp"
...
double MRSSM2_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void MRSSM2_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MRSSM2_input_parameters& input)
{
   ...
   MRSSM2_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MRSSM2_ewsb_solver<Two_scale> >(ewsb_solver));

   MRSSM2_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   MRSSM2_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);
   ...
   MRSSM2_convergence_tester<Two_scale> convergence_tester(
   ...
   MRSSM2_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void MRSSM2_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MRSSM2_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_susy_scale());
}

void MRSSM2_spectrum_generator<Two_scale>::calculate_spectrum()
{
