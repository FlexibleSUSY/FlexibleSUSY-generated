...
#ifndef MRSSM2_SPECTRUM_GENERATOR_INTERFACE_H
#define MRSSM2_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class MRSSM2_spectrum_generator_interface {
public:
   virtual ~MRSSM2_spectrum_generator_interface() = default;

   std::tuple<MRSSM2<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<MRSSM2<T>> get_models_slha() const
   { return std::make_tuple(MRSSM2<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MRSSM2<T>& get_model() const
   { return model; }
   MRSSM2<T>& get_model()
   { return model; }
   MRSSM2<T> get_model_slha() const
   { return MRSSM2<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MRSSM2_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MRSSM2_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(MRSSM2<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MRSSM2_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MRSSM2_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MRSSM2_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MRSSM2_input_parameters& input)
{
 ...
 */
template <class T>
void MRSSM2_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MRSSM2_mass_eigenstates tmp_model(model);
   try {
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return MRSSM2_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(MRSSM2_info::parameter_names),
      std::cend(MRSSM2_info::parameter_names));
 ...
 */
template <class T>
void MRSSM2_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   MRSSM2_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void MRSSM2_spectrum_generator_interface<T>::translate_exception_to_problem(MRSSM2<T>& model)
{
   try {
      throw;
