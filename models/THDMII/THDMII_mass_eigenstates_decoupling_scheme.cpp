...
/**
 * @file THDMII_mass_eigenstates_decoupling_scheme.cpp
 * @brief implementation of the THDMII model class in the decoupling scheme
 *
...
#define CLASSNAME THDMII_mass_eigenstates_decoupling_scheme
...
CLASSNAME::CLASSNAME(const THDMII_mass_eigenstates& model)
{
   fill_from(model);
}
...
{
   return std::make_unique<THDMII_mass_eigenstates_decoupling_scheme>(*this);
}
...
void CLASSNAME::fill_from(const THDMII_mass_eigenstates& model)
{
   set(model.get());
...
std::ostream& operator<<(std::ostream& ostr, const THDMII_mass_eigenstates_decoupling_scheme& model)
{
   model.print(ostr);
   return ostr;
}
