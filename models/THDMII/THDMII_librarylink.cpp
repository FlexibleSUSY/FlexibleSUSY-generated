...
// File generated at Tue 22 Jan 2019 17:07:54
...
public:
   ...
   virtual void fill_slha_io(THDMII_slha_io&) const = 0;
...
public:
   ...
   virtual void fill_slha_io(THDMII_slha_io&) const override;
...
private:
   std::tuple<THDMII_slha<THDMII<Solver_type>>> models{};        ///< running parameters and pole masses
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, THDMII_info::particle_names[i].c_str());
      }
   }
}
...
void put_spectrum(const THDMII_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void THDMII_spectrum_impl<Solver_type>::fill_slha_io(THDMII_slha_io& slha_io) const
{
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSTHDMIICalculateObservables", "error", e.what());
      put_error_output(link);
   }
