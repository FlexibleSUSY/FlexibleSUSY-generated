...
// File generated at Wed 16 Oct 2019 21:28:36
...
template <class T>
class THDMII_spectrum_generator_interface {
public:
   virtual ~THDMII_spectrum_generator_interface() = default;
   ...
   void run(const softsusy::QedQcd&, const THDMII_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "THDMII_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(THDMII<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const THDMII_input_parameters&) = 0;
};
 ...
 */
template <class T>
void THDMII_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void THDMII_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   THDMII_mass_eigenstates tmp_model(model);
   try {
 ...
 */
template <class T>
void THDMII_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   THDMII_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void THDMII_spectrum_generator_interface<T>::translate_exception_to_problem(THDMII<T>& model)
{
   try {
      throw;
