...
std::valarray<std::complex<double>> calculate_Fe_Fe_VP_form_factors (
   ...
   val += std::complex<double> {1., 0.} * FFV_VSF<Fe,Fe,VP,VWm,Hm,Fv>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_SVF<Fe,Fe,VP,Hm,VWm,Fv>::value(indices1, indices2, context, discard_SM_contributions);
...
template std::valarray<std::complex<double>> calculate_form_factors<Fe,Fe,VP>(   int, int,
   const THDMII_mass_eigenstates&, bool);
