...
#include "cxx_qft/THDMII_particle_aliases.hpp"
...
double get_MLCP(const THDMII_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(THDMII_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("THDMII_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("THDMII_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("THDMII_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("THDMII_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const THDMII_mass_eigenstates& model, double ml_pole, int idx)
{
   using namespace THDMII_cxx_diagrams::fields;

   const auto form_factors = THDMII_FFV_form_factors::calculate_form_factors<Lepton,Lepton,VP>(idx, idx, model, true);
...
double calculate_amm_impl_2l_barrzee(const context_base& context, double ml_pole, int idx)
{
   ...
   val += 1.000000000000000 * AMMBarrZeeScalarLoop<Lepton,typename conj<Hm>::type, Ah>::value({idx}, context, ml_pole);
   ...
   val += 1.000000000000000 * AMMBarrZeeScalarLoop<Lepton,typename conj<Hm>::type, hh>::value({idx}, context, ml_pole);
...
template <typename Lepton>
double calculate_amm_impl(const THDMII_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("THDMII_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double THDMII_amm::calculate_amm(const THDMII_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   THDMII_mass_eigenstates model(model_);

   VERBOSE_MSG("THDMII_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("THDMII_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template double THDMII_amm::calculate_amm<THDMII_cxx_diagrams::fields::Fe>(const THDMII_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);

template <typename Lepton>
double THDMII_amm::calculate_amm_uncertainty(const THDMII_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   THDMII_mass_eigenstates model(model_);

   VERBOSE_MSG("THDMII_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("THDMII_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
// spliting templated function into header and cpp
// https://isocpp.org/wiki/faq/templates#separate-template-fn-defn-from-decl
template double THDMII_amm::calculate_amm_uncertainty<THDMII_cxx_diagrams::fields::Fe>(const THDMII_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);
