...
// File generated at Sun 26 Aug 2018 14:36:38
...
public:
   ...
   virtual void fill_slha_io(lowNMSSMTanBetaAtMZ_slha_io&) const = 0;
...
public:
   ...
   virtual void fill_slha_io(lowNMSSMTanBetaAtMZ_slha_io&) const override;
...
private:
   std::tuple<lowNMSSMTanBetaAtMZ_slha<lowNMSSMTanBetaAtMZ<Solver_type>>> models{};        ///< running parameters and pole masses
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, lowNMSSMTanBetaAtMZ_info::particle_names[i].c_str());
      }
   }
}
...
void put_spectrum(const lowNMSSMTanBetaAtMZ_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void lowNMSSMTanBetaAtMZ_spectrum_impl<Solver_type>::fill_slha_io(lowNMSSMTanBetaAtMZ_slha_io& slha_io) const
{
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSlowNMSSMTanBetaAtMZCalculateObservables", "error", e.what());
      put_error_output(link);
   }
