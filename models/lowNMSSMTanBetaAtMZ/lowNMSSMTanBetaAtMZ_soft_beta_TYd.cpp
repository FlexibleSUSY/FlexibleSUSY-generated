...
// File generated at Mon 5 Mar 2018 18:17:39
...
Eigen::Matrix<double,3,3> lowNMSSMTanBetaAtMZ_soft_parameters::calc_beta_TYd_2_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_TYd = (twoLoop*(-0.044444444444444446*Yd*(287*MassB*Quad(g1) + 5*
      ...
      16*traceYdAdjYd*Sqr(g3) + 8*Sqr(g2)*Sqr(g3))*TYd - 3*Lambdax*Sqr(Conj(
      Lambdax))*(Lambdax*TYd + 4*Yd*TLambdax) - Conj(Lambdax)*((3*traceYuAdjYu
      + 2*AbsSqr(Kappa))*Lambdax*TYd + 2*Yd*(3*(traceAdjYuTYu*Lambdax +
      traceYuAdjYu*TLambdax) + 2*Conj(Kappa)*(Lambdax*TKappa + Kappa*TLambdax))
      ) - 0.4*(4*MassB*Sqr(g1) + 15*(3*traceAdjYdTYd + traceAdjYeTYe + 2*MassWB
      *Sqr(g2)) + 15*Conj(Lambdax)*TLambdax)*(Yd*Yd.adjoint()*Yd) + (-12*
      traceYdAdjYd - 4*traceYeAdjYe - 4*AbsSqr(Lambdax) + 1.2*Sqr(g1) + 6*Sqr(
      g2))*(Yd*Yd.adjoint()*TYd) + (-6*traceAdjYuTYu - 1.6*MassB*Sqr(g1) - 2*
      Conj(Lambdax)*TLambdax)*(Yd*Yu.adjoint()*Yu) + (-6*traceYuAdjYu - 2*
