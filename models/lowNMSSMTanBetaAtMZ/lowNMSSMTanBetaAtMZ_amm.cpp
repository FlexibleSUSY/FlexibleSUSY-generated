...
/**
 * @file lowNMSSMTanBetaAtMZ_amm.cpp
 *
...
#include "cxx_qft/lowNMSSMTanBetaAtMZ_particle_aliases.hpp"
...
double get_MLCP(const lowNMSSMTanBetaAtMZ_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(lowNMSSMTanBetaAtMZ_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("lowNMSSMTanBetaAtMZ_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("lowNMSSMTanBetaAtMZ_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("lowNMSSMTanBetaAtMZ_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("lowNMSSMTanBetaAtMZ_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const lowNMSSMTanBetaAtMZ_mass_eigenstates& model, double ml_pole, int idx)
{
...
template <typename Lepton>
double calculate_amm_impl(const lowNMSSMTanBetaAtMZ_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("lowNMSSMTanBetaAtMZ_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double lowNMSSMTanBetaAtMZ_amm::calculate_amm(const lowNMSSMTanBetaAtMZ_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   lowNMSSMTanBetaAtMZ_mass_eigenstates model(model_);

   VERBOSE_MSG("lowNMSSMTanBetaAtMZ_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("lowNMSSMTanBetaAtMZ_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template <typename Lepton>
double lowNMSSMTanBetaAtMZ_amm::calculate_amm_uncertainty(const lowNMSSMTanBetaAtMZ_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   lowNMSSMTanBetaAtMZ_mass_eigenstates model(model_);

   VERBOSE_MSG("lowNMSSMTanBetaAtMZ_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("lowNMSSMTanBetaAtMZ_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
