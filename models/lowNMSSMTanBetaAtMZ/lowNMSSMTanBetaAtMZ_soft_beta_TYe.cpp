...
// File generated at Sun 26 Aug 2018 14:33:31
...
Eigen::Matrix<double,3,3> lowNMSSMTanBetaAtMZ_soft_parameters::calc_beta_TYe_1_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_TYe = (oneOver16PiSqr*((3*traceYdAdjYd + traceYeAdjYe + AbsSqr(Lambdax)
      - 1.8*Sqr(g1) - 3*Sqr(g2))*TYe + 0.4*Ye*(9*MassB*Sqr(g1) + 5*(3*
...
Eigen::Matrix<double,3,3> lowNMSSMTanBetaAtMZ_soft_parameters::calc_beta_TYe_2_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_TYe = (twoLoop*(0.1*(-4*Ye*(135*MassB*Quad(g1) + Sqr(g1)*(2*(
      ...
      traceYdAdjYd*Sqr(g3)))*TYe) - 3*Lambdax*Sqr(Conj(Lambdax))*(Lambdax*TYe +
      4*Ye*TLambdax) - Conj(Lambdax)*((3*traceYuAdjYu + 2*AbsSqr(Kappa))*
      Lambdax*TYe + 2*Ye*(3*(traceAdjYuTYu*Lambdax + traceYuAdjYu*TLambdax) + 2
      *Conj(Kappa)*(Lambdax*TKappa + Kappa*TLambdax))) - 6*(3*traceAdjYdTYd +
      ...
      (g1) + 6*Sqr(g2))*(Ye*Ye.adjoint()*TYe) + (-15*traceYdAdjYd - 5*
      traceYeAdjYe - 5*AbsSqr(Lambdax) - 1.2*Sqr(g1) + 12*Sqr(g2))*(TYe*Ye.
