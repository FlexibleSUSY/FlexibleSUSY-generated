...
#include "lowNMSSMTanBetaAtMZ_FFV_form_factors.hpp"
...
double get_MSUSY(const lowNMSSMTanBetaAtMZ_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(lowNMSSMTanBetaAtMZ_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("lowNMSSMTanBetaAtMZ_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("lowNMSSMTanBetaAtMZ_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("lowNMSSMTanBetaAtMZ_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("lowNMSSMTanBetaAtMZ_a_muon: run_to_MSUSY(): "
...
double calculate_a_muon_impl(const lowNMSSMTanBetaAtMZ_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   VERBOSE_MSG("lowNMSSMTanBetaAtMZ_a_muon: calculating a_mu at Q = " << model.get_scale());
...
std::pair<double,double> vary_scale(const lowNMSSMTanBetaAtMZ_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model,&qedqcd] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("lowNMSSMTanBetaAtMZ_a_muon: scale variation: " << e.what_detailed());
                     }
                     return amu;
                  });
...
double lowNMSSMTanBetaAtMZ_a_muon::calculate_a_muon(const lowNMSSMTanBetaAtMZ_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   lowNMSSMTanBetaAtMZ_mass_eigenstates model(model_);

   VERBOSE_MSG("lowNMSSMTanBetaAtMZ_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("lowNMSSMTanBetaAtMZ_a_muon:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
double lowNMSSMTanBetaAtMZ_a_muon::calculate_a_muon_uncertainty(const lowNMSSMTanBetaAtMZ_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   lowNMSSMTanBetaAtMZ_mass_eigenstates model(model_);

   VERBOSE_MSG("lowNMSSMTanBetaAtMZ_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("lowNMSSMTanBetaAtMZ_a_muon uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
