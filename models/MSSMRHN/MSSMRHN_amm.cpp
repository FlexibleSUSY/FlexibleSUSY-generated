...
/**
 * @file MSSMRHN_amm.cpp
 *
 * This file was generated with FlexibleSUSY 2.8.0 and SARAH 4.15.1 .
 */

#include "MSSMRHN_amm.hpp"
#include "MSSMRHN_mass_eigenstates.hpp"

#include "cxx_qft/MSSMRHN_qft.hpp"
#include "cxx_qft/MSSMRHN_particle_aliases.hpp"
...
double get_MLCP(const MSSMRHN_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(MSSMRHN_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSMRHN_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSMRHN_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSMRHN_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSMRHN_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const MSSMRHN_mass_eigenstates& model, double ml_pole, int idx)
{
...
template <typename Lepton>
double calculate_amm_impl(const MSSMRHN_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("MSSMRHN_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double MSSMRHN_amm::calculate_amm(const MSSMRHN_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   MSSMRHN_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMRHN_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMRHN_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template <typename Lepton>
double MSSMRHN_amm::calculate_amm_uncertainty(const MSSMRHN_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   MSSMRHN_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMRHN_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMRHN_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
