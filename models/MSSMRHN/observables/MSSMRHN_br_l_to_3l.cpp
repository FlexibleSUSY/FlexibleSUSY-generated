...
#include "MSSMRHN_br_l_to_3l.hpp"
...
void run_to_MSUSY(MSSMRHN_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSMRHN_br_l_to_3l: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSMRHN_br_l_to_3l:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSMRHN_br_l_to_3l: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSMRHN_br_l_to_3l: run_to_MSUSY(): "
...
std::complex<double> left(const MSSMRHN_mass_eigenstates& model, int nA) {
 ...
 */
template <class Lepton, class Photon>
std::complex<double> right(const MSSMRHN_mass_eigenstates& model, int nA) {
    context_base context {model};
    using vertex = Vertex<typename Lepton::lorentz_conjugate, Lepton, Photon>;
    std::array<int, 2> indices {nA, nA};
    const auto value =  vertex::evaluate(indices, context);
...
Eigen::Array<std::complex<double>,10,1> fix_tensors_sign(
   const MSSMRHN_mass_eigenstates& model, int nI, int nO, int nA) {
...
double width_same(int nI, int nA, T1 DL, T1 DR, T2 coeffs,
   const MSSMRHN_mass_eigenstates& model, const softsusy::QedQcd& qedqcd) {
...
Eigen::Array<std::complex<double>,13,1> forge(int nI, int nO, int nA,
   const MSSMRHN_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd) {
   ...
   try {
      ...
      ERROR("MSSMRHN_br_l_to_3l:" << e.what_detailed());
