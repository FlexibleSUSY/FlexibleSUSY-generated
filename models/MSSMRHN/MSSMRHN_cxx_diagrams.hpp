...
// File generated at Fri 20 Oct 2017 09:05:45

/**
 * @file MSSMRHN_cxx_diagrams.hpp
 *
 * This file was generated at Fri 20 Oct 2017 09:05:45 with FlexibleSUSY
 * 2.0.1 and SARAH 4.12.0 .
 */

#ifndef MSSMRHN_CXXDIAGRAMS_H
#define MSSMRHN_CXXDIAGRAMS_H
...
struct Fv {
   static constexpr int numberOfGenerations = 6;
   using smFlags = boost::mpl::vector_c<bool, true, true, true, true, true, true>;
   static constexpr int numberOfFieldIndices = 1;
   static constexpr double electric_charge = 0;
...
*/
struct EvaluationContext {
   MSSMRHN_mass_eigenstates& model; ///< The model object.
...
template<> struct VertexData<Fe, Fv, conj<Hpm>::type>
{
   static constexpr impl::IndexBounds<3> index_bounds = { { 0, 0, 0 }, { 3, 6, 2 } };
   static constexpr int fieldIndexStart[4] = { 0, 1, 2, 3 };
   using vertex_type = LeftAndRightComponentedVertex;
...
template<> inline
Vertex<Fe, Fv, conj<Hpm>::type>::vertex_type
Vertex<Fe, Fv, conj<Hpm>::type>::evaluate(const indices_type& indices, const EvaluationContext& context)
{
   const int gt1 = indices[0];
   const int gt2 = indices[1];
   const int gt3 = indices[2];
   ...
   const std::complex<double> right = SUM(j2,0,2,SUM(j1,0,2,Conj(Ye(j1,j2))*ZER(gt1,j1))*UV(gt2,j2))*ZP(gt3,0);
...
Vertex<bar<Fe>::type, Sv, Cha>::evaluate(const indices_type& indices, const EvaluationContext& context)
{
   ...
   const std::complex<double> right = -(g2*SUM(j1,0,2,Conj(ZV(gt2,j1))*ZEL(gt1,j1))*UP(gt3,0)) + SUM(j2,0,2,SUM(j1,0,2,Conj(Yv(j1,j2))*Conj(ZV(gt2,3 + j1)))*ZEL(gt1,j2))*UP(gt3,1);
...
Vertex<Fe, conj<Sv>::type, bar<Cha>::type>::evaluate(const indices_type& indices, const EvaluationContext& context)
{
   ...
   const std::complex<double> left = -(g2*Conj(UP(gt3,0))*SUM(j1,0,2,Conj(ZEL(gt1,j1))*ZV(gt2,j1))) + Conj(UP(gt3,1))*SUM(j2,0,2,Conj(ZEL(gt1,j2))*SUM(j1,0,2,Yv(j1,j2)*ZV(gt2,3 + j1)));
