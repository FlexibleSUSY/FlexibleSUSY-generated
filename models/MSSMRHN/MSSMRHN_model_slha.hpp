...
/**
 * @file MSSMRHN_model_slha.hpp
 * @brief contains wrapper class for model class in SLHA convention
 */

// File generated at Fri 20 Oct 2017 09:05:03

#ifndef MSSMRHN_SLHA_H
#define MSSMRHN_SLHA_H
...
class MSSMRHN_slha : public Model {
public:
   explicit MSSMRHN_slha(const MSSMRHN_input_parameters& input_ = MSSMRHN_input_parameters());
   explicit MSSMRHN_slha(const Model&, bool do_convert_masses_to_slha = true);
   MSSMRHN_slha(const MSSMRHN_slha&) = default;
   MSSMRHN_slha(MSSMRHN_slha&&) = default;
   virtual ~MSSMRHN_slha() = default;
   MSSMRHN_slha& operator=(const MSSMRHN_slha&) = default;
   MSSMRHN_slha& operator=(MSSMRHN_slha&&) = default;
   ...
   const MSSMRHN_physical& get_physical_slha() const; ///< returns pole masses to SLHA convention
   MSSMRHN_physical& get_physical_slha(); ///< returns pole masses to SLHA convention
   ...
   const Eigen::Array<double,6,1>& get_MFv_pole_slha() const { return PHYSICAL_SLHA(MFv); }
   ...
   const Eigen::Matrix<std::complex<double>,6,6>& get_UV_pole_slha() const { return PHYSICAL_SLHA(UV); }
   double get_UV_pole_slha(int i, int k) const { return PHYSICAL_SLHA_REAL(UV(i,k)); }
   ...
   const Eigen::Matrix<std::complex<double>,6,6>& get_UV_slha() const { return UV_slha; }
   std::complex<double> get_UV_slha(int i, int k) const { return UV_slha(i,k); }


private:
   MSSMRHN_physical physical_slha{}; ///< contains the pole masses and mixings in slha convention
   ...
   Eigen::Matrix<std::complex<double>,6,6> UV_slha{Eigen::Matrix<std::complex<double>,6,6>::Zero()};
...
template <class Model>
MSSMRHN_slha<Model>::MSSMRHN_slha(const MSSMRHN_input_parameters& input_)
   : Model(input_)
{
}
 ...
 */
template <class Model>
MSSMRHN_slha<Model>::MSSMRHN_slha(const Model& model_,
...
template <class Model>
void MSSMRHN_slha<Model>::calculate_spectrum()
{
...
template <class Model>
void MSSMRHN_slha<Model>::convert_to_slha()
{
...
template <class Model>
void MSSMRHN_slha<Model>::calculate_ckm_matrix()
{
...
template <class Model>
void MSSMRHN_slha<Model>::calculate_pmns_matrix()
{
 ...
 */
template <class Model>
void MSSMRHN_slha<Model>::convert_yukawa_couplings_to_slha()
{
 ...
 */
template <class Model>
void MSSMRHN_slha<Model>::convert_trilinear_couplings_to_slha()
{
 ...
 */
template <class Model>
void MSSMRHN_slha<Model>::convert_soft_squared_masses_to_slha()
{
...
template <class Model>
const MSSMRHN_physical& MSSMRHN_slha<Model>::get_physical_slha() const
{
   return physical_slha;
}

template <class Model>
MSSMRHN_physical& MSSMRHN_slha<Model>::get_physical_slha()
{
   return physical_slha;
}

template <class Model>
void MSSMRHN_slha<Model>::print(std::ostream& ostr) const
{
   Model::print(ostr);
...
template <class Model>
void MSSMRHN_slha<Model>::set_convert_masses_to_slha(bool flag)
{
   convert_masses_to_slha = flag;
}

template <class Model>
std::ostream& operator<<(std::ostream& ostr, const MSSMRHN_slha<Model>& model)
{
   model.print(ostr);
   return ostr;
}
