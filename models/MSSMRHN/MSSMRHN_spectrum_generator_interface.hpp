...
// File generated at Wed 16 Oct 2019 22:46:43
...
template <class T>
class MSSMRHN_spectrum_generator_interface {
public:
   virtual ~MSSMRHN_spectrum_generator_interface() = default;

   std::tuple<MSSMRHN<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<MSSMRHN_slha<MSSMRHN<T>>> get_models_slha() const
   { return std::make_tuple(MSSMRHN_slha<MSSMRHN<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MSSMRHN<T>& get_model() const
   { return model; }
   MSSMRHN<T>& get_model()
   { return model; }
   MSSMRHN_slha<MSSMRHN<T>> get_model_slha() const
   { return MSSMRHN_slha<MSSMRHN<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MSSMRHN_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MSSMRHN_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(MSSMRHN<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MSSMRHN_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MSSMRHN_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MSSMRHN_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MSSMRHN_input_parameters& input)
{
 ...
 */
template <class T>
void MSSMRHN_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MSSMRHN_mass_eigenstates tmp_model(model);
   try {
   ...
   MSSMRHN_parameter_getter parameter_getter;
   Coupling_monitor<MSSMRHN_mass_eigenstates, MSSMRHN_parameter_getter>
 ...
 */
template <class T>
void MSSMRHN_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   MSSMRHN_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void MSSMRHN_spectrum_generator_interface<T>::translate_exception_to_problem(MSSMRHN<T>& model)
{
   try {
      throw;
