...
public:
   ...
   std::tuple<MSSMRHN<T>> get_models_slha() const
   { return std::make_tuple(MSSMRHN<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MSSMRHN<T>& get_model() const
   { return model; }
   MSSMRHN<T>& get_model()
   { return model; }
   MSSMRHN<T> get_model_slha() const
   { return MSSMRHN<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return MSSMRHN_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(MSSMRHN_info::parameter_names),
      std::cend(MSSMRHN_info::parameter_names));
