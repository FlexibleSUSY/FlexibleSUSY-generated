...
// File generated at Tue 22 Jan 2019 17:42:37
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SqSq() const
{
   ...
   if (get_loops() > 1) {
      anomDim += (twoLoop*(0.8*Sqr(g1)*(Yu.adjoint()*Yu) - 2*(Yd.adjoint()*Yd*
         ...
         trace()) - 3*(Yu.adjoint()*Yu)*(Yu*Yu.adjoint()).trace() - Yu.adjoint(
         )*Yu*(Yv*Yv.adjoint()).trace() + (0.22111111111111112*Quad(g1) + 3.75*
         Quad(g2) - 0.8888888888888888*Quad(g3) + 8*Sqr(g2)*Sqr(g3) +
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SlSl() const
{
   ...
   anomDim = (oneOver16PiSqr*(Ye.adjoint()*Ye + Yv.adjoint()*Yv - 0.3*(Sqr(g1)
      + 5*Sqr(g2))*UNITMATRIX(3))).real();

   if (get_loops() > 1) {
      anomDim += (twoLoop*(-2*(Ye.adjoint()*Ye*Ye.adjoint()*Ye) - 2*(Yv.adjoint
         ()*Yv*Yv.adjoint()*Yv) + Ye.adjoint()*Ye*(1.2*Sqr(g1) - 3*(Yd*Yd.
         adjoint()).trace() - (Ye*Ye.adjoint()).trace()) - 3*(Yv.adjoint()*Yv)*
         (Yu*Yu.adjoint()).trace() - Yv.adjoint()*Yv*(Yv*Yv.adjoint()).trace()
         + 0.03*(69*Quad(g1) + 125*Quad(g2) + 30*Sqr(g1)*Sqr(g2))*UNITMATRIX(3)
         )).real();
   }
   ...
   if (get_loops() > 1) {
      anomDim += Re(twoLoop*(2.07*Quad(g1) + 3.75*Quad(g2) + 0.9*Sqr(g1)*Sqr(g2
         ...
         *(Ye*Ye.adjoint()).trace() - 9*(Yd*Yd.adjoint()*Yd*Yd.adjoint()).trace
         ...
         Ye*Ye.adjoint()).trace() - (Ye*Yv.adjoint()*Yv*Ye.adjoint()).trace()))
         ;
   }
...
double CLASSNAME::get_SHuSHu() const
{
   ...
   anomDim = Re(oneOver16PiSqr*(-0.3*(Sqr(g1) + 5*Sqr(g2)) + 3*(Yu*Yu.adjoint()
      ).trace() + (Yv*Yv.adjoint()).trace()));

   if (get_loops() > 1) {
      anomDim += Re(twoLoop*(2.07*Quad(g1) + 3.75*Quad(g2) + 0.9*Sqr(g1)*Sqr(g2
         ) + 0.8*(Sqr(g1) + 20*Sqr(g3))*(Yu*Yu.adjoint()).trace() - 3*(Yd*Yu.
         adjoint()*Yu*Yd.adjoint()).trace() - (Ye*Yv.adjoint()*Yv*Ye.adjoint())
         .trace() - 9*(Yu*Yu.adjoint()*Yu*Yu.adjoint()).trace() - 3*(Yv*Yv.
         adjoint()*Yv*Yv.adjoint()).trace()));
   }
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SeRSeR() const
{
   ...
   if (get_loops() > 1) {
      anomDim += (twoLoop*(-2*(Ye.conjugate()*Ye.transpose()*Ye.conjugate()*Ye.
         transpose() + Ye.conjugate()*Yv.transpose()*Yv.conjugate()*Ye.
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SvRSvR() const
{
   ...
   if (get_loops() > 1) {
      anomDim += (twoLoop*(-2*(Yv.conjugate()*Ye.transpose()*Ye.conjugate()*Yv.
         transpose() + Yv.conjugate()*Yv.transpose()*Yv.conjugate()*Yv.
         transpose()) + Yv.conjugate()*Yv.transpose()*(1.2*Sqr(g1) + 6*Sqr(g2)
         - 6*(Yu*Yu.adjoint()).trace() - 2*(Yv*Yv.adjoint()).trace()))).real();
   }
