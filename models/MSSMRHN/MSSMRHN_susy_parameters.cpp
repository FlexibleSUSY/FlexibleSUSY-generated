...
// File generated at Fri 20 Oct 2017 09:03:21
...
#define CLASSNAME MSSMRHN_susy_parameters
#define INPUT(parameter) input.parameter
#define TRACE_STRUCT susy_traces
#define TRACE_STRUCT_TYPE Susy_traces
#define CALCULATE_TRACES(l) calc_susy_traces(l);

const int MSSMRHN_susy_parameters::numberOfParameters;

MSSMRHN_susy_parameters::MSSMRHN_susy_parameters(const MSSMRHN_input_parameters& input_)
   : input(input_)
{
...
MSSMRHN_susy_parameters::MSSMRHN_susy_parameters(
   ...
   : Beta_function()
   , Yd(Yd_), Ye(Ye_), Yu(Yu_), Yv(Yv_), Mu(Mu_), Mv(Mv_), g1(g1_), g2(g2_), g3
   (g3_), vd(vd_), vu(vu_)
...
Eigen::ArrayXd MSSMRHN_susy_parameters::beta() const
{
   return calc_beta().get().unaryExpr(Chop<double>(get_zero_threshold()));
}

MSSMRHN_susy_parameters MSSMRHN_susy_parameters::calc_beta(int loops) const
{
   ...
   double beta_Mu = 0.;
   Eigen::Matrix<double,3,3> beta_Mv = Eigen::Matrix<double,3,3>::Zero();
   double beta_g1 = 0.;
   double beta_g2 = 0.;
   double beta_g3 = 0.;
   double beta_vd = 0.;
   ...
   return MSSMRHN_susy_parameters(get_scale(), loops, get_thresholds(), input,
                    beta_Yd, beta_Ye, beta_Yu, beta_Yv, beta_Mu, beta_Mv, beta_g1, beta_g2, beta_g3, beta_vd, beta_vu);
}

MSSMRHN_susy_parameters MSSMRHN_susy_parameters::calc_beta() const
{
...
void MSSMRHN_susy_parameters::clear()
{
   ...
   Mu = 0.;
   Mv = Eigen::Matrix<double,3,3>::Zero();
   g1 = 0.;
   g2 = 0.;
   g3 = 0.;
   vd = 0.;
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SqSq() const
{
   ...
   if (get_loops() > 1) {
      anomDim += (twoLoop*(0.8*Sqr(g1)*(Yu.adjoint()*Yu) - 2*(
         ...
         - Yu.adjoint()*Yu*(Yv*Yv.adjoint()).trace() + (0.22111111111111112*
         Quad(g1) + 3.75*Quad(g2) - 0.8888888888888888*Quad(g3) + 8*Sqr(g2)*Sqr
         (g3) + 0.011111111111111112*Sqr(g1)*(9*Sqr(g2) + 16*Sqr(g3)))*
         UNITMATRIX(3))).real();
   }
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SlSl() const
{
   ...
   anomDim = (oneOver16PiSqr*(Ye.adjoint()*Ye + Yv.adjoint()*Yv - 0.3*(
      Sqr(g1) + 5*Sqr(g2))*UNITMATRIX(3))).real();

   if (get_loops() > 1) {
      anomDim += (twoLoop*(-2*(Ye.adjoint()*Ye*Ye.adjoint()*Ye) - 2*(
         Yv.adjoint()*Yv*Yv.adjoint()*Yv) + Ye.adjoint()*Ye*(1.2*Sqr(g1) - 3*(
         Yd*Yd.adjoint()).trace() - (Ye*Ye.adjoint()).trace()) - 3*(Yv.adjoint(
         )*Yv)*(Yu*Yu.adjoint()).trace() - Yv.adjoint()*Yv*(Yv*Yv.adjoint())
...
double CLASSNAME::get_SHdSHd() const
{
   ...
   if (get_loops() > 1) {
      anomDim += Re(twoLoop*(2.07*Quad(g1) + 3.75*Quad(g2) + 0.9*Sqr(
         g1)*Sqr(g2) - 0.4*(Sqr(g1) - 40*Sqr(g3))*(Yd*Yd.adjoint()).trace() +
         1.2*Sqr(g1)*(Ye*Ye.adjoint()).trace() - 9*(Yd*Yd.adjoint()*Yd*
         Yd.adjoint()).trace() - 3*(Yd*Yu.adjoint()*Yu*Yd.adjoint()).trace() -
         3*(Ye*Ye.adjoint()*Ye*Ye.adjoint()).trace() - (Ye*Yv.adjoint()*Yv*
         Ye.adjoint()).trace()));
   }
...
double CLASSNAME::get_SHuSHu() const
{
   ...
   anomDim = Re(oneOver16PiSqr*(-0.3*(Sqr(g1) + 5*Sqr(g2)) + 3*(Yu*
      Yu.adjoint()).trace() + (Yv*Yv.adjoint()).trace()));

   if (get_loops() > 1) {
      anomDim += Re(twoLoop*(2.07*Quad(g1) + 3.75*Quad(g2) + 0.9*Sqr(
         g1)*Sqr(g2) + 0.8*(Sqr(g1) + 20*Sqr(g3))*(Yu*Yu.adjoint()).trace() - 3
         *(Yd*Yu.adjoint()*Yu*Yd.adjoint()).trace() - (Ye*Yv.adjoint()*Yv*
         Ye.adjoint()).trace() - 9*(Yu*Yu.adjoint()*Yu*Yu.adjoint()).trace() -
         3*(Yv*Yv.adjoint()*Yv*Yv.adjoint()).trace()));
   }
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SuRSuR() const
{
   ...
   if (get_loops() > 1) {
      anomDim += (twoLoop*(-0.4*(5*(Yu.conjugate()*Yd.transpose()*
         ...
         Yu.conjugate()*Yu.transpose()) + Yu.conjugate()*Yu.transpose()*(Sqr(g1
         ) - 15*Sqr(g2) + 15*(Yu*Yu.adjoint()).trace() + 5*(Yv*Yv.adjoint())
         .trace())) + 0.035555555555555556*(107*Quad(g1) - 25*Quad(g3) + 80*Sqr
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SeRSeR() const
{
   ...
   if (get_loops() > 1) {
      anomDim += (twoLoop*(-2*(Ye.conjugate()*Ye.transpose()*
         Ye.conjugate()*Ye.transpose() + Ye.conjugate()*Yv.transpose()*
         Yv.conjugate()*Ye.transpose()) + Ye.conjugate()*Ye.transpose()*(-1.2*
         Sqr(g1) + 6*Sqr(g2) - 6*(Yd*Yd.adjoint()).trace() - 2*(Ye*Ye.adjoint()
...
Eigen::Matrix<double,3,3> CLASSNAME::get_SvRSvR() const
{
   ...
   anomDim = (2*oneOver16PiSqr*(Yv.conjugate()*Yv.transpose())).real();

   if (get_loops() > 1) {
      anomDim += (twoLoop*(-2*(Yv.conjugate()*Ye.transpose()*
         Ye.conjugate()*Yv.transpose() + Yv.conjugate()*Yv.transpose()*
         Yv.conjugate()*Yv.transpose()) + Yv.conjugate()*Yv.transpose()*(1.2*
         Sqr(g1) + 6*Sqr(g2) - 6*(Yu*Yu.adjoint()).trace() - 2*(Yv*Yv.adjoint()
         ).trace()))).real();
   }
...
Eigen::ArrayXd MSSMRHN_susy_parameters::get() const
{
...
void MSSMRHN_susy_parameters::set(const Eigen::ArrayXd& pars)
{
   Yd(0,0) = pars(0);
   Yd(0,1) = pars(1);
   Yd(0,2) = pars(2);
...
const MSSMRHN_input_parameters& MSSMRHN_susy_parameters::get_input() const
{
   return input;
}

MSSMRHN_input_parameters& MSSMRHN_susy_parameters::get_input()
{
   return input;
}

void MSSMRHN_susy_parameters::set_input_parameters(const MSSMRHN_input_parameters& input_)
{
   input = input_;
}

MSSMRHN_susy_parameters::Susy_traces MSSMRHN_susy_parameters::calc_susy_traces(int loops) const
{
...
std::ostream& operator<<(std::ostream& ostr, const MSSMRHN_susy_parameters& susy_pars)
{
