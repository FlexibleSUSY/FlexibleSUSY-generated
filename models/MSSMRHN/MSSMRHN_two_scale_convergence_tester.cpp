...
// File generated at Wed 29 Jun 2016 12:43:17
...
MSSMRHN_convergence_tester<Two_scale>::MSSMRHN_convergence_tester(MSSMRHN<Two_scale>* model, double accuracy_goal)
   : Convergence_tester_DRbar<MSSMRHN<Two_scale> >(model, accuracy_goal)
{
}

MSSMRHN_convergence_tester<Two_scale>::~MSSMRHN_convergence_tester()
{
}

double MSSMRHN_convergence_tester<Two_scale>::max_rel_diff() const
{
   const MSSMRHN<Two_scale>& ol = get_last_iteration_model();
   const MSSMRHN<Two_scale>& ne = get_model();
   ...
   for (unsigned i = 0; i < 6; i++) {
      diff[i + 7] = MaxRelDiff(OLD1(MSu,i),NEW1(MSu,i));
   }
   for (unsigned i = 0; i < 6; i++) {
      diff[i + 13] = MaxRelDiff(OLD1(MSe,i),NEW1(MSe,i));
   }
   for (unsigned i = 0; i < 6; i++) {
      diff[i + 19] = MaxRelDiff(OLD1(MSv,i),NEW1(MSv,i));
   }
   for (unsigned i = 0; i < 2; i++) {
      diff[i + 25] = MaxRelDiff(OLD1(Mhh,i),NEW1(Mhh,i));
   }
