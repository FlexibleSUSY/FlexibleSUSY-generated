...
// File generated at Tue 27 Oct 2015 15:25:49
...
Eigen::Matrix<double,3,3> MSSMRHN_soft_parameters::calc_beta_mq2_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_mq2 = (twoLoop*(-6*traceconjTYdTpTYd*(Yd.adjoint()*Yd) - 2*
      ...
      Yu) - 2*traceconjTYvTpTYv*(Yu.adjoint()*Yu) - 2*traceml2AdjYvYv*(
      Yu.adjoint()*Yu) - 6*tracemq2AdjYuYu*(Yu.adjoint()*Yu) - 6*
      tracemu2YuAdjYu*(Yu.adjoint()*Yu) - 2*tracemv2YvAdjYv*(Yu.adjoint()*Yu) -
      12*mHu2*traceYuAdjYu*(Yu.adjoint()*Yu) - 4*mHu2*traceYvAdjYv*(Yu.adjoint
      ()*Yu) + 1.6*mHu2*Sqr(g1)*(Yu.adjoint()*Yu) - 6*traceconjTYuTpYu*(
      Yu.adjoint()*TYu) - 2*traceconjTYvTpYv*(Yu.adjoint()*TYu) - 6*
      ...
      .adjoint()*TYd) - 6*traceAdjYuTYu*((TYu).adjoint()*Yu) - 2*traceAdjYvTYv*
      ((TYu).adjoint()*Yu) - 1.6*MassB*Sqr(g1)*((TYu).adjoint()*Yu) - 6*
      traceYuAdjYu*((TYu).adjoint()*TYu) - 2*traceYvAdjYv*((TYu).adjoint()*TYu)
      + 1.6*Sqr(g1)*((TYu).adjoint()*TYu) - 3*traceYdAdjYd*(mq2*Yd.adjoint()*
      Yd) - traceYeAdjYe*(mq2*Yd.adjoint()*Yd) + 0.4*Sqr(g1)*(mq2*Yd.adjoint()*
      Yd) - 3*traceYuAdjYu*(mq2*Yu.adjoint()*Yu) - traceYvAdjYv*(mq2*Yu.adjoint
      ()*Yu) + 0.8*Sqr(g1)*(mq2*Yu.adjoint()*Yu) - 6*traceYdAdjYd*(Yd.adjoint()
      *md2*Yd) - 2*traceYeAdjYe*(Yd.adjoint()*md2*Yd) + 0.8*Sqr(g1)*(Yd.adjoint
      ()*md2*Yd) - 3*traceYdAdjYd*(Yd.adjoint()*Yd*mq2) - traceYeAdjYe*(
      Yd.adjoint()*Yd*mq2) + 0.4*Sqr(g1)*(Yd.adjoint()*Yd*mq2) - 6*traceYuAdjYu
      *(Yu.adjoint()*mu2*Yu) - 2*traceYvAdjYv*(Yu.adjoint()*mu2*Yu) + 1.6*Sqr(
      g1)*(Yu.adjoint()*mu2*Yu) - 3*traceYuAdjYu*(Yu.adjoint()*Yu*mq2) -
      traceYvAdjYv*(Yu.adjoint()*Yu*mq2) + 0.8*Sqr(g1)*(Yu.adjoint()*Yu*mq2) -
      ...
      *Tr23*UNITMATRIX(3) + 1.0327955589886444*g1*Tr31*UNITMATRIX(3) + 33*Power
      (g2,4)*AbsSqr(MassWB)*UNITMATRIX(3) + 0.13333333333333333*Tr2U111*Sqr(g1)
      *UNITMATRIX(3) + 0.4*AbsSqr(MassWB)*Sqr(g1)*Sqr(g2)*UNITMATRIX(3) + 0.2*
      MassB*Conj(MassWB)*Sqr(g1)*Sqr(g2)*UNITMATRIX(3) + 32*AbsSqr(MassWB)*Sqr(
      g2)*Sqr(g3)*UNITMATRIX(3) + 16*MassG*Conj(MassWB)*Sqr(g2)*Sqr(g3)*
      UNITMATRIX(3) + 0.35555555555555557*Conj(MassG)*Sqr(g3)*((MassB + 2*MassG
      )*Sqr(g1) + 15*(3*(2*MassG + MassWB)*Sqr(g2) - 8*MassG*Sqr(g3)))*
      ...
      + 16*(2*MassB + MassG)*Sqr(g3)))*UNITMATRIX(3)))).real();
