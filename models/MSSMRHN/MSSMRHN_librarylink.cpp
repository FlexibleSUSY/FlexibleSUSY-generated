...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSMRHNMessage", 1);
...
class MSSMRHN_spectrum {
public:
   virtual ~MSSMRHN_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSMRHN_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSMRHN_observables& get_observables() const = 0;
   virtual const MSSMRHN_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMRHN_input_parameters&) = 0;
...
class MSSMRHN_spectrum_impl : public MSSMRHN_spectrum
{
public:
   virtual ~MSSMRHN_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSMRHN_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSMRHN_observables& get_observables() const override { return observables; }
   virtual const MSSMRHN_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMRHN_input_parameters&) override;
...
private:
   std::tuple<MSSMRHN<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSMRHN_scales scales{};              ///< scale information
   MSSMRHN_observables observables{};    ///< observables
   MSSMRHN_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const MSSMRHN_input_parameters& input_) { input = input_; }
...
private:
   MSSMRHN_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<MSSMRHN_spectrum> spectrum{nullptr};  ///< spectrum information

   MSSMRHN_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSMSSMRHNCalculateSpectrum[]?");
   }
}
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(BMvInput), "BMvInput");
...
MSSMRHN_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const MSSMRHN_slha& model, MLINK link)
{
   MLPutRule(link, MSSMRHN_info::model_name);
   MLPutFunction(link, "List", 101);
   ...
   MLPutRuleTo(link, MODELPARAMETER(UV), "UV");
   ...
   MLPutRuleTo(link, PHYSICALPARAMETER(UV), "UV", {"Pole"});
   ...
   MLPutRuleTo(link, MODELPARAMETER(Mv), "Mv");
   MLPutRuleTo(link, MODELPARAMETER(g1), "g1");
   MLPutRuleTo(link, MODELPARAMETER(g2), "g2");
   MLPutRuleTo(link, MODELPARAMETER(g3), "g3");
   MLPutRuleTo(link, MODELPARAMETER(vd), "vd");
   ...
   MLPutRuleTo(link, MODELPARAMETER(BMv), "Mv", {"B"});
   ...
   MLPutRuleTo(link, MODELPARAMETER(mv2), "mv2");
...
template <typename Solver_type>
void MSSMRHN_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSMRHN_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MSSMRHN_input_parameters& input
)
{
   MSSMRHN_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void MSSMRHN_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MSSMRHN_spectrum_impl<Solver_type>::calculate_model_decays(
   const softsusy::QedQcd& qedqcd, const Physical_input& physical_input, const FlexibleDecay_settings& flexibledecay_settings)
{
   decays = MSSMRHN_decays(std::get<0>(models), qedqcd, physical_input, flexibledecay_settings);
...
template <typename Solver_type>
void MSSMRHN_spectrum_impl<Solver_type>::fill_slha_io(MSSMRHN_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   MSSMRHN_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   MSSMRHN_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = MSSMRHN_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMRHNCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMRHNCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSMRHN_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   INPUTPARAMETER(BMvInput(0,0)) = pars[c++];
   INPUTPARAMETER(BMvInput(0,1)) = pars[c++];
   INPUTPARAMETER(BMvInput(0,2)) = pars[c++];
   INPUTPARAMETER(BMvInput(1,0)) = pars[c++];
   INPUTPARAMETER(BMvInput(1,1)) = pars[c++];
   INPUTPARAMETER(BMvInput(1,2)) = pars[c++];
   INPUTPARAMETER(BMvInput(2,0)) = pars[c++];
   INPUTPARAMETER(BMvInput(2,1)) = pars[c++];
   INPUTPARAMETER(BMvInput(2,2)) = pars[c++];
...
DLLEXPORT int FSMSSMRHNGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMRHNGetSettings"))
...
DLLEXPORT int FSMSSMRHNGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMRHNGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMRHNGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMRHNGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMRHNOpenHandle(
...
DLLEXPORT int FSMSSMRHNCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MSSMRHN_librarylink;
...
DLLEXPORT int FSMSSMRHNGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMRHNGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMRHNToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMRHNToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMRHNGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMRHNGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMRHNCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMRHNCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMRHNCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMRHNCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMRHNCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMRHNCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMRHNCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSMSSMRHNCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMRHNCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMRHNCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMRHNCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMRHNCalculateDecays", "error", e.what());
      put_error_output(link);
   }
