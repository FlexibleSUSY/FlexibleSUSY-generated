...
// File generated at Mon 23 Feb 2015 13:43:36
...
template <class T>
class MSSMRHN_spectrum_generator {
public:
   MSSMRHN_spectrum_generator()
      : solver(), model()
   ...
   void run(const QedQcd& oneset, const MSSMRHN_input_parameters& input);
   void write_running_couplings(const std::string& filename = "MSSMRHN_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "MSSMRHN_spectrum.dat") const;

private:
   RGFlow<T> solver;
   MSSMRHN<T> model;
   MSSMRHN_high_scale_constraint<T> high_scale_constraint;
   MSSMRHN_susy_scale_constraint<T> susy_scale_constraint;
   MSSMRHN_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void MSSMRHN_spectrum_generator<T>::run(const QedQcd& oneset,
                                const MSSMRHN_input_parameters& input)
{
   ...
   MSSMRHN_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   MSSMRHN_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void MSSMRHN_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   MSSMRHN<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   MSSMRHN_parameter_getter parameter_getter;
   Coupling_monitor<MSSMRHN<T>, MSSMRHN_parameter_getter>
 ...
 */
template <class T>
void MSSMRHN_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   MSSMRHN_spectrum_plotter plotter;
