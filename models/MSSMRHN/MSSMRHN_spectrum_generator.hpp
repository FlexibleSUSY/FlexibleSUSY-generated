...
#include "MSSMRHN_spectrum_generator_interface.hpp"
...
class MSSMRHN_spectrum_generator
   : public MSSMRHN_spectrum_generator_interface<T> {
public:
   MSSMRHN_spectrum_generator()
      : MSSMRHN_spectrum_generator_interface<T>()
      , high_scale(0.)
      , susy_scale(0.)
      , low_scale(0.)
   {}
   virtual ~MSSMRHN_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const MSSMRHN_input_parameters&);
   void write_running_couplings(const std::string& filename = "MSSMRHN_rgflow.dat") const;
 ...
 */
template <class T>
void MSSMRHN_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const MSSMRHN_input_parameters& input)
{
   MSSMRHN<T>& model = this->model;
   model.clear();
   ...
   MSSMRHN_high_scale_constraint<T> high_scale_constraint(&model);
   MSSMRHN_susy_scale_constraint<T> susy_scale_constraint(&model, qedqcd);
   MSSMRHN_low_scale_constraint<T>  low_scale_constraint(&model, qedqcd);
   ...
   MSSMRHN_convergence_tester<T> convergence_tester(
   ...
   MSSMRHN_initial_guesser<T> initial_guesser(&model, qedqcd,
 ...
 */
template <class T>
void MSSMRHN_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   MSSMRHN_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
