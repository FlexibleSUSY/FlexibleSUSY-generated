...
/**
 * @file MSSMRHN_two_scale_model_slha.hpp
 * @brief contains wrapper class for model class in SLHA convention
 */

// File generated at Tue 12 Jul 2016 12:04:35

#ifndef MSSMRHN_TWO_SCALE_SLHA_H
#define MSSMRHN_TWO_SCALE_SLHA_H
...
/**
 * @class MSSMRHN_slha<Two_scale>
 * @brief model class wrapper in SLHA convention
 */

template<>
class MSSMRHN_slha<Two_scale> : public MSSMRHN<Two_scale> {
public:
   explicit MSSMRHN_slha(const MSSMRHN_input_parameters& input_ = MSSMRHN_input_parameters());
   explicit MSSMRHN_slha(const MSSMRHN<Two_scale>&, bool do_convert_masses_to_slha = true);
   virtual ~MSSMRHN_slha();
   ...
   const std::complex<double>& get_UV_slha(int i, int k) const { return UV_slha(i,k); }
...
private:
   ...
   Eigen::Matrix<std::complex<double>,6,6> UV_slha;
...
std::ostream& operator<<(std::ostream& ostr, const MSSMRHN_slha<Two_scale>& model);
