...
// File generated at Mon 23 Feb 2015 13:39:55
...
{
   assert(model && "Error: MSSMRHN_high_scale_constraint::apply():"
          " model pointer must not be zero");

   if (std::fabs(model->get_g1()) > 3.0) {
#ifdef ENABLE_VERBOSE
      ERROR("MSSMRHN_high_scale_constraint: Non-perturbative gauge "
   ...
   MODEL->set_mv2(Sqr(m0)*UNITMATRIX(3));
   MODEL->set_mHu2(Sqr(m0));
   MODEL->set_mHd2(Sqr(m0));
   MODEL->set_MassB(m12);
   MODEL->set_MassWB(m12);
   MODEL->set_MassG(m12);
   MODEL->set_BMv(BMvInput);
   MODEL->set_TYv(Azero*Yv);

   {
      ...
      if (MaxAbsValue(Yv) > 3.5449077018110318)
         model->get_problems().flag_non_perturbative_parameter_warning("Yv", MaxAbsValue(Yv), model->get_scale(), 3.5449077018110318);
      else
         model->get_problems().unflag_non_perturbative_parameter_warning("Yv");
...
void MSSMRHN_high_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
   model = cast_model<MSSMRHN<Two_scale>*>(model_);
}

void MSSMRHN_high_scale_constraint<Two_scale>::set_input_parameters(const MSSMRHN_input_parameters& inputPars_)
{
   inputPars = inputPars_;
}
...
{
   assert(model && "MSSMRHN_high_scale_constraint<Two_scale>::"
...
void MSSMRHN_high_scale_constraint<Two_scale>::update_scale()
{
   ...
   const MSSMRHN_soft_parameters beta_functions(model->calc_beta());
   ...
   if (errno == ERANGE) {
#ifdef ENABLE_VERBOSE
      ERROR("MSSMRHN_high_scale_constraint<Two_scale>: Overflow error"
