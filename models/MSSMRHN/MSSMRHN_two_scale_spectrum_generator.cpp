...
// File generated at Fri 20 Oct 2017 09:05:14
...
double MSSMRHN_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void MSSMRHN_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MSSMRHN_input_parameters& input)
{
   ...
   MSSMRHN_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MSSMRHN_ewsb_solver<Two_scale> >(ewsb_solver));

   MSSMRHN_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   MSSMRHN_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   MSSMRHN_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   MSSMRHN_convergence_tester<Two_scale> convergence_tester(
   ...
   MSSMRHN_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void MSSMRHN_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MSSMRHN_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void MSSMRHN_spectrum_generator<Two_scale>::calculate_spectrum()
{
