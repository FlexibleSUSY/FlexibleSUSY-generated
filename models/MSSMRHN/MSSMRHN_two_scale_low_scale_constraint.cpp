...
// File generated at Tue 24 Feb 2015 17:44:48
...
MSSMRHN_low_scale_constraint<Two_scale>::MSSMRHN_low_scale_constraint(
   ...
   const MSSMRHN_input_parameters& inputPars_, const QedQcd& oneset_)
   : Constraint<Two_scale>()
   , model(model_)
...
{
   assert(model && "Error: MSSMRHN_low_scale_constraint::apply():"
...
void MSSMRHN_low_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
   model = cast_model<MSSMRHN<Two_scale>*>(model_);
}

void MSSMRHN_low_scale_constraint<Two_scale>::set_input_parameters(const MSSMRHN_input_parameters& inputPars_)
{
   inputPars = inputPars_;
}

void MSSMRHN_low_scale_constraint<Two_scale>::set_sm_parameters(const QedQcd& oneset_)
{
   oneset = oneset_;
}

const QedQcd& MSSMRHN_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return oneset;
}
...
{
   assert(model && "MSSMRHN_low_scale_constraint<Two_scale>::"
...
double MSSMRHN_low_scale_constraint<Two_scale>::calculate_theta_w(double alpha_em_drbar)
{
   double theta_w = 0.;
 ...
 */
void MSSMRHN_low_scale_constraint<Two_scale>::recalculate_mw_pole()
{
   if (!model->get_thresholds())
      return;
