...
// File generated at Wed 12 Apr 2017 13:08:31
...
Eigen::Matrix<double,3,3> MSSMRHN_soft_parameters::calc_beta_mu2_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_mu2 = (twoLoop*(-0.8*(15*traceconjTYuTpTYu + 5*traceconjTYvTpTYv
      ...
      *(mu2*Yu*Yu.adjoint()*Yu*Yu.adjoint()) - 4*(Yu*mq2*Yd.adjoint()*Yd*
      ...
      Sqr(g1)*(321*MassB*Sqr(g1) + 40*(2*MassB + MassG)*Sqr(g3)) + 5*(30*Power(
      g3,4)*Tr23 - 11.618950038622252*g1*Tr31 + 6*Tr2U111*Sqr(g1) + 8*Conj(
      MassG)*Sqr(g3)*(2*(MassB + 2*MassG)*Sqr(g1) - 15*MassG*Sqr(g3))))*
      UNITMATRIX(3))).real();
