...
// File generated at Sun 26 Aug 2018 14:56:12
...
Eigen::Matrix<double,3,3> MSSMRHN_soft_parameters::calc_beta_mu2_2_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_mu2 = (twoLoop*(-0.8*(15*traceconjTYuTpTYu + 5*traceconjTYvTpTYv + 5*
      ...
      g2))*(Yu*Yu.adjoint()) + (0.8*MassB*Sqr(g1) - 4*(3*traceAdjYuTYu +
      traceAdjYvTYv + 3*MassWB*Sqr(g2)))*(Yu*(TYu).adjoint()) + (0.8*Conj(MassB
      )*Sqr(g1) - 4*(3*traceconjTYuTpYu + traceconjTYvTpYv + 3*Conj(MassWB)*Sqr
      (g2)))*(TYu*Yu.adjoint()) - 0.8*(Sqr(g1) + 5*(3*traceYuAdjYu +
      traceYvAdjYv - 3*Sqr(g2)))*(TYu*(TYu).adjoint()) + (-2*(3*traceYuAdjYu +
      traceYvAdjYv) - 0.4*Sqr(g1) + 6*Sqr(g2))*(mu2*Yu*Yu.adjoint()) - 0.8*(Sqr
      (g1) + 5*(3*traceYuAdjYu + traceYvAdjYv - 3*Sqr(g2)))*(Yu*mq2*Yu.adjoint(
      )) + (-2*(3*traceYuAdjYu + traceYvAdjYv) - 0.4*Sqr(g1) + 6*Sqr(g2))*(Yu*
