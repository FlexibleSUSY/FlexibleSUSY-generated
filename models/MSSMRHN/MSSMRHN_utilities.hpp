...
public:
   Eigen::ArrayXd get_parameters(const MSSMRHN<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const MSSMRHN<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const MSSMRHN<T>&);
...
template <class T>
void MSSMRHN_spectrum_plotter::extract_spectrum(const MSSMRHN<T>& model)
{
   spectrum.clear();
...
template <class Scalar, int M, int N>
std::valarray<double> MSSMRHN_spectrum_plotter::to_valarray(const Eigen::Array<Scalar, M, N>& v)
{
