...
// File generated at Tue 22 Jan 2019 13:51:49
...
CNMSSM_low_scale_constraint<Semi_analytic>::CNMSSM_low_scale_constraint(
...
double CNMSSM_low_scale_constraint<Semi_analytic>::get_scale() const
{
   return scale;
}

void CNMSSM_low_scale_constraint<Semi_analytic>::apply()
{
   check_model_ptr();
...
void CNMSSM_low_scale_constraint<Semi_analytic>::set_model(Model* model_)
{
   model = cast_model<CNMSSM<Semi_analytic>*>(model_);
}

void CNMSSM_low_scale_constraint<Semi_analytic>::clear()
{
   scale = 0.;
...
void CNMSSM_low_scale_constraint<Semi_analytic>::initialize()
{
   check_model_ptr();
...
void CNMSSM_low_scale_constraint<Semi_analytic>::update_scale()
{
   check_model_ptr();
...
void CNMSSM_low_scale_constraint<Semi_analytic>::calculate_threshold_corrections()
{
   check_model_ptr();
...
double CNMSSM_low_scale_constraint<Semi_analytic>::calculate_theta_w()
{
   check_model_ptr();

   double theta_w = std::asin(Electroweak_constants::sinThetaW);

   CNMSSM_weinberg_angle::Sm_parameters sm_pars;
...
void CNMSSM_low_scale_constraint<Semi_analytic>::calculate_DRbar_gauge_couplings()
{
   check_model_ptr();
...
double CNMSSM_low_scale_constraint<Semi_analytic>::calculate_delta_alpha_s(double alphaS) const
{
   check_model_ptr();
...
double CNMSSM_low_scale_constraint<Semi_analytic>::calculate_alpha_s_SM5_at(
...
void CNMSSM_low_scale_constraint<Semi_analytic>::calculate_Yd_DRbar()
{
   check_model_ptr();
...
void CNMSSM_low_scale_constraint<Semi_analytic>::calculate_Ye_DRbar()
{
   check_model_ptr();
...
{
   if (!model)
      throw SetupError("CNMSSM_low_scale_constraint<Semi_analytic>: "
