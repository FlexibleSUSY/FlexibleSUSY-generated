...
/**
 * @file CNMSSM_model_slha.hpp
 * @brief contains wrapper class for model class in SLHA convention
 */

// File generated at Sun 26 Aug 2018 13:59:21

#ifndef CNMSSM_SLHA_H
#define CNMSSM_SLHA_H
...
class CNMSSM_slha : public Model {
public:
   explicit CNMSSM_slha(const CNMSSM_input_parameters& input_ = CNMSSM_input_parameters());
   explicit CNMSSM_slha(const Model&, bool do_convert_masses_to_slha = true);
   CNMSSM_slha(const CNMSSM_slha&) = default;
   CNMSSM_slha(CNMSSM_slha&&) = default;
   virtual ~CNMSSM_slha() = default;
   CNMSSM_slha& operator=(const CNMSSM_slha&) = default;
   CNMSSM_slha& operator=(CNMSSM_slha&&) = default;
   ...
   const CNMSSM_physical& get_physical_slha() const; ///< returns pole masses to SLHA convention
   CNMSSM_physical& get_physical_slha(); ///< returns pole masses to SLHA convention
...
private:
   CNMSSM_physical physical_slha{}; ///< contains the pole masses and mixings in slha convention
...
template <class Model>
CNMSSM_slha<Model>::CNMSSM_slha(const CNMSSM_input_parameters& input_)
   : Model(input_)
{
}
 ...
 */
template <class Model>
CNMSSM_slha<Model>::CNMSSM_slha(const Model& model_,
...
template <class Model>
void CNMSSM_slha<Model>::calculate_spectrum()
{
...
template <class Model>
void CNMSSM_slha<Model>::convert_to_slha()
{
...
template <class Model>
void CNMSSM_slha<Model>::calculate_ckm_matrix()
{
...
template <class Model>
void CNMSSM_slha<Model>::calculate_pmns_matrix()
{
 ...
 */
template <class Model>
void CNMSSM_slha<Model>::convert_yukawa_couplings_to_slha()
{
 ...
 */
template <class Model>
void CNMSSM_slha<Model>::convert_trilinear_couplings_to_slha()
{
 ...
 */
template <class Model>
void CNMSSM_slha<Model>::convert_soft_squared_masses_to_slha()
{
...
template <class Model>
const CNMSSM_physical& CNMSSM_slha<Model>::get_physical_slha() const
{
   return physical_slha;
}

template <class Model>
CNMSSM_physical& CNMSSM_slha<Model>::get_physical_slha()
{
   return physical_slha;
}

template <class Model>
void CNMSSM_slha<Model>::print(std::ostream& ostr) const
{
   Model::print(ostr);
...
template <class Model>
void CNMSSM_slha<Model>::set_convert_masses_to_slha(bool flag)
{
   convert_masses_to_slha = flag;
}

template <class Model>
std::ostream& operator<<(std::ostream& ostr, const CNMSSM_slha<Model>& model)
{
   model.print(ostr);
   return ostr;
}
