...
// File generated at Wed 16 Oct 2019 19:33:28

#ifndef CNMSSM_SPECTRUM_GENERATOR_INTERFACE_H
#define CNMSSM_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class CNMSSM_spectrum_generator_interface {
public:
   virtual ~CNMSSM_spectrum_generator_interface() = default;

   std::tuple<CNMSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<CNMSSM_slha<CNMSSM<T>>> get_models_slha() const
   { return std::make_tuple(CNMSSM_slha<CNMSSM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const CNMSSM<T>& get_model() const
   { return model; }
   CNMSSM<T>& get_model()
   { return model; }
   CNMSSM_slha<CNMSSM<T>> get_model_slha() const
   { return CNMSSM_slha<CNMSSM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const CNMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "CNMSSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(CNMSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const CNMSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void CNMSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void CNMSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const CNMSSM_input_parameters& input)
{
 ...
 */
template <class T>
void CNMSSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   CNMSSM_mass_eigenstates tmp_model(model);
   try {
   ...
   CNMSSM_parameter_getter parameter_getter;
   Coupling_monitor<CNMSSM_mass_eigenstates, CNMSSM_parameter_getter>
 ...
 */
template <class T>
void CNMSSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   CNMSSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void CNMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(CNMSSM<T>& model)
{
   try {
      throw;
