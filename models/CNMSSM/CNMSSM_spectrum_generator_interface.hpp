...
public:
   ...
   std::tuple<CNMSSM<T>> get_models_slha() const
   { return std::make_tuple(CNMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const CNMSSM<T>& get_model() const
   { return model; }
   CNMSSM<T>& get_model()
   { return model; }
   CNMSSM<T> get_model_slha() const
   { return CNMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return CNMSSM_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(CNMSSM_info::parameter_names),
      std::cend(CNMSSM_info::parameter_names));
