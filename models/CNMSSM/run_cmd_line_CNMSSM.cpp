...
{
   std::cout <<
      "Usage: run_cmd_line_CNMSSM.x [options]\n"
      "Options:\n"
      "  --m12=<value>\n"
      "  --TanBeta=<value>\n"
      "  --SignvS=<value>\n"
...
int run_solver(int loop_library, const CNMSSM_input_parameters& input)
{
   ...
   CNMSSM_decays decays;if (settings.get(Spectrum_generator_settings::calculate_sm_masses)) {
...
int run(int solver_type, int loop_library, const CNMSSM_input_parameters& input)
{
   int exit_code = 0;
