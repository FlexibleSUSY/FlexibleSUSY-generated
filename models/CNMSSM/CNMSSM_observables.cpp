...
#include "observables/CNMSSM_br_l_to_3l.hpp"
#include "observables/CNMSSM_br_l_to_l_gamma.hpp"
#include "observables/CNMSSM_l_to_l_conversion.hpp"
...
const int CNMSSM_observables::NUMBER_OF_OBSERVABLES;

CNMSSM_observables::CNMSSM_observables()
   : amm_Fe_1(0)

{
}

Eigen::ArrayXd CNMSSM_observables::get() const
{
   Eigen::ArrayXd vec(CNMSSM_observables::NUMBER_OF_OBSERVABLES);
...
std::vector<std::string> CNMSSM_observables::get_names()
{
   std::vector<std::string> names(CNMSSM_observables::NUMBER_OF_OBSERVABLES);
...
void CNMSSM_observables::clear()
{
   amm_Fe_1 = 0.;

}

void CNMSSM_observables::set(const Eigen::ArrayXd& vec)
{
   assert(vec.rows() == CNMSSM_observables::NUMBER_OF_OBSERVABLES);

   amm_Fe_1 = vec(0);

}

CNMSSM_observables calculate_observables(const CNMSSM_mass_eigenstates& model,
                                              ...
                                              const Spectrum_generator_settings& settings)
{
   ...
   try {
      ...
      observables.AMM1(Fe, 1) = CNMSSM_amm::calculate_amm<CNMSSM_cxx_diagrams::fields::Fe>(MODEL, qedqcd, settings,1);
   } catch (const NonPerturbativeRunningError& e) {
