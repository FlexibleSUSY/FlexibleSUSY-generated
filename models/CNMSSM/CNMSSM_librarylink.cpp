...
// File generated at Fri 10 Apr 2020 17:56:05
...
public:
   ...
   virtual void fill_slha_io(CNMSSM_slha_io&) const = 0;
...
public:
   ...
   virtual void fill_slha_io(CNMSSM_slha_io&) const override;
...
private:
   std::tuple<CNMSSM_slha<CNMSSM<Solver_type>>> models{};        ///< running parameters and pole masses
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, CNMSSM_info::particle_names[i].c_str());
      }
   }
}
...
void put_spectrum(const CNMSSM_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void CNMSSM_spectrum_impl<Solver_type>::fill_slha_io(CNMSSM_slha_io& slha_io) const
{
