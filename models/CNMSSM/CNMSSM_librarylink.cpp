...
template <typename Solver_type>
void CNMSSM_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = CNMSSM_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSCNMSSMCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSCNMSSMCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
