...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSCNMSSMMessage", 1);
...
class CNMSSM_spectrum {
public:
   virtual ~CNMSSM_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(CNMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const CNMSSM_observables& get_observables() const = 0;
   virtual const CNMSSM_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const CNMSSM_input_parameters&) = 0;
...
class CNMSSM_spectrum_impl : public CNMSSM_spectrum
{
public:
   virtual ~CNMSSM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(CNMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const CNMSSM_observables& get_observables() const override { return observables; }
   virtual const CNMSSM_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const CNMSSM_input_parameters&) override;
...
private:
   std::tuple<CNMSSM<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   CNMSSM_scales scales{};              ///< scale information
   CNMSSM_observables observables{};    ///< observables
   CNMSSM_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const CNMSSM_input_parameters& input_) { input = input_; }
...
private:
   CNMSSM_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<CNMSSM_spectrum> spectrum{nullptr};  ///< spectrum information

   CNMSSM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSCNMSSMCalculateSpectrum[]?");
   }
}
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(SignvS), "SignvS");
...
CNMSSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const CNMSSM_slha& model, MLINK link)
{
   MLPutRule(link, CNMSSM_info::model_name);
   MLPutFunction(link, "List", 99);
...
template <typename Solver_type>
void CNMSSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void CNMSSM_spectrum_impl<Solver_type>::calculate_spectrum(
...
{
   CNMSSM_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void CNMSSM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void CNMSSM_spectrum_impl<Solver_type>::calculate_model_decays(
   const softsusy::QedQcd& qedqcd, const Physical_input& physical_input, const FlexibleDecay_settings& flexibledecay_settings)
{
   decays = CNMSSM_decays(std::get<0>(models), qedqcd, physical_input, flexibledecay_settings);
...
template <typename Solver_type>
void CNMSSM_spectrum_impl<Solver_type>::fill_slha_io(CNMSSM_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   CNMSSM_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   CNMSSM_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = CNMSSM_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSCNMSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSCNMSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_SEMI_ANALYTIC_SOLVER
   case 2:
      spectrum.reset(new CNMSSM_spectrum_impl<Semi_analytic>());
...
DLLEXPORT int FSCNMSSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMGetSettings"))
...
DLLEXPORT int FSCNMSSMGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCNMSSMGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCNMSSMOpenHandle(
...
DLLEXPORT int FSCNMSSMCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::CNMSSM_librarylink;
...
DLLEXPORT int FSCNMSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCNMSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCNMSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCNMSSMCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCNMSSMCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSCNMSSMCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSCNMSSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSCNMSSMCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSCNMSSMCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSCNMSSMCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSCNMSSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSCNMSSMCalculateDecays", "error", e.what());
      put_error_output(link);
   }
