...
// File generated at Fri 10 Apr 2020 17:55:48

#ifndef CNMSSM_SLHA_IO_H
#define CNMSSM_SLHA_IO_H
...
public:
   ...
   void fill(CNMSSM_input_parameters&) const;
   void fill(CNMSSM_mass_eigenstates&) const;
   template <class Model> void fill(CNMSSM_slha<Model>&) const;
   ...
   template <class Model> void set_extra(const CNMSSM_slha<Model>&, const CNMSSM_scales&, const CNMSSM_observables&);
   void set_input(const CNMSSM_input_parameters&);
   ...
   template <class Model> void set_spectrum(const CNMSSM_slha<Model>&);
   template <class T> void set_spectrum(const CNMSSM<T>&);
   ...
   static void fill_minpar_tuple(CNMSSM_input_parameters&, int, double);
   static void fill_extpar_tuple(CNMSSM_input_parameters&, int, double);
   static void fill_imminpar_tuple(CNMSSM_input_parameters&, int, double);
   static void fill_imextpar_tuple(CNMSSM_input_parameters&, int, double);

   template <class Model>
   static void fill_slhaea(SLHAea::Coll&, const CNMSSM_slha<Model>&, const softsusy::QedQcd&, const CNMSSM_scales&, const CNMSSM_observables&);

   template <class Model>
   static SLHAea::Coll fill_slhaea(const CNMSSM_slha<Model>&, const softsusy::QedQcd&, const CNMSSM_scales&, const CNMSSM_observables&);

private:
   SLHA_io slha_io; ///< SLHA io class
   bool print_imaginary_parts_of_majorana_mixings;

   void set_extpar(const CNMSSM_input_parameters&);
   void set_imminpar(const CNMSSM_input_parameters&);
   void set_imextpar(const CNMSSM_input_parameters&);
   void set_minpar(const CNMSSM_input_parameters&);
   void set_mass(const CNMSSM_physical&, bool);
   void set_mass(const standard_model::Standard_model_physical&);
   void set_mixing_matrices(const CNMSSM_physical&, bool);
   void set_mixing_matrices(const standard_model::Standard_model_physical&);
   template <class Model> void set_model_parameters(const CNMSSM_slha<Model>&);
   void set_model_parameters(const standard_model::Standard_model&);
   void set_ckm(const Eigen::Matrix<std::complex<double>,3,3>&, double);
   void set_pmns(const Eigen::Matrix<std::complex<double>,3,3>&, double);
   double read_scale() const;
   void fill_drbar_parameters(CNMSSM_mass_eigenstates&) const;
   void fill_physical(CNMSSM_physical&) const;
};
 ...
 */
template <class Model>
void CNMSSM_slha_io::fill(CNMSSM_slha<Model>& model) const
{
...
template <class Model>
void CNMSSM_slha_io::fill_slhaea(
   SLHAea::Coll& slhaea, const CNMSSM_slha<Model>& model,
   const softsusy::QedQcd& qedqcd, const CNMSSM_scales& scales,
   const CNMSSM_observables& observables)
{
   CNMSSM_slha_io slha_io;
   const CNMSSM_input_parameters& input = model.get_input();
...
template <class Model>
SLHAea::Coll CNMSSM_slha_io::fill_slhaea(
   const CNMSSM_slha<Model>& model, const softsusy::QedQcd& qedqcd,
   const CNMSSM_scales& scales, const CNMSSM_observables& observables)
{
   SLHAea::Coll slhaea;
   CNMSSM_slha_io::fill_slhaea(slhaea, model, qedqcd, scales, observables);
 ...
 */
template <class Model>
void CNMSSM_slha_io::set_model_parameters(const CNMSSM_slha<Model>& model)
{
   {
...
void CNMSSM_slha_io::set_extra(
   const CNMSSM_slha<Model>& model, const CNMSSM_scales& scales,
   const CNMSSM_observables& observables)
{
 ...
 */
template <class... Ts>
void CNMSSM_slha_io::set_spectrum(const std::tuple<Ts...>& models)
{
 ...
 */
template <class T>
void CNMSSM_slha_io::set_spectrum(const CNMSSM<T>& model)
{
   set_spectrum(CNMSSM_slha<CNMSSM<T> >(model));
}
 ...
 */
template <class Model>
void CNMSSM_slha_io::set_spectrum(const CNMSSM_slha<Model>& model)
{
   const CNMSSM_physical physical(model.get_physical_slha());
