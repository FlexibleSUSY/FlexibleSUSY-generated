...
// File generated at Mon 5 Mar 2018 15:30:14

/**
 * @file CNMSSM_mass_eigenstates.cpp
 ...
 * parameters.
 *
 * This file was generated at Mon 5 Mar 2018 15:30:14 with FlexibleSUSY
...
#define CLASSNAME CNMSSM_mass_eigenstates
...
CLASSNAME::CNMSSM_mass_eigenstates(const CNMSSM_input_parameters& input_)
   : CNMSSM_soft_parameters(input_)
#if defined(ENABLE_SEMI_ANALYTIC_SOLVER)
   , ewsb_solver(new CNMSSM_ewsb_solver<Semi_analytic>())
#endif
{
}
...
void CLASSNAME::set_ewsb_solver(const std::shared_ptr<CNMSSM_ewsb_solver_interface>& solver)
{
   ewsb_solver = solver;
}
...
{
   if (!ewsb_solver) {
      throw SetupError("CNMSSM_mass_eigenstates::solve_ewsb_tree_level: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("CNMSSM_mass_eigenstates::solve_ewsb_one_loop: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("CNMSSM_mass_eigenstates::solve_ewsb: "
                       "no EWSB solver set");
   }

   VERBOSE_MSG("\t\tSolving CNMSSM EWSB at " << ewsb_loop_order << "-loop order");
...
{
   if (PHYSICAL(MSd).tail<6>().minCoeff() < 0.) problems.flag_pole_tachyon(CNMSSM_info::Sd);
   if (PHYSICAL(MSv).tail<3>().minCoeff() < 0.) problems.flag_pole_tachyon(CNMSSM_info::Sv);
   if (PHYSICAL(MSu).tail<6>().minCoeff() < 0.) problems.flag_pole_tachyon(CNMSSM_info::Su);
   if (PHYSICAL(MSe).tail<6>().minCoeff() < 0.) problems.flag_pole_tachyon(CNMSSM_info::Se);
   if (PHYSICAL(Mhh).tail<3>().minCoeff() < 0.) problems.flag_pole_tachyon(CNMSSM_info::hh);
   if (PHYSICAL(MAh).tail<2>().minCoeff() < 0.) problems.flag_pole_tachyon(CNMSSM_info::Ah);
   if (PHYSICAL(MHpm).tail<1>().minCoeff() < 0.) problems.flag_pole_tachyon(CNMSSM_info::Hpm);
...
void CLASSNAME::calculate_MSd()
{
   ...
   if (MSd.minCoeff() < 0.) {
      problems.flag_running_tachyon(CNMSSM_info::Sd);
   }
...
void CLASSNAME::calculate_MSv()
{
   ...
   if (MSv.minCoeff() < 0.) {
      problems.flag_running_tachyon(CNMSSM_info::Sv);
   }
...
void CLASSNAME::calculate_MSu()
{
   ...
   if (MSu.minCoeff() < 0.) {
      problems.flag_running_tachyon(CNMSSM_info::Su);
   }
...
void CLASSNAME::calculate_MSe()
{
   ...
   if (MSe.minCoeff() < 0.) {
      problems.flag_running_tachyon(CNMSSM_info::Se);
   }
...
void CLASSNAME::calculate_Mhh()
{
   ...
   if (Mhh.minCoeff() < 0.) {
      problems.flag_running_tachyon(CNMSSM_info::hh);
   }
...
void CLASSNAME::calculate_MAh()
{
   ...
   if (MAh.minCoeff() < 0.) {
      problems.flag_running_tachyon(CNMSSM_info::Ah);
   }
...
void CLASSNAME::calculate_MHpm()
{
   ...
   if (MHpm.minCoeff() < 0.) {
      problems.flag_running_tachyon(CNMSSM_info::Hpm);
   }
...
void CLASSNAME::calculate_MVWm()
{
   ...
   if (MVWm < 0.) {
      problems.flag_running_tachyon(CNMSSM_info::VWm);
   }
...
{
   if (!force_output && problems.is_running_tachyon(CNMSSM_info::VZ))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(CNMSSM_info::VZ);
...
{
   if (!force_output && problems.is_running_tachyon(CNMSSM_info::Sd))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(CNMSSM_info::Sv))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(CNMSSM_info::Su))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(CNMSSM_info::Se))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(CNMSSM_info::hh))
      return;
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(CNMSSM_info::hh);
   else
      problems.unflag_no_pole_mass_convergence(CNMSSM_info::hh);
}

void CLASSNAME::calculate_MAh_pole()
{
   if (!force_output && problems.is_running_tachyon(CNMSSM_info::Ah))
      return;
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(CNMSSM_info::Ah);
   else
      problems.unflag_no_pole_mass_convergence(CNMSSM_info::Ah);
}

void CLASSNAME::calculate_MHpm_pole()
{
   if (!force_output && problems.is_running_tachyon(CNMSSM_info::Hpm))
      return;
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(CNMSSM_info::Hpm);
   else
      problems.unflag_no_pole_mass_convergence(CNMSSM_info::Hpm);
}
...
{
   if (!force_output && problems.is_running_tachyon(CNMSSM_info::VWm))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(CNMSSM_info::VWm);
...
std::ostream& operator<<(std::ostream& ostr, const CNMSSM_mass_eigenstates& model)
{
   model.print(ostr);
   return ostr;
}
