...
// File generated at Sun 10 Jan 2016 15:48:19
...
CMSSMNoFV_low_scale_constraint<Two_scale>::CMSSMNoFV_low_scale_constraint()
...
CMSSMNoFV_low_scale_constraint<Two_scale>::CMSSMNoFV_low_scale_constraint(
   CMSSMNoFV<Two_scale>* model_, const softsusy::QedQcd& qedqcd_)
   : Constraint<Two_scale>()
   , model(model_)
   , qedqcd(qedqcd_)
   , new_g1(0.)
...
void CMSSMNoFV_low_scale_constraint<Two_scale>::apply()
{
   assert(model && "Error: CMSSMNoFV_low_scale_constraint::apply():"
...
const Eigen::Matrix<std::complex<double>,3,3>& CMSSMNoFV_low_scale_constraint<Two_scale>::get_ckm()
{
   return ckm;
}

const Eigen::Matrix<std::complex<double>,3,3>& CMSSMNoFV_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}

double CMSSMNoFV_low_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double CMSSMNoFV_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void CMSSMNoFV_low_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
   model = cast_model<CMSSMNoFV<Two_scale>*>(model_);
}

void CMSSMNoFV_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& CMSSMNoFV_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void CMSSMNoFV_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
   initial_scale_guess = 0.;
   model = NULL;
...
void CMSSMNoFV_low_scale_constraint<Two_scale>::initialize()
{
   assert(model && "CMSSMNoFV_low_scale_constraint<Two_scale>::"
...
void CMSSMNoFV_low_scale_constraint<Two_scale>::update_scale()
{
   assert(model && "CMSSMNoFV_low_scale_constraint<Two_scale>::"
...
void CMSSMNoFV_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
...
double CMSSMNoFV_low_scale_constraint<Two_scale>::calculate_theta_w(double alpha_em_drbar)
{
   assert(model && "CMSSMNoFV_low_scale_constraint<Two_scale>::"
...
void CMSSMNoFV_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   assert(model && "CMSSMNoFV_low_scale_constraint<Two_scale>::"
...
double CMSSMNoFV_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   assert(model && "CMSSMNoFV_low_scale_constraint<Two_scale>::"
...
double CMSSMNoFV_low_scale_constraint<Two_scale>::calculate_delta_alpha_s(double alphaS) const
{
   assert(model && "CMSSMNoFV_low_scale_constraint<Two_scale>::"
...
void CMSSMNoFV_low_scale_constraint<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
...
void CMSSMNoFV_low_scale_constraint<Two_scale>::calculate_Yu_DRbar()
{
   assert(model && "CMSSMNoFV_low_scale_constraint<Two_scale>::"
   ...
   if (model->get_thresholds()) {
      topDRbar(2,2) = MODEL->calculate_MFt_DRbar(qedqcd.displayPoleMt());
   }
...
void CMSSMNoFV_low_scale_constraint<Two_scale>::calculate_Yd_DRbar()
{
   assert(model && "CMSSMNoFV_low_scale_constraint<Two_scale>::"
   ...
   if (model->get_thresholds()) {
      bottomDRbar(2,2) = MODEL->calculate_MFb_DRbar(qedqcd.displayMass(softsusy::mBottom));
   }
...
void CMSSMNoFV_low_scale_constraint<Two_scale>::calculate_Ye_DRbar()
{
   assert(model && "CMSSMNoFV_low_scale_constraint<Two_scale>::"
   ...
   if (model->get_thresholds()) {
      electronDRbar(0,0) = MODEL->calculate_MFe_DRbar(qedqcd.displayMass(softsusy::mElectron));
      electronDRbar(1,1) = MODEL->calculate_MFm_DRbar(qedqcd.displayMass(softsusy::mMuon));
      electronDRbar(2,2) = MODEL->calculate_MFtau_DRbar(qedqcd.displayMass(softsusy::mTau));
   }
...
void CMSSMNoFV_low_scale_constraint<Two_scale>::calculate_MNeutrino_DRbar()
{
   assert(model && "CMSSMNoFV_low_scale_constraint<Two_scale>::"
 ...
 */
void CMSSMNoFV_low_scale_constraint<Two_scale>::recalculate_mw_pole()
{
   assert(model && "CMSSMNoFV_low_scale_constraint<Two_scale>::"
   ...
   if (mw_pole_sqr < 0.)
      MODEL->get_problems().flag_tachyon(CMSSMNoFV_info::VWm);
