...
#include "cxx_qft/CMSSMNoFV_particle_aliases.hpp"
#include "CMSSMNoFV_FFV_form_factors.hpp"
...
double get_MLCP(const CMSSMNoFV_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(CMSSMNoFV_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("CMSSMNoFV_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("CMSSMNoFV_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("CMSSMNoFV_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("CMSSMNoFV_amm: run_to_MSUSY(): "
...
template <typename Lepton>
std::enable_if_t<std::is_same_v<Lepton, CMSSMNoFV_cxx_diagrams::fields::Fm>, double>
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const CMSSMNoFV_mass_eigenstates& model, double ml_pole)
{
   using namespace CMSSMNoFV_cxx_diagrams::fields;

   const auto form_factors = CMSSMNoFV_FFV_form_factors::calculate_form_factors<Lepton,Lepton,VP>(model, true);
...
template <typename Lepton>
double calculate_amm_impl(const CMSSMNoFV_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings)
{
   VERBOSE_MSG("CMSSMNoFV_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double CMSSMNoFV_amm::calculate_amm(const CMSSMNoFV_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings)
{
   CMSSMNoFV_mass_eigenstates model(model_);

   VERBOSE_MSG("CMSSMNoFV_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("CMSSMNoFV_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template double CMSSMNoFV_amm::calculate_amm<CMSSMNoFV_cxx_diagrams::fields::Fm>(const CMSSMNoFV_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&);

template <typename Lepton>
double CMSSMNoFV_amm::calculate_amm_uncertainty(const CMSSMNoFV_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings)
{
   CMSSMNoFV_mass_eigenstates model(model_);

   VERBOSE_MSG("CMSSMNoFV_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("CMSSMNoFV_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
// spliting templated function into header and cpp
// https://isocpp.org/wiki/faq/templates#separate-template-fn-defn-from-decl
template double CMSSMNoFV_amm::calculate_amm_uncertainty<CMSSMNoFV_cxx_diagrams::fields::Fm>(const CMSSMNoFV_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&);
