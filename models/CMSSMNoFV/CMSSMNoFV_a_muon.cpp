...
// File generated at Tue 22 Jan 2019 17:48:01

/**
 * @file CMSSMNoFV_a_muon.cpp
 *
 * This file was generated at Tue 22 Jan 2019 17:48:01 with FlexibleSUSY
...
double get_MSUSY(const CMSSMNoFV_mass_eigenstates& model)
{
...
void run_to_MSUSY(CMSSMNoFV_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("CMSSMNoFV_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("CMSSMNoFV_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("CMSSMNoFV_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("CMSSMNoFV_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(CMSSMNoFV_mass_eigenstates& model)
{
   VERBOSE_MSG("CMSSMNoFV_a_muon: calculating a_mu at Q = " << model.get_scale());
...
std::pair<double,double> vary_scale(const CMSSMNoFV_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("CMSSMNoFV_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double CMSSMNoFV_a_muon::calculate_a_muon(const CMSSMNoFV_mass_eigenstates& model_)
{
   CMSSMNoFV_mass_eigenstates model(model_);

   VERBOSE_MSG("CMSSMNoFV_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("CMSSMNoFV_a_muon:" << e.what());
...
double CMSSMNoFV_a_muon::calculate_a_muon_uncertainty(const CMSSMNoFV_mass_eigenstates& model_)
{
   CMSSMNoFV_mass_eigenstates model(model_);

   VERBOSE_MSG("CMSSMNoFV_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("CMSSMNoFV_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
