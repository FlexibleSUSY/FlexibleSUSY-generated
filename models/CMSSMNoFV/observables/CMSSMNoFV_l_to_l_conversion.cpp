...
/**
 * @file CMSSMNoFV_l_to_l_conversion.cpp
 *
 * This file was generated at Sun 25 Feb 2024 21:08:18 with FlexibleSUSY
...
#include "CMSSMNoFV_l_to_l_conversion.hpp"
...
#include "cxx_qft/CMSSMNoFV_npointfunctions_wilsoncoeffs.hpp"
...
double get_MSUSY(const CMSSMNoFV_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(CMSSMNoFV_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("CMSSMNoFV_l_to_l_conversion: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("CMSSMNoFV_l_to_l_conversion:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("CMSSMNoFV_l_to_l_conversion: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("CMSSMNoFV_l_to_l_conversion: run_to_MSUSY(): "
...
using namespace CMSSMNoFV_FFV_form_factors;

namespace CMSSMNoFV_l_to_l_conversion {
...
typedef std::valarray<std::complex<double>>  (*ffv_function)
   (int, int, const CMSSMNoFV_mass_eigenstates&, bool);

typedef std::array<std::complex<double>, 10> (*npf_function)
   (const CMSSMNoFV_mass_eigenstates&, const std::array<int,4>&, const std::array<Eigen::Vector4d,0>&);
...
std::complex<double> left(const CMSSMNoFV_mass_eigenstates& model, int g) {
 ...
 */
template <class Q, class A>
std::complex<double> right(const CMSSMNoFV_mass_eigenstates& model, int g) {
    context_base context {model};
    using vertex = Vertex<typename Q::lorentz_conjugate, Q, A>;
    std::array<int, 2> indices {g, g};
    const auto value =  vertex::evaluate(indices, context);
...
Eigen::Array<std::complex<double>,10,1> embed_photon(
   const CMSSMNoFV_mass_eigenstates& model, const T& ff, int g) {
...
Eigen::Array<std::complex<double>,10,1> fix_tensors_sign(
   const CMSSMNoFV_mass_eigenstates& model, int in, int out, int g) {
 ...
 *    <pi, q| T exp(i L_CMSSMNoFV dx)|pj> =
 *    i * ubari
 ...
 *       for photon, as in CMSSMNoFV, using eq. (t.3).
 ...
 *          <out:q4,l3| T exp(i L_CMSSMNoFV dx) |in:q2,l1> =: G4
...
Eigen::Array<std::complex<double>, 13, 1> forge_conversion(int in, int out,
   const CMSSMNoFV_l_to_l_conversion::Nucleus nucleus,
   ...
   try {
      ...
      ERROR("CMSSMNoFV_l_to_l_conversion:" << e.what_detailed());
