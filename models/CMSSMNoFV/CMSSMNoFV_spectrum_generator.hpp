...
// File generated at Mon 27 Feb 2017 13:52:43

#ifndef CMSSMNoFV_SPECTRUM_GENERATOR_H
#define CMSSMNoFV_SPECTRUM_GENERATOR_H

#include "CMSSMNoFV_spectrum_generator_interface.hpp"
...
#include "CMSSMNoFV_two_scale_initial_guesser.hpp"
...
template <class T>
class CMSSMNoFV_spectrum_generator
   : public CMSSMNoFV_spectrum_generator_interface<T> {
public:
   CMSSMNoFV_spectrum_generator()
      : CMSSMNoFV_spectrum_generator_interface<T>()
      , high_scale(0.)
      , susy_scale(0.)
      , low_scale(0.)
   {}
   virtual ~CMSSMNoFV_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const CMSSMNoFV_input_parameters&);
   void write_running_couplings(const std::string& filename = "CMSSMNoFV_rgflow.dat") const;
 ...
 */
template <class T>
void CMSSMNoFV_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const CMSSMNoFV_input_parameters& input)
{
   CMSSMNoFV<T>& model = this->model;
   model.clear();
   ...
   CMSSMNoFV_high_scale_constraint<T> high_scale_constraint(&model);
   CMSSMNoFV_susy_scale_constraint<T> susy_scale_constraint(&model, qedqcd);
   CMSSMNoFV_low_scale_constraint<T>  low_scale_constraint(&model, qedqcd);
   ...
   CMSSMNoFV_convergence_tester<T> convergence_tester(
   ...
   CMSSMNoFV_initial_guesser<T> initial_guesser(&model, qedqcd,
 ...
 */
template <class T>
void CMSSMNoFV_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   CMSSMNoFV_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
