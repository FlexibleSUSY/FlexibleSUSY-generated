...
// File generated at Wed 12 Apr 2017 14:10:26
...
struct Model_data {
   ...
   CMSSMNoFV_slha<algorithm_type> model; ///< running parameters and pole masses
};
...
void put_spectrum(const CMSSMNoFV_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const CMSSMNoFV_observables& observables, MLINK link)
{
...
void check_spectrum(const Model_data& data, MLINK link)
{
   ...
   if (problems.have_problem()) {
      ...
      put_message(link, "FSCMSSMNoFVCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSCMSSMNoFVCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(Model_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSCMSSMNoFVCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = CMSSMNoFV_slha<algorithm_type>(
