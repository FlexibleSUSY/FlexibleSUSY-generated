...
public:
   virtual ~CMSSMNoFV_spectrum_generator_interface() = default;

   std::tuple<CMSSMNoFV<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<CMSSMNoFV<T>> get_models_slha() const
   { return std::make_tuple(CMSSMNoFV<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const CMSSMNoFV<T>& get_model() const
   { return model; }
   CMSSMNoFV<T>& get_model()
   { return model; }
   CMSSMNoFV<T> get_model_slha() const
   { return CMSSMNoFV<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const CMSSMNoFV_input_parameters&);
...
protected:
   ...
   void translate_exception_to_problem(CMSSMNoFV<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const CMSSMNoFV_input_parameters&) = 0;
};
 ...
 */
template <class T>
void CMSSMNoFV_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const CMSSMNoFV_input_parameters& input)
{
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return CMSSMNoFV_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(CMSSMNoFV_info::parameter_names),
      std::cend(CMSSMNoFV_info::parameter_names));
 ...
 */
template <class T>
void CMSSMNoFV_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   CMSSMNoFV_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void CMSSMNoFV_spectrum_generator_interface<T>::translate_exception_to_problem(CMSSMNoFV<T>& model)
{
   try {
      throw;
