...
{
   return std::make_unique<CMSSMNoFV_mass_eigenstates>(*this);
}
...
double CLASSNAME::calculate_MVZ_DRbar(double m_pole) const
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(CMSSMNoFV_info::VZ);return m_pole;
   }
...
double CLASSNAME::calculate_MVWm_DRbar(double m_pole) const
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(CMSSMNoFV_info::VWm);return m_pole;
   }
