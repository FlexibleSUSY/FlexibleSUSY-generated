...
// File generated at Sat 15 Oct 2016 16:01:04
...
Eigen::Matrix<double,3,3> E6SSMtower_soft_parameters::calc_beta_TYu_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_TYu = (twoLoop*(8.695555555555556*Power(g1,4)*TYu + 16.5*Power(g2
      ...
      *Sqr(gN)*TYu + 0.5366666666666666*Sqr(g1)*Sqr(gN)*TYu + 0.75*Sqr(g2)*Sqr(
      ...
      Conj(Lambdax)*TLambdax)*(Yu*Yd.adjoint()*Yd) + (-6*traceYdAdjYd - 2*
      traceYeAdjYe - 2*AbsSqr(Lambdax) + 0.8*Sqr(g1) + 1.2*Sqr(gN))*(Yu*
      Yd.adjoint()*TYd) + (-18*traceAdjYuTYu - 0.8*MassB*Sqr(g1) - 12*MassWB*
      Sqr(g2) - 1.2*MassBp*Sqr(gN) - 6*Conj(Lambdax)*TLambdax)*(Yu*Yu.adjoint()
      *Yu) + (-12*traceYuAdjYu - 4*AbsSqr(Lambdax) + 1.2*Sqr(g1) + 6*Sqr(g2) +
      0.8*Sqr(gN))*(Yu*Yu.adjoint()*TYu) + (-3*traceYdAdjYd - traceYeAdjYe -
      AbsSqr(Lambdax) + 0.4*Sqr(g1) + 0.6*Sqr(gN))*(TYu*Yd.adjoint()*Yd) + (-15
