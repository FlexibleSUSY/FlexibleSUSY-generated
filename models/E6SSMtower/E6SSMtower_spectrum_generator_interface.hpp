...
template <class T>
class E6SSMtower_spectrum_generator_interface {
public:
   E6SSMtower_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~E6SSMtower_spectrum_generator_interface() {}

   const E6SSMtower<T>& get_model() const { return model; }
   E6SSMtower<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const E6SSMtower_input_parameters&) = 0;
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "E6SSMtower_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(E6SSMtower<T>& model);
};
 ...
 */
template <class T>
void E6SSMtower_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void E6SSMtower_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   E6SSMtower_mass_eigenstates tmp_model(model);
   try {
   ...
   E6SSMtower_parameter_getter parameter_getter;
   Coupling_monitor<E6SSMtower_mass_eigenstates, E6SSMtower_parameter_getter>
 ...
 */
template <class T>
void E6SSMtower_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   E6SSMtower_spectrum_plotter plotter;
 ...
 */
template <class T>
void E6SSMtower_spectrum_generator_interface<T>::translate_exception_to_problem(E6SSMtower<T>& model)
{
   try {
      throw;
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : E6SSMtower_info::parameter_names[parameter_index];
