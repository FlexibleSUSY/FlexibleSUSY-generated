...
// File generated at Wed 12 Apr 2017 12:36:23

#include "E6SSMtower_info.hpp"
#include "E6SSMtower_input_parameters.hpp"
#include "E6SSMtower_observables.hpp"
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSE6SSMtowerMessage", 1);
...
struct Model_data {
   ...
   E6SSMtower_input_parameters input;     ///< model input parameters
   ...
   E6SSMtower_slha<algorithm_type> model; ///< running parameters and pole masses
};
...
void put_spectrum(const E6SSMtower_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const E6SSMtower_observables& observables, MLINK link)
{
...
void check_spectrum(const Model_data& data, MLINK link)
{
   ...
   if (problems.have_problem()) {
      ...
      put_message(link, "FSE6SSMtowerCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSE6SSMtowerCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(Model_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSE6SSMtowerCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = E6SSMtower_slha<algorithm_type>(
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      if (data.model.get_scale() == 0.) {
         put_message(link,
            "FSE6SSMtowerCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSE6SSMtowerCalculateSpectrum[]?");
      }

      E6SSMtower_observables observables;
      ...
      put_message(link, "FSE6SSMtowerCalculateObservables", "error", e.what());
      put_error_output(link);
   }
