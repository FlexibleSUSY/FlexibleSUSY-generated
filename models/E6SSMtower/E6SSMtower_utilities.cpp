...
#include "E6SSMtower_utilities.hpp"
...
E6SSMtower_spectrum_plotter::E6SSMtower_spectrum_plotter()
   : spectrum()
   , scale(0.0)
   , width(16)
{
...
void E6SSMtower_spectrum_plotter::extract_spectrum(const E6SSMtower_mass_eigenstates& model)
{
   spectrum.clear();
...
void E6SSMtower_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("E6SSMtower_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("E6SSMtower_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("E6SSMtower_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void E6SSMtower_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   for (std::size_t s = 0; s < spectrum.size(); ++s) {
      if (!filestr.good()) {
         ERROR("E6SSMtower_spectrum_plotter::write_spectrum: "
...
std::valarray<double> E6SSMtower_spectrum_plotter::to_valarray(double v)
{
   return std::valarray<double>(&v, 1);
}

namespace E6SSMtower_database {
 ...
 */
void to_database(
   const std::string& file_name, const E6SSMtower_mass_eigenstates& model,
   ...
   const E6SSMtower_observables* observables)
{
   ...
   append(names, E6SSMtower_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, E6SSMtower_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill pole masses and mixings
   append(names, E6SSMtower_parameter_getter().get_mass_names());
   append(names, E6SSMtower_parameter_getter().get_mixing_names());
   ...
   if (E6SSMtower_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, E6SSMtower_observables::get_names());
 ...
 */
E6SSMtower_mass_eigenstates from_database(
   const std::string& file_name, std::size_t entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, E6SSMtower_observables* observables)
{
   using utilities::append;

   E6SSMtower_mass_eigenstates model;
   const std::size_t number_of_parameters = model.get_number_of_parameters();
   const std::size_t number_of_masses = E6SSMtower_parameter_getter().get_number_of_masses();
   const std::size_t number_of_mixings = E6SSMtower_info::NUMBER_OF_MIXINGS;
   const std::size_t number_of_input_parameters = E6SSMtower_info::NUMBER_OF_INPUT_PARAMETERS;
   ...
   const std::size_t number_of_observables =
      (observables ? E6SSMtower_observables::NUMBER_OF_OBSERVABLES : 0u);
