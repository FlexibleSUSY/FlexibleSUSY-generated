 ...
 */
template <class T>
void E6SSMtower_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const E6SSMtower_input_parameters& input)
{
   ...
   // convergence tester for E6SSMtower
   E6SSMtower_convergence_tester<T> model_ct(
   ...
   E6SSMtower_standard_model_initial_guesser<T> initial_guesser(&model, &eft, qedqcd,
   ...
   try {
      ...
      // start logarithmic resummation
      {
         ...
         if (eft.get_problems().is_tachyon(standard_model_info::hh))
            this->model.get_problems().flag_tachyon(E6SSMtower_info::hh);
         if (eft.get_problems().is_tachyon(standard_model_info::VZ))
            this->model.get_problems().flag_tachyon(E6SSMtower_info::VZ);
         if (eft.get_problems().is_tachyon(standard_model_info::VWp))
            this->model.get_problems().flag_tachyon(E6SSMtower_info::VWm);
      }
