...
struct E6SSMtower_observables;
class Physical_input;

class E6SSMtower_parameter_getter {
public:
   Eigen::ArrayXd get_parameters(const E6SSMtower_mass_eigenstates& model) {
      return model.get();
   }
...
class E6SSMtower_spectrum_plotter {
public:
   void extract_spectrum(const E6SSMtower_mass_eigenstates&);
...
template <class Scalar, int M, int N>
std::valarray<double> E6SSMtower_spectrum_plotter::to_valarray(const Eigen::Array<Scalar, M, N>& v)
{
...
void to_database(
   const std::string&,
   const E6SSMtower_mass_eigenstates&,
   const softsusy::QedQcd* qedqcd = 0,
   const Physical_input* physical_input = 0,
   const E6SSMtower_observables* observables = 0);
...
E6SSMtower_mass_eigenstates from_database(
   ...
   E6SSMtower_observables* observables = 0);
