...
// File generated at Fri 8 Jan 2016 11:44:05

#ifndef HSSUSY_SPECTRUM_GENERATOR_H
#define HSSUSY_SPECTRUM_GENERATOR_H

#include "HSSUSY_spectrum_generator_interface.hpp"
...
#include "HSSUSY_two_scale_convergence_tester.hpp"
...
template <class T>
class HSSUSY_spectrum_generator
   : public HSSUSY_spectrum_generator_interface<T> {
public:
   HSSUSY_spectrum_generator()
      : HSSUSY_spectrum_generator_interface<T>()
      , solver()
      ...
      , low_scale(0.)
   {}
   virtual ~HSSUSY_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const HSSUSY_input_parameters&);
   void write_running_couplings(const std::string& filename = "HSSUSY_rgflow.dat") const;

private:
   RGFlow<T> solver;
   HSSUSY_high_scale_constraint<T> high_scale_constraint;
   HSSUSY_susy_scale_constraint<T> susy_scale_constraint;
   HSSUSY_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void HSSUSY_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const HSSUSY_input_parameters& input)
{
   HSSUSY<T>& model = this->model;
   model.clear();
   ...
   HSSUSY_convergence_tester<T> convergence_tester(&model, this->precision_goal);
   if (this->max_iterations > 0)
      convergence_tester.set_max_iterations(this->max_iterations);

   HSSUSY_initial_guesser<T> initial_guesser(&model, qedqcd,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
   ...
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : HSSUSY_info::parameter_names[parameter_index];
 ...
 */
template <class T>
void HSSUSY_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   HSSUSY_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
