...
// File generated at Sun 28 Aug 2016 15:03:06
...
HSSUSY_spectrum_plotter::HSSUSY_spectrum_plotter()
   : spectrum()
   , scale(0.0)
   , width(16)
{
...
void HSSUSY_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name.c_str(), std::ios::out);
   VERBOSE_MSG("HSSUSY_spectrum_plotter::write_to_file: opening file: "
               << file_name.c_str());
   if (filestr.fail()) {
      ERROR("HSSUSY_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("HSSUSY_spectrum_plotter::write_to_file: file written: "
               << file_name.c_str());
}

void HSSUSY_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   for (std::size_t s = 0; s < spectrum.size(); ++s) {
      if (!filestr.good()) {
         ERROR("HSSUSY_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const HSSUSY_mass_eigenstates& model,
   ...
   const HSSUSY_observables* observables)
{
   ...
   append(names, HSSUSY_parameter_getter().get_mass_names());
   append(names, HSSUSY_parameter_getter().get_mixing_names());
   ...
   if (HSSUSY_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, HSSUSY_observables::get_names());
 ...
 */
HSSUSY_mass_eigenstates from_database(
   ...
   Physical_input* physical_input, HSSUSY_observables* observables)
{
   ...
   const std::size_t number_of_masses = HSSUSY_parameter_getter().get_number_of_masses();
   const std::size_t number_of_mixings = HSSUSY_info::NUMBER_OF_MIXINGS;
   const std::size_t number_of_input_parameters = HSSUSY_info::NUMBER_OF_INPUT_PARAMETERS;
   ...
   const std::size_t number_of_observables =
      (observables ? HSSUSY_observables::NUMBER_OF_OBSERVABLES : 0u);
