...
double HSSUSY_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void HSSUSY_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const HSSUSY_input_parameters& input)
{
   ...
   HSSUSY_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<HSSUSY_ewsb_solver<Two_scale> >(ewsb_solver));

   HSSUSY_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   HSSUSY_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   HSSUSY_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   HSSUSY_convergence_tester<Two_scale> convergence_tester(
   ...
   HSSUSY_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void HSSUSY_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   HSSUSY_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void HSSUSY_spectrum_generator<Two_scale>::calculate_spectrum()
{
