...
{
   return std::make_unique<HSSUSY_mass_eigenstates>(*this);
}
...
double CLASSNAME::calculate_MVZ_DRbar(double m_pole) const
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(HSSUSY_info::VZ);return m_pole;
   }
...
double CLASSNAME::calculate_MVWp_DRbar(double m_pole) const
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(HSSUSY_info::VWp);return m_pole;
   }
