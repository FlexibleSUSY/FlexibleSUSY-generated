...
// File generated at Tue 22 Jan 2019 16:38:00
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSHSSUSYMessage", 1);
...
class HSSUSY_spectrum_impl : public HSSUSY_spectrum
{
public:
   ...
   virtual void fill_slha_io(HSSUSY_slha_io&) const override;
...
private:
   std::tuple<HSSUSY_slha<HSSUSY<Solver_type>>> models{};        ///< running parameters and pole masses
...
public:
   ...
   void set_input_parameters(const HSSUSY_input_parameters& input_) { input = input_; }
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(TwoLoopAtAs), "TwoLoopAtAs");
   MLPutRuleTo(link, INPUTPARAMETER(TwoLoopAbAs), "TwoLoopAbAs");
   MLPutRuleTo(link, INPUTPARAMETER(TwoLoopAtAb), "TwoLoopAtAb");
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, HSSUSY_info::particle_names[i].c_str());
      }
   }
}
...
HSSUSY_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const HSSUSY_slha<Model>& model, MLINK link)
{
   MLPutRule(link, HSSUSY_info::model_name);
   MLPutFunction(link, "List", 46);
...
template <typename Solver_type>
void HSSUSY_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void HSSUSY_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const HSSUSY_input_parameters& input
)
{
...
template <typename Solver_type>
void HSSUSY_spectrum_impl<Solver_type>::fill_slha_io(HSSUSY_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   HSSUSY_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSHSSUSYCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSHSSUSYCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new HSSUSY_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   INPUTPARAMETER(TwoLoopAtAs) = pars[c++];
   INPUTPARAMETER(TwoLoopAbAs) = pars[c++];
   INPUTPARAMETER(TwoLoopAtAb) = pars[c++];
...
DLLEXPORT int FSHSSUSYGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HSSUSY_librarylink;

   if (!check_number_of_args(link, 1, "FSHSSUSYGetSettings"))
...
DLLEXPORT int FSHSSUSYGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HSSUSY_librarylink;

   if (!check_number_of_args(link, 1, "FSHSSUSYGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHSSUSYGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HSSUSY_librarylink;

   if (!check_number_of_args(link, 1, "FSHSSUSYGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHSSUSYOpenHandle(
...
DLLEXPORT int FSHSSUSYCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::HSSUSY_librarylink;
...
DLLEXPORT int FSHSSUSYGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HSSUSY_librarylink;

   if (!check_number_of_args(link, 1, "FSHSSUSYGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHSSUSYToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HSSUSY_librarylink;

   if (!check_number_of_args(link, 1, "FSHSSUSYToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHSSUSYGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HSSUSY_librarylink;

   if (!check_number_of_args(link, 1, "FSHSSUSYGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHSSUSYCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HSSUSY_librarylink;

   if (!check_number_of_args(link, 1, "FSHSSUSYCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHSSUSYCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HSSUSY_librarylink;

   if (!check_number_of_args(link, 1, "FSHSSUSYCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSHSSUSYCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSHSSUSYCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSHSSUSYCalculateObservables", "error", e.what());
      put_error_output(link);
   }
