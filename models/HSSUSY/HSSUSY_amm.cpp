...
#include "HSSUSY_amm.hpp"
#include "HSSUSY_mass_eigenstates.hpp"

#include "cxx_qft/HSSUSY_qft.hpp"
#include "cxx_qft/HSSUSY_particle_aliases.hpp"
...
double get_MLCP(const HSSUSY_mass_eigenstates_interface& model)
{
   return 0.;
}
...
void run_to_MSUSY(HSSUSY_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("HSSUSY_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("HSSUSY_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("HSSUSY_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("HSSUSY_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const HSSUSY_mass_eigenstates& model, double ml_pole, int idx)
{
...
template <typename Lepton>
double calculate_amm_impl(const HSSUSY_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("HSSUSY_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double HSSUSY_amm::calculate_amm(const HSSUSY_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   HSSUSY_mass_eigenstates model(model_);

   VERBOSE_MSG("HSSUSY_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("HSSUSY_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template <typename Lepton>
double HSSUSY_amm::calculate_amm_uncertainty(const HSSUSY_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   HSSUSY_mass_eigenstates model(model_);

   VERBOSE_MSG("HSSUSY_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("HSSUSY_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
