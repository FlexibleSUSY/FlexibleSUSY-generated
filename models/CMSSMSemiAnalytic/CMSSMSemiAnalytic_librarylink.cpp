...
#include "lowe.h"
#include "CMSSMSemiAnalytic_unitarity.hpp"
...
template <typename Solver_type>
void CMSSMSemiAnalytic_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = CMSSMSemiAnalytic_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
void Model_data::put_unitarity(MLINK link) const
{
   ...
   MLPutFunction(link, "List", 1);
   MLPutRule(link, "CMSSMSemiAnalytic");
...
DLLEXPORT int FSCMSSMSemiAnalyticCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CMSSMSemiAnalytic_librarylink;

   if (!check_number_of_args(link, 1, "FSCMSSMSemiAnalyticCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSCMSSMSemiAnalyticCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
