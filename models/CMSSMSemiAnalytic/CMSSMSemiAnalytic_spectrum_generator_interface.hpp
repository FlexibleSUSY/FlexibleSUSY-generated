...
public:
   ...
   std::tuple<CMSSMSemiAnalytic<T>> get_models_slha() const
   { return std::make_tuple(CMSSMSemiAnalytic<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const CMSSMSemiAnalytic<T>& get_model() const
   { return model; }
   CMSSMSemiAnalytic<T>& get_model()
   { return model; }
   CMSSMSemiAnalytic<T> get_model_slha() const
   { return CMSSMSemiAnalytic<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return CMSSMSemiAnalytic_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(CMSSMSemiAnalytic_info::parameter_names),
      std::cend(CMSSMSemiAnalytic_info::parameter_names));
