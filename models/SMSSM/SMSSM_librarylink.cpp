...
class SMSSM_spectrum {
public:
   virtual ~SMSSM_spectrum() = default;
...
class SMSSM_spectrum_impl : public SMSSM_spectrum
{
public:
   virtual ~SMSSM_spectrum_impl() = default;
...
public:
   ...
   void set_input_parameters(const SMSSM_input_parameters& input_) { input = input_; }
...
private:
   ...
   SMSSM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSSMSSMCalculateSpectrum[]?");
   }
}
...
SMSSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const SMSSM_slha& model, MLINK link)
{
...
template <typename Solver_type>
void SMSSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void SMSSM_spectrum_impl<Solver_type>::calculate_spectrum(
...
template <typename Solver_type>
void SMSSM_spectrum_impl<Solver_type>::calculate_model_observables(
   const softsusy::QedQcd& qedqcd,
...
template <typename Solver_type>
void SMSSM_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = SMSSM_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
{
   check_spectrum_pointer();
   SMSSM_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSSMSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSSMSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new SMSSM_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSSMSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSSMSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSMSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSSMSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSMSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSSMSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSMSSMCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSSMSSMCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSSMSSMCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
