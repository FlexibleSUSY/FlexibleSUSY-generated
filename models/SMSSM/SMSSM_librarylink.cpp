...
// File generated at Thu 15 Dec 2016 12:54:46
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSSMSSMMessage", 1);
...
struct SMSSM_data {
   ...
   SMSSM_input_parameters input;     ///< model input parameters
   ...
   SMSSM_slha<algorithm_type> model; ///< running parameters and pole masses
};

/// current handles
typedef std::map<Handle_id, SMSSM_data> Handle_map;
Handle_map handles_SMSSM;

/******************************************************************/

Handle_id get_new_SMSSM_handle()
{
   static const std::size_t max_handles =
      static_cast<std::size_t>(std::exp2(8*sizeof(Handle_id)) - 1);

   if (handles_SMSSM.size() >= max_handles)
      throw ENotEnoughFreeHandles(handles_SMSSM.size());

   Handle_id hid = 0;

   while (handles_SMSSM.find(hid) != handles_SMSSM.end())
      hid++;
...
SMSSM_data find_SMSSM_data(Handle_id hid)
{
   const Handle_map::iterator handle = handles_SMSSM.find(hid);

   if (handle == handles_SMSSM.end())
...
void put_settings(const SMSSM_data& data, MLINK link)
{
...
void put_sm_input_parameters(const SMSSM_data& data, MLINK link)
{
...
void put_input_parameters(const SMSSM_data& data, MLINK link)
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(BMSInput), "BMSInput");
...
void put_spectrum(const SMSSM_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const SMSSM_observables& observables, MLINK link)
{
...
void check_spectrum(const SMSSM_data& data, MLINK link)
{
   ...
   if (problems.have_problem()) {
      ...
      put_message(link, "FSSMSSMCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSSMSSMCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(SMSSM_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSSMSSMCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = SMSSM_slha<algorithm_type>(
...
SMSSM_data make_SMSSM_data(double* pars, mint npars)
{
   SMSSM_data data;

   const mint n_settings = 23, n_sm_parameters = 29, n_input_pars = 11;
...
DLLEXPORT int FSSMSSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSSMSSMGetSettings"))
      return LIBRARY_TYPE_ERROR;

   const Handle_id hid = get_handle_from(link);

   try {
      const SMSSM_data data = find_SMSSM_data(hid);
...
DLLEXPORT int FSSMSSMGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSSMSSMGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSMSSMGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSSMSSMGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSMSSMOpenHandle(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument Res)
{
   ...
   try {
      SMSSM_data data = make_SMSSM_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);

      const Handle_id hid = get_new_SMSSM_handle();

      handles_SMSSM.insert(std::make_pair(hid, std::move(data)));
...
DLLEXPORT int FSSMSSMCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   if (Argc != 1)
      return LIBRARY_TYPE_ERROR;
   ...
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   ...
   if (handle == handles_SMSSM.end()) {
      std::cerr << "Error: FSSMSSMSet: Unknown handle: "
                << hid << std::endl;
      return LIBRARY_FUNCTION_ERROR;
   }

   try {
      handle->second = make_SMSSM_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);
   } catch (const flexiblesusy::Error& e) {
      std::cerr << e.what() << std::endl;
...
DLLEXPORT int FSSMSSMCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSSMSSMCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;

   const Handle_id hid = get_handle_from(link);

   try {
      SMSSM_data data = find_SMSSM_data(hid);
      ...
      handles_SMSSM[hid] = std::move(data);
...
DLLEXPORT int FSSMSSMCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   if (!check_number_of_args(link, 1, "FSSMSSMCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.model.get_scale() == 0.) {
         put_message(link,
            "FSSMSSMCalculateObservables", "warning",
      ...
      put_message(link, "FSSMSSMCalculateObservables", "error", e.what());
      put_error_output(link);
   }
