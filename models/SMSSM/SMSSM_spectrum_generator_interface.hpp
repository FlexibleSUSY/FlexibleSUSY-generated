...
public:
   ...
   std::tuple<SMSSM<T>> get_models_slha() const
   { return std::make_tuple(SMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const SMSSM<T>& get_model() const
   { return model; }
   SMSSM<T>& get_model()
   { return model; }
   SMSSM<T> get_model_slha() const
   { return SMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return SMSSM_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(SMSSM_info::parameter_names),
      std::cend(SMSSM_info::parameter_names));
