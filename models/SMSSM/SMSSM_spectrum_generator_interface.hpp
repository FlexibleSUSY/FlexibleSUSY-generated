...
// File generated at Sun 26 Aug 2018 14:33:49

#ifndef SMSSM_SPECTRUM_GENERATOR_INTERFACE_H
#define SMSSM_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class SMSSM_spectrum_generator_interface {
public:
   virtual ~SMSSM_spectrum_generator_interface() = default;

   std::tuple<SMSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<SMSSM_slha<SMSSM<T>>> get_models_slha() const
   { return std::make_tuple(SMSSM_slha<SMSSM<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const SMSSM<T>& get_model() const
   { return model; }
   SMSSM<T>& get_model()
   { return model; }
   SMSSM_slha<SMSSM<T>> get_model_slha() const
   { return SMSSM_slha<SMSSM<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const SMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "SMSSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(SMSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const SMSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void SMSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void SMSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const SMSSM_input_parameters& input)
{
 ...
 */
template <class T>
void SMSSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   SMSSM_mass_eigenstates tmp_model(model);
   try {
   ...
   SMSSM_parameter_getter parameter_getter;
   Coupling_monitor<SMSSM_mass_eigenstates, SMSSM_parameter_getter>
 ...
 */
template <class T>
void SMSSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   SMSSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void SMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(SMSSM<T>& model)
{
   try {
      throw;
