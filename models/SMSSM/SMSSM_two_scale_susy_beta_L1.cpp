...
double SMSSM_susy_parameters::calc_beta_L1_one_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_L1 = 2*L1*oneOver16PiSqr*(AbsSqr(Kappa) + AbsSqr(Lambdax));
...
double SMSSM_susy_parameters::calc_beta_L1_two_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_L1 = -0.4*L1*twoLoop*(20*AbsSqr(Kappa)*AbsSqr(Lambdax) + AbsSqr(
      Lambdax)*(15*traceYdAdjYd + 5*traceYeAdjYe + 15*traceYuAdjYu + 10*AbsSqr(
      Lambdax) - 3*Sqr(g1) - 15*Sqr(g2)) + 20*Sqr(Conj(Kappa))*Sqr(Kappa));
