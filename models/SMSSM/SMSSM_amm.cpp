...
#include "SMSSM_amm.hpp"
#include "SMSSM_mass_eigenstates.hpp"

#include "cxx_qft/SMSSM_qft.hpp"
#include "cxx_qft/SMSSM_particle_aliases.hpp"
...
double get_MLCP(const SMSSM_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(SMSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("SMSSM_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("SMSSM_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("SMSSM_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("SMSSM_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const SMSSM_mass_eigenstates& model, double ml_pole, int idx)
{
...
template <typename Lepton>
double calculate_amm_impl(const SMSSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("SMSSM_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double SMSSM_amm::calculate_amm(const SMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   SMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("SMSSM_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("SMSSM_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template <typename Lepton>
double SMSSM_amm::calculate_amm_uncertainty(const SMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   SMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("SMSSM_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("SMSSM_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
