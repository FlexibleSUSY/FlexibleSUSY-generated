...
// File generated at Wed 16 Oct 2019 22:32:55
 ...
 *
 * This file was generated at Wed 16 Oct 2019 22:32:55 with FlexibleSUSY
...
#define CLASSNAME SMSSM_ewsb_solver<Two_scale>

/**
 * This method solves the EWSB conditions iteratively, trying several
 * root finding methods until a solution is found.
 */
int CLASSNAME::solve_iteratively(SMSSM_mass_eigenstates& model_to_solve)
{
   ...
   auto ewsb_stepper = [this, model](const EWSB_vector_t& ewsb_pars) mutable -> EWSB_vector_t {
      const double Mu = ewsb_pars(0);
      const double BMu = ewsb_pars(1);
      const double LL1 = ewsb_pars(2);
...
int CLASSNAME::solve_iteratively_with(
   SMSSM_mass_eigenstates& model_to_solve, EWSB_solver* solver, const EWSB_vector_t& x_init)
{
 ...
 * @param solver solver
 */
void CLASSNAME::set_ewsb_solution(SMSSM_mass_eigenstates& model, const EWSB_solver* solver)
{
   const auto solution = solver->get_solution();

   const double Mu = solution(0);
   const double BMu = solution(1);
   const double LL1 = solution(2);
   model.set_Mu(Mu);
   model.set_BMu(BMu);
   model.set_LL1(LL1);
...
void CLASSNAME::set_best_ewsb_solution(SMSSM_mass_eigenstates& model, It first, It last)
{
...
int CLASSNAME::solve_iteratively_at(SMSSM_mass_eigenstates& model_to_solve, int l)
{
...
int CLASSNAME::solve(SMSSM_mass_eigenstates& model_to_solve)
{
   if (loop_order == 0) {
...
int CLASSNAME::solve_tree_level(SMSSM_mass_eigenstates& model)
{
   ...
   BMu = Re((0.05*(-20*mHd2*vd*vu + 20*mHu2*vd*vu + 10*vu*AbsSqr(Lambdax)*Cube(vd)
      ...
      Sqr(vu) + 7.0710678118654755*MS*vS*Conj(Lambdax)*Sqr(vu) +
...
CLASSNAME::EWSB_vector_t CLASSNAME::initial_guess(const SMSSM_mass_eigenstates& model) const
{
...
CLASSNAME::EWSB_vector_t CLASSNAME::tadpole_equations(const SMSSM_mass_eigenstates& model) const
{
 ...
 */
CLASSNAME::EWSB_vector_t CLASSNAME::ewsb_step(const SMSSM_mass_eigenstates& model) const
{
   ...
   BMu = Re((0.05*(-20*mHd2*vd*vu + 20*mHu2*vd*vu + 10*vu*AbsSqr(Lambdax)*Cube(vd)
      ...
      (vu) + 7.0710678118654755*vS*Conj(TLambdax)*Sqr(vu) + 10*Conj(L1)*Lambdax*
      Sqr(vu) + 7.0710678118654755*vS*Conj(MS)*Lambdax*Sqr(vu) + 5*Conj(Lambdax)*
