...
SMSSM_initial_guesser<Two_scale>::SMSSM_initial_guesser(
   SMSSM<Two_scale>* model_,
   const softsusy::QedQcd& qedqcd_,
   const SMSSM_low_scale_constraint<Two_scale>& low_constraint_,
   const SMSSM_susy_scale_constraint<Two_scale>& susy_constraint_,
   const SMSSM_high_scale_constraint<Two_scale>& high_constraint_
)
   : model(model_)
   , qedqcd(qedqcd_)
...
{
   if (!model)
      throw SetupError("SMSSM_initial_guesser: Error: pointer to model"
                       " SMSSM<Two_scale> must not be zero");
}
 ...
 */
void SMSSM_initial_guesser<Two_scale>::guess()
{
 ...
 * \code{.cpp}
   ...
   MODEL->set_Mu(Re(LowEnergyConstant(MZ)));
   MODEL->set_BMu(Re(Sqr(LowEnergyConstant(MZ))));
   MODEL->set_LL1(Re(Cube(LowEnergyConstant(MZ))));
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
 ...
 * \endcode
 */
void SMSSM_initial_guesser<Two_scale>::guess_susy_parameters()
{
...
void SMSSM_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void SMSSM_initial_guesser<Two_scale>::calculate_running_SM_masses()
{
 ...
 * corrections).
 */
void SMSSM_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
 ...
 */
void SMSSM_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
 ...
 * corrections).
 */
void SMSSM_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
   const auto vd = MODELPARAMETER(vd);
 ...
 */
void SMSSM_initial_guesser<Two_scale>::guess_soft_parameters()
{
