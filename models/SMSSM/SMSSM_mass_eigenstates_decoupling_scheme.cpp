...
/**
 * @file SMSSM_mass_eigenstates_decoupling_scheme.cpp
 * @brief implementation of the SMSSM model class in the decoupling scheme
 *
...
#define CLASSNAME SMSSM_mass_eigenstates_decoupling_scheme
...
CLASSNAME::CLASSNAME(const SMSSM_mass_eigenstates& model)
{
   fill_from(model);
}
...
{
   return std::make_unique<SMSSM_mass_eigenstates_decoupling_scheme>(*this);
}
...
void CLASSNAME::fill_from(const standard_model::Standard_model& sm_input)
{
   ...
   // calculate new gauge couplings
   {
      ...
      if (IsFinite(new_g1)) {
         model->get_problems().unflag_non_perturbative_parameter(SMSSM_info::g1);
      } else {
         model->get_problems().flag_non_perturbative_parameter(
            SMSSM_info::g1, new_g1, get_scale());
         new_g1 = Electroweak_constants::g1;
      }

      if (IsFinite(new_g2)) {
         model->get_problems().unflag_non_perturbative_parameter(SMSSM_info::g2);
      } else {
         model->get_problems().flag_non_perturbative_parameter(
            SMSSM_info::g2, new_g2, get_scale());
...
void CLASSNAME::fill_from(const SMSSM_mass_eigenstates& model)
{
   set(model.get());
...
std::ostream& operator<<(std::ostream& ostr, const SMSSM_mass_eigenstates_decoupling_scheme& model)
{
   model.print(ostr);
   return ostr;
}
