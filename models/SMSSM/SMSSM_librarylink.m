...
FSSMSSMGetProblems = LibraryFunctionLoad[libSMSSM, "FSSMSSMGetProblems", LinkObject, LinkObject];
FSSMSSMGetWarnings = LibraryFunctionLoad[libSMSSM, "FSSMSSMGetWarnings", LinkObject, LinkObject];
FSSMSSMToSLHA = LibraryFunctionLoad[libSMSSM, "FSSMSSMToSLHA", LinkObject, LinkObject];
...
FSSMSSMCalculateDecays = LibraryFunctionLoad[libSMSSM, "FSSMSSMCalculateDecays", LinkObject, LinkObject];
...
FSSMSSMCalculateDecays::error = "`1`";
FSSMSSMCalculateDecays::warning = "`1`";

FSSMSSM::info = "`1`";
FSSMSSM::nonum = "Error: `1` is not a numeric input value!";
FSSMSSMMessage[s_] := Message[FSSMSSM::info, s];

FSSMSSMCheckIsNumeric[a_?NumericQ] := a;
FSSMSSMCheckIsNumeric[a_] := (Message[FSSMSSM::nonum, a]; Abort[]);
...
FSSMSSMOpenHandle[a___, (fsSettings | fsSMParameters | fsModelParameters | fdSettings) -> s_List, r___] :=
    FSSMSSMOpenHandle[a, Sequence @@ s, r];

FSSMSSMOpenHandle[OptionsPattern[]] :=
    FSSMSSMOpenHandleLib[
        FSSMSSMCheckIsNumeric /@ {
...
FSSMSSMSet[handle_Integer, p:OptionsPattern[]] :=
    FSSMSSMSetLib[
        handle,
        ReleaseHold[Hold[FSSMSSMCheckIsNumeric /@ {
            (* spectrum generator settings *)
            OptionValue[precisionGoal],
            OptionValue[maxIterations],
            OptionValue[solver],
