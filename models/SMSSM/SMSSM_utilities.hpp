...
public:
   Eigen::ArrayXd get_parameters(const SMSSM<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const SMSSM<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const SMSSM<T>&);
...
template <class T>
void SMSSM_spectrum_plotter::extract_spectrum(const SMSSM<T>& model)
{
   spectrum.clear();
