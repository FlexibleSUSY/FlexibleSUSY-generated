...
// File generated at Sun 10 Jan 2016 15:36:31

#ifndef SMSSM_SPECTRUM_GENERATOR_H
#define SMSSM_SPECTRUM_GENERATOR_H

#include "SMSSM_spectrum_generator_interface.hpp"
...
class SMSSM_spectrum_generator
   : public SMSSM_spectrum_generator_interface<T> {
public:
   SMSSM_spectrum_generator()
      : SMSSM_spectrum_generator_interface<T>()
      , solver()
      ...
      , low_scale(0.)
   {}
   virtual ~SMSSM_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const SMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename = "SMSSM_rgflow.dat") const;
 ...
 */
template <class T>
void SMSSM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const SMSSM_input_parameters& input)
{
   SMSSM<T>& model = this->model;
   model.clear();
   ...
   SMSSM_convergence_tester<T> convergence_tester(&model, this->precision_goal);
   if (this->max_iterations > 0)
      convergence_tester.set_max_iterations(this->max_iterations);

   SMSSM_initial_guesser<T> initial_guesser(&model, qedqcd,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
   ...
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : SMSSM_info::parameter_names[parameter_index];
 ...
 */
template <class T>
void SMSSM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   SMSSM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
