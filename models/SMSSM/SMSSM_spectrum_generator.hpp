...
#ifndef SMSSM_SPECTRUM_GENERATOR_H
#define SMSSM_SPECTRUM_GENERATOR_H

#include "SMSSM_two_scale_model.hpp"
#include "SMSSM_two_scale_high_scale_constraint.hpp"
#include "SMSSM_two_scale_susy_scale_constraint.hpp"
#include "SMSSM_two_scale_low_scale_constraint.hpp"
#include "SMSSM_two_scale_convergence_tester.hpp"
...
template <class T>
class SMSSM_spectrum_generator {
public:
   SMSSM_spectrum_generator()
      : solver(), model()
   ...
   const SMSSM<T>& get_model() const { return model; }
   ...
   void run(const QedQcd& oneset, const SMSSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "SMSSM_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "SMSSM_spectrum.dat") const;

private:
   RGFlow<T> solver;
   SMSSM<T> model;
   SMSSM_high_scale_constraint<T> high_scale_constraint;
   SMSSM_susy_scale_constraint<T> susy_scale_constraint;
   SMSSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void SMSSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const SMSSM_input_parameters& input)
{
   ...
   SMSSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   SMSSM_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void SMSSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   SMSSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   SMSSM_parameter_getter parameter_getter;
   Coupling_monitor<SMSSM<T>, SMSSM_parameter_getter>
 ...
 */
template <class T>
void SMSSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   SMSSM_spectrum_plotter plotter;
