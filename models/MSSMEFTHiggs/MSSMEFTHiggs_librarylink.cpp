...
namespace MSSMEFTHiggs_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSMEFTHiggsMessage", 1);
...
class MSSMEFTHiggs_spectrum {
public:
   virtual ~MSSMEFTHiggs_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSMEFTHiggs_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSMEFTHiggs_observables& get_observables() const = 0;
   virtual const MSSMEFTHiggs_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMEFTHiggs_input_parameters&) = 0;
...
class MSSMEFTHiggs_spectrum_impl : public MSSMEFTHiggs_spectrum
{
public:
   virtual ~MSSMEFTHiggs_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSMEFTHiggs_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSMEFTHiggs_observables& get_observables() const override { return observables; }
   virtual const MSSMEFTHiggs_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMEFTHiggs_input_parameters&) override;
...
private:
   std::tuple<MSSMEFTHiggs<Solver_type>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSMEFTHiggs_scales scales{};              ///< scale information
   MSSMEFTHiggs_observables observables{};    ///< observables
   MSSMEFTHiggs_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const MSSMEFTHiggs_input_parameters& input_) { input = input_; }
...
private:
   MSSMEFTHiggs_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<MSSMEFTHiggs_spectrum> spectrum{nullptr};  ///< spectrum information

   MSSMEFTHiggs_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSMSSMEFTHiggsCalculateSpectrum[]?");
   }
}
...
MSSMEFTHiggs_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const MSSMEFTHiggs_slha& model, MLINK link)
{
   MLPutRule(link, MSSMEFTHiggs_info::model_name);
...
template <typename Solver_type>
void MSSMEFTHiggs_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MSSMEFTHiggs_input_parameters& input
)
{
...
template <typename Solver_type>
void MSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_model_decays(
   const softsusy::QedQcd& qedqcd, const Physical_input& physical_input, const FlexibleDecay_settings& flexibledecay_settings)
{
   decays = MSSMEFTHiggs_decays(std::get<0>(models), qedqcd, physical_input, flexibledecay_settings);
...
template <typename Solver_type>
void MSSMEFTHiggs_spectrum_impl<Solver_type>::fill_slha_io(MSSMEFTHiggs_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   MSSMEFTHiggs_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   MSSMEFTHiggs_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = MSSMEFTHiggs_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMEFTHiggsCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMEFTHiggsCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSMEFTHiggs_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSMSSMEFTHiggsGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggsGetSettings"))
...
DLLEXPORT int FSMSSMEFTHiggsGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggsGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMEFTHiggsGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggsGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMEFTHiggsOpenHandle(
...
DLLEXPORT int FSMSSMEFTHiggsCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MSSMEFTHiggs_librarylink;
...
DLLEXPORT int FSMSSMEFTHiggsSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::MSSMEFTHiggs_librarylink;
...
DLLEXPORT int FSMSSMEFTHiggsGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggsGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMEFTHiggsToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggsToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMEFTHiggsGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggsGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMEFTHiggsCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggsCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMEFTHiggsCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggsCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMEFTHiggsCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMEFTHiggsCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMEFTHiggsCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSMSSMEFTHiggsCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggsCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMEFTHiggsCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMEFTHiggsCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMEFTHiggsCalculateDecays", "error", e.what());
      put_error_output(link);
   }
