...
#include "lowe.h"
#include "MSSMEFTHiggs_unitarity.hpp"
...
template <typename Solver_type>
void MSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = MSSMEFTHiggs_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSMSSMEFTHiggsCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMEFTHiggsCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSMSSMEFTHiggsCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
