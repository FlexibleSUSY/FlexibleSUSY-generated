...
// File generated at Tue 10 Oct 2017 20:33:18
 ...
 * lambda is calculated from the tree-level MSSMEFTHiggs parameters.
 *
 * @param sm SM parameters
 * @param model_0l MSSMEFTHiggs tree-level parameters
...
Standard_model calculate_SM_tree_level(
   ...
   const MSSMEFTHiggs_mass_eigenstates& model_0l,
   int idx)
{
   auto sm_0l = sm;
...
/**
 * Calculates MSSMEFTHiggs tree-level parameters (g1, g2, g3, Yu, Yd,
 ...
 * @param sm SM parameters
 *
 * @return MSSMEFTHiggs with tree-level parameters
 */
MSSMEFTHiggs_mass_eigenstates calculate_MSSMEFTHiggs_tree_level(
   const MSSMEFTHiggs_mass_eigenstates& model,
   const Standard_model& sm)
{
   auto model_0l = model;
 ...
 */
Eigen::Matrix<double,2,1> calculate_tadpole_over_vevs(
   MSSMEFTHiggs_mass_eigenstates model, int loop_order)
{
   if (loop_order == 0) {
...
/**
 * Calculates tree-level Higgs mass matrix in the MSSMEFTHiggs without
 ...
 *
 * @param model MSSMEFTHiggs parameters
 * @return tree-level Higgs mass matrix in the MSSMEFTHiggs w/o tadpoles
 */
...
/**
 * Calculates squared Higgs pole mass in the MSSMEFTHiggs,
 * \f$(M_h^{\text{MSSMEFTHiggs}})^2\f$.
 *
 * @param model_0l tree-level MSSMEFTHiggs parameters
 * @param model_1l 1-loop MSSMEFTHiggs parameters
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 *
 * @return squared Higgs pole mass in the MSSMEFTHiggs
 */
double calculate_Mh2_pole(
   const MSSMEFTHiggs_mass_eigenstates& model_0l,
   const MSSMEFTHiggs_mass_eigenstates& model_1l,
   int idx)
{
   ...
   const auto M_loop = (mh2_tree - self_energy - Eigen::Matrix<double,2,2>(tadpole.asDiagonal())).eval();
 ...
 * lightest CP-even Higgs boson mass of the MSSMEFTHiggs by requiring
 * that the Higgs pole masses are equal in both models.
 *
 * @param sm Standard Model
 * @param model_1l MSSMEFTHiggs parameters
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 */
void match_high_to_low_scale_model_1loop(
   ...
   int idx)
{
   // tree-level MSSMEFTHiggs parameters
   const auto model_0l = calculate_MSSMEFTHiggs_tree_level(model_1l, sm);
...
double calculate_delta_alpha_em(double alpha_em, const MSSMEFTHiggs_mass_eigenstates& model)
{
...
double calculate_delta_alpha_s(double alpha_s, const MSSMEFTHiggs_mass_eigenstates& model)
{
   const double currentScale = model.get_scale();
...
Eigen::Matrix<double,3,3> calculate_MFu_DRbar_tree_level(const MSSMEFTHiggs_mass_eigenstates& model)
{
...
Eigen::Matrix<double,3,3> calculate_MFd_DRbar_tree_level(const MSSMEFTHiggs_mass_eigenstates& model)
{
   Eigen::Matrix<double,3,3> mf = ZEROMATRIX(3,3);
...
Eigen::Matrix<double,3,3> calculate_MFe_DRbar_tree_level(const MSSMEFTHiggs_mass_eigenstates& model)
{
   Eigen::Matrix<double,3,3> mf = ZEROMATRIX(3,3);
...
double calculate_MW_pole_1loop(const MSSMEFTHiggs_mass_eigenstates& model_0l)
{
...
double calculate_MZ_pole_1loop(const MSSMEFTHiggs_mass_eigenstates& model_0l)
{
...
/**
 * Calculates MSSMEFTHiggs parameters at 1-loop level by performing a
 * 1-loop matching.
 *
 * @param sm_0l SM parameters (lambda is at tree-level)
 * @param sm_1l SM parameters (level is at 1-loop level)
 * @param model_0l MSSMEFTHiggs parameters at tree-level
 * @param model_1l MSSMEFTHiggs parameters at 1-loop level
 *
 * @return MSSMEFTHiggs 1-loop parameters
 */
MSSMEFTHiggs_mass_eigenstates calculate_MSSMEFTHiggs_1loop(
   const Standard_model& sm_0l,
   ...
   const MSSMEFTHiggs_mass_eigenstates& model_1l)
{
   ...
   // running MSSMEFTHiggs W, Z masses (via 1L matching)
   ...
   // running MSSMEFTHiggs quark and lepton masses (via 1L matching)
   ...
   // running MSSMEFTHiggs gauge couplings (via 1L matching)
   const double g1_1L = AbsSqrt(4. * Pi * alpha_em * (1. + delta_alpha_em) * mZ2_1L / mW2_1L) / MSSMEFTHiggs_info::normalization_g1;
   const double g2_1L = AbsSqrt(4. * Pi * alpha_em * (1. + delta_alpha_em) / (1. - mW2_1L/mZ2_1L)) / MSSMEFTHiggs_info::normalization_g2;
   const double g3_1L = AbsSqrt(4. * Pi * alpha_s * (1. + delta_alpha_s)) / MSSMEFTHiggs_info::normalization_g3;
   ...
   {
      auto MODEL = &model;
      const double VEV = 2. * AbsSqrt(mZ2_1L/(Sqr(g1_1L*MSSMEFTHiggs_info::normalization_g1) + Sqr(g2_1L*MSSMEFTHiggs_info::normalization_g2)));
...
{
   // tree-level MSSMEFTHiggs parameters
   const auto model_0l = calculate_MSSMEFTHiggs_tree_level(model, sm);
   const auto sm_0l = calculate_SM_tree_level(sm, model_0l, idx);

   // 1-loop parameters
   const auto model_1l = calculate_MSSMEFTHiggs_1loop(sm_0l, sm, model_0l, model);
 ...
 * CP-even Higgs boson mass of the MSSMEFTHiggs.
 *
...
void match_high_to_low_scale_model_tree_level(
   Standard_model& sm, const MSSMEFTHiggs_mass_eigenstates& model, int idx)
{
...
/**
 * Calculates the gauge and Yukawa couplings and the SM-like VEV in
 * the MSSMEFTHiggs at the tree level from the known Standard Model
 * couplings and the SM vev.
 */
void match_low_to_high_scale_model_tree_level(
   MSSMEFTHiggs_mass_eigenstates& model, const Standard_model& sm_input)
{
   auto sm = sm_input;
   sm.calculate_DRbar_masses();

   model.set_g1(sm.get_g1()*standard_model_info::normalization_g1/MSSMEFTHiggs_info::normalization_g1);
   model.set_g2(sm.get_g2()*standard_model_info::normalization_g2/MSSMEFTHiggs_info::normalization_g2);
   model.set_g3(sm.get_g3()*standard_model_info::normalization_g3/MSSMEFTHiggs_info::normalization_g3);
...
/**
 * Calculates the gauge and Yukawa couplings and the SM-like VEV in
 * the MSSMEFTHiggs at the 1-loop level from the known Standard Model
 * couplings and the SM vev.
 *
 ...
 * determine the MSSMEFTHiggs at the right loop order to avoid the
 ...
 * CP-even Higgs boson mass of the MSSMEFTHiggs by requiring that the
...
void match_high_to_low_scale_model(
   Standard_model& sm, const MSSMEFTHiggs_mass_eigenstates& model, int loop_order, int idx)
{
   if (loop_order == 0) {
