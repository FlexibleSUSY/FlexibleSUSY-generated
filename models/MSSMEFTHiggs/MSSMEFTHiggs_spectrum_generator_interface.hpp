...
template <class T>
class MSSMEFTHiggs_spectrum_generator_interface {
public:
   virtual ~MSSMEFTHiggs_spectrum_generator_interface() = default;
   ...
   MSSMEFTHiggs<T> get_model() const
   { return model; }
   ...
   void run(const softsusy::QedQcd&, const MSSMEFTHiggs_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MSSMEFTHiggs_spectrum.dat") const;

protected:
   MSSMEFTHiggs<T> model;
   ...
   void translate_exception_to_problem(MSSMEFTHiggs<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MSSMEFTHiggs_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MSSMEFTHiggs_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MSSMEFTHiggs_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MSSMEFTHiggs_input_parameters& input)
{
 ...
 */
template <class T>
void MSSMEFTHiggs_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MSSMEFTHiggs_mass_eigenstates tmp_model(model);
   try {
 ...
 */
template <class T>
void MSSMEFTHiggs_spectrum_generator_interface<T>::translate_exception_to_problem(MSSMEFTHiggs<T>& model)
{
   try {
      throw;
