...
void MSSMEFTHiggs_spectrum_plotter::extract_spectrum(const MSSMEFTHiggs_mass_eigenstates& model)
{
   spectrum.clear();
...
void MSSMEFTHiggs_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("MSSMEFTHiggs_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("MSSMEFTHiggs_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("MSSMEFTHiggs_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void MSSMEFTHiggs_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("MSSMEFTHiggs_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const MSSMEFTHiggs_mass_eigenstates& model,
   ...
   const MSSMEFTHiggs_observables* observables)
{
   ...
   if (MSSMEFTHiggs_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, MSSMEFTHiggs_observables::get_names());
   ...
   Physical_input* physical_input, MSSMEFTHiggs_observables* observables)
{
   ...
   const auto number_of_observables =
      (observables ? MSSMEFTHiggs_observables::NUMBER_OF_OBSERVABLES : 0);
