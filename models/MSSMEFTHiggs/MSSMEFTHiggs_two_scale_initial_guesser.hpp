...
#include "MSSMEFTHiggs_initial_guesser.hpp"
...
template <class T>
class MSSMEFTHiggs_standard_model_initial_guesser;

/**
 * @class MSSMEFTHiggs_standard_model_initial_guesser<Two_scale>
 * @brief initial guesser for the MSSMEFTHiggs tower
 */

template<>
class MSSMEFTHiggs_standard_model_initial_guesser<Two_scale> : public Initial_guesser {
public:
   MSSMEFTHiggs_standard_model_initial_guesser(MSSMEFTHiggs<Two_scale>*,
                               standard_model::StandardModel<Two_scale>*,
                               const softsusy::QedQcd&,
                               const standard_model::Standard_model_low_scale_constraint<Two_scale>&,
                               const MSSMEFTHiggs_susy_scale_constraint<Two_scale>&);
   virtual ~MSSMEFTHiggs_standard_model_initial_guesser();
   virtual void guess(); ///< initial guess

   void set_running_precision(double p) { running_precision = p; }

private:
   MSSMEFTHiggs<Two_scale>* model{nullptr}; ///< pointer to model class
   ...
   MSSMEFTHiggs_susy_scale_constraint<Two_scale> susy_constraint{};
