...
double calculate_a_muon_impl(SplitMSSM_mass_eigenstates& model)
{
...
std::pair<double,double> vary_scale(const SplitMSSM_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("SplitMSSM_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double SplitMSSM_a_muon::calculate_a_muon(const SplitMSSM_mass_eigenstates& model_)
{
...
double SplitMSSM_a_muon::calculate_a_muon_uncertainty(const SplitMSSM_mass_eigenstates& model_)
{
   SplitMSSM_mass_eigenstates model(model_);
   ...
   try {
      ...
      ERROR("SplitMSSM_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
