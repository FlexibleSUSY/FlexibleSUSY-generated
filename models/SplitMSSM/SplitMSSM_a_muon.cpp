...
#include "SplitMSSM_FFV_form_factors.hpp"
...
void run_to_MSUSY(SplitMSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("SplitMSSM_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("SplitMSSM_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("SplitMSSM_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("SplitMSSM_a_muon: run_to_MSUSY(): "
...
double calculate_a_muon_impl(const SplitMSSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   VERBOSE_MSG("SplitMSSM_a_muon: calculating a_mu at Q = " << model.get_scale());

   context_base context{ model };

   using namespace SplitMSSM_cxx_diagrams::fields;

   const auto form_factors = SplitMSSM_FFV_form_factors::calculate_Fe_Fe_VP_form_factors(1, 1, model, true);
   ...
   valBarZee += std::complex<double> {1.000000000000000, 0} * AMuonBarZeeScalarLoop<typename conj<Hp>::type, hh>::value({1}, context, qedqcd);
   valBarZee += std::complex<double> {1.000000000000000, 0} * AMuonBarZeeVectorLoop<typename conj<VWp>::type, hh>::value({1}, context, qedqcd);
...
std::pair<double,double> vary_scale(const SplitMSSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model,&qedqcd] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("SplitMSSM_a_muon: scale variation: " << e.what_detailed());
                     }
                     return amu;
                  });
...
double SplitMSSM_a_muon::calculate_a_muon(const SplitMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   SplitMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("SplitMSSM_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("SplitMSSM_a_muon:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
double SplitMSSM_a_muon::calculate_a_muon_uncertainty(const SplitMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd)
{
   SplitMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("SplitMSSM_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("SplitMSSM_a_muon uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
