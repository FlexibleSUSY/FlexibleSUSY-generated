...
// File generated at Tue 10 Oct 2017 21:13:28
...
double SplitMSSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void SplitMSSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const SplitMSSM_input_parameters& input)
{
   ...
   SplitMSSM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<SplitMSSM_ewsb_solver<Two_scale> >(ewsb_solver));

   SplitMSSM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   SplitMSSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   SplitMSSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   SplitMSSM_convergence_tester<Two_scale> convergence_tester(
   ...
   SplitMSSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void SplitMSSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   SplitMSSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void SplitMSSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
