...
// File generated at Wed 29 Jun 2016 11:29:25

#ifndef SplitMSSM_SPECTRUM_GENERATOR_H
#define SplitMSSM_SPECTRUM_GENERATOR_H

#include "SplitMSSM_spectrum_generator_interface.hpp"
...
template <class T>
class SplitMSSM_spectrum_generator
   : public SplitMSSM_spectrum_generator_interface<T> {
public:
   SplitMSSM_spectrum_generator()
      : SplitMSSM_spectrum_generator_interface<T>()
      ...
      , low_scale(0.)
   {}
   virtual ~SplitMSSM_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const SplitMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename = "SplitMSSM_rgflow.dat") const;

private:
   SplitMSSM_high_scale_constraint<T> high_scale_constraint;
   SplitMSSM_susy_scale_constraint<T> susy_scale_constraint;
   SplitMSSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void SplitMSSM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const SplitMSSM_input_parameters& input)
{
   SplitMSSM<T>& model = this->model;
   model.clear();
   ...
   SplitMSSM_convergence_tester<T> convergence_tester(
   ...
   SplitMSSM_initial_guesser<T> initial_guesser(&model, qedqcd,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
   ...
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : SplitMSSM_info::parameter_names[parameter_index];
 ...
 */
template <class T>
void SplitMSSM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   SplitMSSM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
