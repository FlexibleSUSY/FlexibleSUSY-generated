...
SplitMSSM_initial_guesser<Two_scale>::SplitMSSM_initial_guesser(
   SplitMSSM<Two_scale>* model_,
   const softsusy::QedQcd& qedqcd_,
   const SplitMSSM_low_scale_constraint<Two_scale>& low_constraint_,
   const SplitMSSM_susy_scale_constraint<Two_scale>& susy_constraint_,
   const SplitMSSM_high_scale_constraint<Two_scale>& high_constraint_
)
   : model(model_)
   , qedqcd(qedqcd_)
...
{
   if (!model)
      throw SetupError("SplitMSSM_initial_guesser: Error: pointer to model"
                       " SplitMSSM<Two_scale> must not be zero");
}
 ...
 */
void SplitMSSM_initial_guesser<Two_scale>::guess()
{
 ...
 * \endcode
 */
void SplitMSSM_initial_guesser<Two_scale>::guess_susy_parameters()
{
...
void SplitMSSM_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void SplitMSSM_initial_guesser<Two_scale>::calculate_running_SM_masses()
{
 ...
 */
void SplitMSSM_initial_guesser<Two_scale>::guess_soft_parameters()
{
