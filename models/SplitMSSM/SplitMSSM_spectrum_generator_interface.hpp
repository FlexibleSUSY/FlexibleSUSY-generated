...
// File generated at Fri 20 Oct 2017 08:38:37
...
public:
   virtual ~SplitMSSM_spectrum_generator_interface() = default;
   ...
   void run(const softsusy::QedQcd&, const SplitMSSM_input_parameters&);
...
protected:
   ...
   void translate_exception_to_problem(SplitMSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const SplitMSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void SplitMSSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   SplitMSSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void SplitMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(SplitMSSM<T>& model)
{
   try {
      throw;
