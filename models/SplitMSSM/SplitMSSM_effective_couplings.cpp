...
// File generated at Sun 26 Aug 2018 14:10:28
...
void SplitMSSM_effective_couplings::calculate_eff_CphhVPVP()
{
   ...
   for (int gI1 = 0; gI1 < 2; ++gI1) {
      result += CpbarChaChahhPL(gI1, gI1) * vev * AS12(decay_scale / Sqr(MCha(gI1)
         )) / MCha(gI1);
   }
...
void SplitMSSM_effective_couplings::calculate_eff_CpAhVPVP()
{
   ...
   for (int gI1 = 0; gI1 < 2; ++gI1) {
      result += CpbarChaChaAhPL(gI1, gI1) * vev * AP12(decay_scale / Sqr(MCha(gI1)
         )) / MCha(gI1);
   }
   result *= 2.0;
