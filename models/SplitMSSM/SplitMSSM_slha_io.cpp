 ...
 */
void SplitMSSM_slha_io::set_decays(const SplitMSSM_decay_table& decay_table, FlexibleDecay_settings const& flexibledecay_settings)
{
   for (const auto& particle : decay_table) {
      set_decay_block(particle, flexibledecay_settings);
   }
}
void SplitMSSM_slha_io::fill_decays_data(const SplitMSSM_decays& decays, FlexibleDecay_settings const& flexibledecay_settings)
{
