...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSSplitMSSMMessage", 1);
...
class SplitMSSM_spectrum {
public:
   virtual ~SplitMSSM_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(SplitMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const SplitMSSM_observables& get_observables() const = 0;
   virtual const SplitMSSM_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const SplitMSSM_input_parameters&) = 0;
...
class SplitMSSM_spectrum_impl : public SplitMSSM_spectrum
{
public:
   virtual ~SplitMSSM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(SplitMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const SplitMSSM_observables& get_observables() const override { return observables; }
   virtual const SplitMSSM_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const SplitMSSM_input_parameters&) override;
...
private:
   std::tuple<SplitMSSM<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   SplitMSSM_scales scales{};              ///< scale information
   SplitMSSM_observables observables{};    ///< observables
   SplitMSSM_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const SplitMSSM_input_parameters& input_) { input = input_; }
...
private:
   ...
   std::unique_ptr<SplitMSSM_spectrum> spectrum{nullptr};  ///< spectrum information

   SplitMSSM_slha_io get_slha_io() const;
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(msl2), "msl2");
   MLPutRuleTo(link, INPUTPARAMETER(mse2), "mse2");
...
SplitMSSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const SplitMSSM_slha& model, MLINK link)
{
   MLPutRule(link, SplitMSSM_info::model_name);
   MLPutFunction(link, "List", 66);
   ...
   MLPutRuleTo(link, MODELPARAMETER(Yd), "Yd");
   MLPutRuleTo(link, MODELPARAMETER(Ye), "Ye");
   MLPutRuleTo(link, MODELPARAMETER(gYd), "gYd");
   MLPutRuleTo(link, MODELPARAMETER(g2d), "g2d");
   MLPutRuleTo(link, MODELPARAMETER(gYu), "gYu");
   MLPutRuleTo(link, MODELPARAMETER(g2u), "g2u");
...
template <typename Solver_type>
void SplitMSSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void SplitMSSM_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const SplitMSSM_input_parameters& input
)
{
...
template <typename Solver_type>
void SplitMSSM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void SplitMSSM_spectrum_impl<Solver_type>::calculate_model_decays(
   const softsusy::QedQcd& qedqcd, const Physical_input& physical_input, const FlexibleDecay_settings& flexibledecay_settings)
{
   decays = SplitMSSM_decays(std::get<0>(models), qedqcd, physical_input, flexibledecay_settings);
...
template <typename Solver_type>
void SplitMSSM_spectrum_impl<Solver_type>::fill_slha_io(SplitMSSM_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   SplitMSSM_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   SplitMSSM_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = SplitMSSM_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSSplitMSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSSplitMSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new SplitMSSM_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   INPUTPARAMETER(msq2(1,0)) = pars[c++];
   INPUTPARAMETER(msq2(1,1)) = pars[c++];
   INPUTPARAMETER(msq2(1,2)) = pars[c++];
   INPUTPARAMETER(msq2(2,0)) = pars[c++];
   INPUTPARAMETER(msq2(2,1)) = pars[c++];
   INPUTPARAMETER(msq2(2,2)) = pars[c++];
   INPUTPARAMETER(msu2(0,0)) = pars[c++];
   INPUTPARAMETER(msu2(0,1)) = pars[c++];
   INPUTPARAMETER(msu2(0,2)) = pars[c++];
   INPUTPARAMETER(msu2(1,0)) = pars[c++];
   INPUTPARAMETER(msu2(1,1)) = pars[c++];
   INPUTPARAMETER(msu2(1,2)) = pars[c++];
   INPUTPARAMETER(msu2(2,0)) = pars[c++];
   INPUTPARAMETER(msu2(2,1)) = pars[c++];
   INPUTPARAMETER(msu2(2,2)) = pars[c++];
   INPUTPARAMETER(msd2(0,0)) = pars[c++];
   INPUTPARAMETER(msd2(0,1)) = pars[c++];
   INPUTPARAMETER(msd2(0,2)) = pars[c++];
   INPUTPARAMETER(msd2(1,0)) = pars[c++];
   INPUTPARAMETER(msd2(1,1)) = pars[c++];
   INPUTPARAMETER(msd2(1,2)) = pars[c++];
   INPUTPARAMETER(msd2(2,0)) = pars[c++];
   INPUTPARAMETER(msd2(2,1)) = pars[c++];
   INPUTPARAMETER(msd2(2,2)) = pars[c++];
   INPUTPARAMETER(msl2(0,0)) = pars[c++];
   INPUTPARAMETER(msl2(0,1)) = pars[c++];
   INPUTPARAMETER(msl2(0,2)) = pars[c++];
   INPUTPARAMETER(msl2(1,0)) = pars[c++];
   INPUTPARAMETER(msl2(1,1)) = pars[c++];
   INPUTPARAMETER(msl2(1,2)) = pars[c++];
   INPUTPARAMETER(msl2(2,0)) = pars[c++];
   INPUTPARAMETER(msl2(2,1)) = pars[c++];
   INPUTPARAMETER(msl2(2,2)) = pars[c++];
   INPUTPARAMETER(mse2(0,0)) = pars[c++];
   INPUTPARAMETER(mse2(0,1)) = pars[c++];
   INPUTPARAMETER(mse2(0,2)) = pars[c++];
   INPUTPARAMETER(mse2(1,0)) = pars[c++];
   INPUTPARAMETER(mse2(1,1)) = pars[c++];
   INPUTPARAMETER(mse2(1,2)) = pars[c++];
   INPUTPARAMETER(mse2(2,0)) = pars[c++];
   INPUTPARAMETER(mse2(2,1)) = pars[c++];
   INPUTPARAMETER(mse2(2,2)) = pars[c++];
...
{
   using namespace flexiblesusy::SplitMSSM_librarylink;
...
DLLEXPORT int FSSplitMSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SplitMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSSplitMSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSplitMSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SplitMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSSplitMSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSplitMSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SplitMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSSplitMSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSSplitMSSMCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSSplitMSSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSSplitMSSMCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSSplitMSSMCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SplitMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSSplitMSSMCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSSplitMSSMCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSSplitMSSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSSplitMSSMCalculateDecays", "error", e.what());
      put_error_output(link);
   }
