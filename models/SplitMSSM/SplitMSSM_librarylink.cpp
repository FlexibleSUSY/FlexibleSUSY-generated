...
// File generated at Wed 12 Apr 2017 11:17:56
...
struct Model_data {
   ...
   SplitMSSM_slha<algorithm_type> model; ///< running parameters and pole masses
};
...
void put_spectrum(const SplitMSSM_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const SplitMSSM_observables& observables, MLINK link)
{
...
void check_spectrum(const Model_data& data, MLINK link)
{
   ...
   if (problems.have_problem()) {
      ...
      put_message(link, "FSSplitMSSMCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSSplitMSSMCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(Model_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSSplitMSSMCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = SplitMSSM_slha<algorithm_type>(
