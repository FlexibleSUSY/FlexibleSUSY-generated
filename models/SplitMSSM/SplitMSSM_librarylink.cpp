...
#include "lowe.h"
#include "SplitMSSM_unitarity.hpp"
...
template <typename Solver_type>
void SplitMSSM_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = SplitMSSM_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSSplitMSSMCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SplitMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSSplitMSSMCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSSplitMSSMCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
