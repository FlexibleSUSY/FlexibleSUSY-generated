...
#include "THDMIIMSSMBC_edm.hpp"
#include "THDMIIMSSMBC_l_to_lgamma.hpp"
...
const int THDMIIMSSMBC_observables::NUMBER_OF_OBSERVABLES;
...
{
   Eigen::ArrayXd vec(THDMIIMSSMBC_observables::NUMBER_OF_OBSERVABLES);
...
{
   std::vector<std::string> names(THDMIIMSSMBC_observables::NUMBER_OF_OBSERVABLES);
...
{
   assert(vec.rows() == THDMIIMSSMBC_observables::NUMBER_OF_OBSERVABLES);
                                              ...
                                              const Physical_input& physical_input)
{
   ...
   try {
      THDMIIMSSMBC_effective_couplings effective_couplings(model, qedqcd, physical_input);
      effective_couplings.calculate_effective_couplings();

      observables.AMU = THDMIIMSSMBC_a_muon::calculate_a_muon(MODEL, qedqcd);
