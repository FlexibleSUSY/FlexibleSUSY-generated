 ...
 */
void THDMIIMSSMBC_slha_io::set_dcinfo(const FlexibleDecay_problems& problems)
{
 ...
 * @param warnings vector of warning strings
 */
void THDMIIMSSMBC_slha_io::set_dcinfo(
   const std::vector<std::string>& problems,
   const std::vector<std::string>& warnings)
{
   ...
   dcinfo << FORMAT_SPINFO(5, THDMIIMSSMBC_info::model_name)
          << FORMAT_SPINFO(9, SARAH_VERSION);
 ...
 */
void THDMIIMSSMBC_slha_io::set_decay_block(const Decays_list& decays_list, FlexibleDecay_settings const& flexibledecay_settings)
{
   const auto pdg = decays_list.get_particle_id();
   const auto width = decays_list.get_total_width();
   const std::string name = THDMIIMSSMBC_info::get_particle_name_from_pdg(pdg);
   ...
   if (!is_zero(width, 1e-100)) {
      ...
      for (const auto& channel : sorted_decays_list) {
         ...
         for (auto id : final_state) {
            comment += " " + THDMIIMSSMBC_info::get_particle_name_from_pdg(id);
         }
         comment += ")";
 ...
 * @param settings struct of spectrum generator settings to be filled
 */
void THDMIIMSSMBC_slha_io::fill(FlexibleDecay_settings& settings) const
{
   slha_io.fill(settings);
}
