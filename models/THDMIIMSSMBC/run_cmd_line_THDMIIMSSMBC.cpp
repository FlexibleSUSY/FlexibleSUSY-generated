...
{
   std::cout <<
      "Usage: run_cmd_line_THDMIIMSSMBC.x [options]\n"
      "Options:\n"
      "  --TanBeta=<value>\n"
      "  --MSUSY=<value>\n"
      "  --MEWSB=<value>\n"
...
int main(int argc, char* argv[])
{
   ...
   const THDMIIMSSMBC_slha<algorithm_type> model(spectrum_generator.get_model());

   THDMIIMSSMBC_scales scales;
   scales.HighScale = spectrum_generator.get_high_scale();
   scales.SUSYScale = spectrum_generator.get_susy_scale();
   scales.LowScale  = spectrum_generator.get_low_scale();

   const THDMIIMSSMBC_observables observables(calculate_observables(model, qedqcd, physical_input));

   // SLHA output
   SLHAea::Coll slhaea(THDMIIMSSMBC_slha_io::fill_slhaea(model, qedqcd, scales, observables));
