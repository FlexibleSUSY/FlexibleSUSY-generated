...
public:
   ...
   std::tuple<THDMIIMSSMBC<T>> get_models_slha() const
   { return std::make_tuple(THDMIIMSSMBC<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const THDMIIMSSMBC<T>& get_model() const
   { return model; }
   THDMIIMSSMBC<T>& get_model()
   { return model; }
   THDMIIMSSMBC<T> get_model_slha() const
   { return THDMIIMSSMBC<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return THDMIIMSSMBC_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(THDMIIMSSMBC_info::parameter_names),
      std::cend(THDMIIMSSMBC_info::parameter_names));
