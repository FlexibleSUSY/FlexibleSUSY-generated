...
// File generated at Wed 16 Oct 2019 21:28:14
...
template <class T>
class THDMIIMSSMBC_spectrum_generator_interface {
public:
   virtual ~THDMIIMSSMBC_spectrum_generator_interface() = default;

   std::tuple<THDMIIMSSMBC<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<THDMIIMSSMBC_slha<THDMIIMSSMBC<T>>> get_models_slha() const
   { return std::make_tuple(THDMIIMSSMBC_slha<THDMIIMSSMBC<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const THDMIIMSSMBC<T>& get_model() const
   { return model; }
   THDMIIMSSMBC<T>& get_model()
   { return model; }
   THDMIIMSSMBC_slha<THDMIIMSSMBC<T>> get_model_slha() const
   { return THDMIIMSSMBC_slha<THDMIIMSSMBC<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const THDMIIMSSMBC_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "THDMIIMSSMBC_spectrum.dat") const;

protected:
   THDMIIMSSMBC<T> model;
   ...
   void translate_exception_to_problem(THDMIIMSSMBC<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const THDMIIMSSMBC_input_parameters&) = 0;
};
 ...
 */
template <class T>
void THDMIIMSSMBC_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void THDMIIMSSMBC_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const THDMIIMSSMBC_input_parameters& input)
{
 ...
 */
template <class T>
void THDMIIMSSMBC_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   THDMIIMSSMBC_mass_eigenstates tmp_model(model);
   try {
   ...
   THDMIIMSSMBC_parameter_getter parameter_getter;
   Coupling_monitor<THDMIIMSSMBC_mass_eigenstates, THDMIIMSSMBC_parameter_getter>
 ...
 */
template <class T>
void THDMIIMSSMBC_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   THDMIIMSSMBC_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void THDMIIMSSMBC_spectrum_generator_interface<T>::translate_exception_to_problem(THDMIIMSSMBC<T>& model)
{
   try {
      throw;
