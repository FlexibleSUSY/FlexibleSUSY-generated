...
public:
   THDMIIMSSMBC_spectrum_generator_interface()
      : model()
      , settings()
   ...
   virtual void run(const softsusy::QedQcd&, const THDMIIMSSMBC_input_parameters&) = 0;
 ...
 */
template <class T>
void THDMIIMSSMBC_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   THDMIIMSSMBC_spectrum_plotter plotter;
