...
struct Model_data {
   ...
   THDMIIMSSMBC_slha<algorithm_type> model; ///< running parameters and pole masses
};
...
void put_spectrum(const THDMIIMSSMBC_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const THDMIIMSSMBC_observables& observables, MLINK link)
{
...
void check_spectrum(const Model_data& data, MLINK link)
{
   ...
   if (problems.have_problem()) {
      ...
      put_message(link, "FSTHDMIIMSSMBCCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSTHDMIIMSSMBCCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(Model_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSTHDMIIMSSMBCCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = THDMIIMSSMBC_slha<algorithm_type>(
