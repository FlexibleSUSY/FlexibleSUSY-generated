...
#include "THDMIIMSSMBC_two_scale_spectrum_generator.hpp"
#endif
...
#include "lowe.h"
#include "THDMIIMSSMBC_unitarity.hpp"
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSTHDMIIMSSMBCMessage", 1);
...
class THDMIIMSSMBC_spectrum {
public:
   virtual ~THDMIIMSSMBC_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(THDMIIMSSMBC_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const THDMIIMSSMBC_observables& get_observables() const = 0;

   virtual const UnitarityInfiniteS& get_unitarity() const = 0;
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDMIIMSSMBC_input_parameters&) = 0;
...
class THDMIIMSSMBC_spectrum_impl : public THDMIIMSSMBC_spectrum
{
public:
   virtual ~THDMIIMSSMBC_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(THDMIIMSSMBC_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const THDMIIMSSMBC_observables& get_observables() const override { return observables; }

   virtual const UnitarityInfiniteS& get_unitarity() const { return unitarityData; };
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDMIIMSSMBC_input_parameters&) override;
   virtual void calculate_model_observables(
      const softsusy::QedQcd&,
...
private:
   std::tuple<THDMIIMSSMBC<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   THDMIIMSSMBC_scales scales{};              ///< scale information
   THDMIIMSSMBC_observables observables{};    ///< observables
...
public:
   ...
   void set_input_parameters(const THDMIIMSSMBC_input_parameters& input_) { input = input_; }
...
private:
   THDMIIMSSMBC_input_parameters input{};                    ///< model input parameters
   ...
   std::unique_ptr<THDMIIMSSMBC_spectrum> spectrum{nullptr}; ///< spectrum information

   THDMIIMSSMBC_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSTHDMIIMSSMBCCalculateSpectrum[]?");
   }
}
...
THDMIIMSSMBC_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const THDMIIMSSMBC_slha& model, MLINK link)
{
   MLPutRule(link, THDMIIMSSMBC_info::model_name);
...
template <typename Solver_type>
void THDMIIMSSMBC_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void THDMIIMSSMBC_spectrum_impl<Solver_type>::calculate_spectrum(
...
template <typename Solver_type>
void THDMIIMSSMBC_spectrum_impl<Solver_type>::calculate_model_observables(
   const softsusy::QedQcd& qedqcd,
...
template <typename Solver_type>
void THDMIIMSSMBC_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = THDMIIMSSMBC_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}

/******************************************************************/

template <typename Solver_type>
void THDMIIMSSMBC_spectrum_impl<Solver_type>::fill_slha_io(THDMIIMSSMBC_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   THDMIIMSSMBC_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSTHDMIIMSSMBCCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSTHDMIIMSSMBCCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new THDMIIMSSMBC_spectrum_impl<Two_scale>());
...
{
   using namespace flexiblesusy::THDMIIMSSMBC_librarylink;
...
DLLEXPORT int FSTHDMIIMSSMBCGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDMIIMSSMBCGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDMIIMSSMBCToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDMIIMSSMBCToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDMIIMSSMBCGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDMIIMSSMBCGetWarnings"))
      return LIBRARY_TYPE_ERROR;
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSTHDMIIMSSMBCCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSTHDMIIMSSMBCCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSTHDMIIMSSMBCCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSTHDMIIMSSMBCCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDMIIMSSMBCCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSTHDMIIMSSMBCCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
