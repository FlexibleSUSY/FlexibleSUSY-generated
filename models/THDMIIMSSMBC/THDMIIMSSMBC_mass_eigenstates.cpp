...
// File generated at Mon 5 Mar 2018 17:34:40
...
#include "THDMIIMSSMBC_two_scale_ewsb_solver.hpp"
#endif
...
CLASSNAME::THDMIIMSSMBC_mass_eigenstates(const THDMIIMSSMBC_input_parameters& input_)
   ...
   , ewsb_solver(new THDMIIMSSMBC_ewsb_solver<Two_scale>())
#endif
{
}
...
void CLASSNAME::set_ewsb_solver(const std::shared_ptr<THDMIIMSSMBC_ewsb_solver_interface>& solver)
{
   ewsb_solver = solver;
}
...
{
   if (!ewsb_solver) {
      throw SetupError("THDMIIMSSMBC_mass_eigenstates::solve_ewsb_tree_level: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("THDMIIMSSMBC_mass_eigenstates::solve_ewsb_one_loop: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("THDMIIMSSMBC_mass_eigenstates::solve_ewsb: "
                       "no EWSB solver set");
   }

   VERBOSE_MSG("\t\tSolving THDMIIMSSMBC EWSB at " << ewsb_loop_order << "-loop order");
...
{
   if (!force_output && problems.is_running_tachyon(THDMIIMSSMBC_info::VZ
      ))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(THDMIIMSSMBC_info::VZ);
...
{
   if (!force_output && problems.is_running_tachyon(THDMIIMSSMBC_info::hh
      ))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(THDMIIMSSMBC_info::Ah
      ))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(THDMIIMSSMBC_info::Hm
      ))
      return;
...
void CLASSNAME::calculate_MVWm_pole()
{
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(THDMIIMSSMBC_info::VWm);
