...
{
   return std::make_unique<THDMIIMSSMBC_mass_eigenstates>(*this);
}
...
{
   if (!force_output && problems.is_running_tachyon(THDMIIMSSMBC_info::VWm))
      return;
...
double CLASSNAME::calculate_MVZ_DRbar(double m_pole) const
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(THDMIIMSSMBC_info::VZ);return m_pole;
   }
...
double CLASSNAME::calculate_MVWm_DRbar(double m_pole) const
{
   ...
   if (mass_sqr < 0.) {
      problems.flag_pole_tachyon(THDMIIMSSMBC_info::VWm);return m_pole;
   }
