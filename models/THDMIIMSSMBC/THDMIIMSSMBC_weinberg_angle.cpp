...
{
   const auto gY = MODEL->get_g1() * THDMIIMSSMBC_info::normalization_g1;
   const auto g2 = MODEL->get_g2() * THDMIIMSSMBC_info::normalization_g2;
...
double CLASSNAME::calculate_mw_pole(double sinThetaW) const
{
   ...
   sm.set_g3(model->get_g3() * THDMIIMSSMBC_info::normalization_g3 /
...
double CLASSNAME::calculate_delta_alpha_hat_bsm(double alpha_em) const
{
   ...
   delta_alpha_hat_bsm += alpha_em/(2.*Pi)*(-0.3333333333333333*FiniteLog(Abs(MHm(
      1)/currentScale)));
