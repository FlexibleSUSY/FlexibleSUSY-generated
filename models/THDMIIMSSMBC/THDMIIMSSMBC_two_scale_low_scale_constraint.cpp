...
// File generated at Wed 16 Oct 2019 21:28:25
...
void THDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   ...
   if (IsFinite(new_g1)) {
      model->get_problems().unflag_non_perturbative_parameter(THDMIIMSSMBC_info::
         g1);
   } else {
...
double THDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   ...
   const double delta_alpha_em = -0.05305164769729845*alphaEm*FiniteLog(Abs(MHm(1)
      /currentScale));
...
void THDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_running_SM_masses()
{
   check_model_ptr();
