...
#include "cxx_qft/SM_particle_aliases.hpp"
...
double get_MLCP(const SM_mass_eigenstates_interface& model)
{
   return 0.;
}
...
void run_to_MSUSY(SM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("SM_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("SM_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("SM_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("SM_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const SM_mass_eigenstates& model, double ml_pole, int idx)
{
   using namespace SM_cxx_diagrams::fields;

   const auto form_factors = SM_FFV_form_factors::calculate_form_factors<Lepton,Lepton,VP>(idx, idx, model, true);
...
double calculate_amm_impl_2l_barrzee(const context_base& context, double ml_pole, int idx)
{
   ...
   val += 1.000000000000000 * AMMBarrZeeScalarLoop<Lepton,typename conj<Hp>::type, hh>::value({idx}, context, ml_pole);
   val += 1.000000000000000 * AMMBarrZeeVectorLoop<Lepton,typename conj<VWp>::type, hh>::value({idx}, context, ml_pole);
...
template <typename Lepton>
double calculate_amm_impl(const SM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("SM_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double SM_amm::calculate_amm(const SM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   SM_mass_eigenstates model(model_);

   VERBOSE_MSG("SM_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("SM_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template double SM_amm::calculate_amm<SM_cxx_diagrams::fields::Fe>(const SM_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);

template <typename Lepton>
double SM_amm::calculate_amm_uncertainty(const SM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   SM_mass_eigenstates model(model_);

   VERBOSE_MSG("SM_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("SM_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
// spliting templated function into header and cpp
// https://isocpp.org/wiki/faq/templates#separate-template-fn-defn-from-decl
template double SM_amm::calculate_amm_uncertainty<SM_cxx_diagrams::fields::Fe>(const SM_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);
