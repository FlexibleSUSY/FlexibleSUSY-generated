...
/**
 * @file SM_model_slha.hpp
 * @brief contains wrapper class for model class in SLHA convention
 */

// File generated at Fri 10 Apr 2020 19:47:30
...
class SM_slha : public Model {
public:
   explicit SM_slha(const SM_input_parameters& input_ = SM_input_parameters());
   explicit SM_slha(const Model&, bool do_convert_masses_to_slha = true);
   SM_slha(const SM_slha&) = default;
   SM_slha(SM_slha&&) = default;
   virtual ~SM_slha() = default;
   SM_slha& operator=(const SM_slha&) = default;
   SM_slha& operator=(SM_slha&&) = default;
...
template <class Model>
SM_slha<Model>::SM_slha(const SM_input_parameters& input_)
   : Model(input_)
{
}
 ...
 */
template <class Model>
SM_slha<Model>::SM_slha(const Model& model_,
...
template <class Model>
void SM_slha<Model>::calculate_spectrum()
{
...
template <class Model>
void SM_slha<Model>::convert_to_slha()
{
...
template <class Model>
void SM_slha<Model>::calculate_ckm_matrix()
{
...
template <class Model>
void SM_slha<Model>::calculate_pmns_matrix()
{
 ...
 */
template <class Model>
void SM_slha<Model>::convert_yukawa_couplings_to_slha()
{
 ...
 */
template <class Model>
void SM_slha<Model>::convert_trilinear_couplings_to_slha()
{
 ...
 */
template <class Model>
void SM_slha<Model>::convert_soft_squared_masses_to_slha()
{

}

template <class Model>
const SM_physical& SM_slha<Model>::get_physical_slha() const
{
   return physical_slha;
}

template <class Model>
SM_physical& SM_slha<Model>::get_physical_slha()
{
   return physical_slha;
}

template <class Model>
void SM_slha<Model>::print(std::ostream& ostr) const
{
   Model::print(ostr);
...
template <class Model>
void SM_slha<Model>::set_convert_masses_to_slha(bool flag)
{
   convert_masses_to_slha = flag;
}

template <class Model>
std::ostream& operator<<(std::ostream& ostr, const SM_slha<Model>& model)
{
   model.print(ostr);
   return ostr;
}
