...
// File generated at Sun 26 Aug 2018 14:10:45
 ...
 */
double SM_soft_parameters::calc_beta_mu2_1_loop(const Soft_traces& soft_traces) const
{
 ...
 */
double SM_soft_parameters::calc_beta_mu2_2_loop(const Soft_traces& soft_traces) const
{
   const double traceYdAdjYd = TRACE_STRUCT.traceYdAdjYd;
   const double traceYeAdjYe = TRACE_STRUCT.traceYeAdjYe;
   const double traceYuAdjYu = TRACE_STRUCT.traceYuAdjYu;
 ...
 */
double SM_soft_parameters::calc_beta_mu2_3_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_mu2 = Re(2*mu2*threeLoop*(128.25*Cube(Lambdax) + 8.378314604562993*
      Power6(g1) + 301.7235994495886*Power6(g2) + 154.40506064218175*Power6(Yd(
      2,2)) + 3.4683535473939138*Power6(Ye(2,2)) + 154.40506064218175*Power6(Yu
      (2,2)) + 173.69714554123618*Lambdax*Quad(Yd(2,2)) + 75.89904851374538*
      Lambdax*Quad(Ye(2,2)) + 173.69714554123618*Lambdax*Quad(Yu(2,2)) -
      209.24048513745396*Quad(Yd(2,2))*Sqr(g3) - 209.24048513745396*Quad(Yu(2,2
      ))*Sqr(g3) + 178.48396765750311*Quad(g3)*Sqr(Yd(2,2)) + 72.*Quad(Ye(2,2))
      *Sqr(Yd(2,2)) + 296.2115485137454*Quad(Yu(2,2))*Sqr(Yd(2,2)) +
      40.19238810996319*Lambdax*Sqr(g3)*Sqr(Yd(2,2)) + 37.125*Sqr(Lambdax)*Sqr(
      Yd(2,2)) + 72.*Quad(Yd(2,2))*Sqr(Ye(2,2)) + 72.*Quad(Yu(2,2))*Sqr(Ye(2,2)
      ) + 12.375*Sqr(Lambdax)*Sqr(Ye(2,2)) - 54.*Lambdax*Sqr(Yd(2,2))*Sqr(Ye(2,
      2)) + Quad(g2)*(-32.25723415592714*Lambdax - 28.572145541236182*Sqr(g3) -
      102.62651936694535*Sqr(Yd(2,2)) - 34.208839788981784*Sqr(Ye(2,2)) -
      102.62651936694535*Sqr(Yu(2,2))) + Quad(g1)*(-32.90278936623708*Lambdax +
      9.778005607297105*Sqr(g2) - 4.190581346047974*Sqr(g3) -
      11.837888914984243*Sqr(Yd(2,2)) - 20.542464319265452*Sqr(Ye(2,2)) -
      27.721423523790055*Sqr(Yu(2,2))) + 178.48396765750311*Quad(g3)*Sqr(Yu(2,2
      )) + 296.2115485137454*Quad(Yd(2,2))*Sqr(Yu(2,2)) + 72.*Quad(Ye(2,2))*Sqr
      (Yu(2,2)) + 40.19238810996319*Lambdax*Sqr(g3)*Sqr(Yu(2,2)) + 37.125*Sqr(
      ...
      Sqr(g2)*(-3.829281688145728*Quad(Yd(2,2)) + 3.223572770618091*Quad(Ye(2,2
      ...
      *Sqr(Yu(2,2)) - 2.7*Sqr(Ye(2,2))*Sqr(Yu(2,2)) + Sqr(g3)*(-
      2.091983828751535*Sqr(Yd(2,2)) + 8.727254982244787*Sqr(Yu(2,2))) + Sqr(g2
      )*(-18.911535445876382*Lambdax - 8.109375*Sqr(Yd(2,2)) +
