...
struct SM_scan_result {
   Spectrum_generator_problems problems;
   double higgs{0.};
};

template <class solver_type>
SM_scan_result run_parameter_point(const softsusy::QedQcd& qedqcd,
   SM_input_parameters& input)
{
...
void scan(int solver_type, SM_input_parameters& input,
          const std::vector<double>& range)
{
   softsusy::QedQcd qedqcd;

   for (const auto p: range) {
      INPUTPARAMETER(LambdaIN) = p;
