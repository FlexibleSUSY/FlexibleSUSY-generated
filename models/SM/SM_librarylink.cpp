...
// File generated at Wed 16 Oct 2019 21:36:19
...
class SM_spectrum_impl : public SM_spectrum
{
public:
   ...
   virtual void fill_slha_io(SM_slha_io&) const override;
...
private:
   std::tuple<SM_slha<SM<Solver_type>>> models{};        ///< running parameters and pole masses
...
public:
   ...
   void set_input_parameters(const SM_input_parameters& input_) { input = input_; }
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSSMCalculateSpectrum[]?");
   }
}
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, SM_info::particle_names[i].c_str());
      }
   }
}
...
SM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   SM_slha_io slha_io;
...
void put_spectrum(const SM_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void SM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void SM_spectrum_impl<Solver_type>::calculate_spectrum(
...
{
   SM_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void SM_spectrum_impl<Solver_type>::fill_slha_io(SM_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   SM_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new SM_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SM_librarylink;

   if (!check_number_of_args(link, 1, "FSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SM_librarylink;

   if (!check_number_of_args(link, 1, "FSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::SM_librarylink;

   if (!check_number_of_args(link, 1, "FSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSSMCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
