...
class nPointFeFeFeFe_3917881239509984 : public correlation_function_context<4,0>
{
   ...
   public:
   nPointFeFeFeFe_3917881239509984( const SM_mass_eigenstates &model,
   ...
   std::array<std::complex<double>,10> calculate( void ) {
      ...
      for ( std::size_t i=0; i<coeffsLength; i++ ) {
         genericSummation[i] += genericsum1[i]+genericsum2[i]+genericsum3[i];
      }
      return genericSummation;
   } // End of calculate()
}; // End of nPointFeFeFeFe_3917881239509984

std::array<std::complex<double>, 10> scalars1loop(const SM_mass_eigenstates &model,
const std::array<int, 4> &indices,
const std::array<Eigen::Vector4d, 0> &momenta
) {
   nPointFeFeFeFe_3917881239509984 helper{model, indices, momenta};
   return helper.calculate();
}
class nPointFeFeFeFe_3917881243318570 : public correlation_function_context<4,0>
{
   using generic_sum_base = correlation_function_context<4,0>;
   ...
   public:
   nPointFeFeFeFe_3917881243318570( const SM_mass_eigenstates &model,
   const std::array<int, 4> &indices,
   const std::array<Eigen::Vector4d, 0> &momenta
    ) :
   correlation_function_context<4,0> { model, indices, momenta }
...
}; // End of nPointFeFeFeFe_3917881243318570

std::array<std::complex<double>, 10> vectors1loop(const SM_mass_eigenstates &model,
const std::array<int, 4> &indices,
const std::array<Eigen::Vector4d, 0> &momenta
) {
   nPointFeFeFeFe_3917881243318570 helper{model, indices, momenta};
   return helper.calculate();
}
class nPointFeFeFeFe_3917881249578249 : public correlation_function_context<4,0>
{
   using generic_sum_base = correlation_function_context<4,0>;
   ...
   std::array<std::complex<double>,10> genericSum1( void ) {
      ...
      using GenericInsertions = boost::mpl::vector<
         boost::mpl::vector<fields::Fv, fields::Fv, typename cxx_diagrams::conj<fields::Hp>::type, typename cxx_diagrams::conj<fields::Hp>::type>
         >;
      using combinatorial_factors = boost::mpl::vector<
         boost::mpl::int_<1>
         >;
   ...
   public:
   nPointFeFeFeFe_3917881249578249( const SM_mass_eigenstates &model,
   const std::array<int, 4> &indices,
   const std::array<Eigen::Vector4d, 0> &momenta
    ) :
   correlation_function_context<4,0> { model, indices, momenta }
   ...
   std::array<std::complex<double>,10> calculate( void ) {
      ...
      for ( std::size_t i=0; i<coeffsLength; i++ ) {
         genericSummation[i] += genericsum1[i]+genericsum2[i];
      }
      return genericSummation;
   } // End of calculate()
}; // End of nPointFeFeFeFe_3917881249578249

std::array<std::complex<double>, 10> boxes1loop(const SM_mass_eigenstates &model,
const std::array<int, 4> &indices,
const std::array<Eigen::Vector4d, 0> &momenta
) {
   nPointFeFeFeFe_3917881249578249 helper{model, indices, momenta};
   return helper.calculate();
}
...
void run_to_MSUSY(SM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("SM_br_l_to_3l: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("SM_br_l_to_3l:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("SM_br_l_to_3l: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("SM_br_l_to_3l: run_to_MSUSY(): "
...
std::complex<double> left(const SM_mass_eigenstates& model, int nA) {
 ...
 */
template <class Lepton, class Photon>
std::complex<double> right(const SM_mass_eigenstates& model, int nA) {
    context_base context {model};
    using vertex = Vertex<typename Lepton::lorentz_conjugate, Lepton, Photon>;
    std::array<int, 2> indices {nA, nA};
    const auto value =  vertex::evaluate(indices, context);
...
Eigen::Array<std::complex<double>,10,1> fix_tensors_sign(
   const SM_mass_eigenstates& model, int nI, int nO, int nA) {
...
double width_same(int nI, int nA, T1 DL, T1 DR, T2 coeffs,
   const SM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd) {
...
Eigen::Array<std::complex<double>,13,1> forge(int nI, int nO, int nA,
   const SM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd) {
   ...
   try {
      ...
      ERROR("SM_br_l_to_3l:" << e.what_detailed());
