...
const Eigen::Matrix<std::complex<double>,3,3>& SM_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}
...
void SM_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<SM<Two_scale>*>(model_);
}

void SM_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& SM_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}
...
double SM_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();
   ...
   SM_weinberg_angle::Sm_parameters sm_pars;
   ...
   SM_weinberg_angle weinberg(MODEL, sm_pars);
...
double SM_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
void SM_low_scale_constraint<Two_scale>::calculate_running_SM_masses()
{
   check_model_ptr();
...
void SM_low_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("SM_low_scale_constraint<Two_scale>: "
