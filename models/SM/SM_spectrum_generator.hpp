...
// File generated at Mon 23 Feb 2015 12:25:46
...
public:
   ...
   void run(const QedQcd& oneset, const SM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "SM_rge_running.dat") const;
 ...
 */
template <class T>
void SM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   SM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   SM_parameter_getter parameter_getter;
   Coupling_monitor<SM<T>, SM_parameter_getter>
 ...
 */
template <class T>
void SM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
