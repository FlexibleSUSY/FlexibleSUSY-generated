...
#include "SM_spectrum_generator_interface.hpp"
...
template <class T>
class SM_spectrum_generator
   : public SM_spectrum_generator_interface<T> {
public:
   SM_spectrum_generator()
      : SM_spectrum_generator_interface<T>()
      , high_scale(0.)
      , susy_scale(0.)
      , low_scale(0.)
   {}
   virtual ~SM_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const SM_input_parameters&);
   void write_running_couplings(const std::string& filename = "SM_rgflow.dat") const;
 ...
 */
template <class T>
void SM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const SM_input_parameters& input)
{
   SM<T>& model = this->model;
   model.clear();
   ...
   SM_high_scale_constraint<T> high_scale_constraint(&model);
   SM_susy_scale_constraint<T> susy_scale_constraint(&model, qedqcd);
   SM_low_scale_constraint<T>  low_scale_constraint(&model, qedqcd);
   ...
   SM_convergence_tester<T> convergence_tester(
   ...
   SM_initial_guesser<T> initial_guesser(&model, qedqcd,
 ...
 */
template <class T>
void SM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   SM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
