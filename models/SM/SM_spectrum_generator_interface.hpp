...
public:
   SM_spectrum_generator_interface()
      : model()
      , settings()
   ...
   virtual void run(const softsusy::QedQcd&, const SM_input_parameters&) = 0;
 ...
 */
template <class T>
void SM_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   SM_spectrum_plotter plotter;
