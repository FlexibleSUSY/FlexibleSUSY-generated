...
CLASSNAME::SM(const SM_input_parameters& input_)
   ...
   , physical()
   , problems(SM_info::particle_names)
...
const Problems<SM_info::NUMBER_OF_PARTICLES>& CLASSNAME::get_problems() const
{
   return problems;
}

Problems<SM_info::NUMBER_OF_PARTICLES>& CLASSNAME::get_problems()
{
   return problems;
}
...
int CLASSNAME::ewsb_step(double ewsb_parameters[number_of_ewsb_equations]) const
{
   ...
   mu2 = (0.5*(Power(v,3)*Lambdax - 2*tadpole[0]))/v;
...
{
   const double mass_matrix_Hp = -mu2 + 0.5*Lambdax*Sqr(v) + 0.25*Sqr(g2)
      *Sqr(v);
...
{
   MHp = get_mass_matrix_Hp();

   if (MHp < 0.)
      problems.flag_tachyon(SM_info::Hp);
...
{
   const double mass_matrix_Ah = 0.25*(-4*mu2 + 2*Lambdax*Sqr(v) + Sqr(v)
...
{
   MAh = get_mass_matrix_Ah();

   if (MAh < 0.)
      problems.flag_tachyon(SM_info::Ah);
...
{
   const double mass_matrix_hh = -mu2 + 1.5*Lambdax*Sqr(v);
...
{
   Mhh = get_mass_matrix_hh();

   if (Mhh < 0.)
      problems.flag_tachyon(SM_info::hh);
...
{
   const double mass_matrix_VZ = 0.25*Sqr(v)*Sqr(g2*Cos(ThetaW()) +
...
void CLASSNAME::calculate_MVZ()
{
   ...
   if (MVZ < 0.)
      problems.flag_tachyon(SM_info::VZ);
...
{
   const double mass_matrix_VWp = 0.25*Sqr(g2)*Sqr(v);
...
{
   MVWp = get_mass_matrix_VWp();

   if (MVWp < 0.)
      problems.flag_tachyon(SM_info::VWp);
...
std::complex<double> CLASSNAME::self_energy_Fd_1(double p , unsigned gO1, unsigned gO2) const
{
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_233 += B0(p,MFd(gI2),0)*Conj(CpbarUFdVGFdPR(gO2,gI2))*
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_235 += B0(p,MFd(gI2),0)*Conj(CpbarUFdVPFdPR(gO2,gI2))*
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_237 += B0(p,MFd(gI2),MVZ)*Conj(CpbarUFdVZFdPR(gO2,gI2))*
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_240 += B0(p,MFu(gI2),MVWp)*Conj(CpbarUFdconjVWpFuPR(gO2,gI2))*
...
std::complex<double> CLASSNAME::self_energy_Fu_1(double p , unsigned gO1, unsigned gO2) const
{
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_272 += B0(p,MFd(gI2),MVWp)*Conj(CpbarUFuVWpFdPR(gO2,gI2))*
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_275 += B0(p,MFu(gI2),0)*Conj(CpbarUFuVGFuPR(gO2,gI2))*
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_277 += B0(p,MFu(gI2),0)*Conj(CpbarUFuVPFuPR(gO2,gI2))*
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_279 += B0(p,MFu(gI2),MVZ)*Conj(CpbarUFuVZFuPR(gO2,gI2))*
...
std::complex<double> CLASSNAME::self_energy_Fe_1(double p , unsigned gO1, unsigned gO2) const
{
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_311 += B0(p,MFe(gI2),0)*Conj(CpbarUFeVPFePR(gO2,gI2))*
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_313 += B0(p,MFe(gI2),MVZ)*Conj(CpbarUFeVZFePR(gO2,gI2))*
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_316 += B0(p,MFv(gI2),MVWp)*Conj(CpbarUFeconjVWpFvPR(gO2,gI2))*
...
std::complex<double> CLASSNAME::self_energy_Fd_1_heavy_rotated(double p , unsigned gO1, unsigned gO2) const
{
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_344 += B0(p,MFd(gI2),MVZ)*Conj(CpbarFdVZFdPR(gO2,gI2))*
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_347 += B0(p,MFu(gI2),MVWp)*Conj(CpbarFdconjVWpFuPR(gO2,gI2))*
...
std::complex<double> CLASSNAME::self_energy_Fe_1_heavy_rotated(double p , unsigned gO1, unsigned gO2) const
{
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_371 += B0(p,MFe(gI2),MVZ)*Conj(CpbarFeVZFePR(gO2,gI2))*
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_374 += B0(p,MFv(gI2),MVWp)*Conj(CpbarFeconjVWpFvPR(gO2,gI2))*
...
std::complex<double> CLASSNAME::self_energy_Fu_1_heavy_rotated(double p , unsigned gO1, unsigned gO2) const
{
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_398 += B0(p,MFd(gI2),MVWp)*Conj(CpbarFuVWpFdPR(gO2,gI2))*
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_401 += B0(p,MFu(gI2),0)*Conj(CpbarFuVPFuPR(gO2,gI2))*CpbarFuVPFuPL
         (gO1,gI2)*MFu(gI2);
   }
   tmp_400 += tmp_401;
   result += (-4) * tmp_400;
   ...
   for (unsigned gI2 = 0; gI2 < 3; ++gI2) {
      tmp_403 += B0(p,MFu(gI2),MVZ)*Conj(CpbarFuVZFuPR(gO2,gI2))*
...
std::complex<double> CLASSNAME::tadpole_hh() const
{
   ...
   for (unsigned gI1 = 0; gI1 < 3; ++gI1) {
      tmp_429 += A0(MFd(gI1))*(CphhbarFdFdPL(gI1,gI1) + CphhbarFdFdPR(gI1,
         gI1))*MFd(gI1);
   }
   tmp_428 += tmp_429;
   result += (6) * tmp_428;
   ...
   for (unsigned gI1 = 0; gI1 < 3; ++gI1) {
      tmp_431 += A0(MFe(gI1))*(CphhbarFeFePL(gI1,gI1) + CphhbarFeFePR(gI1,
         gI1))*MFe(gI1);
   }
   tmp_430 += tmp_431;
   result += (2) * tmp_430;
   ...
   for (unsigned gI1 = 0; gI1 < 3; ++gI1) {
      tmp_433 += A0(MFu(gI1))*(CphhbarFuFuPL(gI1,gI1) + CphhbarFuFuPR(gI1,
         gI1))*MFu(gI1);
   }
   tmp_432 += tmp_433;
   result += (6) * tmp_432;
...
{
   if (!force_output && problems.is_tachyon(Hp))
      return;
   ...
   do {
      const double M_tree(get_mass_matrix_Hp());
...
void CLASSNAME::calculate_MAh_pole()
{
   ...
   do {
      const double M_tree(get_mass_matrix_Ah());
...
void CLASSNAME::calculate_MFd_pole()
{
   ...
   for (unsigned es = 0; es < 3; ++es) {
      ...
      problems.flag_bad_mass(SM_info::Fd, eigenvalue_error > precision
         * Abs(eigen_values(0)));
   #else
...
void CLASSNAME::calculate_MFu_pole()
{
   ...
   for (unsigned es = 0; es < 3; ++es) {
      ...
      problems.flag_bad_mass(SM_info::Fu, eigenvalue_error > precision
         * Abs(eigen_values(0)));
   #else
...
void CLASSNAME::calculate_MFe_pole()
{
   ...
   for (unsigned es = 0; es < 3; ++es) {
      ...
      problems.flag_bad_mass(SM_info::Fe, eigenvalue_error > precision
         * Abs(eigen_values(0)));
   #else
