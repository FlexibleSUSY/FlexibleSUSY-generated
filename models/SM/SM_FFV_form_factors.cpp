...
std::valarray<std::complex<double>> calculate_Fe_Fe_VP_form_factors (
   ...
   val += std::complex<double> {1., 0.} * FFV_VSF<Fe,Fe,VP,typename conj<VWp>::type,typename conj<Hp>::type,Fv>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_SVF<Fe,Fe,VP,typename conj<Hp>::type,typename conj<VWp>::type,Fv>::value(indices1, indices2, context, discard_SM_contributions);
   val += std::complex<double> {1., 0.} * FFV_VVF<Fe,Fe,VP,typename conj<VWp>::type,typename conj<VWp>::type,Fv>::value(indices1, indices2, context, discard_SM_contributions);
...
template std::valarray<std::complex<double>> calculate_form_factors<Fe,Fe,VP>(   int, int,
   const SM_mass_eigenstates&, bool);
