...
std::vector<std::string> SM_parameter_getter::get_mass_names(const std::string& head)
{
...
std::array<std::string, SM_info::NUMBER_OF_MIXINGS> SM_parameter_getter::get_mixing_names()
{
   return SM_info::particle_mixing_names;
}

Eigen::ArrayXd SM_parameter_getter::get_parameters(const SM_mass_eigenstates& model)
{
   return model.get();
}

std::array<std::string, SM_info::NUMBER_OF_PARAMETERS> SM_parameter_getter::get_parameter_names()
{
   return SM_info::parameter_names;
}

std::array<std::string, SM_info::NUMBER_OF_PARTICLES> SM_parameter_getter::get_particle_names()
{
   return SM_info::particle_names;
}

std::vector<std::string> SM_parameter_getter::get_DRbar_mass_names()
{
   return get_mass_names();
}

std::vector<std::string> SM_parameter_getter::get_pole_mass_names()
{
   return get_mass_names("Pole");
}

std::array<std::string, SM_info::NUMBER_OF_MIXINGS> SM_parameter_getter::get_DRbar_mixing_names()
{
   return get_mixing_names();
}

std::array<std::string, SM_info::NUMBER_OF_MIXINGS> SM_parameter_getter::get_pole_mixing_names()
{
...
std::array<std::string, SM_info::NUMBER_OF_INPUT_PARAMETERS> SM_parameter_getter::get_input_parameter_names()
{
   return SM_info::input_parameter_names;
}

std::array<std::string, SM_info::NUMBER_OF_EXTRA_PARAMETERS> SM_parameter_getter::get_extra_parameter_names()
{
   return SM_info::extra_parameter_names;
}

decltype(SM_info::NUMBER_OF_MASSES) SM_parameter_getter::get_number_of_masses()
{
   return SM_info::NUMBER_OF_MASSES;
}

// SM_spectrum_plotter ////////////////////////////////////////

SM_spectrum_plotter::SM_spectrum_plotter(const SM_mass_eigenstates& model)
{
...
void SM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("SM_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("SM_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("SM_spectrum_plotter::write_to_file: file written: "
               << file_name);
}
 ...
 */
void to_database(
   const std::string& file_name, const SM_mass_eigenstates& model,
   ...
   const SM_observables* observables)
{
   ...
   append(names, SM_parameter_getter::get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, SM_parameter_getter::get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, SM_parameter_getter::get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, SM_parameter_getter::get_DRbar_mass_names());
   append(names, SM_parameter_getter::get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, SM_parameter_getter::get_pole_mass_names());
   append(names, SM_parameter_getter::get_pole_mixing_names());
   ...
   if (SM_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, SM_observables::get_names());
 ...
 */
SM_mass_eigenstates from_database(
   const std::string& file_name, long long entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, SM_observables* observables)
{
   SM_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = SM_parameter_getter::get_number_of_masses();
   const auto number_of_mixings = SM_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = SM_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = SM_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? SM_observables::NUMBER_OF_OBSERVABLES : 0);
