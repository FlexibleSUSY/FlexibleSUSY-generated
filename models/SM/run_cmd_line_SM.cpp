...
{
   std::cout <<
      "Usage: run_cmd_line_SM.x [options]\n"
      "Options:\n"
      "  --LambdaIN=<value>\n"
      "  --Qin=<value>\n"
      "  --QEWSB=<value>\n"
...
int run_solver(int loop_library, const SM_input_parameters& input)
{
   ...
   SM_decays decays;decays = SM_decays(std::get<0>(models), qedqcd, physical_input, flexibledecay_settings);
...
int run(int solver_type, int loop_library, const SM_input_parameters& input)
{
   int exit_code = 0;
