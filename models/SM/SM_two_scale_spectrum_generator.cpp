...
// File generated at Fri 20 Oct 2017 08:39:39
...
double SM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void SM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const SM_input_parameters& input)
{
   ...
   SM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   SM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   SM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   SM_convergence_tester<Two_scale> convergence_tester(
   ...
   SM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void SM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   SM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void SM_spectrum_generator<Two_scale>::calculate_spectrum()
{
