...
namespace MRSSMEFTHiggs_cxx_diagrams
{
   struct context_base {
      ...
      context_base(MRSSMEFTHiggs_mass_eigenstates& m) : model(m) {}
...
template<> inline
double context_base::mass_impl<fields::sigmaO>(const std::array<int, 1>& indices) const
{ return model.get_MsigmaO(); }

template<> inline
double context_base::mass_impl<fields::phiO>(const std::array<int, 1>& indices) const
