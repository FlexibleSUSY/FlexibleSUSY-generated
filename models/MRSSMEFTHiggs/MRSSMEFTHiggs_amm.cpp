...
/**
 * @file MRSSMEFTHiggs_amm.cpp
 *
 * This file was generated with FlexibleSUSY 2.8.0 and SARAH 4.15.1 .
 */

#include "MRSSMEFTHiggs_amm.hpp"
#include "MRSSMEFTHiggs_mass_eigenstates.hpp"

#include "cxx_qft/MRSSMEFTHiggs_qft.hpp"
#include "cxx_qft/MRSSMEFTHiggs_particle_aliases.hpp"
...
double get_MLCP(const MRSSMEFTHiggs_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(MRSSMEFTHiggs_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MRSSMEFTHiggs_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MRSSMEFTHiggs_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MRSSMEFTHiggs_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MRSSMEFTHiggs_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const MRSSMEFTHiggs_mass_eigenstates& model, double ml_pole, int idx)
{
   using namespace MRSSMEFTHiggs_cxx_diagrams::fields;

   const auto form_factors = MRSSMEFTHiggs_FFV_form_factors::calculate_form_factors<Lepton,Lepton,VP>(idx, idx, model, true);
...
double calculate_amm_impl_2l_barrzee(const context_base& context, double ml_pole, int idx)
{
   ...
   val += 1.000000000000000 * AMMBarrZeeFermionLoop<Lepton,typename bar<Cha1>::type, Ah>::value({idx}, context, ml_pole);
   val += 1.000000000000000 * AMMBarrZeeFermionLoop<Lepton,typename bar<Cha2>::type, Ah>::value({idx}, context, ml_pole);
   ...
   val += 1.000000000000000 * AMMBarrZeeScalarLoop<Lepton,typename conj<SRdp>::type, Ah>::value({idx}, context, ml_pole);
   val += 1.000000000000000 * AMMBarrZeeScalarLoop<Lepton,typename conj<SRum>::type, Ah>::value({idx}, context, ml_pole);
   val += 3.000000000000000 * AMMBarrZeeScalarLoop<Lepton,typename conj<Su>::type, Ah>::value({idx}, context, ml_pole);
   val += 1.000000000000000 * AMMBarrZeeFermionLoop<Lepton,typename bar<Cha1>::type, hh>::value({idx}, context, ml_pole);
   val += 1.000000000000000 * AMMBarrZeeFermionLoop<Lepton,typename bar<Cha2>::type, hh>::value({idx}, context, ml_pole);
   ...
   val += 1.000000000000000 * AMMBarrZeeScalarLoop<Lepton,typename conj<SRdp>::type, hh>::value({idx}, context, ml_pole);
   val += 1.000000000000000 * AMMBarrZeeScalarLoop<Lepton,typename conj<SRum>::type, hh>::value({idx}, context, ml_pole);
   ...
   val += 1.000000000000000 * AMMBarrZeeFermionLoopZ<Lepton,typename bar<Cha1>::type, Ah>::value({idx}, context, ml_pole);
   val += 1.000000000000000 * AMMBarrZeeFermionLoopZ<Lepton,typename bar<Cha2>::type, Ah>::value({idx}, context, ml_pole);
   ...
   val += 1.000000000000000 * AMMBarrZeeFermionLoopZ<Lepton,typename bar<Cha1>::type, hh>::value({idx}, context, ml_pole);
   val += 1.000000000000000 * AMMBarrZeeFermionLoopZ<Lepton,typename bar<Cha2>::type, hh>::value({idx}, context, ml_pole);
...
template <typename Lepton>
double calculate_amm_impl(const MRSSMEFTHiggs_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("MRSSMEFTHiggs_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double MRSSMEFTHiggs_amm::calculate_amm(const MRSSMEFTHiggs_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   MRSSMEFTHiggs_mass_eigenstates model(model_);

   VERBOSE_MSG("MRSSMEFTHiggs_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MRSSMEFTHiggs_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template double MRSSMEFTHiggs_amm::calculate_amm<MRSSMEFTHiggs_cxx_diagrams::fields::Fe>(const MRSSMEFTHiggs_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);

template <typename Lepton>
double MRSSMEFTHiggs_amm::calculate_amm_uncertainty(const MRSSMEFTHiggs_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   MRSSMEFTHiggs_mass_eigenstates model(model_);

   VERBOSE_MSG("MRSSMEFTHiggs_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MRSSMEFTHiggs_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
// spliting templated function into header and cpp
// https://isocpp.org/wiki/faq/templates#separate-template-fn-defn-from-decl
template double MRSSMEFTHiggs_amm::calculate_amm_uncertainty<MRSSMEFTHiggs_cxx_diagrams::fields::Fe>(const MRSSMEFTHiggs_mass_eigenstates&, const softsusy::QedQcd&, const Spectrum_generator_settings&, int);
