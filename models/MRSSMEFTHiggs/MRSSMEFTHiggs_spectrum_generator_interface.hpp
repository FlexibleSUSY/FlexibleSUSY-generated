...
// File generated at Sun 26 Aug 2018 15:18:01

#ifndef MRSSMEFTHiggs_SPECTRUM_GENERATOR_INTERFACE_H
#define MRSSMEFTHiggs_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class MRSSMEFTHiggs_spectrum_generator_interface {
public:
   virtual ~MRSSMEFTHiggs_spectrum_generator_interface() = default;

   std::tuple<MRSSMEFTHiggs<T>, standard_model::StandardModel<T>> get_models() const
   { return std::make_tuple(model, eft); }
   std::tuple<MRSSMEFTHiggs_slha<MRSSMEFTHiggs<T>>, standard_model::StandardModel<T>> get_models_slha() const
   { return std::make_tuple(MRSSMEFTHiggs_slha<MRSSMEFTHiggs<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.), eft); }

   const MRSSMEFTHiggs<T>& get_model() const
   { return model; }
   MRSSMEFTHiggs<T>& get_model()
   { return model; }
   MRSSMEFTHiggs_slha<MRSSMEFTHiggs<T>> get_model_slha() const
   { return MRSSMEFTHiggs_slha<MRSSMEFTHiggs<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MRSSMEFTHiggs_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MRSSMEFTHiggs_spectrum.dat") const;

protected:
   MRSSMEFTHiggs<T> model;
   ...
   void translate_exception_to_problem(MRSSMEFTHiggs<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MRSSMEFTHiggs_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MRSSMEFTHiggs_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MRSSMEFTHiggs_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MRSSMEFTHiggs_input_parameters& input)
{
 ...
 */
template <class T>
void MRSSMEFTHiggs_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   MRSSMEFTHiggs_mass_eigenstates tmp_model(model);
   try {
   ...
   MRSSMEFTHiggs_parameter_getter parameter_getter;
   Coupling_monitor<MRSSMEFTHiggs_mass_eigenstates, MRSSMEFTHiggs_parameter_getter>
 ...
 */
template <class T>
void MRSSMEFTHiggs_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   MRSSMEFTHiggs_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void MRSSMEFTHiggs_spectrum_generator_interface<T>::translate_exception_to_problem(MRSSMEFTHiggs<T>& model)
{
   try {
      throw;
