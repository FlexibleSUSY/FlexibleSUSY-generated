...
// File generated at Wed 16 Oct 2019 20:06:44
...
MRSSMEFTHiggs_low_scale_constraint<Two_scale>::MRSSMEFTHiggs_low_scale_constraint(
...
void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::apply()
{
   check_model_ptr();
...
const Eigen::Matrix<std::complex<double>,3,3>& MRSSMEFTHiggs_low_scale_constraint<Two_scale>::get_ckm()
{
   return ckm;
}

const Eigen::Matrix<std::complex<double>,3,3>& MRSSMEFTHiggs_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}

double MRSSMEFTHiggs_low_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double MRSSMEFTHiggs_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<MRSSMEFTHiggs<Two_scale>*>(model_);
}

void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& MRSSMEFTHiggs_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
...
void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::initialize()
{
   check_model_ptr();
...
void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::update_scale()
{
   check_model_ptr();
...
void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
   check_model_ptr();
...
double MRSSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();

   double theta_w = std::asin(Electroweak_constants::sinThetaW);

   MRSSMEFTHiggs_weinberg_angle::Sm_parameters sm_pars;
   ...
   MRSSMEFTHiggs_weinberg_angle weinberg(MODEL, sm_pars);
...
void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   check_model_ptr();
   ...
   if (IsFinite(new_g1)) {
      model->get_problems().unflag_non_perturbative_parameter(MRSSMEFTHiggs_info::
         g1);
   } else {
...
double MRSSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   check_model_ptr();
...
double MRSSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_delta_alpha_s(double alphaS) const
{
   check_model_ptr();
   ...
   const double delta_alpha_s = 0.15915494309189535*alphaS*(0.5 - 4*FiniteLog(Abs(
      MGlu/currentScale)) - 0.5*FiniteLog(Abs(MphiO/currentScale)) - 0.5*FiniteLog
      (Abs(MsigmaO/currentScale)) - 0.16666666666666666*FiniteLog(Abs(MSd(0)/
...
double MRSSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_running_SM_masses()
{
   check_model_ptr();
...
void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_Yu_DRbar()
{
   check_model_ptr();
...
void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_Yd_DRbar()
{
   check_model_ptr();
...
void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::calculate_Ye_DRbar()
{
   check_model_ptr();
...
void MRSSMEFTHiggs_low_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("MRSSMEFTHiggs_low_scale_constraint<Two_scale>: "
