...
#include "lowe.h"
#include "MRSSMEFTHiggs_unitarity.hpp"
...
template <typename Solver_type>
void MRSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = MRSSMEFTHiggs_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSMRSSMEFTHiggsCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMEFTHiggsCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSMRSSMEFTHiggsCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
