...
// File generated at Wed 16 Oct 2019 20:07:34
...
public:
   ...
   virtual void fill_slha_io(MRSSMEFTHiggs_slha_io&) const = 0;
...
public:
   ...
   virtual void fill_slha_io(MRSSMEFTHiggs_slha_io&) const override;
...
private:
   std::tuple<MRSSMEFTHiggs_slha<MRSSMEFTHiggs<Solver_type>>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, MRSSMEFTHiggs_info::particle_names[i].c_str());
      }
   }
}
...
void put_spectrum(const MRSSMEFTHiggs_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void MRSSMEFTHiggs_spectrum_impl<Solver_type>::fill_slha_io(MRSSMEFTHiggs_slha_io& slha_io) const
{
