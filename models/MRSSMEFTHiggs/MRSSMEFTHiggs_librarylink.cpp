...
namespace MRSSMEFTHiggs_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMRSSMEFTHiggsMessage", 1);
...
class MRSSMEFTHiggs_spectrum {
public:
   virtual ~MRSSMEFTHiggs_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MRSSMEFTHiggs_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MRSSMEFTHiggs_observables& get_observables() const = 0;
   virtual const MRSSMEFTHiggs_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MRSSMEFTHiggs_input_parameters&) = 0;
...
class MRSSMEFTHiggs_spectrum_impl : public MRSSMEFTHiggs_spectrum
{
public:
   virtual ~MRSSMEFTHiggs_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MRSSMEFTHiggs_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MRSSMEFTHiggs_observables& get_observables() const override { return observables; }
   virtual const MRSSMEFTHiggs_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MRSSMEFTHiggs_input_parameters&) override;
...
private:
   std::tuple<MRSSMEFTHiggs<Solver_type>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MRSSMEFTHiggs_scales scales{};              ///< scale information
   MRSSMEFTHiggs_observables observables{};    ///< observables
   MRSSMEFTHiggs_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const MRSSMEFTHiggs_input_parameters& input_) { input = input_; }
...
private:
   MRSSMEFTHiggs_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<MRSSMEFTHiggs_spectrum> spectrum{nullptr};  ///< spectrum information

   MRSSMEFTHiggs_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSMRSSMEFTHiggsCalculateSpectrum[]?");
   }
}
...
{
   MLPutFunction(link, "List", 22);

   MLPutRuleTo(link, INPUTPARAMETER(TanBeta), "TanBeta");
   MLPutRuleTo(link, INPUTPARAMETER(MS), "MS");
   MLPutRuleTo(link, INPUTPARAMETER(LamTDInput), "LamTDInput");
   MLPutRuleTo(link, INPUTPARAMETER(LamTUInput), "LamTUInput");
   MLPutRuleTo(link, INPUTPARAMETER(LamSDInput), "LamSDInput");
   MLPutRuleTo(link, INPUTPARAMETER(LamSUInput), "LamSUInput");
   MLPutRuleTo(link, INPUTPARAMETER(MuDInput), "MuDInput");
   MLPutRuleTo(link, INPUTPARAMETER(MuUInput), "MuUInput");
   ...
   MLPutRuleTo(link, INPUTPARAMETER(mS2Input), "mS2Input");
   MLPutRuleTo(link, INPUTPARAMETER(mT2Input), "mT2Input");
   MLPutRuleTo(link, INPUTPARAMETER(moc2Input), "moc2Input");
   MLPutRuleTo(link, INPUTPARAMETER(mRd2Input), "mRd2Input");
   MLPutRuleTo(link, INPUTPARAMETER(mRu2Input), "mRu2Input");
   MLPutRuleTo(link, INPUTPARAMETER(MDBSInput), "MDBSInput");
   MLPutRuleTo(link, INPUTPARAMETER(MDWBTInput), "MDWBTInput");
   MLPutRuleTo(link, INPUTPARAMETER(MDGocInput), "MDGocInput");
...
MRSSMEFTHiggs_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   MRSSMEFTHiggs_slha_io slha_io;
...
void put_spectrum(const MRSSMEFTHiggs_slha& model, MLINK link)
{
   ...
   MLPutRuleTo(link, MODELPARAMETER(MFv), "Fv", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MSRdp), "SRdp", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MSRum), "SRum", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MsigmaO), "sigmaO", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MphiO), "phiO", {"M"});
   ...
   MLPutRuleTo(link, MODELPARAMETER(Mhh), "hh", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MAh), "Ah", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MRh), "Rh", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MHpm), "Hpm", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MChi), "Chi", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MCha1), "Cha1", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MCha2), "Cha2", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MFe), "Fe", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MFd), "Fd", {"M"});
   MLPutRuleTo(link, MODELPARAMETER(MFu), "Fu", {"M"});
   ...
   MLPutRuleTo(link, MODELPARAMETER(ZHR), "ZHR");
   MLPutRuleTo(link, MODELPARAMETER(ZP), "ZP");
   MLPutRuleTo(link, MODELPARAMETER(ZN1), "ZN1");
   MLPutRuleTo(link, MODELPARAMETER(ZN2), "ZN2");
   MLPutRuleTo(link, MODELPARAMETER(UM1), "UM1");
   MLPutRuleTo(link, MODELPARAMETER(UP1), "UP1");
   MLPutRuleTo(link, MODELPARAMETER(UM2), "UM2");
   MLPutRuleTo(link, MODELPARAMETER(UP2), "UP2");
   ...
   MLPutRuleTo(link, PHYSICALPARAMETER(MFv), "Fv", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MSRdp), "SRdp", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MSRum), "SRum", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MsigmaO), "sigmaO", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MphiO), "phiO", {"Pole", "M"});
   ...
   MLPutRuleTo(link, PHYSICALPARAMETER(Mhh), "hh", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MAh), "Ah", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MRh), "Rh", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MHpm), "Hpm", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MChi), "Chi", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MCha1), "Cha1", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MCha2), "Cha2", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MFe), "Fe", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MFd), "Fd", {"Pole", "M"});
   MLPutRuleTo(link, PHYSICALPARAMETER(MFu), "Fu", {"Pole", "M"});
   ...
   MLPutRuleTo(link, PHYSICALPARAMETER(ZHR), "ZHR", {"Pole"});
   MLPutRuleTo(link, PHYSICALPARAMETER(ZP), "ZP", {"Pole"});
   MLPutRuleTo(link, PHYSICALPARAMETER(ZN1), "ZN1", {"Pole"});
   MLPutRuleTo(link, PHYSICALPARAMETER(ZN2), "ZN2", {"Pole"});
   MLPutRuleTo(link, PHYSICALPARAMETER(UM1), "UM1", {"Pole"});
   MLPutRuleTo(link, PHYSICALPARAMETER(UP1), "UP1", {"Pole"});
   MLPutRuleTo(link, PHYSICALPARAMETER(UM2), "UM2", {"Pole"});
   MLPutRuleTo(link, PHYSICALPARAMETER(UP2), "UP2", {"Pole"});
   ...
   MLPutRuleTo(link, MODELPARAMETER(Yd), "Yd");
   MLPutRuleTo(link, MODELPARAMETER(Ye), "Ye");
   MLPutRuleTo(link, MODELPARAMETER(LamTD), "LamTD");
   MLPutRuleTo(link, MODELPARAMETER(LamTU), "LamTU");
   MLPutRuleTo(link, MODELPARAMETER(LamSD), "LamSD");
   MLPutRuleTo(link, MODELPARAMETER(LamSU), "LamSU");
   MLPutRuleTo(link, MODELPARAMETER(Yu), "Yu");
   MLPutRuleTo(link, MODELPARAMETER(Mu), "\u03bc");
   MLPutRuleTo(link, MODELPARAMETER(MuD), "MuD");
   MLPutRuleTo(link, MODELPARAMETER(MuU), "MuU");
   MLPutRuleTo(link, MODELPARAMETER(g1), "g1");
   MLPutRuleTo(link, MODELPARAMETER(g2), "g2");
   MLPutRuleTo(link, MODELPARAMETER(g3), "g3");
   MLPutRuleTo(link, MODELPARAMETER(vd), "vd");
   ...
   MLPutRuleTo(link, MODELPARAMETER(BMuD), "MuD", {"B"});
   MLPutRuleTo(link, MODELPARAMETER(BMuU), "MuU", {"B"});
   ...
   MLPutRuleTo(link, MODELPARAMETER(moc2), "moc2");
   MLPutRuleTo(link, MODELPARAMETER(mRd2), "mRd2");
   MLPutRuleTo(link, MODELPARAMETER(mRu2), "mRu2");
   MLPutRuleTo(link, MODELPARAMETER(MDBS), "MDBS");
   MLPutRuleTo(link, MODELPARAMETER(MDWBT), "MDWBT");
   MLPutRuleTo(link, MODELPARAMETER(MDGoc), "MDGoc");
   MLPutRuleTo(link, MODELPARAMETER(scale), "SCALE");
...
template <typename Solver_type>
void MRSSMEFTHiggs_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MRSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MRSSMEFTHiggs_input_parameters& input
)
{
...
template <typename Solver_type>
void MRSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void MRSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_model_decays(
   const softsusy::QedQcd& qedqcd, const Physical_input& physical_input, const FlexibleDecay_settings& flexibledecay_settings)
{
   decays = MRSSMEFTHiggs_decays(std::get<0>(models), qedqcd, physical_input, flexibledecay_settings);
...
template <typename Solver_type>
void MRSSMEFTHiggs_spectrum_impl<Solver_type>::fill_slha_io(MRSSMEFTHiggs_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   MRSSMEFTHiggs_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   MRSSMEFTHiggs_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = MRSSMEFTHiggs_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMRSSMEFTHiggsCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMRSSMEFTHiggsCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MRSSMEFTHiggs_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   INPUTPARAMETER(MS) = pars[c++];
   INPUTPARAMETER(LamTDInput) = pars[c++];
   INPUTPARAMETER(LamTUInput) = pars[c++];
   INPUTPARAMETER(LamSDInput) = pars[c++];
   INPUTPARAMETER(LamSUInput) = pars[c++];
   INPUTPARAMETER(MuDInput) = pars[c++];
   INPUTPARAMETER(MuUInput) = pars[c++];
   ...
   INPUTPARAMETER(mS2Input) = pars[c++];
   INPUTPARAMETER(mT2Input) = pars[c++];
   INPUTPARAMETER(moc2Input) = pars[c++];
   INPUTPARAMETER(mRd2Input) = pars[c++];
   INPUTPARAMETER(mRu2Input) = pars[c++];
   INPUTPARAMETER(MDBSInput) = pars[c++];
   INPUTPARAMETER(MDWBTInput) = pars[c++];
   INPUTPARAMETER(MDGocInput) = pars[c++];
...
DLLEXPORT int FSMRSSMEFTHiggsGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMEFTHiggsGetSettings"))
...
DLLEXPORT int FSMRSSMEFTHiggsGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMEFTHiggsGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSMEFTHiggsGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMEFTHiggsGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSMEFTHiggsOpenHandle(
...
DLLEXPORT int FSMRSSMEFTHiggsCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MRSSMEFTHiggs_librarylink;
...
DLLEXPORT int FSMRSSMEFTHiggsSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::MRSSMEFTHiggs_librarylink;
...
DLLEXPORT int FSMRSSMEFTHiggsGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMEFTHiggsGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSMEFTHiggsToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMEFTHiggsToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSMEFTHiggsGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMEFTHiggsGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSMEFTHiggsCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMEFTHiggsCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMRSSMEFTHiggsCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMEFTHiggsCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMRSSMEFTHiggsCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMRSSMEFTHiggsCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMRSSMEFTHiggsCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSMRSSMEFTHiggsCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MRSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSMRSSMEFTHiggsCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMRSSMEFTHiggsCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMRSSMEFTHiggsCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMRSSMEFTHiggsCalculateDecays", "error", e.what());
      put_error_output(link);
   }
