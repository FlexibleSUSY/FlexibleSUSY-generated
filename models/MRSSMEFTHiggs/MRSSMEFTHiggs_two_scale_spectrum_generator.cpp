...
// File generated at Tue 22 Jan 2019 14:34:21
...
double MRSSMEFTHiggs_spectrum_generator<Two_scale>::get_pole_mass_scale(double susy_scale) const
{
...
double MRSSMEFTHiggs_spectrum_generator<Two_scale>::get_eft_pole_mass_scale(double susy_scale, double Mt) const
{
   double Q_higgs =
 ...
 */
void MRSSMEFTHiggs_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const MRSSMEFTHiggs_input_parameters& input)
{
   ...
   MRSSMEFTHiggs_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<MRSSMEFTHiggs_ewsb_solver<Two_scale> >(ewsb_solver));

   MRSSMEFTHiggs_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   ...
   MRSSMEFTHiggs_standard_model_matching_down<Two_scale> matching_down(
   ...
   // convergence tester for MRSSMEFTHiggs
   MRSSMEFTHiggs_convergence_tester<Two_scale> model_ct(
   ...
   MRSSMEFTHiggs_standard_model_initial_guesser<Two_scale> initial_guesser(&model, &eft, qedqcd,
 ...
 */
void MRSSMEFTHiggs_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   MRSSMEFTHiggs_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_susy_scale());
}

void MRSSMEFTHiggs_spectrum_generator<Two_scale>::calculate_spectrum(double Mt, double MW)
{
   ...
   if (eft.get_problems().is_running_tachyon(standard_model_info::hh))
      model.get_problems().flag_running_tachyon(MRSSMEFTHiggs_info::hh);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::hh))
      model.get_problems().flag_pole_tachyon(MRSSMEFTHiggs_info::hh);
   if (eft.get_problems().is_running_tachyon(standard_model_info::VZ))
      model.get_problems().flag_running_tachyon(MRSSMEFTHiggs_info::VZ);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::VZ))
      model.get_problems().flag_pole_tachyon(MRSSMEFTHiggs_info::VZ);
   if (eft.get_problems().is_running_tachyon(standard_model_info::VWp))
      model.get_problems().flag_running_tachyon(MRSSMEFTHiggs_info::VWm);
   if (eft.get_problems().is_pole_tachyon(standard_model_info::VWp))
      model.get_problems().flag_pole_tachyon(MRSSMEFTHiggs_info::VWm);
}
