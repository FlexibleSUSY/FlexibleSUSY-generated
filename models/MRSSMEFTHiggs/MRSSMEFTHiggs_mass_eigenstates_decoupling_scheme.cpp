...
/**
 * @file MRSSMEFTHiggs_mass_eigenstates_decoupling_scheme.cpp
 * @brief implementation of the MRSSMEFTHiggs model class in the decoupling scheme
 *
...
#define CLASSNAME MRSSMEFTHiggs_mass_eigenstates_decoupling_scheme
...
CLASSNAME::CLASSNAME(const MRSSMEFTHiggs_mass_eigenstates& model)
{
   fill_from(model);
}
...
{
   return std::make_unique<MRSSMEFTHiggs_mass_eigenstates_decoupling_scheme>(*this);
}
   ...
   // calculate new gauge couplings
   {
      ...
      if (IsFinite(new_g1)) {
         ...
         model->get_problems().flag_non_perturbative_parameter(
            MRSSMEFTHiggs_info::g1, new_g1, get_scale());
      ...
      if (IsFinite(new_g2)) {
         ...
         model->get_problems().flag_non_perturbative_parameter(
            MRSSMEFTHiggs_info::g2, new_g2, get_scale());
...
void CLASSNAME::fill_from(const MRSSMEFTHiggs_mass_eigenstates& model)
{
   set(model.get());
...
std::ostream& operator<<(std::ostream& ostr, const MRSSMEFTHiggs_mass_eigenstates_decoupling_scheme& model)
{
   model.print(ostr);
   return ostr;
}
