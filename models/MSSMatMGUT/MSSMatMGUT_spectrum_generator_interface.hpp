...
template <class T>
class MSSMatMGUT_spectrum_generator_interface {
public:
   virtual ~MSSMatMGUT_spectrum_generator_interface() = default;

   std::tuple<MSSMatMGUT<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<MSSMatMGUT<T>> get_models_slha() const
   { return std::make_tuple(MSSMatMGUT<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MSSMatMGUT<T>& get_model() const
   { return model; }
   MSSMatMGUT<T>& get_model()
   { return model; }
   MSSMatMGUT<T> get_model_slha() const
   { return MSSMatMGUT<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const MSSMatMGUT_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "MSSMatMGUT_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(MSSMatMGUT<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const MSSMatMGUT_input_parameters&) = 0;
};
 ...
 */
template <class T>
void MSSMatMGUT_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void MSSMatMGUT_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const MSSMatMGUT_input_parameters& input)
{
 ...
 */
template <class T>
void MSSMatMGUT_spectrum_generator_interface<T>::write_running_couplings(
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return MSSMatMGUT_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(MSSMatMGUT_info::parameter_names),
      std::cend(MSSMatMGUT_info::parameter_names));
 ...
 */
template <class T>
void MSSMatMGUT_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   MSSMatMGUT_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void MSSMatMGUT_spectrum_generator_interface<T>::translate_exception_to_problem(MSSMatMGUT<T>& model)
{
   try {
      throw;
