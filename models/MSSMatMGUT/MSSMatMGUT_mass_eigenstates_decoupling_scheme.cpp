...
/**
 * @file MSSMatMGUT_mass_eigenstates_decoupling_scheme.cpp
 * @brief implementation of the MSSMatMGUT model class in the decoupling scheme
 *
...
#include "MSSMatMGUT_mass_eigenstates_decoupling_scheme.hpp"
...
#define CLASSNAME MSSMatMGUT_mass_eigenstates_decoupling_scheme
...
CLASSNAME::CLASSNAME(const MSSMatMGUT_mass_eigenstates& model)
{
   fill_from(model);
}
...
{
   return std::make_unique<MSSMatMGUT_mass_eigenstates_decoupling_scheme>(*this);
}
   ...
   // calculate new gauge couplings
   {
      ...
      if (IsFinite(new_g1)) {
         ...
         model->get_problems().flag_non_perturbative_parameter(
            MSSMatMGUT_info::g1, new_g1, get_scale());
      ...
      if (IsFinite(new_g2)) {
         ...
         model->get_problems().flag_non_perturbative_parameter(
            MSSMatMGUT_info::g2, new_g2, get_scale());
...
void CLASSNAME::fill_from(const MSSMatMGUT_mass_eigenstates& model)
{
   set(model.get());
...
std::ostream& operator<<(std::ostream& ostr, const MSSMatMGUT_mass_eigenstates_decoupling_scheme& model)
{
   model.print(ostr);
   return ostr;
}
