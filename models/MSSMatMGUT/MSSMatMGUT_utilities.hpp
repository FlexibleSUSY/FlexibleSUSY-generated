...
public:
   Eigen::ArrayXd get_parameters(const MSSMatMGUT<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const MSSMatMGUT<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const MSSMatMGUT<T>&);
...
template <class T>
void MSSMatMGUT_spectrum_plotter::extract_spectrum(const MSSMatMGUT<T>& model)
{
   spectrum.clear();
