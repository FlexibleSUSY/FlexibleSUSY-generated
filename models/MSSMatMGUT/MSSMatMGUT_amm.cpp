...
/**
 * @file MSSMatMGUT_amm.cpp
 *
 * This file was generated with FlexibleSUSY 2.8.0 and SARAH 4.15.1 .
 */

#include "MSSMatMGUT_amm.hpp"
#include "MSSMatMGUT_mass_eigenstates.hpp"

#include "cxx_qft/MSSMatMGUT_qft.hpp"
#include "cxx_qft/MSSMatMGUT_particle_aliases.hpp"
...
double get_MLCP(const MSSMatMGUT_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(MSSMatMGUT_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSMatMGUT_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSMatMGUT_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSMatMGUT_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSMatMGUT_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const MSSMatMGUT_mass_eigenstates& model, double ml_pole, int idx)
{
...
template <typename Lepton>
double calculate_amm_impl(const MSSMatMGUT_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("MSSMatMGUT_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double MSSMatMGUT_amm::calculate_amm(const MSSMatMGUT_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   MSSMatMGUT_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMatMGUT_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMatMGUT_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template <typename Lepton>
double MSSMatMGUT_amm::calculate_amm_uncertainty(const MSSMatMGUT_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   MSSMatMGUT_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMatMGUT_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMatMGUT_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
