...
template <class T>
class MSSMatMGUT_spectrum_generator {
public:
   ...
   const MSSMatMGUT<T>& get_model() const { return model; }
   ...
   void run(const QedQcd& oneset, const MSSMatMGUT_input_parameters& input);
   void write_running_couplings(const std::string& filename = "MSSMatMGUT_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "MSSMatMGUT_spectrum.dat") const;

private:
   RGFlow<T> solver;
   MSSMatMGUT<T> model;
   MSSMatMGUT_high_scale_constraint<T> high_scale_constraint;
 ...
 */
template <class T>
void MSSMatMGUT_spectrum_generator<T>::run(const QedQcd& oneset,
                                const MSSMatMGUT_input_parameters& input)
{
   ...
   MSSMatMGUT_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   MSSMatMGUT_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void MSSMatMGUT_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   MSSMatMGUT<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   MSSMatMGUT_parameter_getter parameter_getter;
   Coupling_monitor<MSSMatMGUT<T>, MSSMatMGUT_parameter_getter>
 ...
 */
template <class T>
void MSSMatMGUT_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   MSSMatMGUT_spectrum_plotter plotter;
