...
#include "MSSMatMGUT_spectrum_generator_interface.hpp"
...
class MSSMatMGUT_spectrum_generator
   : public MSSMatMGUT_spectrum_generator_interface<T> {
public:
   MSSMatMGUT_spectrum_generator()
      : MSSMatMGUT_spectrum_generator_interface<T>()
      , high_scale(0.)
      , susy_scale(0.)
      , low_scale(0.)
   {}
   virtual ~MSSMatMGUT_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const MSSMatMGUT_input_parameters&);
   void write_running_couplings(const std::string& filename = "MSSMatMGUT_rgflow.dat") const;
                                ...
                                const MSSMatMGUT_input_parameters& input)
{
   ...
   MSSMatMGUT_high_scale_constraint<T> high_scale_constraint(&model);
   MSSMatMGUT_susy_scale_constraint<T> susy_scale_constraint(&model, qedqcd);
   MSSMatMGUT_low_scale_constraint<T>  low_scale_constraint(&model, qedqcd);
   ...
   MSSMatMGUT_convergence_tester<T> convergence_tester(
 ...
 */
template <class T>
void MSSMatMGUT_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   MSSMatMGUT_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
