...
#include "lowe.h"
#include "lowNMSSM_unitarity.hpp"
...
template <typename Solver_type>
void lowNMSSM_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = lowNMSSM_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
DLLEXPORT int FSlowNMSSMCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::lowNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSlowNMSSMCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSlowNMSSMCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
