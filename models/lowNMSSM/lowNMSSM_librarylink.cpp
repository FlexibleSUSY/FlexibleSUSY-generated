...
namespace lowNMSSM_librarylink {
...
class lowNMSSM_spectrum {
public:
   virtual ~lowNMSSM_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(lowNMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const lowNMSSM_observables& get_observables() const = 0;
   virtual const lowNMSSM_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const lowNMSSM_input_parameters&) = 0;
...
class lowNMSSM_spectrum_impl : public lowNMSSM_spectrum
{
public:
   virtual ~lowNMSSM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(lowNMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const lowNMSSM_observables& get_observables() const override { return observables; }
   virtual const lowNMSSM_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const lowNMSSM_input_parameters&) override;
...
private:
   std::tuple<lowNMSSM<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   lowNMSSM_scales scales{};              ///< scale information
   lowNMSSM_observables observables{};    ///< observables
   lowNMSSM_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const lowNMSSM_input_parameters& input_) { input = input_; }
...
private:
   ...
   std::unique_ptr<lowNMSSM_spectrum> spectrum{nullptr};  ///< spectrum information

   lowNMSSM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSlowNMSSMCalculateSpectrum[]?");
   }
}
...
lowNMSSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const lowNMSSM_slha& model, MLINK link)
{
   MLPutRule(link, lowNMSSM_info::model_name);
...
template <typename Solver_type>
void lowNMSSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void lowNMSSM_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const lowNMSSM_input_parameters& input
)
{
...
template <typename Solver_type>
void lowNMSSM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void lowNMSSM_spectrum_impl<Solver_type>::calculate_model_decays(
   const softsusy::QedQcd& qedqcd, const Physical_input& physical_input, const FlexibleDecay_settings& flexibledecay_settings)
{
   decays = lowNMSSM_decays(std::get<0>(models), qedqcd, physical_input, flexibledecay_settings);
...
template <typename Solver_type>
void lowNMSSM_spectrum_impl<Solver_type>::fill_slha_io(lowNMSSM_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   lowNMSSM_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   lowNMSSM_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = lowNMSSM_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSlowNMSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSlowNMSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new lowNMSSM_spectrum_impl<Two_scale>());
...
{
   using namespace flexiblesusy::lowNMSSM_librarylink;
...
DLLEXPORT int FSlowNMSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::lowNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSlowNMSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSlowNMSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::lowNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSlowNMSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSlowNMSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::lowNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSlowNMSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSlowNMSSMCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSlowNMSSMCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::lowNMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSlowNMSSMCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSlowNMSSMCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSlowNMSSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSlowNMSSMCalculateDecays", "error", e.what());
      put_error_output(link);
   }
