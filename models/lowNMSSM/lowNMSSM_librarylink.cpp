...
// File generated at Fri 10 Apr 2020 20:23:36
...
public:
   ...
   virtual void fill_slha_io(lowNMSSM_slha_io&) const = 0;
...
public:
   ...
   virtual void fill_slha_io(lowNMSSM_slha_io&) const override;
...
private:
   std::tuple<lowNMSSM_slha<lowNMSSM<Solver_type>>> models{};        ///< running parameters and pole masses
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, lowNMSSM_info::particle_names[i].c_str());
      }
   }
}
...
void put_spectrum(const lowNMSSM_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void lowNMSSM_spectrum_impl<Solver_type>::fill_slha_io(lowNMSSM_slha_io& slha_io) const
{
