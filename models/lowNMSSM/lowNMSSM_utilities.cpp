...
#include "lowNMSSM_utilities.hpp"
...
// lowNMSSM_parameter_getter ////////////////////////////////////////

std::vector<std::string> lowNMSSM_parameter_getter::get_mass_names(const std::string& head)
{
...
std::array<std::string, lowNMSSM_info::NUMBER_OF_MIXINGS> lowNMSSM_parameter_getter::get_mixing_names()
{
   return lowNMSSM_info::particle_mixing_names;
}

Eigen::ArrayXd lowNMSSM_parameter_getter::get_parameters(const lowNMSSM_mass_eigenstates& model)
{
   return model.get();
}

std::array<std::string, lowNMSSM_info::NUMBER_OF_PARAMETERS> lowNMSSM_parameter_getter::get_parameter_names()
{
   return lowNMSSM_info::parameter_names;
}

std::array<std::string, lowNMSSM_info::NUMBER_OF_PARTICLES> lowNMSSM_parameter_getter::get_particle_names()
{
   return lowNMSSM_info::particle_names;
}

std::vector<std::string> lowNMSSM_parameter_getter::get_DRbar_mass_names()
{
   return get_mass_names();
}

std::vector<std::string> lowNMSSM_parameter_getter::get_pole_mass_names()
{
   return get_mass_names("Pole");
}

std::array<std::string, lowNMSSM_info::NUMBER_OF_MIXINGS> lowNMSSM_parameter_getter::get_DRbar_mixing_names()
{
   return get_mixing_names();
}

std::array<std::string, lowNMSSM_info::NUMBER_OF_MIXINGS> lowNMSSM_parameter_getter::get_pole_mixing_names()
{
...
std::array<std::string, lowNMSSM_info::NUMBER_OF_INPUT_PARAMETERS> lowNMSSM_parameter_getter::get_input_parameter_names()
{
   return lowNMSSM_info::input_parameter_names;
}

std::array<std::string, lowNMSSM_info::NUMBER_OF_EXTRA_PARAMETERS> lowNMSSM_parameter_getter::get_extra_parameter_names()
{
   return lowNMSSM_info::extra_parameter_names;
}

decltype(lowNMSSM_info::NUMBER_OF_MASSES) lowNMSSM_parameter_getter::get_number_of_masses()
{
   return lowNMSSM_info::NUMBER_OF_MASSES;
}

// lowNMSSM_spectrum_plotter ////////////////////////////////////////

lowNMSSM_spectrum_plotter::lowNMSSM_spectrum_plotter(const lowNMSSM_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void lowNMSSM_spectrum_plotter::extract_spectrum(const lowNMSSM_mass_eigenstates& model)
{
   spectrum.clear();
...
void lowNMSSM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("lowNMSSM_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("lowNMSSM_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("lowNMSSM_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void lowNMSSM_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("lowNMSSM_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const lowNMSSM_mass_eigenstates& model,
   ...
   const lowNMSSM_observables* observables)
{
   ...
   append(names, lowNMSSM_parameter_getter::get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, lowNMSSM_parameter_getter::get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, lowNMSSM_parameter_getter::get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, lowNMSSM_parameter_getter::get_DRbar_mass_names());
   append(names, lowNMSSM_parameter_getter::get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, lowNMSSM_parameter_getter::get_pole_mass_names());
   append(names, lowNMSSM_parameter_getter::get_pole_mixing_names());
   ...
   if (lowNMSSM_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, lowNMSSM_observables::get_names());
 ...
 */
lowNMSSM_mass_eigenstates from_database(
   const std::string& file_name, long long entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, lowNMSSM_observables* observables)
{
   lowNMSSM_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = lowNMSSM_parameter_getter::get_number_of_masses();
   const auto number_of_mixings = lowNMSSM_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = lowNMSSM_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = lowNMSSM_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? lowNMSSM_observables::NUMBER_OF_OBSERVABLES : 0);
