...
/**
 * @file lowNMSSM_two_scale_model_slha.hpp
...
/**
 * @class lowNMSSM_slha<Two_scale>
 * @brief model class wrapper in SLHA convention
 */

template<>
class lowNMSSM_slha<Two_scale> : public lowNMSSM<Two_scale> {
public:
   explicit lowNMSSM_slha(const lowNMSSM_input_parameters& input_ = lowNMSSM_input_parameters());
   explicit lowNMSSM_slha(const lowNMSSM<Two_scale>&, bool do_convert_masses_to_slha = true);
   virtual ~lowNMSSM_slha();
