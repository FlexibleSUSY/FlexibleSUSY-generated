...
public:
   lowNMSSM_spectrum_generator_interface()
      : model()
      , settings()
   ...
   virtual void run(const softsusy::QedQcd&, const lowNMSSM_input_parameters&) = 0;
 ...
 */
template <class T>
void lowNMSSM_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
...
void lowNMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(lowNMSSM<T>& model)
{
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : lowNMSSM_info::parameter_names[parameter_index];
