...
template <class T>
class lowNMSSM_spectrum_generator_interface {
public:
   virtual ~lowNMSSM_spectrum_generator_interface() = default;

   std::tuple<lowNMSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<lowNMSSM<T>> get_models_slha() const
   { return std::make_tuple(lowNMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const lowNMSSM<T>& get_model() const
   { return model; }
   lowNMSSM<T>& get_model()
   { return model; }
   lowNMSSM<T> get_model_slha() const
   { return lowNMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const lowNMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "lowNMSSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(lowNMSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const lowNMSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void lowNMSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void lowNMSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const lowNMSSM_input_parameters& input)
{
 ...
 */
template <class T>
void lowNMSSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   lowNMSSM_mass_eigenstates tmp_model(model);
   try {
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return lowNMSSM_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(lowNMSSM_info::parameter_names),
      std::cend(lowNMSSM_info::parameter_names));
 ...
 */
template <class T>
void lowNMSSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   lowNMSSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void lowNMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(lowNMSSM<T>& model)
{
   try {
      throw;
