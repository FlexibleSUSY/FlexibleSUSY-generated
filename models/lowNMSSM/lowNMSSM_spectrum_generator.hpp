...
#include "lowNMSSM_two_scale_convergence_tester.hpp"
...
template <class T>
class lowNMSSM_spectrum_generator {
public:
   ...
   void run(const QedQcd& oneset, const lowNMSSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "lowNMSSM_rge_running.dat") const;
 ...
 */
template <class T>
void lowNMSSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const lowNMSSM_input_parameters& input)
{
   ...
   lowNMSSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   lowNMSSM_initial_guesser<T> initial_guesser(&model, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint);
 ...
 */
template <class T>
void lowNMSSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   lowNMSSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   lowNMSSM_parameter_getter parameter_getter;
   Coupling_monitor<lowNMSSM<T>, lowNMSSM_parameter_getter>
 ...
 */
template <class T>
void lowNMSSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   lowNMSSM_spectrum_plotter plotter;
