...
#include "lowNMSSM_spectrum_generator_interface.hpp"
...
class lowNMSSM_spectrum_generator
   : public lowNMSSM_spectrum_generator_interface<T> {
public:
   lowNMSSM_spectrum_generator()
      ...
      , low_scale(0.)
   {}
   virtual ~lowNMSSM_spectrum_generator() {}

   double get_high_scale() const { return 0.; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const lowNMSSM_input_parameters&);
 ...
 */
template <class T>
void lowNMSSM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const lowNMSSM_input_parameters& input)
{
   lowNMSSM<T>& model = this->model;
   model.clear();
   ...
   lowNMSSM_susy_scale_constraint<T> susy_scale_constraint(&model, qedqcd);
   lowNMSSM_low_scale_constraint<T>  low_scale_constraint(&model, qedqcd);
   ...
   lowNMSSM_convergence_tester<T> convergence_tester(
   ...
   lowNMSSM_initial_guesser<T> initial_guesser(&model, qedqcd,
 ...
 */
template <class T>
void lowNMSSM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   lowNMSSM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, susy_scale);
}
