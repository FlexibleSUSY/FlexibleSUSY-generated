...
#include "lowNMSSM_two_scale_initial_guesser.hpp"
...
lowNMSSM_initial_guesser<Two_scale>::lowNMSSM_initial_guesser(
   lowNMSSM<Two_scale>* model_,
   const softsusy::QedQcd& qedqcd_,
   const lowNMSSM_low_scale_constraint<Two_scale>& low_constraint_,
   const lowNMSSM_susy_scale_constraint<Two_scale>& susy_constraint_
)
   : model(model_)
   , qedqcd(qedqcd_)
...
{
   if (!model)
      throw SetupError("lowNMSSM_initial_guesser: Error: pointer to model"
                       " lowNMSSM<Two_scale> must not be zero");
}
 ...
 */
void lowNMSSM_initial_guesser<Two_scale>::guess()
{
 ...
 * \endcode
 */
void lowNMSSM_initial_guesser<Two_scale>::guess_susy_parameters()
{
...
void lowNMSSM_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void lowNMSSM_initial_guesser<Two_scale>::calculate_running_SM_masses()
{
 ...
 * corrections).
 */
void lowNMSSM_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
 ...
 */
void lowNMSSM_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
 ...
 * corrections).
 */
void lowNMSSM_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
   const auto vd = MODELPARAMETER(vd);
 ...
 */
void lowNMSSM_initial_guesser<Two_scale>::guess_soft_parameters()
{
