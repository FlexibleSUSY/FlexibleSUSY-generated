...
#ifndef lowNMSSM_TWO_SCALE_INITIAL_GUESSER_H
#define lowNMSSM_TWO_SCALE_INITIAL_GUESSER_H

#include "lowNMSSM_initial_guesser.hpp"
...
/**
 * @class lowNMSSM_initial_guesser<Two_scale>
 * @brief initial guesser for the lowNMSSM
 */

template<>
class lowNMSSM_initial_guesser<Two_scale> : public Initial_guesser {
public:
   lowNMSSM_initial_guesser(lowNMSSM<Two_scale>*,
                               const softsusy::QedQcd&,
                               const lowNMSSM_low_scale_constraint<Two_scale>&,
                               const lowNMSSM_susy_scale_constraint<Two_scale>&);
   virtual ~lowNMSSM_initial_guesser() = default;
   virtual void guess() override; ///< initial guess

   void set_running_precision(double p) { running_precision = p; }

private:
   lowNMSSM<Two_scale>* model{nullptr}; ///< pointer to model class
   ...
   lowNMSSM_low_scale_constraint<Two_scale> low_constraint{};
   lowNMSSM_susy_scale_constraint<Two_scale> susy_constraint{};
