...
public:
   Eigen::ArrayXd get_parameters(const lowNMSSM<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const lowNMSSM<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const lowNMSSM<T>&);
...
template <class T>
void lowNMSSM_spectrum_plotter::extract_spectrum(const lowNMSSM<T>& model)
{
   spectrum.clear();
