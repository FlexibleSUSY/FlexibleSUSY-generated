...
/**
 * @file lowNMSSM_amm.cpp
 *
 * This file was generated with FlexibleSUSY 2.8.0 and SARAH 4.15.1 .
 */

#include "lowNMSSM_amm.hpp"
#include "lowNMSSM_mass_eigenstates.hpp"

#include "cxx_qft/lowNMSSM_qft.hpp"
#include "cxx_qft/lowNMSSM_particle_aliases.hpp"
...
double get_MLCP(const lowNMSSM_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(lowNMSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("lowNMSSM_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("lowNMSSM_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("lowNMSSM_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("lowNMSSM_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const lowNMSSM_mass_eigenstates& model, double ml_pole, int idx)
{
...
template <typename Lepton>
double calculate_amm_impl(const lowNMSSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("lowNMSSM_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double lowNMSSM_amm::calculate_amm(const lowNMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   lowNMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("lowNMSSM_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("lowNMSSM_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template <typename Lepton>
double lowNMSSM_amm::calculate_amm_uncertainty(const lowNMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   lowNMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("lowNMSSM_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("lowNMSSM_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
