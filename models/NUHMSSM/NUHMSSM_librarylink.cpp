...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSNUHMSSMMessage", 1);
...
class NUHMSSM_spectrum_impl : public NUHMSSM_spectrum
{
public:
   virtual ~NUHMSSM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(NUHMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const NUHMSSM_observables& get_observables() const override { return observables; }

   virtual const UnitarityInfiniteS& get_unitarity() const { return unitarityData; };
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const NUHMSSM_input_parameters&) override;
   virtual void calculate_model_observables(
      const softsusy::QedQcd&,
...
private:
   std::tuple<NUHMSSM<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   NUHMSSM_scales scales{};              ///< scale information
   NUHMSSM_observables observables{};    ///< observables
...
public:
   ...
   void set_input_parameters(const NUHMSSM_input_parameters& input_) { input = input_; }
...
private:
   NUHMSSM_input_parameters input{};                    ///< model input parameters
   ...
   std::unique_ptr<NUHMSSM_spectrum> spectrum{nullptr}; ///< spectrum information

   NUHMSSM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSNUHMSSMCalculateSpectrum[]?");
   }
}
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(BInput), "BInput");
...
NUHMSSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const NUHMSSM_slha& model, MLINK link)
{
   MLPutRule(link, NUHMSSM_info::model_name);
...
template <typename Solver_type>
void NUHMSSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void NUHMSSM_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const NUHMSSM_input_parameters& input
)
{
...
template <typename Solver_type>
void NUHMSSM_spectrum_impl<Solver_type>::calculate_model_observables(
   const softsusy::QedQcd& qedqcd,
...
template <typename Solver_type>
void NUHMSSM_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = NUHMSSM_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}

/******************************************************************/

template <typename Solver_type>
void NUHMSSM_spectrum_impl<Solver_type>::fill_slha_io(NUHMSSM_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   NUHMSSM_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSNUHMSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSNUHMSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new NUHMSSM_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSNUHMSSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMGetSettings"))
...
DLLEXPORT int FSNUHMSSMGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUHMSSMGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUHMSSMOpenHandle(
...
DLLEXPORT int FSNUHMSSMCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::NUHMSSM_librarylink;
...
DLLEXPORT int FSNUHMSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUHMSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUHMSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUHMSSMCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSNUHMSSMCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSNUHMSSMCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSNUHMSSMCalculateSpectrum[]?");
      }
...
DLLEXPORT int FSNUHMSSMCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::NUHMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSNUHMSSMCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSNUHMSSMCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
