...
// File generated at Sun 26 Aug 2018 15:02:30
...
public:
   ...
   virtual void fill_slha_io(NUHMSSM_slha_io&) const = 0;
...
public:
   ...
   virtual void fill_slha_io(NUHMSSM_slha_io&) const override;
...
private:
   std::tuple<NUHMSSM_slha<NUHMSSM<Solver_type>>> models{};        ///< running parameters and pole masses
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, NUHMSSM_info::particle_names[i].c_str());
      }
   }
}
...
void put_spectrum(const NUHMSSM_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void NUHMSSM_spectrum_impl<Solver_type>::fill_slha_io(NUHMSSM_slha_io& slha_io) const
{
