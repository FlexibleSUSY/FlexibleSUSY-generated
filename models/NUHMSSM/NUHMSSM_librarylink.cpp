...
// File generated at Thu 15 Dec 2016 13:05:34
...
struct NUHMSSM_data {
   ...
   NUHMSSM_slha<algorithm_type> model; ///< running parameters and pole masses
};

/// current handles
typedef std::map<Handle_id, NUHMSSM_data> Handle_map;
Handle_map handles_NUHMSSM;

/******************************************************************/

Handle_id get_new_NUHMSSM_handle()
{
   static const std::size_t max_handles =
      static_cast<std::size_t>(std::exp2(8*sizeof(Handle_id)) - 1);

   if (handles_NUHMSSM.size() >= max_handles)
      throw ENotEnoughFreeHandles(handles_NUHMSSM.size());

   Handle_id hid = 0;

   while (handles_NUHMSSM.find(hid) != handles_NUHMSSM.end())
      hid++;
...
NUHMSSM_data find_NUHMSSM_data(Handle_id hid)
{
   const Handle_map::iterator handle = handles_NUHMSSM.find(hid);

   if (handle == handles_NUHMSSM.end())
...
void put_settings(const NUHMSSM_data& data, MLINK link)
{
...
void put_sm_input_parameters(const NUHMSSM_data& data, MLINK link)
{
...
void put_input_parameters(const NUHMSSM_data& data, MLINK link)
{
...
void put_spectrum(const NUHMSSM_slha<algorithm_type>& model, MLINK link)
{
...
void put_observables(const NUHMSSM_observables& observables, MLINK link)
{
...
void check_spectrum(const NUHMSSM_data& data, MLINK link)
{
   ...
   if (problems.have_problem()) {
      ...
      put_message(link, "FSNUHMSSMCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSNUHMSSMCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(NUHMSSM_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSNUHMSSMCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = NUHMSSM_slha<algorithm_type>(
...
NUHMSSM_data make_NUHMSSM_data(double* pars, mint npars)
{
   NUHMSSM_data data;

   const mint n_settings = 23, n_sm_parameters = 29, n_input_pars = 7;
...
DLLEXPORT int FSNUHMSSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      const NUHMSSM_data data = find_NUHMSSM_data(hid);
   ...
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument Res)
{
   ...
   try {
      NUHMSSM_data data = make_NUHMSSM_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);

      const Handle_id hid = get_new_NUHMSSM_handle();

      handles_NUHMSSM.insert(std::make_pair(hid, std::move(data)));
   ...
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   ...
   if (handle == handles_NUHMSSM.end()) {
      std::cerr << "Error: FSNUHMSSMSet: Unknown handle: "
                << hid << std::endl;
      return LIBRARY_FUNCTION_ERROR;
   }

   try {
      handle->second = make_NUHMSSM_data(
         libData->MTensor_getRealData(pars),
         libData->MTensor_getDimensions(pars)[0]);
   } catch (const flexiblesusy::Error& e) {
      std::cerr << e.what() << std::endl;
   ...
   WolframLibraryData /* libData */, MLINK link)
{
   ...
   try {
      NUHMSSM_data data = find_NUHMSSM_data(hid);
      ...
      handles_NUHMSSM[hid] = std::move(data);
