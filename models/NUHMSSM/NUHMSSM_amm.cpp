...
#include "cxx_qft/NUHMSSM_particle_aliases.hpp"
...
double get_MLCP(const NUHMSSM_mass_eigenstates_interface& model)
{
...
void run_to_MSUSY(NUHMSSM_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("NUHMSSM_amm: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("NUHMSSM_amm:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("NUHMSSM_amm: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("NUHMSSM_amm: run_to_MSUSY(): "
...
template <typename Lepton>
double calculate_amm_impl_1l(const context_base& context, const NUHMSSM_mass_eigenstates& model, double ml_pole, int idx)
{
...
template <typename Lepton>
double calculate_amm_impl(const NUHMSSM_mass_eigenstates& model, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   VERBOSE_MSG("NUHMSSM_amm: calculating a_mu at Q = " << model.get_scale());
...
template <typename Lepton>
double NUHMSSM_amm::calculate_amm(const NUHMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   NUHMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("NUHMSSM_amm: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUHMSSM_amm:" << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
...
template <typename Lepton>
double NUHMSSM_amm::calculate_amm_uncertainty(const NUHMSSM_mass_eigenstates& model_, const softsusy::QedQcd& qedqcd, const Spectrum_generator_settings& settings, int idx)
{
   NUHMSSM_mass_eigenstates model(model_);

   VERBOSE_MSG("NUHMSSM_amm: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("NUHMSSM_amm uncertainty: " << e.what_detailed());
      return std::numeric_limits<double>::quiet_NaN();
   }
