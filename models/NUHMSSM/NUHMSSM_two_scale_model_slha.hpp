...
/**
 * @class NUHMSSM_slha<Two_scale>
 * @brief model class wrapper in SLHA convention
 */

template<>
class NUHMSSM_slha<Two_scale> : public NUHMSSM<Two_scale> {
public:
   explicit NUHMSSM_slha(const NUHMSSM_input_parameters& input_ = NUHMSSM_input_parameters());
   explicit NUHMSSM_slha(const NUHMSSM<Two_scale>&, bool do_convert_masses_to_slha = true);
   virtual ~NUHMSSM_slha();
...
std::ostream& operator<<(std::ostream& ostr, const NUHMSSM_slha<Two_scale>& model);
