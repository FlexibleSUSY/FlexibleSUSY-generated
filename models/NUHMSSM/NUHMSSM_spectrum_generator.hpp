...
#include "NUHMSSM_spectrum_generator_interface.hpp"
...
class NUHMSSM_spectrum_generator
   : public NUHMSSM_spectrum_generator_interface<T> {
public:
   NUHMSSM_spectrum_generator()
      : NUHMSSM_spectrum_generator_interface<T>()
      ...
      , low_scale(0.)
   {}
   virtual ~NUHMSSM_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const NUHMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename = "NUHMSSM_rgflow.dat") const;
 ...
 */
template <class T>
void NUHMSSM_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const NUHMSSM_input_parameters& input)
{
   NUHMSSM<T>& model = this->model;
   model.clear();
   ...
   NUHMSSM_convergence_tester<T> convergence_tester(&model, this->precision_goal);
   if (this->max_iterations > 0)
      convergence_tester.set_max_iterations(this->max_iterations);

   NUHMSSM_initial_guesser<T> initial_guesser(&model, qedqcd,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
   ...
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : NUHMSSM_info::parameter_names[parameter_index];
 ...
 */
template <class T>
void NUHMSSM_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   NUHMSSM_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
