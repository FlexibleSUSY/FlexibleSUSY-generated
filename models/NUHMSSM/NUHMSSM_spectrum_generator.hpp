...
template <class T>
class NUHMSSM_spectrum_generator {
public:
   NUHMSSM_spectrum_generator()
      : solver(), model()
   ...
   const NUHMSSM<T>& get_model() const { return model; }
   ...
   void run(const QedQcd& oneset, const NUHMSSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "NUHMSSM_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "NUHMSSM_spectrum.dat") const;

private:
   RGFlow<T> solver;
   NUHMSSM<T> model;
   NUHMSSM_high_scale_constraint<T> high_scale_constraint;
   NUHMSSM_susy_scale_constraint<T> susy_scale_constraint;
   NUHMSSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void NUHMSSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const NUHMSSM_input_parameters& input)
{
   ...
   NUHMSSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   NUHMSSM_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void NUHMSSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   NUHMSSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   NUHMSSM_parameter_getter parameter_getter;
   Coupling_monitor<NUHMSSM<T>, NUHMSSM_parameter_getter>
 ...
 */
template <class T>
void NUHMSSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
