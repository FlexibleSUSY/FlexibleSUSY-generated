...
// NUHMSSM_parameter_getter ////////////////////////////////////////

std::vector<std::string> NUHMSSM_parameter_getter::get_mass_names(const std::string& head)
{
...
std::array<std::string, NUHMSSM_info::NUMBER_OF_MIXINGS> NUHMSSM_parameter_getter::get_mixing_names()
{
   return NUHMSSM_info::particle_mixing_names;
}

Eigen::ArrayXd NUHMSSM_parameter_getter::get_parameters(const NUHMSSM_mass_eigenstates& model)
{
   return model.get();
}

std::array<std::string, NUHMSSM_info::NUMBER_OF_PARAMETERS> NUHMSSM_parameter_getter::get_parameter_names()
{
   return NUHMSSM_info::parameter_names;
}

std::array<std::string, NUHMSSM_info::NUMBER_OF_PARTICLES> NUHMSSM_parameter_getter::get_particle_names()
{
   return NUHMSSM_info::particle_names;
}

std::vector<std::string> NUHMSSM_parameter_getter::get_DRbar_mass_names()
{
   return get_mass_names();
}

std::vector<std::string> NUHMSSM_parameter_getter::get_pole_mass_names()
{
   return get_mass_names("Pole");
}

std::array<std::string, NUHMSSM_info::NUMBER_OF_MIXINGS> NUHMSSM_parameter_getter::get_DRbar_mixing_names()
{
   return get_mixing_names();
}

std::array<std::string, NUHMSSM_info::NUMBER_OF_MIXINGS> NUHMSSM_parameter_getter::get_pole_mixing_names()
{
...
std::array<std::string, NUHMSSM_info::NUMBER_OF_INPUT_PARAMETERS> NUHMSSM_parameter_getter::get_input_parameter_names()
{
   return NUHMSSM_info::input_parameter_names;
}

std::array<std::string, NUHMSSM_info::NUMBER_OF_EXTRA_PARAMETERS> NUHMSSM_parameter_getter::get_extra_parameter_names()
{
   return NUHMSSM_info::extra_parameter_names;
}

decltype(NUHMSSM_info::NUMBER_OF_MASSES) NUHMSSM_parameter_getter::get_number_of_masses()
{
   return NUHMSSM_info::NUMBER_OF_MASSES;
}

// NUHMSSM_spectrum_plotter ////////////////////////////////////////

NUHMSSM_spectrum_plotter::NUHMSSM_spectrum_plotter(const NUHMSSM_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void NUHMSSM_spectrum_plotter::extract_spectrum(const NUHMSSM_mass_eigenstates& model)
{
   spectrum.clear();
...
void NUHMSSM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;
...
{
   if (!filestr.good()) {
      ERROR("NUHMSSM_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const NUHMSSM_mass_eigenstates& model,
   ...
   const NUHMSSM_observables* observables)
{
   ...
   append(names, NUHMSSM_parameter_getter::get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, NUHMSSM_parameter_getter::get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, NUHMSSM_parameter_getter::get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, NUHMSSM_parameter_getter::get_DRbar_mass_names());
   append(names, NUHMSSM_parameter_getter::get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, NUHMSSM_parameter_getter::get_pole_mass_names());
   append(names, NUHMSSM_parameter_getter::get_pole_mixing_names());
   ...
   if (NUHMSSM_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, NUHMSSM_observables::get_names());
...
NUHMSSM_mass_eigenstates from_database(
   ...
   Physical_input* physical_input, NUHMSSM_observables* observables)
{
   NUHMSSM_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = NUHMSSM_parameter_getter::get_number_of_masses();
   const auto number_of_mixings = NUHMSSM_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = NUHMSSM_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = NUHMSSM_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? NUHMSSM_observables::NUMBER_OF_OBSERVABLES : 0);
