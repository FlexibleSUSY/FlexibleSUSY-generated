...
template <class T>
class NUHMSSM_spectrum_generator_interface {
public:
   virtual ~NUHMSSM_spectrum_generator_interface() = default;

   std::tuple<NUHMSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<NUHMSSM<T>> get_models_slha() const
   { return std::make_tuple(NUHMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const NUHMSSM<T>& get_model() const
   { return model; }
   NUHMSSM<T>& get_model()
   { return model; }
   NUHMSSM<T> get_model_slha() const
   { return NUHMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const NUHMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "NUHMSSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(NUHMSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const NUHMSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void NUHMSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void NUHMSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const NUHMSSM_input_parameters& input)
{
 ...
 */
template <class T>
void NUHMSSM_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   NUHMSSM_mass_eigenstates tmp_model(model);
   try {
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return NUHMSSM_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(NUHMSSM_info::parameter_names),
      std::cend(NUHMSSM_info::parameter_names));
 ...
 */
template <class T>
void NUHMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(NUHMSSM<T>& model)
{
   try {
      throw;
