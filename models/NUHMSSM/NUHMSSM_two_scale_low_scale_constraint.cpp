...
// File generated at Wed 16 Oct 2019 22:42:28
...
const Eigen::Matrix<std::complex<double>,3,3>& NUHMSSM_low_scale_constraint<Two_scale>::get_ckm()
{
   return ckm;
}

const Eigen::Matrix<std::complex<double>,3,3>& NUHMSSM_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}

double NUHMSSM_low_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double NUHMSSM_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void NUHMSSM_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<NUHMSSM<Two_scale>*>(model_);
}

void NUHMSSM_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& NUHMSSM_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void NUHMSSM_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
...
void NUHMSSM_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
   check_model_ptr();
...
double NUHMSSM_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();

   double theta_w = std::asin(Electroweak_constants::sinThetaW);

   NUHMSSM_weinberg_angle::Sm_parameters sm_pars;
   ...
   NUHMSSM_weinberg_angle weinberg(MODEL, sm_pars);
...
void NUHMSSM_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   check_model_ptr();
...
double NUHMSSM_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   check_model_ptr();
...
double NUHMSSM_low_scale_constraint<Two_scale>::calculate_delta_alpha_s(double alphaS) const
{
   check_model_ptr();
...
double NUHMSSM_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
void NUHMSSM_low_scale_constraint<Two_scale>::calculate_running_SM_masses()
{
   check_model_ptr();
...
void NUHMSSM_low_scale_constraint<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void NUHMSSM_low_scale_constraint<Two_scale>::calculate_Yu_DRbar()
{
   check_model_ptr();
...
void NUHMSSM_low_scale_constraint<Two_scale>::calculate_Yd_DRbar()
{
   check_model_ptr();
...
void NUHMSSM_low_scale_constraint<Two_scale>::calculate_Ye_DRbar()
{
   check_model_ptr();
...
void NUHMSSM_low_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("NUHMSSM_low_scale_constraint<Two_scale>: "
