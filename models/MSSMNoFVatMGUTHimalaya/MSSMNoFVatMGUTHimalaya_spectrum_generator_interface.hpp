...
public:
   ...
   std::tuple<MSSMNoFVatMGUTHimalaya<T>> get_models_slha() const
   { return std::make_tuple(MSSMNoFVatMGUTHimalaya<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const MSSMNoFVatMGUTHimalaya<T>& get_model() const
   { return model; }
   MSSMNoFVatMGUTHimalaya<T>& get_model()
   { return model; }
   MSSMNoFVatMGUTHimalaya<T> get_model_slha() const
   { return MSSMNoFVatMGUTHimalaya<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return MSSMNoFVatMGUTHimalaya_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(MSSMNoFVatMGUTHimalaya_info::parameter_names),
      std::cend(MSSMNoFVatMGUTHimalaya_info::parameter_names));
