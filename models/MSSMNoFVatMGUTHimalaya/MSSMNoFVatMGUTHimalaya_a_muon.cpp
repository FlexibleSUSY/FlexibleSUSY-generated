...
// File generated at Tue 22 Jan 2019 13:27:21

/**
 * @file MSSMNoFVatMGUTHimalaya_a_muon.cpp
 *
 * This file was generated at Tue 22 Jan 2019 13:27:21 with FlexibleSUSY
 * 2.3.0 and SARAH 4.14.1 .
 */

#include "MSSMNoFVatMGUTHimalaya_a_muon.hpp"
...
double get_MSUSY(const MSSMNoFVatMGUTHimalaya_mass_eigenstates& model)
{
...
void run_to_MSUSY(MSSMNoFVatMGUTHimalaya_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("MSSMNoFVatMGUTHimalaya_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("MSSMNoFVatMGUTHimalaya_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("MSSMNoFVatMGUTHimalaya_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("MSSMNoFVatMGUTHimalaya_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(MSSMNoFVatMGUTHimalaya_mass_eigenstates& model)
{
   VERBOSE_MSG("MSSMNoFVatMGUTHimalaya_a_muon: calculating a_mu at Q = " << model.get_scale());
   ...
   val += AMuonVertexCorrectionFS<Fm, Ah>::value(indices, context);
   val += AMuonVertexCorrectionSF<Sm, Chi>::value(indices, context);
   val += AMuonVertexCorrectionSF<Hpm, Fvm>::value(indices, context);
   val += AMuonVertexCorrectionFS<Fm, hh>::value(indices, context);
   val += AMuonVertexCorrectionFS<Cha, SvmL>::value(indices, context);
...
std::pair<double,double> vary_scale(const MSSMNoFVatMGUTHimalaya_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("MSSMNoFVatMGUTHimalaya_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
{
   return context.model.get_physical().MFm;
}

} // anonymous namespace

double MSSMNoFVatMGUTHimalaya_a_muon::calculate_a_muon(const MSSMNoFVatMGUTHimalaya_mass_eigenstates& model_)
{
   MSSMNoFVatMGUTHimalaya_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMNoFVatMGUTHimalaya_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMNoFVatMGUTHimalaya_a_muon:" << e.what());
...
double MSSMNoFVatMGUTHimalaya_a_muon::calculate_a_muon_uncertainty(const MSSMNoFVatMGUTHimalaya_mass_eigenstates& model_)
{
   MSSMNoFVatMGUTHimalaya_mass_eigenstates model(model_);

   VERBOSE_MSG("MSSMNoFVatMGUTHimalaya_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("MSSMNoFVatMGUTHimalaya_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
