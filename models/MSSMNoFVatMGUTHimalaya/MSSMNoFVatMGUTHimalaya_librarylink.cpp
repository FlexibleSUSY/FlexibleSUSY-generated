...
#include "lowe.h"
#include "MSSMNoFVatMGUTHimalaya_unitarity.hpp"
...
namespace MSSMNoFVatMGUTHimalaya_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMSSMNoFVatMGUTHimalayaMessage", 1);
...
class MSSMNoFVatMGUTHimalaya_spectrum {
public:
   virtual ~MSSMNoFVatMGUTHimalaya_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(MSSMNoFVatMGUTHimalaya_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const MSSMNoFVatMGUTHimalaya_observables& get_observables() const = 0;

   virtual const UnitarityInfiniteS& get_unitarity() const = 0;
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMNoFVatMGUTHimalaya_input_parameters&) = 0;
...
class MSSMNoFVatMGUTHimalaya_spectrum_impl : public MSSMNoFVatMGUTHimalaya_spectrum
{
public:
   virtual ~MSSMNoFVatMGUTHimalaya_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(MSSMNoFVatMGUTHimalaya_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const MSSMNoFVatMGUTHimalaya_observables& get_observables() const override { return observables; }

   virtual const UnitarityInfiniteS& get_unitarity() const { return unitarityData; };
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const MSSMNoFVatMGUTHimalaya_input_parameters&) override;
   virtual void calculate_model_observables(
      const softsusy::QedQcd&,
...
private:
   std::tuple<MSSMNoFVatMGUTHimalaya<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   MSSMNoFVatMGUTHimalaya_scales scales{};              ///< scale information
   MSSMNoFVatMGUTHimalaya_observables observables{};    ///< observables
...
public:
   ...
   void set_input_parameters(const MSSMNoFVatMGUTHimalaya_input_parameters& input_) { input = input_; }
...
private:
   MSSMNoFVatMGUTHimalaya_input_parameters input{};                    ///< model input parameters
   ...
   std::unique_ptr<MSSMNoFVatMGUTHimalaya_spectrum> spectrum{nullptr}; ///< spectrum information

   MSSMNoFVatMGUTHimalaya_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSMSSMNoFVatMGUTHimalayaCalculateSpectrum[]?");
   }
}
...
MSSMNoFVatMGUTHimalaya_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const MSSMNoFVatMGUTHimalaya_slha& model, MLINK link)
{
   MLPutRule(link, MSSMNoFVatMGUTHimalaya_info::model_name);
...
template <typename Solver_type>
void MSSMNoFVatMGUTHimalaya_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void MSSMNoFVatMGUTHimalaya_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const MSSMNoFVatMGUTHimalaya_input_parameters& input
)
{
...
template <typename Solver_type>
void MSSMNoFVatMGUTHimalaya_spectrum_impl<Solver_type>::calculate_model_observables(
   const softsusy::QedQcd& qedqcd,
...
template <typename Solver_type>
void MSSMNoFVatMGUTHimalaya_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = MSSMNoFVatMGUTHimalaya_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}

/******************************************************************/

template <typename Solver_type>
void MSSMNoFVatMGUTHimalaya_spectrum_impl<Solver_type>::fill_slha_io(MSSMNoFVatMGUTHimalaya_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   MSSMNoFVatMGUTHimalaya_observables observables = spectrum->get_observables();
...
void Model_data::put_unitarity(MLINK link) const
{
   ...
   MLPutFunction(link, "List", 1);
   MLPutRule(link, "MSSMNoFVatMGUTHimalaya");
   MLPutFunction(link, "List", 3);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSMSSMNoFVatMGUTHimalayaCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSMSSMNoFVatMGUTHimalayaCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new MSSMNoFVatMGUTHimalaya_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSMSSMNoFVatMGUTHimalayaGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUTHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTHimalayaGetSettings"))
...
DLLEXPORT int FSMSSMNoFVatMGUTHimalayaGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUTHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTHimalayaGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVatMGUTHimalayaGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUTHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTHimalayaGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVatMGUTHimalayaOpenHandle(
...
DLLEXPORT int FSMSSMNoFVatMGUTHimalayaCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::MSSMNoFVatMGUTHimalaya_librarylink;
...
DLLEXPORT int FSMSSMNoFVatMGUTHimalayaSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::MSSMNoFVatMGUTHimalaya_librarylink;
...
DLLEXPORT int FSMSSMNoFVatMGUTHimalayaGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUTHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTHimalayaGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVatMGUTHimalayaToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUTHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTHimalayaToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVatMGUTHimalayaGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUTHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTHimalayaGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVatMGUTHimalayaCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUTHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTHimalayaCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMSSMNoFVatMGUTHimalayaCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUTHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTHimalayaCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSMSSMNoFVatMGUTHimalayaCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMSSMNoFVatMGUTHimalayaCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMSSMNoFVatMGUTHimalayaCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSMSSMNoFVatMGUTHimalayaCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::MSSMNoFVatMGUTHimalaya_librarylink;

   if (!check_number_of_args(link, 1, "FSMSSMNoFVatMGUTHimalayaCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSMSSMNoFVatMGUTHimalayaCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
