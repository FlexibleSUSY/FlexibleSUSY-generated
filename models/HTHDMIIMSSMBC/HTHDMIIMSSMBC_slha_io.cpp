 ...
 * SLHA output file.
 */
void HTHDMIIMSSMBC_slha_io::fill(HTHDMIIMSSMBC_slha& model) const
{
 ...
 */
void HTHDMIIMSSMBC_slha_io::set_model_parameters(const HTHDMIIMSSMBC_slha& model)
{
   {
 ...
 */
void HTHDMIIMSSMBC_slha_io::set_dcinfo(const FlexibleDecay_problems& problems)
{
 ...
 * @param warnings vector of warning strings
 */
void HTHDMIIMSSMBC_slha_io::set_dcinfo(
   const std::vector<std::string>& problems,
   const std::vector<std::string>& warnings)
{
   ...
   dcinfo << FORMAT_SPINFO(5, HTHDMIIMSSMBC_info::model_name)
          << FORMAT_SPINFO(9, SARAH_VERSION);
 ...
 */
void HTHDMIIMSSMBC_slha_io::set_decay_block(const Decays_list& decays_list, FlexibleDecay_settings const& flexibledecay_settings)
{
   const auto pdg = decays_list.get_particle_id();
   const auto width = decays_list.get_total_width();
   const std::string name = HTHDMIIMSSMBC_info::get_particle_name_from_pdg(pdg);
   ...
   if (!is_zero(width, 1e-100)) {
      ...
      for (const auto& channel : sorted_decays_list) {
         ...
         for (auto id : final_state) {
            comment += " " + HTHDMIIMSSMBC_info::get_particle_name_from_pdg(id);
         }
         comment += ")";
 ...
 */
void HTHDMIIMSSMBC_slha_io::set_spectrum(const HTHDMIIMSSMBC_slha& model)
{
...
void HTHDMIIMSSMBC_slha_io::set_extra(
   const HTHDMIIMSSMBC_slha& model,
   const HTHDMIIMSSMBC_scales& scales,
...
void HTHDMIIMSSMBC_slha_io::write_to_file(const std::string& file_name) const
{
   slha_io.write_to_file(file_name);
}

void HTHDMIIMSSMBC_slha_io::write_to_stream() const
{
   write_to_stream(std::cout);
}

void HTHDMIIMSSMBC_slha_io::write_to_stream(std::ostream& ostr) const
{
 ...
 * @param settings struct of spectrum generator settings to be filled
 */
void HTHDMIIMSSMBC_slha_io::fill(FlexibleDecay_settings& settings) const
{
   slha_io.fill(settings);
}
