...
public:
   HTHDMIIMSSMBC_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~HTHDMIIMSSMBC_spectrum_generator_interface() {}

   const HTHDMIIMSSMBC<T>& get_model() const { return model; }
   HTHDMIIMSSMBC<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const HTHDMIIMSSMBC_input_parameters&) = 0;
 ...
 */
template <class T>
void HTHDMIIMSSMBC_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   HTHDMIIMSSMBC_spectrum_plotter plotter;
