...
// File generated at Sun 26 Aug 2018 14:08:05

#ifndef HTHDMIIMSSMBC_SPECTRUM_GENERATOR_INTERFACE_H
#define HTHDMIIMSSMBC_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class HTHDMIIMSSMBC_spectrum_generator_interface {
public:
   virtual ~HTHDMIIMSSMBC_spectrum_generator_interface() = default;

   std::tuple<HTHDMIIMSSMBC<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<HTHDMIIMSSMBC_slha<HTHDMIIMSSMBC<T>>> get_models_slha() const
   { return std::make_tuple(HTHDMIIMSSMBC_slha<HTHDMIIMSSMBC<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const HTHDMIIMSSMBC<T>& get_model() const
   { return model; }
   HTHDMIIMSSMBC<T>& get_model()
   { return model; }
   HTHDMIIMSSMBC_slha<HTHDMIIMSSMBC<T>> get_model_slha() const
   { return HTHDMIIMSSMBC_slha<HTHDMIIMSSMBC<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const HTHDMIIMSSMBC_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "HTHDMIIMSSMBC_spectrum.dat") const;

protected:
   HTHDMIIMSSMBC<T> model;
   ...
   void translate_exception_to_problem(HTHDMIIMSSMBC<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const HTHDMIIMSSMBC_input_parameters&) = 0;
};
 ...
 */
template <class T>
void HTHDMIIMSSMBC_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void HTHDMIIMSSMBC_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const HTHDMIIMSSMBC_input_parameters& input)
{
 ...
 */
template <class T>
void HTHDMIIMSSMBC_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   HTHDMIIMSSMBC_mass_eigenstates tmp_model(model);
   try {
   ...
   HTHDMIIMSSMBC_parameter_getter parameter_getter;
   Coupling_monitor<HTHDMIIMSSMBC_mass_eigenstates, HTHDMIIMSSMBC_parameter_getter>
 ...
 */
template <class T>
void HTHDMIIMSSMBC_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   HTHDMIIMSSMBC_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void HTHDMIIMSSMBC_spectrum_generator_interface<T>::translate_exception_to_problem(HTHDMIIMSSMBC<T>& model)
{
   try {
      throw;
