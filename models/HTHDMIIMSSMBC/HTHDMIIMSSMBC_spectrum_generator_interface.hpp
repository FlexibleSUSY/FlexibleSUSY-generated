...
public:
   ...
   std::tuple<HTHDMIIMSSMBC<T>> get_models_slha() const
   { return std::make_tuple(HTHDMIIMSSMBC<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const HTHDMIIMSSMBC<T>& get_model() const
   { return model; }
   HTHDMIIMSSMBC<T>& get_model()
   { return model; }
   HTHDMIIMSSMBC<T> get_model_slha() const
   { return HTHDMIIMSSMBC<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return HTHDMIIMSSMBC_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(HTHDMIIMSSMBC_info::parameter_names),
      std::cend(HTHDMIIMSSMBC_info::parameter_names));
