...
// File generated at Wed 29 Jun 2016 11:22:46

#include "HTHDMIIMSSMBC_two_scale_low_scale_constraint.hpp"
...
HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::HTHDMIIMSSMBC_low_scale_constraint()
...
void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::apply()
{
   assert(model && "Error: HTHDMIIMSSMBC_low_scale_constraint::apply():"
   ...
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
   MODEL->set_v1(Re((2*MZDRbar)/(Sqrt(0.6*Sqr(g1) + Sqr(g2))*Sqrt(1 + Sqr(
      TanBeta)))));
   MODEL->set_v2(Re((2*MZDRbar*TanBeta)/(Sqrt(0.6*Sqr(g1) + Sqr(g2))*Sqrt(1 +
      Sqr(TanBeta)))));
   MODEL->set_g1(new_g1);
   MODEL->set_g2(new_g2);
   MODEL->set_g3(new_g3);
...
const Eigen::Matrix<std::complex<double>,3,3>& HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::get_ckm()
{
   return ckm;
}

const Eigen::Matrix<std::complex<double>,3,3>& HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}

double HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
   model = cast_model<HTHDMIIMSSMBC<Two_scale>*>(model_);
}

void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
   initial_scale_guess = 0.;
   model = NULL;
...
void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::initialize()
{
   assert(model && "HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::"
          "initialize(): model pointer is zero.");

   initial_scale_guess = qedqcd.displayPoleMt();
...
void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::update_scale()
{
   assert(model && "HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::"
          "update_scale(): model pointer is zero.");

   scale = qedqcd.displayPoleMt();


}

void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
...
double HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_theta_w(double alpha_em_drbar)
{
   assert(model && "HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::"
...
void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   assert(model && "HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::"
...
double HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   assert(model && "HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::"
   ...
   const double delta_alpha_em = 0.15915494309189535*alphaEm*(
      -1.3333333333333333*FiniteLog(Abs(MCha/currentScale)) - 0.3333333333333333*
      FiniteLog(Abs(MHm(1)/currentScale)));

   return delta_alpha_em + delta_alpha_em_SM;

}

double HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_delta_alpha_s(double alphaS) const
{
   assert(model && "HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::"
...
void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
...
void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_Yu_DRbar()
{
   assert(model && "HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::"
   ...
   MODEL->set_Yu((-((1.4142135623730951*topDRbar)/v2).transpose()).real());

}

void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_Yd_DRbar()
{
   assert(model && "HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::"
   ...
   MODEL->set_Yd((((1.4142135623730951*bottomDRbar)/v1).transpose()).real());

}

void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_Ye_DRbar()
{
   assert(model && "HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::"
   ...
   const auto v1 = MODELPARAMETER(v1);
   MODEL->set_Ye((((1.4142135623730951*electronDRbar)/v1).transpose()).real());

}

void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_MNeutrino_DRbar()
{
   assert(model && "HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::"
 ...
 */
void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::recalculate_mw_pole()
{
   assert(model && "HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::"
   ...
   if (mw_pole_sqr < 0.)
      MODEL->get_problems().flag_tachyon(HTHDMIIMSSMBC_info::VWm);
