...
// File generated at Wed 16 Oct 2019 21:26:59
...
void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   ...
   if (IsFinite(new_g1)) {
      model->get_problems().unflag_non_perturbative_parameter(HTHDMIIMSSMBC_info::
         g1);
   } else {
...
double HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   ...
   const double delta_alpha_em = 0.15915494309189535*alphaEm*(-1.3333333333333333*
      FiniteLog(Abs(MCha/currentScale)) - 0.3333333333333333*FiniteLog(Abs(MHm(1)/
      currentScale)));
...
void HTHDMIIMSSMBC_low_scale_constraint<Two_scale>::calculate_running_SM_masses()
{
   check_model_ptr();
