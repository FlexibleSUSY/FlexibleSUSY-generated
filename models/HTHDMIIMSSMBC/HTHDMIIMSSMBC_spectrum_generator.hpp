...
// File generated at Tue 8 Mar 2016 18:08:05

#ifndef HTHDMIIMSSMBC_SPECTRUM_GENERATOR_H
#define HTHDMIIMSSMBC_SPECTRUM_GENERATOR_H

#include "HTHDMIIMSSMBC_spectrum_generator_interface.hpp"
...
#include "HTHDMIIMSSMBC_two_scale_convergence_tester.hpp"
#include "HTHDMIIMSSMBC_two_scale_initial_guesser.hpp"
...
template <class T>
class HTHDMIIMSSMBC_spectrum_generator
   : public HTHDMIIMSSMBC_spectrum_generator_interface<T> {
public:
   HTHDMIIMSSMBC_spectrum_generator()
      : HTHDMIIMSSMBC_spectrum_generator_interface<T>()
      , solver()
      ...
      , low_scale(0.)
   {}
   virtual ~HTHDMIIMSSMBC_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const HTHDMIIMSSMBC_input_parameters&);
   void write_running_couplings(const std::string& filename = "HTHDMIIMSSMBC_rgflow.dat") const;

private:
   RGFlow<T> solver;
   HTHDMIIMSSMBC_high_scale_constraint<T> high_scale_constraint;
   HTHDMIIMSSMBC_susy_scale_constraint<T> susy_scale_constraint;
   HTHDMIIMSSMBC_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void HTHDMIIMSSMBC_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const HTHDMIIMSSMBC_input_parameters& input)
{
   HTHDMIIMSSMBC<T>& model = this->model;
   model.clear();
   ...
   HTHDMIIMSSMBC_convergence_tester<T> convergence_tester(&model, this->precision_goal);
   if (this->max_iterations > 0)
      convergence_tester.set_max_iterations(this->max_iterations);

   HTHDMIIMSSMBC_initial_guesser<T> initial_guesser(&model, qedqcd,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
   ...
   try {
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : HTHDMIIMSSMBC_info::parameter_names[parameter_index];
 ...
 */
template <class T>
void HTHDMIIMSSMBC_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   HTHDMIIMSSMBC_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
