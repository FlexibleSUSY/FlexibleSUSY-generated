...
// File generated at Sun 4 Aug 2019 18:59:49
...
public:
   ...
   virtual void fill_slha_io(HTHDMIIMSSMBC_slha_io&) const = 0;
...
public:
   ...
   virtual void fill_slha_io(HTHDMIIMSSMBC_slha_io&) const override;
...
private:
   std::tuple<HTHDMIIMSSMBC_slha<HTHDMIIMSSMBC<Solver_type>>> models{};        ///< running parameters and pole masses
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, HTHDMIIMSSMBC_info::particle_names[i].c_str());
      }
   }
}
...
void put_spectrum(const HTHDMIIMSSMBC_slha<Model>& model, MLINK link)
{
...
template <typename Solver_type>
void HTHDMIIMSSMBC_spectrum_impl<Solver_type>::fill_slha_io(HTHDMIIMSSMBC_slha_io& slha_io) const
{
