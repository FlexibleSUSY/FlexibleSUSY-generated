...
#include "lowe.h"
#include "HTHDMIIMSSMBC_unitarity.hpp"
...
namespace HTHDMIIMSSMBC_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSHTHDMIIMSSMBCMessage", 1);
...
class HTHDMIIMSSMBC_spectrum {
public:
   virtual ~HTHDMIIMSSMBC_spectrum() = default;
...
class HTHDMIIMSSMBC_spectrum_impl : public HTHDMIIMSSMBC_spectrum
{
public:
   virtual ~HTHDMIIMSSMBC_spectrum_impl() = default;
   ...
   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const HTHDMIIMSSMBC_input_parameters&) override;
   virtual void calculate_model_observables(
      const softsusy::QedQcd&,
...
private:
   std::tuple<HTHDMIIMSSMBC<Solver_type>> models{};        ///< running parameters and pole masses
...
public:
   ...
   void set_input_parameters(const HTHDMIIMSSMBC_input_parameters& input_) { input = input_; }
...
private:
   HTHDMIIMSSMBC_input_parameters input{};                    ///< model input parameters
   ...
   HTHDMIIMSSMBC_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSHTHDMIIMSSMBCCalculateSpectrum[]?");
   }
}
...
HTHDMIIMSSMBC_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const HTHDMIIMSSMBC_slha& model, MLINK link)
{
   MLPutRule(link, HTHDMIIMSSMBC_info::model_name);
   MLPutFunction(link, "List", 68);
...
template <typename Solver_type>
void HTHDMIIMSSMBC_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void HTHDMIIMSSMBC_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const HTHDMIIMSSMBC_input_parameters& input
)
{
   HTHDMIIMSSMBC_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void HTHDMIIMSSMBC_spectrum_impl<Solver_type>::calculate_model_observables(
   const softsusy::QedQcd& qedqcd,
...
template <typename Solver_type>
void HTHDMIIMSSMBC_spectrum_impl<Solver_type>::calculate_unitarity() {
   unitarityData = HTHDMIIMSSMBC_unitarity::max_scattering_eigenvalue_infinite_s(std::get<0>(models));
}
...
{
   check_spectrum_pointer();
   HTHDMIIMSSMBC_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSHTHDMIIMSSMBCCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSHTHDMIIMSSMBCCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new HTHDMIIMSSMBC_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   HTHDMIIMSSMBC_input_parameters input;
...
DLLEXPORT int FSHTHDMIIMSSMBCGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HTHDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSHTHDMIIMSSMBCGetSettings"))
...
DLLEXPORT int FSHTHDMIIMSSMBCGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HTHDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSHTHDMIIMSSMBCGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHTHDMIIMSSMBCGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HTHDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSHTHDMIIMSSMBCGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHTHDMIIMSSMBCOpenHandle(
...
DLLEXPORT int FSHTHDMIIMSSMBCCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::HTHDMIIMSSMBC_librarylink;
...
DLLEXPORT int FSHTHDMIIMSSMBCSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::HTHDMIIMSSMBC_librarylink;
...
DLLEXPORT int FSHTHDMIIMSSMBCGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HTHDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSHTHDMIIMSSMBCGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHTHDMIIMSSMBCToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HTHDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSHTHDMIIMSSMBCToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHTHDMIIMSSMBCGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HTHDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSHTHDMIIMSSMBCGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHTHDMIIMSSMBCCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HTHDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSHTHDMIIMSSMBCCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSHTHDMIIMSSMBCCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HTHDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSHTHDMIIMSSMBCCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSHTHDMIIMSSMBCCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSHTHDMIIMSSMBCCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSHTHDMIIMSSMBCCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSHTHDMIIMSSMBCCalculateUnitarity(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::HTHDMIIMSSMBC_librarylink;

   if (!check_number_of_args(link, 1, "FSHTHDMIIMSSMBCCalculateUnitarity"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      put_message(link, "FSHTHDMIIMSSMBCCalculateUnitarity", "error", e.what());
      put_error_output(link);
   }
