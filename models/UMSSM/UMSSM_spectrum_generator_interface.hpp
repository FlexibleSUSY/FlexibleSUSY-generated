...
template <class T>
class UMSSM_spectrum_generator_interface {
public:
   virtual ~UMSSM_spectrum_generator_interface() = default;

   std::tuple<UMSSM<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<UMSSM<T>> get_models_slha() const
   { return std::make_tuple(UMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   const UMSSM<T>& get_model() const
   { return model; }
   UMSSM<T>& get_model()
   { return model; }
   UMSSM<T> get_model_slha() const
   { return UMSSM<T>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const UMSSM_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "UMSSM_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(UMSSM<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const UMSSM_input_parameters&) = 0;
};
 ...
 */
template <class T>
void UMSSM_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void UMSSM_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const UMSSM_input_parameters& input)
{
 ...
 */
template <class T>
void UMSSM_spectrum_generator_interface<T>::write_running_couplings(
   ...
   double start, double stop) const
{
   ...
   auto data_getter = [&tmp_model](double scale) {
      ...
      return UMSSM_parameter_getter::get_parameters(tmp_model);
   };

   std::vector<std::string> parameter_names(
      std::cbegin(UMSSM_info::parameter_names),
      std::cend(UMSSM_info::parameter_names));
 ...
 */
template <class T>
void UMSSM_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   UMSSM_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void UMSSM_spectrum_generator_interface<T>::translate_exception_to_problem(UMSSM<T>& model)
{
   try {
      throw;
