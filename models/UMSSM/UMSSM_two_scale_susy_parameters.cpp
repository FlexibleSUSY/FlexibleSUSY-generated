...
// File generated at Wed 12 Apr 2017 12:25:04
...
UMSSM_susy_parameters::UMSSM_susy_parameters(const UMSSM_input_parameters& input_)
   ...
   )), Lambdax(0), Yv(Eigen::Matrix<double,3,3>::Zero()), Yu(Eigen::Matrix<
   double,3,3>::Zero()), g1(0), g2(0), g3(0), gp(0), vd(0), vu(0), vS(0)
...
double CLASSNAME::get_SsRSsR() const
{
   ...
   if (get_loops() > 1) {
      anomDim += Re(twoLoop*(2*Power(gp,4)*Sqr(Qs)*(9*Sqr(Qd) + 3*Sqr(
         Qe) + 2*Sqr(QHd) + 2*Sqr(QHu) + 6*Sqr(Ql) + 18*Sqr(Qq) + 3*Sqr(Qs) + 9
