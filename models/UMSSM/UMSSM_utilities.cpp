...
// UMSSM_parameter_getter ////////////////////////////////////////

std::vector<std::string> UMSSM_parameter_getter::get_mass_names(const std::string& head)
{
...
std::array<std::string, UMSSM_info::NUMBER_OF_MIXINGS> UMSSM_parameter_getter::get_mixing_names()
{
   return UMSSM_info::particle_mixing_names;
}

Eigen::ArrayXd UMSSM_parameter_getter::get_parameters(const UMSSM_mass_eigenstates& model)
{
   return model.get();
}

std::array<std::string, UMSSM_info::NUMBER_OF_PARAMETERS> UMSSM_parameter_getter::get_parameter_names()
{
   return UMSSM_info::parameter_names;
}

std::array<std::string, UMSSM_info::NUMBER_OF_PARTICLES> UMSSM_parameter_getter::get_particle_names()
{
   return UMSSM_info::particle_names;
}

std::vector<std::string> UMSSM_parameter_getter::get_DRbar_mass_names()
{
   return get_mass_names();
}

std::vector<std::string> UMSSM_parameter_getter::get_pole_mass_names()
{
   return get_mass_names("Pole");
}

std::array<std::string, UMSSM_info::NUMBER_OF_MIXINGS> UMSSM_parameter_getter::get_DRbar_mixing_names()
{
   return get_mixing_names();
}

std::array<std::string, UMSSM_info::NUMBER_OF_MIXINGS> UMSSM_parameter_getter::get_pole_mixing_names()
{
...
std::array<std::string, UMSSM_info::NUMBER_OF_INPUT_PARAMETERS> UMSSM_parameter_getter::get_input_parameter_names()
{
   return UMSSM_info::input_parameter_names;
}

std::array<std::string, UMSSM_info::NUMBER_OF_EXTRA_PARAMETERS> UMSSM_parameter_getter::get_extra_parameter_names()
{
   return UMSSM_info::extra_parameter_names;
}

decltype(UMSSM_info::NUMBER_OF_MASSES) UMSSM_parameter_getter::get_number_of_masses()
{
   return UMSSM_info::NUMBER_OF_MASSES;
}

// UMSSM_spectrum_plotter ////////////////////////////////////////

UMSSM_spectrum_plotter::UMSSM_spectrum_plotter(const UMSSM_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void UMSSM_spectrum_plotter::extract_spectrum(const UMSSM_mass_eigenstates& model)
{
   spectrum.clear();
...
void UMSSM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;
...
{
   if (!filestr.good()) {
      ERROR("UMSSM_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const UMSSM_mass_eigenstates& model,
   ...
   const UMSSM_observables* observables)
{
   ...
   append(names, UMSSM_parameter_getter::get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, UMSSM_parameter_getter::get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, UMSSM_parameter_getter::get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, UMSSM_parameter_getter::get_DRbar_mass_names());
   append(names, UMSSM_parameter_getter::get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, UMSSM_parameter_getter::get_pole_mass_names());
   append(names, UMSSM_parameter_getter::get_pole_mixing_names());
   ...
   if (UMSSM_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, UMSSM_observables::get_names());
 ...
 */
UMSSM_mass_eigenstates from_database(
   const std::string& file_name, long long entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, UMSSM_observables* observables)
{
   UMSSM_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = UMSSM_parameter_getter::get_number_of_masses();
   const auto number_of_mixings = UMSSM_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = UMSSM_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = UMSSM_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? UMSSM_observables::NUMBER_OF_OBSERVABLES : 0);
