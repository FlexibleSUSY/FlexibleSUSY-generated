...
class UMSSM_slha : public Model {
public:
   explicit UMSSM_slha(const UMSSM_input_parameters& input_ = UMSSM_input_parameters());
   explicit UMSSM_slha(const Model&, bool do_convert_masses_to_slha = true);
   UMSSM_slha(const UMSSM_slha&) = default;
   UMSSM_slha(UMSSM_slha&&) = default;
   virtual ~UMSSM_slha() = default;
   UMSSM_slha& operator=(const UMSSM_slha&) = default;
   UMSSM_slha& operator=(UMSSM_slha&&) = default;
   ...
   const UMSSM_physical& get_physical_slha() const; ///< returns pole masses to SLHA convention
   UMSSM_physical& get_physical_slha(); ///< returns pole masses to SLHA convention
   ...
   std::complex<double> get_ZVL_pole_slha(int i, int k) const { return PHYSICAL_SLHA(ZVL(i,k)); }
   const Eigen::Matrix<std::complex<double>,3,3>& get_ZVR_pole_slha() const { return PHYSICAL_SLHA(ZVR); }
   std::complex<double> get_ZVR_pole_slha(int i, int k) const { return PHYSICAL_SLHA(ZVR(i,k)); }
   ...
   std::complex<double> get_ZVL_slha(int i, int k) const { return ZVL_slha(i,k); }


private:
   UMSSM_physical physical_slha{}; ///< contains the pole masses and mixings in slha convention
   ...
   Eigen::Matrix<std::complex<double>,3,3> ZVL_slha{Eigen::Matrix<std::complex<double>,3,3>::Zero()};
...
template <class Model>
UMSSM_slha<Model>::UMSSM_slha(const UMSSM_input_parameters& input_)
   : Model(input_)
{
}
 ...
 */
template <class Model>
UMSSM_slha<Model>::UMSSM_slha(const Model& model_,
...
template <class Model>
void UMSSM_slha<Model>::calculate_spectrum()
{
...
template <class Model>
void UMSSM_slha<Model>::convert_to_slha()
{
...
template <class Model>
void UMSSM_slha<Model>::calculate_ckm_matrix()
{
 ...
 */
template <class Model>
void UMSSM_slha<Model>::convert_yukawa_couplings_to_slha()
{
 ...
 */
template <class Model>
void UMSSM_slha<Model>::convert_trilinear_couplings_to_slha()
{
 ...
 */
template <class Model>
void UMSSM_slha<Model>::convert_soft_squared_masses_to_slha()
{
...
template <class Model>
const UMSSM_physical& UMSSM_slha<Model>::get_physical_slha() const
{
   return physical_slha;
}

template <class Model>
UMSSM_physical& UMSSM_slha<Model>::get_physical_slha()
{
   return physical_slha;
}

template <class Model>
void UMSSM_slha<Model>::print(std::ostream& ostr) const
{
   Model::print(ostr);
...
template <class Model>
void UMSSM_slha<Model>::set_convert_masses_to_slha(bool flag)
{
   convert_masses_to_slha = flag;
}

template <class Model>
std::ostream& operator<<(std::ostream& ostr, const UMSSM_slha<Model>& model)
{
   model.print(ostr);
   return ostr;
}
