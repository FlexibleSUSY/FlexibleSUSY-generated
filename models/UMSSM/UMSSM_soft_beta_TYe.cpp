...
// File generated at Tue 10 Oct 2017 22:14:16
...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_TYe_1_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_TYe = (oneOver16PiSqr*(3*traceYdAdjYd*TYe + traceYeAdjYe*TYe +
      ...
      *TYe - 2*Sqr(gp)*Sqr(QHd)*TYe - 2*Sqr(gp)*Sqr(Ql)*TYe + 0.4*Ye*(9*MassB*
      Sqr(g1) + 5*(3*traceAdjYdTYd + traceAdjYeTYe + 3*MassWB*Sqr(g2) + 2*MassU
      *Sqr(gp)*(Sqr(Qe) + Sqr(QHd) + Sqr(Ql))) + 5*Conj(Lambdax)*TLambdax) + 4*
      (Ye*Ye.adjoint()*TYe) + 2*(Ye*Yv.conjugate()*(TYv).transpose()) + 5*(TYe*
      Ye.adjoint()*Ye) + TYe*Yv.conjugate()*Yv.transpose())).real();
...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_TYe_2_loop(const Soft_traces& soft_traces) const
{
   ...
   const Eigen::Matrix<double,3,3> beta_TYe_1 = ((-0.4*twoLoop*Ye*(135*
      MassB*Quad(g1) + Sqr(g1)*(9*(MassB + MassWB)*Sqr(g2) + 2*(traceAdjYdTYd -
      3*traceAdjYeTYe - MassB*traceYdAdjYd + 3*MassB*traceYeAdjYe + 3*(MassB +
      MassU)*Sqr(gp)*(-(QHd*QHu) + 4*QHd*Ql - QHu*Ql + Qd*(6*Qe - 3*(QHd + Ql)
      ) - 3*QHd*Qq - 3*Ql*Qq + Qe*(-5*QHd + 2*QHu - 9*Ql + 6*Qq - 12*Qu) + 6*
      QHd*Qu + 6*Ql*Qu + 10*Sqr(Qe) + 2*Sqr(QHd) + 4*Sqr(Ql)))) + 5*AbsSqr(
      Lambdax)*(3*traceAdjYuTYu + traceAdjYvTYv + 2*MassU*Sqr(gp)*(-Sqr(QHd) +
      Sqr(QHu) + Sqr(Qs))) + 5*(traceAdjYeTYeconjYvTpYv +
      traceAdjYvTpYeconjYeTYv + 18*traceYdAdjYdTYdAdjYd + 3*
      traceYdAdjYuTYuAdjYd + 6*traceYeAdjYeTYeAdjYe + 3*traceYuAdjYdTYdAdjYu +
      15*MassWB*Quad(g2) - 16*traceAdjYdTYd*Sqr(g3) + 16*MassG*traceYdAdjYd*Sqr
      (g3) + 6*(MassU + MassWB)*Sqr(g2)*Sqr(gp)*(Sqr(QHd) + Sqr(Ql)) - 2*Sqr(gp
      )*(3*(traceAdjYdTYd - MassU*traceYdAdjYd)*Sqr(Qd) + traceAdjYeTYe*Sqr(Qe)
      - MassU*traceYeAdjYe*Sqr(Qe) + (-3*traceAdjYdTYd - traceAdjYeTYe + 3*
      MassU*traceYdAdjYd + MassU*traceYeAdjYe)*Sqr(QHd) + traceAdjYeTYe*Sqr(Ql)
      - MassU*traceYeAdjYe*Sqr(Ql) + 3*traceAdjYdTYd*Sqr(Qq) - 3*MassU*
      traceYdAdjYd*Sqr(Qq)) + 4*MassU*Quad(gp)*(5*Quad(Qe) + 4*Quad(QHd) + 8*
      ...
      Sqr(Qv))))) - 4*twoLoop*(3*MassWB*Sqr(g2) - MassU*Sqr(gp)*(Sqr(Qe) - 3*
      Sqr(QHd)))*(Ye*Ye.adjoint()*Ye))*UNITMATRIX(3)).real();
   const Eigen::Matrix<double,3,3> beta_TYe_2 = ((0.1*twoLoop*(135*Quad(
      g1) + 2*Sqr(g1)*(9*Sqr(g2) + 2*(-traceYdAdjYd + 3*traceYeAdjYe + 3*Sqr(gp
      )*(-(QHd*QHu) + 4*QHd*Ql - QHu*Ql + Qd*(6*Qe - 3*(QHd + Ql)) - 3*QHd*Qq -
      3*Ql*Qq + Qe*(-5*QHd + 2*QHu - 9*Ql + 6*Qq - 12*Qu) + 6*QHd*Qu + 6*Ql*Qu
      + 10*Sqr(Qe) + 2*Sqr(QHd) + 4*Sqr(Ql)))) + 5*(15*Quad(g2) + 12*Sqr(g2)*
      Sqr(gp)*(Sqr(QHd) + Sqr(Ql)) + 2*(-9*traceYdAdjYdYdAdjYd - 3*
      ...
      *Sqr(gp)*Sqr(Ql))*(Ye*Ye.adjoint()*Ye) + twoLoop*(-12*traceYdAdjYd - 4*
      ...
      traceAdjYvTYv + 2*MassU*Sqr(gp)*(Sqr(QHu) - Sqr(Ql) + Sqr(Qv)))*(Ye*
      Yv.conjugate()*Yv.transpose()) + twoLoop*(-2*(3*traceYuAdjYu +
      traceYvAdjYv) - 2*AbsSqr(Lambdax) + 4*Sqr(gp)*(Sqr(QHu) - Sqr(Ql) + Sqr(
      Qv)))*(Ye*Yv.conjugate()*(TYv).transpose()) + twoLoop*(-15*traceYdAdjYd -
      5*traceYeAdjYe - 5*AbsSqr(Lambdax) - 1.2*Sqr(g1) + 12*Sqr(g2) - 6*Sqr(gp
      ...
      *(Sqr(QHu) - Sqr(Ql) + Sqr(Qv)))*(TYe*Yv.conjugate()*Yv.transpose()) - 6*
      ...
      )*TYe*Ye.adjoint()*Ye) - 2*twoLoop*(Ye*Yv.conjugate()*Yv.transpose()*
      Ye.adjoint()*TYe) - 4*twoLoop*(Ye*Yv.conjugate()*Yv.transpose()*
      Yv.conjugate()*(TYv).transpose()) - 4*twoLoop*(Ye*Yv.conjugate()*(TYv)
      .transpose()*Ye.adjoint()*Ye) - 4*twoLoop*(Ye*Yv.conjugate()*(TYv)
      .transpose()*Yv.conjugate()*Yv.transpose()) - 6*twoLoop*(TYe*Ye.adjoint()
      *Ye*Ye.adjoint()*Ye) - 4*twoLoop*(TYe*Yv.conjugate()*Yv.transpose()*
      Ye.adjoint()*Ye) - 2*twoLoop*(TYe*Yv.conjugate()*Yv.transpose()*
