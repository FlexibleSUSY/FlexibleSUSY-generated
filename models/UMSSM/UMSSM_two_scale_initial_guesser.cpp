...
UMSSM_initial_guesser<Two_scale>::UMSSM_initial_guesser(
   UMSSM<Two_scale>* model_,
   const softsusy::QedQcd& qedqcd_,
   const UMSSM_low_scale_constraint<Two_scale>& low_constraint_,
   const UMSSM_susy_scale_constraint<Two_scale>& susy_constraint_,
   const UMSSM_high_scale_constraint<Two_scale>& high_constraint_
)
   : model(model_)
   , qedqcd(qedqcd_)
...
{
   if (!model)
      throw SetupError("UMSSM_initial_guesser: Error: pointer to model"
                       " UMSSM<Two_scale> must not be zero");
}
 ...
 */
void UMSSM_initial_guesser<Two_scale>::guess()
{
 ...
 * \endcode
 */
void UMSSM_initial_guesser<Two_scale>::guess_susy_parameters()
{
...
void UMSSM_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void UMSSM_initial_guesser<Two_scale>::calculate_running_SM_masses()
{
 ...
 * corrections).
 */
void UMSSM_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
 ...
 */
void UMSSM_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
 ...
 * corrections).
 */
void UMSSM_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
   const auto vd = MODELPARAMETER(vd);
 ...
 */
void UMSSM_initial_guesser<Two_scale>::guess_soft_parameters()
{
