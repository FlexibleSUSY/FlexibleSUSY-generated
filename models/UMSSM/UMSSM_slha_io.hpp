...
public:
   ...
   static void convert_to_slha_convention(UMSSM_physical&);
...
private:
   ...
   template <class T> void fill_drbar_parameters(UMSSM_slha<T>&) const;
   template <class T> void fill_physical(UMSSM_slha<T>&) const;
};
 ...
 */
template <class T>
void UMSSM_slha_io::fill_drbar_parameters(UMSSM_slha<T>& model) const
{
 ...
 */
template <class T>
void UMSSM_slha_io::fill_physical(UMSSM_slha<T>& model) const
{
   {
      DEFINE_PARAMETER(ZD);
