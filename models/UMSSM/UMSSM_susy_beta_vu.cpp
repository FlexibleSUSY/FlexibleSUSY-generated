...
// File generated at Fri 20 Oct 2017 08:51:46
...
double UMSSM_susy_parameters::calc_beta_vu_2_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_vu = Re(-0.005*twoLoop*vu*(207*Quad(g1) + 10*Sqr(g1)*(34*
      ...
      *QHu - 3*Ql + 3*Qq - 6*Qu)*Sqr(gp)) + 20*AbsSqr(Lambdax)*(3*Sqr(g1) + 5*(
      ...
      Sqr(Qs)))) + 25*(11*Quad(g2) + 12*Sqr(g2)*(3*traceYuAdjYu + traceYvAdjYv
      + 2*Sqr(gp)*Sqr(QHu)) + 8*(-3*traceYdAdjYuYuAdjYd - 9*traceYuAdjYuYuAdjYu
      - traceYvAdjYvTpYeconjYe - 3*traceYvAdjYvYvAdjYv + 16*traceYuAdjYu*Sqr(
      g3) + Quad(gp)*Sqr(QHu)*(9*Sqr(Qd) + 3*Sqr(Qe) + 2*Sqr(QHd) + 4*Sqr(QHu)
      + 6*Sqr(Ql) + 18*Sqr(Qq) + Sqr(Qs) + 9*Sqr(Qu) + 3*Sqr(Qv)) + 2*Sqr(gp)*(
      3*traceYuAdjYu*Sqr(Qq) + 3*traceYuAdjYu*Sqr(Qu) + traceYvAdjYv*(Sqr(Ql) +
