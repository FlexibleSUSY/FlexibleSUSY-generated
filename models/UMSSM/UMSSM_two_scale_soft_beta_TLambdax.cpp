...
// File generated at Thu 15 Dec 2016 12:52:18
...
double UMSSM_soft_parameters::calc_beta_TLambdax_two_loop(const Soft_traces& soft_traces) const
{
   ...
   const double beta_TLambdax_1 = Re(-0.08*twoLoop*Lambdax*(207*Power(g1,
      ...
      *Power(g2,4)*MassWB + 6*(MassU + MassWB)*Sqr(g2)*Sqr(gp)*(Sqr(QHd) + Sqr(
      QHu)) + 2*(traceAdjYeTYeconjYvTpYv + traceAdjYvTpYeconjYeTYv + 9*
      ...
      *Sqr(Qq) - 3*traceAdjYuTYu*Sqr(Qq) + 3*MassU*traceYdAdjYd*Sqr(Qq) + 3*
      ...
      ) + 2*Power(gp,4)*MassU*(4*Power(QHd,4) + 4*Power(QHu,4) + 3*Power(Qs,4)
      + 4*Sqr(QHd)*Sqr(QHu) + 6*Sqr(QHd)*Sqr(Ql) + 6*Sqr(QHu)*Sqr(Ql) + 18*Sqr(
      QHd)*Sqr(Qq) + 18*Sqr(QHu)*Sqr(Qq) + 3*Sqr(QHd)*Sqr(Qs) + 3*Sqr(QHu)*Sqr(
      Qs) + 6*Sqr(Ql)*Sqr(Qs) + 18*Sqr(Qq)*Sqr(Qs) + 9*Sqr(Qd)*(Sqr(QHd) + Sqr(
      QHu) + Sqr(Qs)) + 3*Sqr(Qe)*(Sqr(QHd) + Sqr(QHu) + Sqr(Qs)) + 9*Sqr(QHd)*
      Sqr(Qu) + 9*Sqr(QHu)*Sqr(Qu) + 9*Sqr(Qs)*Sqr(Qu) + 3*Sqr(QHd)*Sqr(Qv) + 3
      *Sqr(QHu)*Sqr(Qv) + 3*Sqr(Qs)*Sqr(Qv))))));
   const double beta_TLambdax_2 = Re(0.02*twoLoop*((207*Power(g1,4) + 10*
      Sqr(g1)*(9*Sqr(g2) - 2*(traceYdAdjYd - 3*traceYeAdjYe - 2*traceYuAdjYu +
      3*Sqr(gp)*(3*Qd*QHd + 3*Qe*QHd - 3*Qd*QHu - 3*Qe*QHu + 2*QHd*QHu - 3*QHd*
      Ql + 3*QHu*Ql + 3*QHd*Qq - 3*QHu*Qq - 6*QHd*Qu + 6*QHu*Qu - 2*Sqr(QHd) -
      2*Sqr(QHu)))) + 25*(15*Power(g2,4) + 12*Sqr(g2)*Sqr(gp)*(Sqr(QHd) + Sqr(
      QHu)) + 2*(-9*traceYdAdjYdYdAdjYd - 6*traceYdAdjYuYuAdjYd - 3*
