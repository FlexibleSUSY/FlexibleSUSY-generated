...
Vertex<typename bar<fields::Fe>::type, fields::Chi, fields::Se>::evaluate(const indices_type& indices, const context_base& context)
{
   ...
   const std::complex<double> right = 0.1*(1.4142135623730951*SUM(j1,0,2,Conj(ZE(gt3,j1))*ZEL(gt1,j1))*(-10*gp*Ql*ZN(gt2,0) + 3.872983346207417*g1*ZN(gt2,1) + 5*g2*ZN(gt2,2)) - 10*SUM(j2,0,2,SUM(j1,0,2,Conj(Ye(j1,j2))*Conj(ZE(gt3,3 + j1)))*ZEL(gt1,j2))*ZN(gt2,3));
