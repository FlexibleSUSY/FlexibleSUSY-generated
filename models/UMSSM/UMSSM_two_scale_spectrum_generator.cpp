...
double UMSSM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void UMSSM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const UMSSM_input_parameters& input)
{
   ...
   UMSSM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<UMSSM_ewsb_solver<Two_scale> >(ewsb_solver));

   UMSSM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   UMSSM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   UMSSM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   UMSSM_convergence_tester<Two_scale> convergence_tester(
   ...
   UMSSM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void UMSSM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   UMSSM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void UMSSM_spectrum_generator<Two_scale>::calculate_spectrum()
{
