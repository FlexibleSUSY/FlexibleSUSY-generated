...
// File generated at Mon 23 Feb 2015 13:04:05
...
void UMSSM_soft_parameters::calc_soft_traces(Soft_traces& soft_traces) const
{
   ...
   TRACE_STRUCT.Tr14 = gp*(2*mHd2*QHd + 2*mHu2*QHu + ms2*Qs + 3*Qd*(md2).trace(
   ...
   TRACE_STRUCT.Tr2U114 = 0.7745966692414834*g1*gp*(-(mHd2*QHd) + mHu2*QHu + Qd
   ...
   TRACE_STRUCT.Tr2U141 = 0.7745966692414834*g1*gp*(-(mHd2*QHd) + mHu2*QHu + Qd
      *(md2).trace() + Qe*(me2).trace() - Ql*(ml2).trace() + Qq*(mq2).trace() - 2*
      Qu*(mu2).trace());
   TRACE_STRUCT.Tr2U144 = Sqr(gp)*(2*mHd2*Sqr(QHd) + 2*mHu2*Sqr(QHu) + ms2*Sqr(
      Qs) + 3*Sqr(Qd)*(md2).trace() + Sqr(Qe)*(me2).trace() + 2*Sqr(Ql)*(ml2)
      .trace() + 6*Sqr(Qq)*(mq2).trace() + 3*Sqr(Qu)*(mu2).trace());
   TRACE_STRUCT.Tr34 = 0.1*gp*(-10*(mHd2*QHd + mHu2*QHu + ms2*Qs)*AbsSqr(
