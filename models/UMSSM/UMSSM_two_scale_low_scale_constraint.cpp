...
// File generated at Wed 12 Apr 2017 12:28:06
...
UMSSM_low_scale_constraint<Two_scale>::UMSSM_low_scale_constraint()
...
UMSSM_low_scale_constraint<Two_scale>::UMSSM_low_scale_constraint(
   UMSSM<Two_scale>* model_, const softsusy::QedQcd& qedqcd_)
   : Constraint<Two_scale>()
   , model(model_)
   , qedqcd(qedqcd_)
   , new_g1(0.)
...
void UMSSM_low_scale_constraint<Two_scale>::apply()
{
   assert(model && "Error: UMSSM_low_scale_constraint::apply():"
...
const Eigen::Matrix<std::complex<double>,3,3>& UMSSM_low_scale_constraint<Two_scale>::get_ckm()
{
   return ckm;
}

const Eigen::Matrix<std::complex<double>,3,3>& UMSSM_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}

double UMSSM_low_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double UMSSM_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void UMSSM_low_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
   model = cast_model<UMSSM<Two_scale>*>(model_);
}

void UMSSM_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& UMSSM_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void UMSSM_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
   initial_scale_guess = 0.;
   model = NULL;
...
void UMSSM_low_scale_constraint<Two_scale>::initialize()
{
   assert(model && "UMSSM_low_scale_constraint<Two_scale>::"
...
void UMSSM_low_scale_constraint<Two_scale>::update_scale()
{
   assert(model && "UMSSM_low_scale_constraint<Two_scale>::"
...
void UMSSM_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
...
double UMSSM_low_scale_constraint<Two_scale>::calculate_theta_w(double alpha_em_drbar)
{
   assert(model && "UMSSM_low_scale_constraint<Two_scale>::"
   ...
   const double msnue         = AbsSqr(ZV(0,0))*MSv(0) + AbsSqr(ZV(1,0))*MSv(1)
      + AbsSqr(ZV(2,0))*MSv(2) + AbsSqr(ZV(3,0))*MSv(3) + AbsSqr(ZV(4,0))*MSv(4)
      + AbsSqr(ZV(5,0))*MSv(5);
   const double msnumu        = AbsSqr(ZV(0,1))*MSv(0) + AbsSqr(ZV(1,1))*MSv(1)
      + AbsSqr(ZV(2,1))*MSv(2) + AbsSqr(ZV(3,1))*MSv(3) + AbsSqr(ZV(4,1))*MSv(4)
      + AbsSqr(ZV(5,1))*MSv(5);
...
void UMSSM_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   assert(model && "UMSSM_low_scale_constraint<Two_scale>::"
...
double UMSSM_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   assert(model && "UMSSM_low_scale_constraint<Two_scale>::"
...
double UMSSM_low_scale_constraint<Two_scale>::calculate_delta_alpha_s(double alphaS) const
{
   assert(model && "UMSSM_low_scale_constraint<Two_scale>::"
...
void UMSSM_low_scale_constraint<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
...
void UMSSM_low_scale_constraint<Two_scale>::calculate_Yu_DRbar()
{
   assert(model && "UMSSM_low_scale_constraint<Two_scale>::"
...
void UMSSM_low_scale_constraint<Two_scale>::calculate_Yd_DRbar()
{
   assert(model && "UMSSM_low_scale_constraint<Two_scale>::"
...
void UMSSM_low_scale_constraint<Two_scale>::calculate_Ye_DRbar()
{
   assert(model && "UMSSM_low_scale_constraint<Two_scale>::"
...
void UMSSM_low_scale_constraint<Two_scale>::calculate_MNeutrino_DRbar()
{
   assert(model && "UMSSM_low_scale_constraint<Two_scale>::"
 ...
 */
double UMSSM_low_scale_constraint<Two_scale>::recalculate_mw_pole(double mw_pole)
{
   assert(model && "UMSSM_low_scale_constraint<Two_scale>::"
