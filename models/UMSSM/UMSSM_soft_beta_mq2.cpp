...
// File generated at Sun 26 Aug 2018 14:22:31
...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_mq2_2_loop(const Soft_traces& soft_traces) const
{
   ...
   const Eigen::Matrix<double,3,3> beta_mq2_2 = (UNITMATRIX(3)*(-2*twoLoop*
      ...
      *twoLoop*Conj(Lambdax)*TLambdax*((TYu).adjoint()*Yu) + twoLoop*(-3*
      ...
      (QHu) - Sqr(Qq) + Sqr(Qu))))*(Yu.adjoint()*Yu*mq2) - 8*mHd2*twoLoop*(Yd.
      ...
      *(Yu.adjoint()*Yu*Yu.adjoint()*Yu) - 4*twoLoop*(Yu.adjoint()*Yu*(TYu).
      ...
      twoLoop*(Conj(MassB)*Sqr(g1)*(597*MassB*Sqr(g1) + 5*(9*(2*MassB + MassWB)
      *Sqr(g2) + 4*(4*(2*MassB + MassG)*Sqr(g3) + 3*(2*MassB + MassU)*Qq*(9*Qd
      + 9*Qe - 3*QHd + 3*QHu - 9*Ql + 10*Qq - 18*Qu)*Sqr(gp)))) + 10*(8*Conj(
      MassG)*Sqr(g3)*((MassB + 2*MassG)*Sqr(g1) + 15*(3*(2*MassG + MassWB)*Sqr(
      g2) - 8*MassG*Sqr(g3) + 4*(2*MassG + MassU)*Sqr(gp)*Sqr(Qq))) + 3*(
      7.745966692414834*g1*gp*Qq*Tr2U114 + 7.745966692414834*g1*gp*Qq*Tr2U141 +
      7.745966692414834*g1*Tr31 + 60*gp*Qq*Tr34 + 45*Tr22*Quad(g2) + 80*Tr23*
      Quad(g3) + Tr2U111*Sqr(g1) + 60*Tr2U144*Sqr(gp)*Sqr(Qq) + 2*Qq*Conj(MassU
      )*Sqr(gp)*((MassB + 2*MassU)*(9*Qd + 9*Qe - 3*QHd + 3*QHu - 9*Ql + 10*Qq)
      *Sqr(g1) + 5*Qq*(16*(MassG + 2*MassU)*Sqr(g3) + 9*((2*MassU + MassWB)*Sqr
      (g2) + 2*MassU*Sqr(gp)*(9*Sqr(Qd) + 3*Sqr(Qe) + 2*(Sqr(QHd) + Sqr(QHu) +
      3*Sqr(Ql) + 10*Sqr(Qq)))))))))*UNITMATRIX(3))).real();
   const Eigen::Matrix<double,3,3> beta_mq2_3 = (0.2*twoLoop*(Conj(MassWB)*Sqr(
      g2)*((MassB + 2*MassWB)*Sqr(g1) + 5*(16*(MassG + 2*MassWB)*Sqr(g3) + 3*(
      11*MassWB*Sqr(g2) + 4*(MassU + 2*MassWB)*Sqr(gp)*Sqr(Qq)))) + 24*Qq*Conj(
      MassU)*Sqr(gp)*(-((MassB + 2*MassU)*Qu*Sqr(g1)) + 5*MassU*Qq*Sqr(gp)*(Sqr
