...
// File generated at Sun 26 Aug 2018 14:23:24
...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_mu2_2_loop(const Soft_traces& soft_traces) const
{
   ...
   const Eigen::Matrix<double,3,3> beta_mu2_1 = (UNITMATRIX(3)*(-0.8*twoLoop*(
      ...
      *AbsSqr(MassU)*Sqr(gp)*(Sqr(QHu) + Sqr(Qq) - Sqr(Qu)) + 10*mHu2*Sqr(gp)*
      ...
      + mHu2)*twoLoop*(Yu*Yd.adjoint()*Yd*Yu.adjoint()) - 4*twoLoop*(Yu*Yd.
      adjoint()*TYd*(TYu).adjoint()) - 8*mHu2*twoLoop*(Yu*Yu.adjoint()*Yu*Yu.
      adjoint()) - 4*twoLoop*(Yu*Yu.adjoint()*TYu*(TYu).adjoint()) - 4*twoLoop*
      (Yu*(TYd).adjoint()*TYd*Yu.adjoint()) - 4*twoLoop*(Yu*(TYu).adjoint()*TYu
      *Yu.adjoint()) - 4*twoLoop*(TYu*Yd.adjoint()*Yd*(TYu).adjoint()) - 4*
      twoLoop*(TYu*Yu.adjoint()*Yu*(TYu).adjoint()) - 4*twoLoop*(TYu*(TYd).
      adjoint()*Yd*Yu.adjoint()) - 4*twoLoop*(TYu*(TYu).adjoint()*Yu*Yu.adjoint
      ()) - 2*twoLoop*(mu2*Yu*Yd.adjoint()*Yd*Yu.adjoint()) - 2*twoLoop*(mu2*Yu
      *Yu.adjoint()*Yu*Yu.adjoint()) - 4*twoLoop*(Yu*mq2*Yd.adjoint()*Yd*Yu.
      adjoint()) - 4*twoLoop*(Yu*mq2*Yu.adjoint()*Yu*Yu.adjoint()) - 4*twoLoop*
      (Yu*Yd.adjoint()*md2*Yd*Yu.adjoint()) - 4*twoLoop*(Yu*Yd.adjoint()*Yd*mq2
      *Yu.adjoint()) - 2*twoLoop*(Yu*Yd.adjoint()*Yd*Yu.adjoint()*mu2) - 4*
      twoLoop*(Yu*Yu.adjoint()*mu2*Yu*Yu.adjoint()) - 4*twoLoop*(Yu*Yu.adjoint(
      )*Yu*mq2*Yu.adjoint()) - 2*twoLoop*(Yu*Yu.adjoint()*Yu*Yu.adjoint()*mu2)
      + 0.035555555555555556*twoLoop*(15*(-7.745966692414834*g1*(gp*Qu*(Tr2U114
       + Tr2U141) + Tr31) + 15*gp*Qu*(gp*Qu*Tr2U144 + Tr34) + 20*Tr23*Quad(g3)
      + 4*Tr2U111*Sqr(g1)) + 4*AbsSqr(MassB)*Sqr(g1)*(321*Sqr(g1) + 80*Sqr(g3))
      )*UNITMATRIX(3))).real();
   const Eigen::Matrix<double,3,3> beta_mu2_2 = (0.17777777777777778*twoLoop*(2
      *Conj(MassB)*Sqr(g1)*(16*MassG*Sqr(g3) - 3*(2*MassB + MassU)*(9*Qd + 9*Qe
       - 3*QHd + 3*QHu - 9*Ql + 9*Qq - 22*Qu)*Qu*Sqr(gp)) + 8*Conj(MassG)*Sqr(
      g3)*(4*(MassB + 2*MassG)*Sqr(g1) + 15*(-2*MassG*Sqr(g3) + (2*MassG +
      MassU)*Sqr(gp)*Sqr(Qu))) + 3*Qu*Conj(MassU)*Sqr(gp)*(-2*(MassB + 2*MassU)
      *(9*Qd + 9*Qe - 3*QHd + 3*QHu - 9*Ql + 9*Qq - 22*Qu)*Sqr(g1) + 5*Qu*(8*(
      MassG + 2*MassU)*Sqr(g3) + 9*MassU*Sqr(gp)*(9*Sqr(Qd) + 3*Sqr(Qe) + 2*Sqr
      (QHd) + 2*Sqr(QHu) + 6*Sqr(Ql) + 18*Sqr(Qq) + Sqr(Qs) + 11*Sqr(Qu) + 3*
