...
// File generated at Wed 12 Apr 2017 12:27:50
...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_mvR2_two_loop(const Soft_traces& soft_traces) const
{
   ...
   const Eigen::Matrix<double,3,3> beta_mvR2_1 = (UNITMATRIX(3)*(-0.8*
      ...
      *mvR2) + 8*gp*Qv*twoLoop*(gp*Qv*Tr2U144 + Tr34 + 3*Power(gp,3)*Qv*AbsSqr(
      MassU)*(9*Sqr(Qd) + 3*Sqr(Qe) + 2*Sqr(QHd) + 2*Sqr(QHu) + 6*Sqr(Ql) + 18*
      Sqr(Qq) + Sqr(Qs)))*UNITMATRIX(3))).real();
