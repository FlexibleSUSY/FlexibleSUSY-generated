...
// File generated at Sat 15 Oct 2016 15:33:08
...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_TYe_two_loop(const Soft_traces& soft_traces) const
{
   ...
   const Eigen::Matrix<double,3,3> beta_TYe_2 = ((twoLoop*(13.5*Power(g1,
      4)*TYe + 7.5*Power(g2,4)*TYe + 10*Power(gp,4)*Power(Qe,4)*TYe + 8*Power(
      gp,4)*Power(QHd,4)*TYe + 16*Power(gp,4)*Power(Ql,4)*TYe - 9*
      ...
      *Sqr(Qq)*TYe + 36*Power(gp,4)*Sqr(QHd)*Sqr(Qq)*TYe + 36*Power(gp,4)*Sqr(
      ...
      Sqr(QHd)*Sqr(Qv)*TYe + 6*Power(gp,4)*Sqr(Ql)*Sqr(Qv)*TYe) - 2*twoLoop*(9*
      traceAdjYdTYd + 3*traceAdjYeTYe + 2*MassU*Sqr(gp)*Sqr(Ql))*(Ye*Ye.adjoint
      ()*Ye) + twoLoop*(-12*traceYdAdjYd - 4*traceYeAdjYe - 4*AbsSqr(Lambdax) +
      1.2*Sqr(g1) + 6*Sqr(g2) + 8*Sqr(gp)*Sqr(QHd))*(Ye*Ye.adjoint()*TYe) +
      twoLoop*(-6*traceAdjYuTYu - 2*traceAdjYvTYv - 4*MassU*Sqr(gp)*Sqr(QHu) +
      4*MassU*Sqr(gp)*Sqr(Ql) - 4*MassU*Sqr(gp)*Sqr(Qv))*(Ye*Yv.conjugate()*
      Yv.transpose()) + twoLoop*(-6*traceYuAdjYu - 2*traceYvAdjYv - 2*AbsSqr(
      Lambdax) + 4*Sqr(gp)*Sqr(QHu) - 4*Sqr(gp)*Sqr(Ql) + 4*Sqr(gp)*Sqr(Qv))*(
      Ye*Yv.conjugate()*(TYv).transpose()) + twoLoop*(-15*traceYdAdjYd - 5*
      ...
      Sqr(QHu) - 2*Sqr(gp)*Sqr(Ql) + 2*Sqr(gp)*Sqr(Qv))*(TYe*Yv.conjugate()*
      Yv.transpose()) - 6*twoLoop*(Ye*Ye.adjoint()*Ye*Ye.adjoint()*TYe) - 8*
      twoLoop*(Ye*Ye.adjoint()*TYe*Ye.adjoint()*Ye) - 2*twoLoop*(Ye*
      Yv.conjugate()*Yv.transpose()*Ye.adjoint()*TYe) - 4*twoLoop*(Ye*
      Yv.conjugate()*Yv.transpose()*Yv.conjugate()*(TYv).transpose()) - 4*
      twoLoop*(Ye*Yv.conjugate()*(TYv).transpose()*Ye.adjoint()*Ye) - 4*twoLoop
      *(Ye*Yv.conjugate()*(TYv).transpose()*Yv.conjugate()*Yv.transpose()) - 6*
      twoLoop*(TYe*Ye.adjoint()*Ye*Ye.adjoint()*Ye) - 4*twoLoop*(TYe*
      Yv.conjugate()*Yv.transpose()*Ye.adjoint()*Ye) - 2*twoLoop*(TYe*
      Yv.conjugate()*Yv.transpose()*Yv.conjugate()*Yv.transpose()))*UNITMATRIX(
      3)).real();
   const Eigen::Matrix<double,3,3> beta_TYe_3 = ((twoLoop*((-3*
      ...
      QHd) - Sqr(QHu) - Sqr(Qs))) - 3*Sqr(Conj(Lambdax))*Sqr(Lambdax))*TYe - 6*
      traceYuAdjYu*Ye*Conj(Lambdax)*TLambdax - 2*traceYvAdjYv*Ye*Conj(Lambdax)*
      TLambdax - 4*Ye*Conj(Lambdax)*Sqr(gp)*Sqr(QHd)*TLambdax + 4*Ye*Conj(
      Lambdax)*Sqr(gp)*Sqr(QHu)*TLambdax + 4*Ye*Conj(Lambdax)*Sqr(gp)*Sqr(Qs)*
      TLambdax - 12*Ye*Lambdax*Sqr(Conj(Lambdax))*TLambdax) - 6*twoLoop*Conj(
