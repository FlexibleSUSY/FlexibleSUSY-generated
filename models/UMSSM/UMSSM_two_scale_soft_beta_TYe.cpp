...
// File generated at Thu 15 Dec 2016 12:52:11
...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_TYe_two_loop(const Soft_traces& soft_traces) const
{
   ...
   const Eigen::Matrix<double,3,3> beta_TYe_1 = ((-0.4*twoLoop*Ye*(135*
      Power(g1,4)*MassB + Sqr(g1)*(9*(MassB + MassWB)*Sqr(g2) + 2*(
      traceAdjYdTYd - 3*traceAdjYeTYe - MassB*traceYdAdjYd + 3*MassB*
      traceYeAdjYe + 3*(MassB + MassU)*Sqr(gp)*(-(QHd*QHu) + 4*QHd*Ql - QHu*Ql
      + Qd*(6*Qe - 3*(QHd + Ql)) - 3*QHd*Qq - 3*Ql*Qq + Qe*(-5*QHd + 2*QHu - 9*
      Ql + 6*Qq - 12*Qu) + 6*QHd*Qu + 6*Ql*Qu + 10*Sqr(Qe) + 2*Sqr(QHd) + 4*Sqr
      (Ql)))) + 5*AbsSqr(Lambdax)*(3*traceAdjYuTYu + traceAdjYvTYv + 2*MassU*
      Sqr(gp)*(-Sqr(QHd) + Sqr(QHu) + Sqr(Qs))) + 5*(15*Power(g2,4)*MassWB +
      ...
      *Sqr(g3) + 6*(MassU + MassWB)*Sqr(g2)*Sqr(gp)*(Sqr(QHd) + Sqr(Ql)) - 2*
      ...
      Sqr(Qu) + 3*Sqr(Qv))))) - 4*twoLoop*(3*MassWB*Sqr(g2) - MassU*Sqr(gp)*(
      Sqr(Qe) - 3*Sqr(QHd)))*(Ye*Ye.adjoint()*Ye))*UNITMATRIX(3)).real();
   const Eigen::Matrix<double,3,3> beta_TYe_2 = ((0.1*twoLoop*(135*Power(
      g1,4) + 2*Sqr(g1)*(9*Sqr(g2) + 2*(-traceYdAdjYd + 3*traceYeAdjYe + 3*Sqr(
      gp)*(-(QHd*QHu) + 4*QHd*Ql - QHu*Ql + Qd*(6*Qe - 3*(QHd + Ql)) - 3*QHd*Qq
      - 3*Ql*Qq + Qe*(-5*QHd + 2*QHu - 9*Ql + 6*Qq - 12*Qu) + 6*QHd*Qu + 6*Ql*
      Qu + 10*Sqr(Qe) + 2*Sqr(QHd) + 4*Sqr(Ql)))) + 5*(15*Power(g2,4) + 12*Sqr(
      g2)*Sqr(gp)*(Sqr(QHd) + Sqr(Ql)) + 2*(-9*traceYdAdjYdYdAdjYd - 3*
      ...
      *Sqr(Qd) + traceYeAdjYe*Sqr(Qe) - (3*traceYdAdjYd + traceYeAdjYe)*Sqr(QHd
      ) + 3*traceYdAdjYd*Sqr(Qq)) + 2*Power(gp,4)*(5*Power(Qe,4) + 4*Power(QHd,
      4) + 8*Power(Ql,4) + 2*Sqr(QHd)*Sqr(QHu) + 8*Sqr(QHd)*Sqr(Ql) + 2*Sqr(QHu
      )*Sqr(Ql) + 9*Sqr(Qd)*(Sqr(Qe) + Sqr(QHd) + Sqr(Ql)) + 18*Sqr(QHd)*Sqr(Qq
      ) + 18*Sqr(Ql)*Sqr(Qq) + Sqr(QHd)*Sqr(Qs) + Sqr(Ql)*Sqr(Qs) + 9*Sqr(QHd)*
      Sqr(Qu) + 9*Sqr(Ql)*Sqr(Qu) + 3*Sqr(QHd)*Sqr(Qv) + 3*Sqr(Ql)*Sqr(Qv) +
      Sqr(Qe)*(5*Sqr(QHd) + 2*Sqr(QHu) + 9*Sqr(Ql) + 18*Sqr(Qq) + Sqr(Qs) + 9*
      Sqr(Qu) + 3*Sqr(Qv))))))*TYe - 2*twoLoop*(9*traceAdjYdTYd + 3*
      traceAdjYeTYe + 2*MassU*Sqr(gp)*Sqr(Ql))*(Ye*Ye.adjoint()*Ye) + twoLoop*(
      -12*traceYdAdjYd - 4*traceYeAdjYe - 4*AbsSqr(Lambdax) + 1.2*Sqr(g1) + 6*
      Sqr(g2) + 8*Sqr(gp)*Sqr(QHd))*(Ye*Ye.adjoint()*TYe) - 2*twoLoop*(3*
      traceAdjYuTYu + traceAdjYvTYv + 2*MassU*Sqr(gp)*(Sqr(QHu) - Sqr(Ql) + Sqr
      (Qv)))*(Ye*Yv.conjugate()*Yv.transpose()) + twoLoop*(-2*(3*traceYuAdjYu +
