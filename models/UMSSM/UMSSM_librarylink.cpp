...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSUMSSMMessage", 1);
...
class UMSSM_spectrum {
public:
   virtual ~UMSSM_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(UMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const UMSSM_observables& get_observables() const = 0;
   virtual const UMSSM_decays& get_decays() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const UMSSM_input_parameters&) = 0;
...
class UMSSM_spectrum_impl : public UMSSM_spectrum
{
public:
   virtual ~UMSSM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(UMSSM_slha_io&, const Spectrum_generator_settings&, const FlexibleDecay_settings&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const UMSSM_observables& get_observables() const override { return observables; }
   virtual const UMSSM_decays& get_decays() const override { return decays; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const UMSSM_input_parameters&) override;
...
private:
   std::tuple<UMSSM<Solver_type>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   UMSSM_scales scales{};              ///< scale information
   UMSSM_observables observables{};    ///< observables
   UMSSM_decays decays{};              ///< decays
};
...
public:
   ...
   void set_input_parameters(const UMSSM_input_parameters& input_) { input = input_; }
...
private:
   UMSSM_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<UMSSM_spectrum> spectrum{nullptr};  ///< spectrum information

   UMSSM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSUMSSMCalculateSpectrum[]?");
   }
}
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(Qq), "Qq");
   MLPutRuleTo(link, INPUTPARAMETER(Ql), "Ql");
   MLPutRuleTo(link, INPUTPARAMETER(QHd), "QHd");
   MLPutRuleTo(link, INPUTPARAMETER(QHu), "QHu");
   MLPutRuleTo(link, INPUTPARAMETER(Qd), "Qd");
   MLPutRuleTo(link, INPUTPARAMETER(Qu), "Qu");
   MLPutRuleTo(link, INPUTPARAMETER(Qe), "Qe");
   MLPutRuleTo(link, INPUTPARAMETER(Qs), "Qs");
   MLPutRuleTo(link, INPUTPARAMETER(Qv), "Qv");
...
UMSSM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const UMSSM_slha& model, MLINK link)
{
   MLPutRule(link, UMSSM_info::model_name);
   MLPutFunction(link, "List", 107);
   ...
   MLPutRuleTo(link, MODELPARAMETER(ZVL), "ZVL");
   MLPutRuleTo(link, MODELPARAMETER(ZVR), "ZVR");
   ...
   MLPutRuleTo(link, PHYSICALPARAMETER(ZVL), "ZVL", {"Pole"});
   MLPutRuleTo(link, PHYSICALPARAMETER(ZVR), "ZVR", {"Pole"});
   ...
   MLPutRuleTo(link, MODELPARAMETER(Ye), "Ye");
   MLPutRuleTo(link, MODELPARAMETER(Lambdax), "\u03bb");
   MLPutRuleTo(link, MODELPARAMETER(Yv), "Yv");
   MLPutRuleTo(link, MODELPARAMETER(Yu), "Yu");
   MLPutRuleTo(link, MODELPARAMETER(g1), "g1");
   MLPutRuleTo(link, MODELPARAMETER(g2), "g2");
   MLPutRuleTo(link, MODELPARAMETER(g3), "g3");
   MLPutRuleTo(link, MODELPARAMETER(gp), "gp");
   MLPutRuleTo(link, MODELPARAMETER(vd), "vd");
   MLPutRuleTo(link, MODELPARAMETER(vu), "vu");
   ...
   MLPutRuleTo(link, MODELPARAMETER(TYv), "Yv", {"T"});
   ...
   MLPutRuleTo(link, MODELPARAMETER(mvR2), "mvR2");
   ...
   MLPutRuleTo(link, MODELPARAMETER(MassU), "MassU");
   MLPutRuleTo(link, MODELPARAMETER(scale), "SCALE");
...
template <typename Solver_type>
void UMSSM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void UMSSM_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const UMSSM_input_parameters& input
)
{
   UMSSM_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void UMSSM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void UMSSM_spectrum_impl<Solver_type>::calculate_model_decays(
   const softsusy::QedQcd& qedqcd, const Physical_input& physical_input, const FlexibleDecay_settings& flexibledecay_settings)
{
   decays = UMSSM_decays(std::get<0>(models), qedqcd, physical_input, flexibledecay_settings);
...
template <typename Solver_type>
void UMSSM_spectrum_impl<Solver_type>::fill_slha_io(UMSSM_slha_io& slha_io,
...
{
   check_spectrum_pointer();
   UMSSM_observables observables = spectrum->get_observables();
...
{
   check_spectrum_pointer();
   UMSSM_decays decays = spectrum->get_decays();
   ...
   for (const auto& decays_list : decay_table) {
      ...
      const auto multiplet_and_index_pair = UMSSM_info::get_multiplet_and_index_from_pdg(pid);
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSUMSSMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSUMSSMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new UMSSM_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   INPUTPARAMETER(Qq) = pars[c++];
   INPUTPARAMETER(Ql) = pars[c++];
   INPUTPARAMETER(QHd) = pars[c++];
   INPUTPARAMETER(QHu) = pars[c++];
   INPUTPARAMETER(Qd) = pars[c++];
   INPUTPARAMETER(Qu) = pars[c++];
   INPUTPARAMETER(Qe) = pars[c++];
   INPUTPARAMETER(Qs) = pars[c++];
   INPUTPARAMETER(Qv) = pars[c++];
...
DLLEXPORT int FSUMSSMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::UMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSUMSSMGetSettings"))
...
DLLEXPORT int FSUMSSMGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::UMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSUMSSMGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSUMSSMGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::UMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSUMSSMGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSUMSSMOpenHandle(
...
DLLEXPORT int FSUMSSMCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::UMSSM_librarylink;
...
DLLEXPORT int FSUMSSMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::UMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSUMSSMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSUMSSMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::UMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSUMSSMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSUMSSMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::UMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSUMSSMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSUMSSMCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::UMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSUMSSMCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSUMSSMCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::UMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSUMSSMCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSUMSSMCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSUMSSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSUMSSMCalculateObservables", "error", e.what_detailed());
      put_error_output(link);
   }
...
DLLEXPORT int FSUMSSMCalculateDecays(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::UMSSM_librarylink;

   if (!check_number_of_args(link, 1, "FSUMSSMCalculateDecays"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSUMSSMCalculateDecays", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSUMSSMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSUMSSMCalculateDecays", "error", e.what());
      put_error_output(link);
   }
