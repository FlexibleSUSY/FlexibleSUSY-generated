...
double UMSSM_soft_parameters::calc_beta_ms2_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_ms2 = twoLoop*(8*gp*Qs*Tr34 - 12*traceYdAdjYd*AbsSqr(TLambdax) -
      ...
      + 18*Sqr(Qq) + 3*Sqr(Qs) + 9*Sqr(Qu)) + Conj(Lambdax)*(Sqr(QHd) + Sqr(
      QHu) - Sqr(Qs))*(2*MassU*Lambdax - TLambdax)) + 0.8*Conj(Lambdax)*(-15*
      ...
      *mHu2*traceYuAdjYu*Lambdax - 15*ms2*traceYuAdjYu*Lambdax - 40*AbsSqr(
      ...
      15*ms2*Lambdax*Sqr(g2) + 10*mHd2*Lambdax*Sqr(gp)*Sqr(QHd) + 10*mHu2*
      Lambdax*Sqr(gp)*Sqr(QHd) + 10*ms2*Lambdax*Sqr(gp)*Sqr(QHd) + 10*mHd2*
      Lambdax*Sqr(gp)*Sqr(QHu) + 10*mHu2*Lambdax*Sqr(gp)*Sqr(QHu) + 10*ms2*
      Lambdax*Sqr(gp)*Sqr(QHu) - 10*mHd2*Lambdax*Sqr(gp)*Sqr(Qs) - 10*mHu2*
