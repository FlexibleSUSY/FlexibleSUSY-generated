...
// File generated at Sun 31 May 2015 12:37:54

#ifndef UMSSM_SPECTRUM_GENERATOR_H
#define UMSSM_SPECTRUM_GENERATOR_H
...
template <class T>
class UMSSM_spectrum_generator {
public:
   UMSSM_spectrum_generator()
      : solver(), model()
   ...
   const UMSSM<T>& get_model() const { return model; }
   ...
   void run(const QedQcd& oneset, const UMSSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "UMSSM_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "UMSSM_spectrum.dat") const;

private:
   RGFlow<T> solver;
   UMSSM<T> model;
   UMSSM_high_scale_constraint<T> high_scale_constraint;
   UMSSM_susy_scale_constraint<T> susy_scale_constraint;
   UMSSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void UMSSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const UMSSM_input_parameters& input)
{
   ...
   UMSSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   UMSSM_initial_guesser<T> initial_guesser(&model, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void UMSSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   UMSSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   UMSSM_parameter_getter parameter_getter;
   Coupling_monitor<UMSSM<T>, UMSSM_parameter_getter>
 ...
 */
template <class T>
void UMSSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   UMSSM_spectrum_plotter plotter;
