...
// File generated at Wed 16 Oct 2019 22:26:50
...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_TYd_1_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_TYd = (oneOver16PiSqr*(0.06666666666666667*(90*traceAdjYdTYd*Yd + 30*
      ...
      *Yd*Sqr(g3) + 60*MassU*Yd*Sqr(gp)*Sqr(Qd) + 60*MassU*Yd*Sqr(gp)*Sqr(QHd)
      + 60*MassU*Yd*Sqr(gp)*Sqr(Qq) + 45*traceYdAdjYd*TYd + 15*traceYeAdjYe*TYd
       + 15*AbsSqr(Lambdax)*TYd - 7*Sqr(g1)*TYd - 45*Sqr(g2)*TYd - 80*Sqr(g3)*
      TYd - 30*Sqr(gp)*Sqr(Qd)*TYd - 30*Sqr(gp)*Sqr(QHd)*TYd - 30*Sqr(gp)*Sqr(
      Qq)*TYd + 30*Yd*Conj(Lambdax)*TLambdax) + 4*(Yd*Yd.adjoint()*TYd) + 2*(Yd
...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_TYd_2_loop(const Soft_traces& soft_traces) const
{
   ...
   const Eigen::Matrix<double,3,3> beta_TYd_1 = (-0.044444444444444446*twoLoop*
      Yd*(45*traceAdjYeTYeconjYvTpYv + 45*traceAdjYvTpYeconjYeTYv + 810*
   ...
   const Eigen::Matrix<double,3,3> beta_TYd_2 = ((-0.011111111111111112*twoLoop
      *(540*traceYuAdjYdTYdAdjYu*Yd + 540*traceAdjYuTYu*Yd*AbsSqr(Lambdax) +
      180*traceAdjYvTYv*Yd*AbsSqr(Lambdax) - 360*MassU*Yd*AbsSqr(Lambdax)*Sqr(
      gp)*Sqr(QHd) + 360*MassU*Yd*AbsSqr(Lambdax)*Sqr(gp)*Sqr(QHu) + 360*MassU*
      Yd*AbsSqr(Lambdax)*Sqr(gp)*Sqr(Qs) - 287*Quad(g1)*TYd - 675*Quad(g2)*TYd
      ...
      *Qu*Sqr(g1)*Sqr(gp)*TYd - 264*Sqr(g1)*Sqr(gp)*Sqr(Qd)*TYd - 960*Sqr(g3)*
      ...
      Sqr(Qs)*TYd) - 0.4*twoLoop*(45*traceAdjYdTYd + 15*traceAdjYeTYe + 4*MassB
      *Sqr(g1) + 30*MassWB*Sqr(g2) - 10*MassU*Sqr(gp)*Sqr(Qd) + 30*MassU*Sqr(gp
      )*Sqr(QHd) + 10*MassU*Sqr(gp)*Sqr(Qq))*(Yd*Yd.adjoint()*Yd) + 0.4*twoLoop
      *(-30*traceYdAdjYd - 10*traceYeAdjYe - 10*AbsSqr(Lambdax) + 3*Sqr(g1) +
      15*Sqr(g2) + 20*Sqr(gp)*Sqr(QHd))*(Yd*Yd.adjoint()*TYd) - 0.4*twoLoop*(15
      *traceAdjYuTYu + 5*traceAdjYvTYv + 4*MassB*Sqr(g1) + 10*MassU*Sqr(gp)*Sqr
      (QHu) - 10*MassU*Sqr(gp)*Sqr(Qq) + 10*MassU*Sqr(gp)*Sqr(Qu))*(Yd*Yu.
      adjoint()*Yu) + 0.4*twoLoop*(-15*traceYuAdjYu - 5*traceYvAdjYv - 5*AbsSqr
      (Lambdax) + 4*Sqr(g1) + 10*Sqr(gp)*Sqr(QHu) - 10*Sqr(gp)*Sqr(Qq) + 10*Sqr
      (gp)*Sqr(Qu))*(Yd*Yu.adjoint()*TYu) + 0.2*twoLoop*(-75*traceYdAdjYd - 25*
      traceYeAdjYe - 25*AbsSqr(Lambdax) + 6*Sqr(g1) + 60*Sqr(g2) - 30*Sqr(gp)*
      Sqr(Qd) + 50*Sqr(gp)*Sqr(QHd) + 30*Sqr(gp)*Sqr(Qq))*(TYd*Yd.adjoint()*Yd)
      + 0.2*twoLoop*(-15*traceYuAdjYu - 5*traceYvAdjYv - 5*AbsSqr(Lambdax) + 4*
   ...
   const Eigen::Matrix<double,3,3> beta_TYd_3 = ((0.2*twoLoop*(-45*
      traceYdAdjYdYdAdjYd*TYd - 15*traceYdAdjYuYuAdjYd*TYd - 15*
      traceYeAdjYeYeAdjYe*TYd - 5*traceYvAdjYvTpYeconjYe*TYd - 15*traceYuAdjYu*
      AbsSqr(Lambdax)*TYd - 5*traceYvAdjYv*AbsSqr(Lambdax)*TYd - 2*traceYdAdjYd
      *Sqr(g1)*TYd + 6*traceYeAdjYe*Sqr(g1)*TYd + 80*traceYdAdjYd*Sqr(g3)*TYd +
      36*QHd*Qu*Sqr(g1)*Sqr(gp)*TYd - 12*Qq*Qu*Sqr(g1)*Sqr(gp)*TYd + 30*
      traceYdAdjYd*Sqr(gp)*Sqr(Qd)*TYd + 10*traceYeAdjYe*Sqr(gp)*Sqr(Qe)*TYd -
      30*traceYdAdjYd*Sqr(gp)*Sqr(QHd)*TYd - 10*traceYeAdjYe*Sqr(gp)*Sqr(QHd)*
      TYd - 10*AbsSqr(Lambdax)*Sqr(gp)*Sqr(QHd)*TYd + 10*AbsSqr(Lambdax)*Sqr(gp
      )*Sqr(QHu)*TYd + 10*traceYeAdjYe*Sqr(gp)*Sqr(Ql)*TYd + 30*traceYdAdjYd*
      Sqr(gp)*Sqr(Qq)*TYd + 10*AbsSqr(Lambdax)*Sqr(gp)*Sqr(Qs)*TYd + 90*Quad(gp
      )*Sqr(Qd)*Sqr(Qu)*TYd + 90*Quad(gp)*Sqr(QHd)*Sqr(Qu)*TYd + 90*Quad(gp)*
      Sqr(Qq)*Sqr(Qu)*TYd + 30*Quad(gp)*Sqr(Qd)*Sqr(Qv)*TYd + 30*Quad(gp)*Sqr(
      QHd)*Sqr(Qv)*TYd + 30*Quad(gp)*Sqr(Qq)*Sqr(Qv)*TYd - 15*Sqr(Conj(Lambdax)
      )*Sqr(Lambdax)*TYd - 30*traceYuAdjYu*Yd*Conj(Lambdax)*TLambdax - 10*
      traceYvAdjYv*Yd*Conj(Lambdax)*TLambdax - 20*Yd*Conj(Lambdax)*Sqr(gp)*Sqr(
      QHd)*TLambdax + 20*Yd*Conj(Lambdax)*Sqr(gp)*Sqr(QHu)*TLambdax + 20*Yd*
      Conj(Lambdax)*Sqr(gp)*Sqr(Qs)*TLambdax - 60*Yd*Lambdax*Sqr(Conj(Lambdax))
      *TLambdax) - 6*twoLoop*Conj(Lambdax)*TLambdax*(Yd*Yd.adjoint()*Yd) - 2*
      twoLoop*Conj(Lambdax)*TLambdax*(Yd*Yu.adjoint()*Yu))*UNITMATRIX(3)).real(
      );
