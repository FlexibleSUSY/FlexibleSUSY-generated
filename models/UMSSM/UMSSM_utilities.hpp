...
public:
   Eigen::ArrayXd get_parameters(const UMSSM<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const UMSSM<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const UMSSM<T>&);
...
template <class T>
void UMSSM_spectrum_plotter::extract_spectrum(const UMSSM<T>& model)
{
   spectrum.clear();
