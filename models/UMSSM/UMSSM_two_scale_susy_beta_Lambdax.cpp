...
// File generated at Wed 12 Apr 2017 12:25:10
...
double UMSSM_susy_parameters::calc_beta_Lambdax_two_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_Lambdax = Re(-0.02*twoLoop*Lambdax*(-207*Power(g1,4) - 10*Sqr(g1)
      *(9*Sqr(g2) - 2*(traceYdAdjYd - 3*traceYeAdjYe - 2*traceYuAdjYu + 3*Sqr(
      gp)*(3*Qd*QHd + 3*Qe*QHd - 3*Qd*QHu - 3*Qe*QHu + 2*QHd*QHu - 3*QHd*Ql + 3
      *QHu*Ql + 3*QHd*Qq - 3*QHu*Qq - 6*QHd*Qu + 6*QHu*Qu - 2*Sqr(QHd) - 2*Sqr(
      QHu)))) - 10*AbsSqr(Lambdax)*(6*Sqr(g1) + 5*(-3*(3*traceYdAdjYd +
      traceYeAdjYe + 3*traceYuAdjYu + traceYvAdjYv) + 6*Sqr(g2) + 4*Sqr(gp)*(
      Sqr(QHd) + Sqr(QHu)))) - 25*(15*Power(g2,4) + 12*Sqr(g2)*Sqr(gp)*(Sqr(QHd
      ...
      *Sqr(QHu)*Sqr(Qs) + 6*Sqr(Ql)*Sqr(Qs) + 18*Sqr(Qq)*Sqr(Qs) + 9*Sqr(Qd)*(
      ...
      QHd)*Sqr(Qv) + 3*Sqr(QHu)*Sqr(Qv) + 3*Sqr(Qs)*Sqr(Qv)))) + 500*Sqr(Conj(
      Lambdax))*Sqr(Lambdax)));
