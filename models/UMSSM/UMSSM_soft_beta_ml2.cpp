...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_ml2_1_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_ml2 = (2*mHd2*(Ye.adjoint()*Ye) + 2*((TYe).adjoint()*TYe) + 2*mHu2*(Yv.
      conjugate()*Yv.transpose()) + 2*(TYv.conjugate()*(TYv).transpose()) + ml2
      *Ye.adjoint()*Ye + ml2*Yv.conjugate()*Yv.transpose() + 2*(Ye.adjoint()*
      me2*Ye) + Ye.adjoint()*Ye*ml2 + 2*(Yv.conjugate()*mvR2*Yv.transpose()) +
      Yv.conjugate()*Yv.transpose()*ml2 - 0.2*(3.872983346207417*g1*Tr11 - 10*
      gp*Ql*Tr14 + 6*AbsSqr(MassB)*Sqr(g1) + 30*AbsSqr(MassWB)*Sqr(g2) + 40*
      AbsSqr(MassU)*Sqr(gp)*Sqr(Ql))*UNITMATRIX(3)).real();
...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_ml2_2_loop(const Soft_traces& soft_traces) const
{
   ...
   const Eigen::Matrix<double,3,3> beta_ml2_1 = ((0.4*(-15*traceconjTYdTpTYd -
      ...
      AbsSqr(Lambdax) + 6*mHd2*Sqr(g1) + 12*AbsSqr(MassB)*Sqr(g1) + 10*mHd2*Sqr
      (gp)*Sqr(Qe) + 20*AbsSqr(MassU)*Sqr(gp)*Sqr(Qe) + 10*mHd2*Sqr(gp)*Sqr(QHd
      ) + 20*AbsSqr(MassU)*Sqr(gp)*Sqr(QHd) - 10*mHd2*Sqr(gp)*Sqr(Ql) - 20*
      AbsSqr(MassU)*Sqr(gp)*Sqr(Ql))*(Ye.adjoint()*Ye) - 0.4*(15*
      traceconjTYdTpYd + 5*traceconjTYeTpYe + 5*Conj(TLambdax)*Lambdax + 6*Conj
      (MassB)*Sqr(g1) + 10*Conj(MassU)*Sqr(gp)*Sqr(Qe) + 10*Conj(MassU)*Sqr(gp)
      *Sqr(QHd) - 10*Conj(MassU)*Sqr(gp)*Sqr(Ql))*(Ye.adjoint()*TYe) - 0.4*(15*
      traceAdjYdTYd + 5*traceAdjYeTYe + 6*MassB*Sqr(g1) + 10*MassU*Sqr(gp)*Sqr(
      Qe) + 10*MassU*Sqr(gp)*Sqr(QHd) - 10*MassU*Sqr(gp)*Sqr(Ql))*((TYe).
      adjoint()*Ye) + 0.4*(-15*traceYdAdjYd - 5*traceYeAdjYe - 5*AbsSqr(Lambdax
      ) + 6*Sqr(g1) + 10*Sqr(gp)*Sqr(Qe) + 10*Sqr(gp)*Sqr(QHd) - 10*Sqr(gp)*Sqr
      (Ql))*((TYe).adjoint()*TYe) + 2*(-3*traceconjTYuTpTYu - traceconjTYvTpTYv
       - 3*tracemq2AdjYuYu - 3*tracemu2YuAdjYu - 6*mHu2*traceYuAdjYu - 2*mHu2*
      ...
      Sqr(gp)*Sqr(Qv))*(Yv.conjugate()*Yv.transpose()) - 2*(3*traceconjTYuTpYu
      + traceconjTYvTpYv + Conj(TLambdax)*Lambdax + 2*Conj(MassU)*Sqr(gp)*Sqr(
      QHu) - 2*Conj(MassU)*Sqr(gp)*Sqr(Ql) + 2*Conj(MassU)*Sqr(gp)*Sqr(Qv))*(Yv
      .conjugate()*(TYv).transpose()) - 2*(3*traceAdjYuTYu + traceAdjYvTYv + 2*
      MassU*Sqr(gp)*Sqr(QHu) - 2*MassU*Sqr(gp)*Sqr(Ql) + 2*MassU*Sqr(gp)*Sqr(Qv
      ))*(TYv.conjugate()*Yv.transpose()) + 2*(-3*traceYuAdjYu - traceYvAdjYv -
      AbsSqr(Lambdax) + 2*Sqr(gp)*Sqr(QHu) - 2*Sqr(gp)*Sqr(Ql) + 2*Sqr(gp)*Sqr(
      Qv))*(TYv.conjugate()*(TYv).transpose()) + 0.2*(-15*traceYdAdjYd - 5*
      traceYeAdjYe - 5*AbsSqr(Lambdax) + 6*Sqr(g1) + 10*Sqr(gp)*Sqr(Qe) + 10*
      Sqr(gp)*Sqr(QHd) - 10*Sqr(gp)*Sqr(Ql))*(ml2*Ye.adjoint()*Ye) + (-3*
      traceYuAdjYu - traceYvAdjYv - AbsSqr(Lambdax) + 2*Sqr(gp)*Sqr(QHu) - 2*
      Sqr(gp)*Sqr(Ql) + 2*Sqr(gp)*Sqr(Qv))*(ml2*Yv.conjugate()*Yv.transpose())
      + 0.4*(-15*traceYdAdjYd - 5*traceYeAdjYe - 5*AbsSqr(Lambdax) + 6*Sqr(g1)
      + 10*Sqr(gp)*Sqr(Qe) + 10*Sqr(gp)*Sqr(QHd) - 10*Sqr(gp)*Sqr(Ql))*(Ye.
      adjoint()*me2*Ye) + 0.2*(-15*traceYdAdjYd - 5*traceYeAdjYe - 5*AbsSqr(
      Lambdax) + 6*Sqr(g1) + 10*Sqr(gp)*Sqr(Qe) + 10*Sqr(gp)*Sqr(QHd) - 10*Sqr(
      gp)*Sqr(Ql))*(Ye.adjoint()*Ye*ml2))*UNITMATRIX(3)).real();
   const Eigen::Matrix<double,3,3> beta_ml2_2 = (UNITMATRIX(3)*(-2*AbsSqr(
      TLambdax)*(Ye.adjoint()*Ye) - 2*Conj(Lambdax)*TLambdax*((TYe).adjoint()*
      Ye) - 2*AbsSqr(TLambdax)*(Yv.conjugate()*Yv.transpose()) - 2*Conj(Lambdax
      )*TLambdax*(TYv.conjugate()*Yv.transpose()) + 2*(-3*traceYuAdjYu -
      traceYvAdjYv - AbsSqr(Lambdax) + 2*Sqr(gp)*Sqr(QHu) - 2*Sqr(gp)*Sqr(Ql) +
      2*Sqr(gp)*Sqr(Qv))*(Yv.conjugate()*mvR2*Yv.transpose()) + (-3*
      traceYuAdjYu - traceYvAdjYv - AbsSqr(Lambdax) + 2*Sqr(gp)*Sqr(QHu) - 2*
      Sqr(gp)*Sqr(Ql) + 2*Sqr(gp)*Sqr(Qv))*(Yv.conjugate()*Yv.transpose()*ml2)
      ...
      - 8*mHu2*(Yv.conjugate()*Yv.transpose()*Yv.conjugate()*Yv.transpose()) -
      4*(Yv.conjugate()*Yv.transpose()*TYv.conjugate()*(TYv).transpose()) - 4*(
      Yv.conjugate()*(TYv).transpose()*TYv.conjugate()*Yv.transpose()) - 4*(TYv
      .conjugate()*Yv.transpose()*Yv.conjugate()*(TYv).transpose()) - 4*(TYv.
      conjugate()*(TYv).transpose()*Yv.conjugate()*Yv.transpose()) - 2*(ml2*Ye.
      adjoint()*Ye*Ye.adjoint()*Ye) - 2*(ml2*Yv.conjugate()*Yv.transpose()*Yv.
      conjugate()*Yv.transpose()) - 4*(Ye.adjoint()*me2*Ye*Ye.adjoint()*Ye) - 4
      *(Ye.adjoint()*Ye*ml2*Ye.adjoint()*Ye) - 4*(Ye.adjoint()*Ye*Ye.adjoint()*
      me2*Ye) - 2*(Ye.adjoint()*Ye*Ye.adjoint()*Ye*ml2) - 4*(Yv.conjugate()*
      mvR2*Yv.transpose()*Yv.conjugate()*Yv.transpose()) - 4*(Yv.conjugate()*Yv
      .transpose()*ml2*Yv.conjugate()*Yv.transpose()) - 4*(Yv.conjugate()*Yv.
      transpose()*Yv.conjugate()*mvR2*Yv.transpose()) - 2*(Yv.conjugate()*Yv.
      transpose()*Yv.conjugate()*Yv.transpose()*ml2) + 0.04*(-77.45966692414834
      *g1*gp*Ql*Tr2U114 - 77.45966692414834*g1*gp*Ql*Tr2U141 -
