...
// File generated at Tue 10 Oct 2017 22:15:17
...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_ml2_2_loop(const Soft_traces& soft_traces) const
{
   ...
   const Eigen::Matrix<double,3,3> beta_ml2_2 = (UNITMATRIX(3)*(-2*
      ...
      *mvR2*Yv.transpose()) - 2*twoLoop*(Yv.conjugate()*Yv.transpose()*
      ...
      )*Ql*(-3*Qd - 3*Qe + QHd - QHu + 4*Ql - 3*Qq + 6*Qu)*Sqr(gp))) + 5*(
      -15.491933384829668*g1*gp*Ql*Tr2U114 - 15.491933384829668*g1*gp*Ql*
      Tr2U141 - 15.491933384829668*g1*Tr31 + 40*gp*Ql*Tr34 + 30*Tr22*Quad(g2) +
      165*AbsSqr(MassWB)*Quad(g2) + 6*Tr2U111*Sqr(g1) + 18*AbsSqr(MassWB)*Sqr(
      g1)*Sqr(g2) + 9*MassB*Conj(MassWB)*Sqr(g1)*Sqr(g2) + 40*Tr2U144*Sqr(gp)*
      Sqr(Ql) + 120*AbsSqr(MassWB)*Sqr(g2)*Sqr(gp)*Sqr(Ql) + 60*MassU*Conj(
      MassWB)*Sqr(g2)*Sqr(gp)*Sqr(Ql) + 12*Ql*Conj(MassU)*Sqr(gp)*(-((MassB + 2
      *MassU)*(3*Qd + 3*Qe - QHd + QHu - 4*Ql + 3*Qq - 6*Qu)*Sqr(g1)) + 5*Ql*((
      2*MassU + MassWB)*Sqr(g2) + 2*MassU*Sqr(gp)*(9*Sqr(Qd) + 3*Sqr(Qe) + 2*
      Sqr(QHd) + 2*Sqr(QHu) + 8*Sqr(Ql) + 18*Sqr(Qq) + Sqr(Qs) + 9*Sqr(Qu) + 3*
