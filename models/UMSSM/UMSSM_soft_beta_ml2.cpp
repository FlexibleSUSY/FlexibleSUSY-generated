...
// File generated at Sun 26 Aug 2018 14:22:40
...
Eigen::Matrix<double,3,3> UMSSM_soft_parameters::calc_beta_ml2_2_loop(const Soft_traces& soft_traces) const
{
   ...
   const Eigen::Matrix<double,3,3> beta_ml2_2 = (UNITMATRIX(3)*(-2*twoLoop*
      ...
      ) + 2*Sqr(gp)*(Sqr(QHu) - Sqr(Ql) + Sqr(Qv)))*(Yv.conjugate()*mvR2*Yv.
      transpose()) + twoLoop*(-3*traceYuAdjYu - traceYvAdjYv - AbsSqr(Lambdax)
      + 2*Sqr(gp)*(Sqr(QHu) - Sqr(Ql) + Sqr(Qv)))*(Yv.conjugate()*Yv.transpose(
      )*ml2) - 8*mHd2*twoLoop*(Ye.adjoint()*Ye*Ye.adjoint()*Ye) - 4*twoLoop*(Ye
      .adjoint()*Ye*(TYe).adjoint()*TYe) - 4*twoLoop*(Ye.adjoint()*TYe*(TYe).
      adjoint()*Ye) - 4*twoLoop*((TYe).adjoint()*Ye*Ye.adjoint()*TYe) - 4*
      twoLoop*((TYe).adjoint()*TYe*Ye.adjoint()*Ye) - 8*mHu2*twoLoop*(Yv.
      conjugate()*Yv.transpose()*Yv.conjugate()*Yv.transpose()) - 4*twoLoop*(Yv
      .conjugate()*Yv.transpose()*TYv.conjugate()*(TYv).transpose()) - 4*
      ...
      - 4*twoLoop*(Ye.adjoint()*me2*Ye*Ye.adjoint()*Ye) - 4*twoLoop*(Ye.adjoint
      ()*Ye*ml2*Ye.adjoint()*Ye) - 4*twoLoop*(Ye.adjoint()*Ye*Ye.adjoint()*me2*
      Ye) - 2*twoLoop*(Ye.adjoint()*Ye*Ye.adjoint()*Ye*ml2) - 4*twoLoop*(Yv.
      conjugate()*mvR2*Yv.transpose()*Yv.conjugate()*Yv.transpose()) - 4*
      twoLoop*(Yv.conjugate()*Yv.transpose()*ml2*Yv.conjugate()*Yv.transpose())
      - 4*twoLoop*(Yv.conjugate()*Yv.transpose()*Yv.conjugate()*mvR2*Yv.
      transpose()) - 2*twoLoop*(Yv.conjugate()*Yv.transpose()*Yv.conjugate()*Yv
      .transpose()*ml2) + 0.04*twoLoop*(3*Conj(MassB)*Sqr(g1)*(207*MassB*Sqr(g1
      ) + 5*(3*(2*MassB + MassWB)*Sqr(g2) + 4*(2*MassB + MassU)*Ql*(-3*Qd - 3*
      Qe + QHd - QHu + 4*Ql - 3*Qq + 6*Qu)*Sqr(gp))) + 5*(-15.491933384829668*
      g1*gp*Ql*Tr2U114 - 15.491933384829668*g1*gp*Ql*Tr2U141 -
      15.491933384829668*g1*Tr31 + 40*gp*Ql*Tr34 + 30*Tr22*Quad(g2) + 165*
      AbsSqr(MassWB)*Quad(g2) + 6*Tr2U111*Sqr(g1) + 18*AbsSqr(MassWB)*Sqr(g1)*
      Sqr(g2) + 9*MassB*Conj(MassWB)*Sqr(g1)*Sqr(g2) + 40*Tr2U144*Sqr(gp)*Sqr(
      Ql) + 120*AbsSqr(MassWB)*Sqr(g2)*Sqr(gp)*Sqr(Ql) + 60*MassU*Conj(MassWB)*
      Sqr(g2)*Sqr(gp)*Sqr(Ql) + 12*Ql*Conj(MassU)*Sqr(gp)*(-((MassB + 2*MassU)*
      (3*Qd + 3*Qe - QHd + QHu - 4*Ql + 3*Qq - 6*Qu)*Sqr(g1)) + 5*Ql*((2*MassU
      + MassWB)*Sqr(g2) + 2*MassU*Sqr(gp)*(9*Sqr(Qd) + 3*Sqr(Qe) + 2*Sqr(QHd) +
      2*Sqr(QHu) + 8*Sqr(Ql) + 18*Sqr(Qq) + Sqr(Qs) + 9*Sqr(Qu) + 3*Sqr(Qv)))))
      )*UNITMATRIX(3))).real();
