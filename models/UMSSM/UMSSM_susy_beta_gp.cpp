...
double UMSSM_susy_parameters::calc_beta_gp_1_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_gp = Re(Cube(gp)*(9*Sqr(Qd) + 3*Sqr(Qe) + 2*Sqr(QHd) + 2*Sqr(QHu) + 6*
      Sqr(Ql) + 18*Sqr(Qq) + Sqr(Qs) + 9*Sqr(Qu) + 3*Sqr(Qv)));


   return oneLoop * beta_gp;
}
...
double UMSSM_susy_parameters::calc_beta_gp_2_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_gp = Re(0.4*Cube(gp)*(90*Quad(Qd)*Sqr(gp) + 30*Quad(Qe)*Sqr(gp) + 20*
      Quad(QHd)*Sqr(gp) + 20*Quad(QHu)*Sqr(gp) + 60*Quad(Ql)*Sqr(gp) + 180*Quad
      (Qq)*Sqr(gp) + 10*Quad(Qs)*Sqr(gp) + 90*Quad(Qu)*Sqr(gp) + 30*Quad(Qv)*
      Sqr(gp) - 30*traceYdAdjYd*Sqr(Qd) + 6*Sqr(g1)*Sqr(Qd) + 120*Sqr(g3)*Sqr(
      Qd) - 10*traceYeAdjYe*Sqr(Qe) + 18*Sqr(g1)*Sqr(Qe) - 30*traceYdAdjYd*Sqr(
      QHd) - 10*traceYeAdjYe*Sqr(QHd) - 10*AbsSqr(Lambdax)*Sqr(QHd) + 3*Sqr(g1)
      *Sqr(QHd) + 15*Sqr(g2)*Sqr(QHd) - 30*traceYuAdjYu*Sqr(QHu) - 10*
