...
// File generated at Sat 27 Aug 2016 12:17:29
...
double UMSSM_susy_parameters::calc_beta_vS_two_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_vS = Re(-0.2*twoLoop*vS*(2*AbsSqr(Lambdax)*(-15*traceYdAdjYd - 5*
      traceYeAdjYe - 15*traceYuAdjYu - 5*traceYvAdjYv + 3*Sqr(g1) + 15*Sqr(g2)
      + 10*Sqr(gp)*Sqr(QHd) + 10*Sqr(gp)*Sqr(QHu)) + 5*Power(gp,4)*Sqr(Qs)*(9*
      Sqr(Qd) + 3*Sqr(Qe) + 2*Sqr(QHd) + 2*Sqr(QHu) + 6*Sqr(Ql) + 18*Sqr(Qq) +
      3*Sqr(Qs) + 9*Sqr(Qu) + 3*Sqr(Qv)) - 20*Sqr(Conj(Lambdax))*Sqr(Lambdax)))
      ;
