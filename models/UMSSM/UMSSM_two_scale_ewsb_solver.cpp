...
/**
 * @file UMSSM_two_scale_ewsb_solver.cpp
 *
 * @brief implementation of EWSB solver for two-scale iteration
 *
 * This file was generated at Fri 20 Oct 2017 09:01:36 with FlexibleSUSY
...
#define CLASSNAME UMSSM_ewsb_solver<Two_scale>

/**
 * This method solves the EWSB conditions iteratively, trying several
 * root finding methods until a solution is found.
 */
int CLASSNAME::solve_iteratively(UMSSM_mass_eigenstates& model_to_solve)
{
...
int CLASSNAME::solve_iteratively_with(
   UMSSM_mass_eigenstates& model_to_solve, EWSB_solver* solver, const EWSB_vector_t& x_init)
{
 ...
 * @param solver solver
 */
void CLASSNAME::set_ewsb_solution(UMSSM_mass_eigenstates& model, const EWSB_solver* solver)
{
...
void CLASSNAME::set_best_ewsb_solution(UMSSM_mass_eigenstates& model, It first, It last)
{
...
int CLASSNAME::solve_iteratively_at(UMSSM_mass_eigenstates& model_to_solve, int l)
{
...
int CLASSNAME::solve(UMSSM_mass_eigenstates& model_to_solve)
{
   if (loop_order == 0) {
...
int CLASSNAME::solve_tree_level(UMSSM_mass_eigenstates& model)
{
...
CLASSNAME::EWSB_vector_t CLASSNAME::initial_guess(const UMSSM_mass_eigenstates& model) const
{
...
CLASSNAME::EWSB_vector_t CLASSNAME::tadpole_equations(const UMSSM_mass_eigenstates& model) const
{
 ...
 */
CLASSNAME::EWSB_vector_t CLASSNAME::ewsb_step(const UMSSM_mass_eigenstates& model) const
{
   ...
   mHd2 = Re((0.025*(14.142135623730951*vS*vu*Conj(TLambdax) + 40*tadpole[0] -
      3*Cube(vd)*Sqr(g1) - 5*Cube(vd)*Sqr(g2) - 20*Cube(vd)*Sqr(gp)*Sqr(QHd) - 20*
      vd*AbsSqr(Lambdax)*Sqr(vS) - 20*QHd*Qs*vd*Sqr(gp)*Sqr(vS) - 20*vd*AbsSqr(
   ...
   mHu2 = Re((0.025*(14.142135623730951*vd*vS*Conj(TLambdax) + 40*tadpole[1] -
      3*Cube(vu)*Sqr(g1) - 5*Cube(vu)*Sqr(g2) - 20*Cube(vu)*Sqr(gp)*Sqr(QHu) - 20*
      vu*AbsSqr(Lambdax)*Sqr(vd) + 3*vu*Sqr(g1)*Sqr(vd) + 5*vu*Sqr(g2)*Sqr(vd) -
      20*QHd*QHu*vu*Sqr(gp)*Sqr(vd) - 20*vu*AbsSqr(Lambdax)*Sqr(vS) - 20*QHu*Qs*vu
      *Sqr(gp)*Sqr(vS) + 14.142135623730951*vd*vS*TLambdax))/vu);
   ms2 = Re((0.25*(1.4142135623730951*vd*vu*Conj(TLambdax) + 4*tadpole[2] - 2*
      Cube(vS)*Sqr(gp)*Sqr(Qs) - 2*vS*AbsSqr(Lambdax)*Sqr(vd) - 2*QHd*Qs*vS*Sqr(gp
      )*Sqr(vd) - 2*vS*AbsSqr(Lambdax)*Sqr(vu) - 2*QHu*Qs*vS*Sqr(gp)*Sqr(vu) +
