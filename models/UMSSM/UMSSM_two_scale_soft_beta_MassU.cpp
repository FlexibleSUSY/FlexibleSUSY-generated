...
double UMSSM_soft_parameters::calc_beta_MassU_one_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_MassU = 2*MassU*oneOver16PiSqr*Sqr(gp)*(9*Sqr(Qd) + 3*Sqr(Qe) + 2
      *Sqr(QHd) + 2*Sqr(QHu) + 6*Sqr(Ql) + 18*Sqr(Qq) + Sqr(Qs) + 9*Sqr(Qu));
...
double UMSSM_soft_parameters::calc_beta_MassU_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_MassU = 0.8*twoLoop*Sqr(gp)*(180*MassU*Power(Qd,4)*Sqr(gp) + 60*
      MassU*Power(Qe,4)*Sqr(gp) + 40*MassU*Power(QHd,4)*Sqr(gp) + 40*MassU*
